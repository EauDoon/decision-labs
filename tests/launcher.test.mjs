...
import { createLauncher, parsePort, PUBLIC_PATHS, publicFile, CONTENT_SECURITY_POLICY, notFoundPage, catalogVersionLine, catalogJobs, catalogFirstWhatsNewHeading, catalogFirstWorkbenchHeading, catalogFirstOpenHref, catalogSkipLinks } from '../scripts/serve.mjs';
...
test('launcher serves only workbenches and refuses hostile hosts and methods', async (t) => {
  ...
  assert.match(page.body, /Weekend Gap 1\.6\.0/);
...
test('launcher 404 body names the catalog and still returns 404', async (t) => {
  ...
  assert.doesNotMatch(missing.body, /id="copy-(first|last)-/);
  assert.doesNotMatch(missing.body, /id="copy-version-line"/);
  assert.doesNotMatch(missing.body, /id="copy-skips"/);
...
test('404 copy tools derive from the printed page and stay local', async () => {
  const page = notFoundPage();
  const script = page.match(/<script>([\s\S]*)<\/script>/)?.[1] ?? '';
  assert.doesNotMatch(script, /\bfetch\s*\(|XMLHttpRequest|WebSocket/);
  assert.match(script, /querySelector\('\.version-line'\)/);
  assert.match(script, /querySelectorAll\('#catalog-jobs li'\)/);
  assert.match(script, /querySelectorAll\('#how-it-works ul li'\)/);
  assert.match(script, /querySelectorAll\('#trust ul li'\)/);
  for (const id of ['copy-lede', 'copy-versions', 'copy-jobs', 'copy-how', 'copy-trust']) {
    assert.match(script, new RegExp(`wireCopy\\('${id}'`));
  }
  ...
  assert.equal(page.includes(catalogJobs()[0].name), true);
