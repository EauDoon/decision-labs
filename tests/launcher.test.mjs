...
test('404 copy first unlabelled skip target text markdown is the target text, or empty if missing', async () => {
  ...
  const versionLine = {
    getAttribute() { return null; },
    tagName: 'P',
    textContent: 'Partnership Breakpoint 1.5.50, Common Cart 1.4.50. Each workbench versions itself.',
  };
  const extra = {
    getAttribute() { return null; },
    tagName: 'P',
  ...
  await clickFirst();
  assert.equal(copied, '- Partnership Breakpoint 1.5.50, Common Cart 1.4.50. Each workbench versions itself.');
  assert.doesNotMatch(copied, /#version-line/);
  assert.doesNotMatch(copied, /Skip to catalog versions/);
  ...
  assert.notEqual(copied, '- Partnership Breakpoint 1.5.50, Common Cart 1.4.50. Each workbench versions itself.');
  skips = [];
  copied = 'stale';
  await clickFirst();
  assert.equal(copied, '');
