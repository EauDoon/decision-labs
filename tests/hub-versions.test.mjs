...
test('catalog versions stay Partnership Breakpoint 1.5.32, Common Cart 1.4.31, The Smallest Agreement 1.5.31, Weekend Gap 1.5.31', () => {
  assert.match(html, /data-app="partnership-breakpoint">\s*1\.5\.32\s*</);
  assert.match(html, /data-app="common-cart">\s*1\.4\.31\s*</);
  assert.match(html, /data-app="smallest-agreement">\s*1\.5\.31\s*</);
  assert.match(html, /data-app="weekend-gap">\s*1\.5\.31\s*</);
  assert.match(html, /data-app-version="partnership-breakpoint">\s*1\.5\.32\s*</);
  assert.match(html, /data-app-version="common-cart">\s*1\.4\.31\s*</);
  assert.match(html, /data-app-version="smallest-agreement">\s*1\.5\.31\s*</);
  assert.match(html, /data-app-version="weekend-gap">\s*1\.5\.31\s*</);
  assert.match(html, /Partnership Breakpoint 1\.5\.32, Common Cart 1\.4\.31, The Smallest Agreement 1\.5\.31, Weekend Gap 1.5\.31/);
  assert.match(readme, /\[Partnership Breakpoint\]\(apps\/partnership-breakpoint\/\) \| 1\.5\.32 \|/);
  assert.match(readme, /\[Common Cart\]\(apps\/common-cart\/\) \| 1\.4\.31 \|/);
  assert.match(readme, /\[The Smallest Agreement\]\(apps\/smallest-agreement\/\) \| 1\.5\.31 \|/);
  assert.match(readme, /\[Weekend Gap\]\(apps\/weekend-gap\/\) \| 1\.5\.31 \|/);
});
