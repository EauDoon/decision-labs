...
`redactConfiguration` copies a valid case, deletes `deal.title` and `deal.notes` if present, and replaces each participant `name` with `Participant 1` through `N`. Identifiers, shares, costs, stress settings, and currency are unchanged. This is a sharing aid, not encryption. Print redacted uses the same Participant 1 through N labels on the print path and in the print stylesheet without changing the saved case. Print one-pager includes the least-headroom participant line, an allocation-balance line, a deal title line, a currency code line, the first-breakpoint participant label line, the first-breakpoint remaining-to-hold line, the first-breakpoint volume-to-hold line, the over-capacity participant count line, the first over-capacity participant label line, the first over-capacity remaining listed capacity line, the last over-capacity participant label line, and the last over-capacity remaining listed capacity line when the case is valid. The remaining-to-hold line is honest when that amount is unavailable. The first over-capacity participant label line is an honest empty when none are over. The first over-capacity remaining listed capacity line is an honest empty when none are over. The last over-capacity participant label line is an honest empty when none are over. The last over-capacity remaining listed capacity line is an honest empty when none are over. The saved case is unchanged.

Copy first-breakpoint remaining-to-hold copies the volume or share still needed for the first-breakpoint participant as one Markdown line, or an honest empty line when that amount is unavailable. Copy first-breakpoint volume-to-hold copies that participant's exit volume from the same result, or an honest empty line when exit volume is unavailable. Copy first over-capacity remaining listed capacity copies how far over listed capacity the first over-capacity roster row currently is, or an honest empty line when none are over. Copy last over-capacity participant label copies the name of the last roster row currently over listed capacity as one Markdown line, or an honest empty line when none are over. Copy last over-capacity remaining listed capacity copies how far over listed capacity the last over-capacity roster row currently is, or an honest empty line when none are over. Copy last first-breakpoint participant label copies the name of the last roster row that is the first-breakpoint as one Markdown line, or an honest empty line when none. Those copy controls are distinct from each other and from least-headroom, first-breakpoint label, over-capacity count, and first over-capacity label copy. Counts remain counts. Ranking is not a forecast of who will exit.
