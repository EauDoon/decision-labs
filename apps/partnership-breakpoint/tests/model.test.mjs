...
test('Domestique cycling carnival split preset is a distinct committee club-hire first-aid starting point', () => {
  ...
  const others = ['balanced', 'thinMargin', 'growthAtCost', 'creatorTakeRate', 'threePartyJv', 'twoPartyStudio', 'fourPartyMarketplace', 'licensorDistributor', 'talentAgentPlatform', 'threePartyJointVenture', 'podcastHostNetwork', 'communityHallSplit', 'festivalStallSplit', 'popupCinemaSplit', 'communityRadioSplit', 'schoolConcertSplit', 'sportsCarnivalSplit', 'netballCarnivalSplit', 'swimmingCarnivalSplit', 'athleticsCarnivalSplit', 'cricketCarnivalSplit', 'tennisCarnivalSplit', 'basketballCarnivalSplit', 'volleyballCarnivalSplit', 'rugbyCarnivalSplit', 'hockeyCarnivalSplit', 'baseballCarnivalSplit', 'softballCarnivalSplit', 'lacrosseCarnivalSplit', 'waterPoloCarnivalSplit', 'rowingCarnivalSplit', 'sailingCarnivalSplit', 'canoeingCarnivalSplit', 'kayakingCarnivalSplit', 'dragonBoatCarnivalSplit', 'surfCarnivalSplit', 'triathlonCarnivalSplit', 'cyclingCarnivalSplit', 'mountainBikeCarnivalSplit', 'bmxCarnivalSplit', 'cycloCrossCarnivalSplit', 'trackCyclingCarnivalSplit', 'gravelCyclingCarnivalSplit', 'roadCyclingCarnivalSplit', 'criteriumCyclingCarnivalSplit', 'timeTrialCyclingCarnivalSplit', 'hillClimbCyclingCarnivalSplit', 'keirinCyclingCarnivalSplit', 'madisonCyclingCarnivalSplit', 'omniumCyclingCarnivalSplit', 'pointsRaceCyclingCarnivalSplit', 'scratchCyclingCarnivalSplit', 'individualPursuitCyclingCarnivalSplit', 'teamPursuitCyclingCarnivalSplit', 'teamSprintCyclingCarnivalSplit', 'firstAidCyclingCarnivalSplit', 'feedZoneCyclingCarnivalSplit', 'leadOutCyclingCarnivalSplit', 'soigneurCyclingCarnivalSplit', 'bottleHandCyclingCarnivalSplit', 'echelonCyclingCarnivalSplit'];
  for (const key of others) {
    const other = clonePreset(key);
...
test('Bottle-hand cycling carnival split preset is a distinct committee club-hire first-aid starting point', () => {
  const keys = Object.keys(PRESETS);
  assert.ok(keys.indexOf('domestiqueCyclingCarnivalSplit') < keys.indexOf('bottleHandCyclingCarnivalSplit'));
  assert.equal(keys.indexOf('bottleHandCyclingCarnivalSplit'), keys.indexOf('domestiqueCyclingCarnivalSplit') + 1);
  assert.equal(keys.indexOf('domestiqueCyclingCarnivalSplit'), keys.indexOf('soigneurCyclingCarnivalSplit') + 1);
  const bottleHand = clonePreset('bottleHandCyclingCarnivalSplit');
  const domestique = clonePreset('domestiqueCyclingCarnivalSplit');
  const soigneur = clonePreset('soigneurCyclingCarnivalSplit');
  const leadOut = clonePreset('leadOutCyclingCarnivalSplit');
  const feedZone = clonePreset('feedZoneCyclingCarnivalSplit');
  const firstAid = clonePreset('firstAidCyclingCarnivalSplit');
  assert.equal(PRESETS.bottleHandCyclingCarnivalSplit.name, 'Bottle-hand cycling carnival split');
  assert.equal(bottleHand.participants.length, 3);
  assert.deepEqual(bottleHand.participants.map((item) => item.id), ['bottle-hand-cycling-committee', 'bottle-hand-cycling-club-hire', 'bottle-hand-cycling-first-aid']);
  assert.deepEqual(bottleHand.participants.map((item) => item.name), ['Carnival committee', 'Bottle-hand cycling club hire', 'First-aid']);
  assert.deepEqual(bottleHand.participants.map((item) => item.revenueShare), [0.37, 0.38, 0.25]);
  assert.equal(bottleHand.deal.monthlyVolume, 7300);
  assert.equal(bottleHand.deal.feePerTransaction, 8);
  assert.equal(bottleHand.deal.addressableVolume, 9300);
  assert.deepEqual(bottleHand.participants.map((item) => item.capacity), [8850, 9950, 7300]);
  assert.equal(bottleHand.participants[2].variableCostPerTransaction, 1.18);
  assert.notEqual(bottleHand.participants[2].variableCostPerTransaction, 1.20);
  assert.notEqual(bottleHand.participants[2].variableCostPerTransaction, 1.56);
  assert.notEqual(bottleHand.participants[2].variableCostPerTransaction, 1.46);
  assert.notEqual(bottleHand.participants[2].variableCostPerTransaction, 1.48);
  assert.notEqual(bottleHand.participants[2].variableCostPerTransaction, 1.50);
  assert.notEqual(bottleHand.participants[2].variableCostPerTransaction, 1.44);
  assert.notEqual(bottleHand.participants[2].variableCostPerTransaction, 1.42);
  assert.notEqual(bottleHand.participants[2].variableCostPerTransaction, 1.40);
  assert.notEqual(bottleHand.participants[2].variableCostPerTransaction, 1.38);
  assert.notEqual(bottleHand.participants[2].variableCostPerTransaction, 1.36);
  assert.notEqual(bottleHand.participants[2].variableCostPerTransaction, 1.34);
  assert.notEqual(bottleHand.participants[2].variableCostPerTransaction, 1.32);
  assert.notEqual(bottleHand.participants[2].variableCostPerTransaction, 1.30);
  assert.notEqual(bottleHand.participants[2].variableCostPerTransaction, 1.28);
  assert.notEqual(bottleHand.participants[2].variableCostPerTransaction, 1.26);
  assert.notEqual(bottleHand.participants[2].variableCostPerTransaction, 1.24);
  assert.notEqual(bottleHand.participants[2].variableCostPerTransaction, 1.22);
  assert.ok(bottleHand.participants.every((item) => item.capacity != null && Number.isFinite(item.capacity)));
  assert.notEqual(bottleHand.participants[0].variableCostPerTransaction, bottleHand.participants[1].variableCostPerTransaction);
  assert.notEqual(bottleHand.participants[1].variableCostPerTransaction, bottleHand.participants[2].variableCostPerTransaction);
  assert.notEqual(bottleHand.participants[0].fixedMonthlyCost, bottleHand.participants[1].fixedMonthlyCost);
  assert.notEqual(bottleHand.participants[1].fixedMonthlyCost, bottleHand.participants[2].fixedMonthlyCost);
  const others = ['balanced', 'thinMargin', 'growthAtCost', 'creatorTakeRate', 'threePartyJv', 'twoPartyStudio', 'fourPartyMarketplace', 'licensorDistributor', 'talentAgentPlatform', 'threePartyJointVenture', 'podcastHostNetwork', 'communityHallSplit', 'festivalStallSplit', 'popupCinemaSplit', 'communityRadioSplit', 'schoolConcertSplit', 'sportsCarnivalSplit', 'netballCarnivalSplit', 'swimmingCarnivalSplit', 'athleticsCarnivalSplit', 'cricketCarnivalSplit', 'tennisCarnivalSplit', 'basketballCarnivalSplit', 'volleyballCarnivalSplit', 'rugbyCarnivalSplit', 'hockeyCarnivalSplit', 'baseballCarnivalSplit', 'softballCarnivalSplit', 'lacrosseCarnivalSplit', 'waterPoloCarnivalSplit', 'rowingCarnivalSplit', 'sailingCarnivalSplit', 'canoeingCarnivalSplit', 'kayakingCarnivalSplit', 'dragonBoatCarnivalSplit', 'surfCarnivalSplit', 'triathlonCarnivalSplit', 'cyclingCarnivalSplit', 'mountainBikeCarnivalSplit', 'bmxCarnivalSplit', 'cycloCrossCarnivalSplit', 'trackCyclingCarnivalSplit', 'gravelCyclingCarnivalSplit', 'roadCyclingCarnivalSplit', 'criteriumCyclingCarnivalSplit', 'timeTrialCyclingCarnivalSplit', 'hillClimbCyclingCarnivalSplit', 'keirinCyclingCarnivalSplit', 'madisonCyclingCarnivalSplit', 'omniumCyclingCarnivalSplit', 'pointsRaceCyclingCarnivalSplit', 'scratchCyclingCarnivalSplit', 'individualPursuitCyclingCarnivalSplit', 'teamPursuitCyclingCarnivalSplit', 'teamSprintCyclingCarnivalSplit', 'firstAidCyclingCarnivalSplit', 'feedZoneCyclingCarnivalSplit', 'leadOutCyclingCarnivalSplit', 'soigneurCyclingCarnivalSplit', 'domestiqueCyclingCarnivalSplit', 'echelonCyclingCarnivalSplit'];
  for (const key of others) {
    const other = clonePreset(key);
    assert.notEqual(bottleHand.participants.map((item) => item.id).join(','), other.participants.map((item) => item.id).join(','), key);
    assert.notEqual(JSON.stringify(bottleHand.deal), JSON.stringify(other.deal), key);
    assert.notEqual(JSON.stringify(bottleHand.participants.map((item) => item.variableCostPerTransaction)), JSON.stringify(other.participants.map((item) => item.variableCostPerTransaction)), key);
    assert.notEqual(JSON.stringify(bottleHand.participants.map((item) => item.fixedMonthlyCost)), JSON.stringify(other.participants.map((item) => item.fixedMonthlyCost)), key);
  }
  const teamSprint = clonePreset('teamSprintCyclingCarnivalSplit');
  const teamPursuit = clonePreset('teamPursuitCyclingCarnivalSplit');
  const pursuit = clonePreset('individualPursuitCyclingCarnivalSplit');
  ...
  const track = clonePreset('trackCyclingCarnivalSplit');
  const cycloCross = clonePreset('cycloCrossCarnivalSplit');
  assert.notEqual(JSON.stringify(bottleHand), JSON.stringify(domestique));
  assert.notEqual(JSON.stringify(bottleHand), JSON.stringify(soigneur));
  assert.notEqual(JSON.stringify(bottleHand), JSON.stringify(leadOut));
  assert.notEqual(JSON.stringify(bottleHand), JSON.stringify(feedZone));
  assert.notEqual(JSON.stringify(bottleHand), JSON.stringify(firstAid));
  assert.equal(soigneur.participants[2].variableCostPerTransaction, 1.22);
  assert.equal(leadOut.participants[2].variableCostPerTransaction, 1.24);
  assert.notEqual(JSON.stringify(bottleHand), JSON.stringify(teamSprint));
  assert.notEqual(JSON.stringify(bottleHand), JSON.stringify(teamPursuit));
  assert.notEqual(JSON.stringify(bottleHand), JSON.stringify(pursuit));
  assert.notEqual(JSON.stringify(bottleHand), JSON.stringify(scratch));
  assert.notEqual(JSON.stringify(bottleHand), JSON.stringify(pointsRace));
  assert.notEqual(JSON.stringify(bottleHand), JSON.stringify(omnium));
  assert.notEqual(JSON.stringify(bottleHand), JSON.stringify(madison));
  assert.notEqual(JSON.stringify(bottleHand), JSON.stringify(keirin));
  assert.notEqual(JSON.stringify(bottleHand), JSON.stringify(hillClimb));
  assert.notEqual(bottleHand.participants[1].id, 'domestique-cycling-club-hire');
  assert.notEqual(bottleHand.participants[1].id, 'soigneur-cycling-club-hire');
  assert.notEqual(bottleHand.participants[1].id, 'lead-out-cycling-club-hire');
  assert.notEqual(bottleHand.participants[1].id, 'feed-zone-cycling-club-hire');
  assert.notEqual(bottleHand.participants[1].id, 'first-aid-cycling-club-hire');
  assert.notEqual(bottleHand.participants[1].id, 'team-sprint-cycling-club-hire');
  assert.notEqual(bottleHand.participants[1].id, 'team-pursuit-cycling-club-hire');
  assert.notEqual(bottleHand.participants[1].id, 'individual-pursuit-cycling-club-hire');
  assert.notEqual(bottleHand.participants[1].id, 'scratch-cycling-club-hire');
  assert.notEqual(bottleHand.participants[1].id, 'points-race-cycling-club-hire');
  assert.notEqual(bottleHand.participants[1].id, 'omnium-cycling-club-hire');
  assert.equal(bottleHand.participants[1].name, 'Bottle-hand cycling club hire');
  assert.match(bottleHand.participants[1].name, /Bottle-hand cycling club/);
  assert.doesNotMatch(bottleHand.participants[1].name, /Domestique cycling/);
  assert.doesNotMatch(bottleHand.participants[1].name, /Soigneur cycling/);
  assert.doesNotMatch(bottleHand.participants[1].name, /Lead-out cycling/);
  assert.doesNotMatch(bottleHand.participants[1].name, /Feed-zone cycling/);
  assert.doesNotMatch(bottleHand.participants[1].name, /First-aid cycling/);
  assert.doesNotMatch(bottleHand.participants[1].name, /Team-sprint cycling/);
  assert.doesNotMatch(bottleHand.participants[1].name, /Team-pursuit cycling/);
  assert.doesNotMatch(bottleHand.participants[1].name, /Individual-pursuit cycling/);
  assert.doesNotMatch(bottleHand.participants[1].name, /Scratch cycling/);
  assert.doesNotMatch(bottleHand.participants[1].name, /Points-race cycling/);
  assert.doesNotMatch(bottleHand.participants[1].name, /Omnium cycling/);
  assert.doesNotMatch(bottleHand.participants[1].name, /Madison cycling/);
  assert.doesNotMatch(bottleHand.participants[1].name, /Keirin cycling/);
  assert.doesNotMatch(bottleHand.participants[1].name, /Hill-climb cycling/);
  assert.doesNotMatch(bottleHand.participants[1].name, /Time-trial cycling/);
  assert.doesNotMatch(bottleHand.participants[1].name, /Criterium cycling/);
  assert.doesNotMatch(bottleHand.participants[1].name, /soigneur-station/i);
  assert.doesNotMatch(bottleHand.participants[1].name, /cape-bag/i);
  assert.doesNotMatch(bottleHand.participants[1].name, /rice-cake/i);
  assert.doesNotMatch(bottleHand.participants[1].name, /start-ramp/i);
  assert.doesNotMatch(bottleHand.participants[1].name, /time-check/i);
  assert.doesNotMatch(bottleHand.participants[1].name, /timing-hut/i);
  assert.doesNotMatch(bottleHand.participants[1].name, /pit-lane/i);
  assert.doesNotMatch(bottleHand.participants[1].name, /sealed-road/i);
  assert.doesNotMatch(bottleHand.participants[1].name, /circuit/i);
  assert.doesNotMatch(bottleHand.participants[1].name, /train-line/i);
  assert.doesNotMatch(bottleHand.participants[1].name, /last-rider/i);
  assert.doesNotMatch(bottleHand.participants[1].name, /sprinter-launch/i);
  assert.doesNotMatch(bottleHand.participants[1].name, /musette-line/i);
  assert.doesNotMatch(bottleHand.participants[1].name, /sticky-bottle/i);
  assert.doesNotMatch(bottleHand.participants[1].name, /bottle-hand-up/i);
  assert.doesNotMatch(bottleHand.participants[1].name, /treatment-tent/i);
  assert.doesNotMatch(bottleHand.participants[1].name, /ice-pack-rota/i);
  assert.doesNotMatch(bottleHand.participants[1].name, /triage-board/i);
  assert.doesNotMatch(bottleHand.participants[1].name, /banked-boards/i);
  assert.doesNotMatch(bottleHand.participants[1].name, /summit-marshal/i);
  assert.doesNotMatch(bottleHand.participants[1].name, /hairpin/i);
  assert.doesNotMatch(bottleHand.participants[1].name, /climb-chip/i);
  assert.doesNotMatch(bottleHand.participants[1].name, /derny-pacer/i);
  assert.doesNotMatch(bottleHand.participants[1].name, /keirin-chip/i);
  assert.doesNotMatch(bottleHand.participants[1].name, /keirin-gate/i);
  assert.doesNotMatch(bottleHand.participants[1].name, /madison-pair/i);
  assert.doesNotMatch(bottleHand.participants[1].name, /handsling/i);
  assert.doesNotMatch(bottleHand.participants[1].name, /omnium-board/i);
  assert.doesNotMatch(bottleHand.participants[1].name, /flying-200/i);
  assert.doesNotMatch(bottleHand.participants[1].name, /elimination-heat/i);
  assert.doesNotMatch(bottleHand.participants[1].name, /tempo-lap/i);
  assert.doesNotMatch(bottleHand.participants[1].name, /points-race/i);
  assert.doesNotMatch(bottleHand.participants[1].name, /points-sprint/i);
  assert.doesNotMatch(bottleHand.participants[1].name, /bell-lap/i);
  assert.doesNotMatch(bottleHand.participants[1].name, /prime-board/i);
  assert.doesNotMatch(bottleHand.participants[1].name, /scratch-bunch/i);
  assert.doesNotMatch(bottleHand.participants[1].name, /bunch-finish/i);
  assert.doesNotMatch(bottleHand.participants[1].name, /scratch-chip/i);
  assert.doesNotMatch(bottleHand.participants[1].name, /pursuit-line/i);
  assert.doesNotMatch(bottleHand.participants[1].name, /catch-bell/i);
  assert.doesNotMatch(bottleHand.participants[1].name, /pursuit-chip/i);
  assert.doesNotMatch(bottleHand.participants[1].name, /team-pursuit-rota/i);
  assert.doesNotMatch(bottleHand.participants[1].name, /changeover-bell/i);
  assert.doesNotMatch(bottleHand.participants[1].name, /quartet-board/i);
  assert.doesNotMatch(bottleHand.participants[1].name, /start-gate/i);
  assert.doesNotMatch(bottleHand.participants[1].name, /flying-relay/i);
  assert.doesNotMatch(bottleHand.participants[1].name, /trio-board/i);
  const payload = JSON.stringify(bottleHand);
  assert.doesNotMatch(payload, /live roster/i);
  assert.doesNotMatch(payload, /hosted/i);
  assert.doesNotMatch(payload, /\bapi\b/i);
  assert.doesNotMatch(payload, /forecast/i);
  assert.doesNotMatch(payload, /live capacity/i);
  assert.doesNotMatch(payload, /wheel-change/i);
  assert.doesNotMatch(payload, /neutral-service/i);
  assert.doesNotMatch(payload, /team-car/i);
  assert.doesNotMatch(payload, /soigneur-station/i);
  assert.doesNotMatch(payload, /cape-bag/i);
  assert.doesNotMatch(payload, /rice-cake/i);
  assert.doesNotMatch(payload, /start-ramp/i);
  ...
  assert.doesNotMatch(payload, /flying-relay/i);
  assert.doesNotMatch(payload, /trio-board/i);
  assert.match(payload, /Bottle-hand cycling club hire/);
  assert.doesNotMatch(payload, /Domestique cycling club hire/);
  assert.doesNotMatch(payload, /Soigneur cycling club hire/);
  assert.doesNotMatch(payload, /Lead-out cycling club hire/);
  assert.doesNotMatch(payload, /train-line/i);
  assert.doesNotMatch(payload, /last-rider/i);
  ...
  assert.equal(teamSprint.participants[2].variableCostPerTransaction, 1.30);
  assert.equal(timeTrial.participants[2].variableCostPerTransaction, 1.48);
  const result = calculatePartnership(bottleHand);
  assert.equal(validateConfiguration(bottleHand).valid, true);
  assert.equal(domestique.participants[2].variableCostPerTransaction, 1.20);
  assert.equal(soigneur.participants[2].variableCostPerTransaction, 1.22);
  assert.equal(feedZone.participants[2].variableCostPerTransaction, 1.26);
  assert.equal(firstAid.participants[2].variableCostPerTransaction, 1.28);
  assert.equal(new Set(result.participants.map((item) => item.id)).size, 3);
  assert.equal(result.effectiveVolume, 7300);
  assert.equal(result.viable, true);
  assert.ok(result.participants.every((item) => item.profitPass));
  assert.equal(track.deal.monthlyVolume, 5400);
  assert.equal(gravel.deal.monthlyVolume, 5500);
...
test('Domestique cycling carnival remaining listed capacity is distinct from soigneur lead-out feed-zone first-aid team-sprint team-pursuit individual-pursuit scratch points-race omnium madison keirin hill-climb time-trial criterium road gravel track and cyclo-cross', () => {
  const domestique = clonePreset('domestiqueCyclingCarnivalSplit');
  const result = calculatePartnership(domestique);
  assert.equal(result.effectiveVolume, 7200);
  const remaining = domestique.participants.map((item) => item.capacity - result.effectiveVolume);
  assert.deepEqual(remaining, [1550, 2650, 0]);
  const lastWithin = [...domestique.participants].reverse().find((item) => item.capacity - result.effectiveVolume >= -1e-9);
  assert.equal(lastWithin.id, 'domestique-cycling-first-aid');
  const lastSpare = [...domestique.participants].reverse().find((item) => item.capacity - result.effectiveVolume > 1e-9);
  assert.equal(lastSpare.id, 'domestique-cycling-club-hire');
  const firstSpare = domestique.participants.find((item) => item.capacity - result.effectiveVolume > 1e-9);
  assert.equal(firstSpare.id, 'domestique-cycling-committee');
  assert.notEqual(lastSpare.id, lastWithin.id);
  const soigneur = clonePreset('soigneurCyclingCarnivalSplit');
  const soigneurResult = calculatePartnership(soigneur);
  assert.notEqual(JSON.stringify(domestique.participants.map((item) => item.capacity)), JSON.stringify(soigneur.participants.map((item) => item.capacity)));
  assert.notEqual(result.effectiveVolume, soigneurResult.effectiveVolume);
  assert.deepEqual(soigneur.participants.map((item) => item.capacity), [8550, 9650, 7100]);
  assert.equal(soigneur.deal.monthlyVolume, 7100);
  assert.equal(soigneur.participants[2].variableCostPerTransaction, 1.22);
  const bottleHand = clonePreset('bottleHandCyclingCarnivalSplit');
  const bottleHandResult = calculatePartnership(bottleHand);
  assert.notEqual(JSON.stringify(domestique.participants.map((item) => item.capacity)), JSON.stringify(bottleHand.participants.map((item) => item.capacity)));
  assert.notEqual(result.effectiveVolume, bottleHandResult.effectiveVolume);
  assert.deepEqual(bottleHand.participants.map((item) => item.capacity), [8850, 9950, 7300]);
  assert.equal(bottleHand.deal.monthlyVolume, 7300);
  assert.equal(bottleHand.participants[2].variableCostPerTransaction, 1.18);
  const leadOut = clonePreset('leadOutCyclingCarnivalSplit');
  const leadOutResult = calculatePartnership(leadOut);
  assert.notEqual(JSON.stringify(domestique.participants.map((item) => item.capacity)), JSON.stringify(leadOut.participants.map((item) => item.capacity)));
  assert.notEqual(result.effectiveVolume, leadOutResult.effectiveVolume);
  assert.deepEqual(leadOut.participants.map((item) => item.capacity), [8400, 9500, 7000]);
  assert.equal(leadOut.deal.monthlyVolume, 7000);
  assert.equal(leadOut.participants[2].variableCostPerTransaction, 1.24);
  ...
  const hillClimbResult = calculatePartnership(hillClimb);
  const timeTrialResult = calculatePartnership(timeTrial);
  assert.notEqual(JSON.stringify(domestique.participants.map((item) => item.capacity)), JSON.stringify(feedZone.participants.map((item) => item.capacity)));
  assert.notEqual(JSON.stringify(domestique.participants.map((item) => item.capacity)), JSON.stringify(firstAid.participants.map((item) => item.capacity)));
  assert.notEqual(JSON.stringify(domestique.participants.map((item) => item.capacity)), JSON.stringify(teamSprint.participants.map((item) => item.capacity)));
  assert.notEqual(JSON.stringify(domestique.participants.map((item) => item.capacity)), JSON.stringify(teamPursuit.participants.map((item) => item.capacity)));
  assert.notEqual(JSON.stringify(domestique.participants.map((item) => item.capacity)), JSON.stringify(pursuit.participants.map((item) => item.capacity)));
  assert.notEqual(JSON.stringify(domestique.participants.map((item) => item.capacity)), JSON.stringify(scratch.participants.map((item) => item.capacity)));
  assert.notEqual(JSON.stringify(domestique.participants.map((item) => item.capacity)), JSON.stringify(pointsRace.participants.map((item) => item.capacity)));
  assert.notEqual(JSON.stringify(domestique.participants.map((item) => item.capacity)), JSON.stringify(omnium.participants.map((item) => item.capacity)));
  assert.notEqual(JSON.stringify(domestique.participants.map((item) => item.capacity)), JSON.stringify(madison.participants.map((item) => item.capacity)));
  assert.notEqual(JSON.stringify(domestique.participants.map((item) => item.capacity)), JSON.stringify(keirin.participants.map((item) => item.capacity)));
  assert.notEqual(JSON.stringify(domestique.participants.map((item) => item.capacity)), JSON.stringify(hillClimb.participants.map((item) => item.capacity)));
  assert.notEqual(JSON.stringify(domestique.participants.map((item) => item.capacity)), JSON.stringify(timeTrial.participants.map((item) => item.capacity)));
  assert.notEqual(result.effectiveVolume, feedZoneResult.effectiveVolume);
  assert.notEqual(result.effectiveVolume, firstAidResult.effectiveVolume);
  assert.notEqual(result.effectiveVolume, teamSprintResult.effectiveVolume);
  assert.notEqual(result.effectiveVolume, teamPursuitResult.effectiveVolume);
  ...
  const cycloCross = clonePreset('cycloCrossCarnivalSplit');
  assert.equal(cycloCross.participants[2].variableCostPerTransaction, 1.56);
  const lastZeroShare = [...domestique.participants].reverse().find((item) => item.revenueShare === 0);
  assert.equal(lastZeroShare, undefined);
  const over = clonePreset('domestiqueCyclingCarnivalSplit');
  over.deal.monthlyVolume = 9200;
  const overResult = calculatePartnership(over);
  const firstOver = over.participants.find((item) => item.capacity < overResult.effectiveVolume);
  const lastOver = [...over.participants].reverse().find((item) => item.capacity < overResult.effectiveVolume);
  assert.equal(overResult.effectiveVolume, 9200);
  assert.equal(firstOver.id, 'domestique-cycling-committee');
  assert.equal(lastOver.id, 'domestique-cycling-first-aid');
  assert.notEqual(firstOver.id, lastOver.id);
  assert.equal(lastOver.capacity - overResult.effectiveVolume, -2000);
  assert.equal(firstOver.capacity - overResult.effectiveVolume, -450);
});

test('Bottle-hand cycling carnival remaining listed capacity is distinct from domestique soigneur lead-out feed-zone first-aid team-sprint team-pursuit individual-pursuit scratch points-race omnium madison keirin hill-climb time-trial criterium road gravel track and cyclo-cross', () => {
  const bottleHand = clonePreset('bottleHandCyclingCarnivalSplit');
  const result = calculatePartnership(bottleHand);
  assert.equal(result.effectiveVolume, 7300);
  const remaining = bottleHand.participants.map((item) => item.capacity - result.effectiveVolume);
  assert.deepEqual(remaining, [1550, 2650, 0]);
  const lastWithin = [...bottleHand.participants].reverse().find((item) => item.capacity - result.effectiveVolume >= -1e-9);
  assert.equal(lastWithin.id, 'bottle-hand-cycling-first-aid');
  const lastSpare = [...bottleHand.participants].reverse().find((item) => item.capacity - result.effectiveVolume > 1e-9);
  assert.equal(lastSpare.id, 'bottle-hand-cycling-club-hire');
  const firstSpare = bottleHand.participants.find((item) => item.capacity - result.effectiveVolume > 1e-9);
  assert.equal(firstSpare.id, 'bottle-hand-cycling-committee');
  assert.notEqual(lastSpare.id, lastWithin.id);
  const domestique = clonePreset('domestiqueCyclingCarnivalSplit');
  const domestiqueResult = calculatePartnership(domestique);
  assert.notEqual(JSON.stringify(bottleHand.participants.map((item) => item.capacity)), JSON.stringify(domestique.participants.map((item) => item.capacity)));
  assert.notEqual(result.effectiveVolume, domestiqueResult.effectiveVolume);
  assert.deepEqual(domestique.participants.map((item) => item.capacity), [8750, 9850, 7200]);
  assert.equal(domestique.deal.monthlyVolume, 7200);
  assert.equal(domestique.participants[2].variableCostPerTransaction, 1.20);
  const soigneur = clonePreset('soigneurCyclingCarnivalSplit');
  const soigneurResult = calculatePartnership(soigneur);
  assert.notEqual(JSON.stringify(bottleHand.participants.map((item) => item.capacity)), JSON.stringify(soigneur.participants.map((item) => item.capacity)));
  assert.notEqual(result.effectiveVolume, soigneurResult.effectiveVolume);
  assert.deepEqual(soigneur.participants.map((item) => item.capacity), [8550, 9650, 7100]);
  assert.equal(soigneur.deal.monthlyVolume, 7100);
  assert.equal(soigneur.participants[2].variableCostPerTransaction, 1.22);
  const leadOut = clonePreset('leadOutCyclingCarnivalSplit');
  const leadOutResult = calculatePartnership(leadOut);
  assert.notEqual(JSON.stringify(bottleHand.participants.map((item) => item.capacity)), JSON.stringify(leadOut.participants.map((item) => item.capacity)));
  assert.notEqual(result.effectiveVolume, leadOutResult.effectiveVolume);
  assert.deepEqual(leadOut.participants.map((item) => item.capacity), [8400, 9500, 7000]);
  assert.equal(leadOut.deal.monthlyVolume, 7000);
  assert.equal(leadOut.participants[2].variableCostPerTransaction, 1.24);
  ...
  const hillClimbResult = calculatePartnership(hillClimb);
  const timeTrialResult = calculatePartnership(timeTrial);
  assert.notEqual(JSON.stringify(bottleHand.participants.map((item) => item.capacity)), JSON.stringify(feedZone.participants.map((item) => item.capacity)));
  assert.notEqual(JSON.stringify(bottleHand.participants.map((item) => item.capacity)), JSON.stringify(firstAid.participants.map((item) => item.capacity)));
  assert.notEqual(JSON.stringify(bottleHand.participants.map((item) => item.capacity)), JSON.stringify(teamSprint.participants.map((item) => item.capacity)));
  assert.notEqual(JSON.stringify(bottleHand.participants.map((item) => item.capacity)), JSON.stringify(teamPursuit.participants.map((item) => item.capacity)));
  assert.notEqual(JSON.stringify(bottleHand.participants.map((item) => item.capacity)), JSON.stringify(pursuit.participants.map((item) => item.capacity)));
  assert.notEqual(JSON.stringify(bottleHand.participants.map((item) => item.capacity)), JSON.stringify(scratch.participants.map((item) => item.capacity)));
  assert.notEqual(JSON.stringify(bottleHand.participants.map((item) => item.capacity)), JSON.stringify(pointsRace.participants.map((item) => item.capacity)));
  assert.notEqual(JSON.stringify(bottleHand.participants.map((item) => item.capacity)), JSON.stringify(omnium.participants.map((item) => item.capacity)));
  assert.notEqual(JSON.stringify(bottleHand.participants.map((item) => item.capacity)), JSON.stringify(madison.participants.map((item) => item.capacity)));
  assert.notEqual(JSON.stringify(bottleHand.participants.map((item) => item.capacity)), JSON.stringify(keirin.participants.map((item) => item.capacity)));
  assert.notEqual(JSON.stringify(bottleHand.participants.map((item) => item.capacity)), JSON.stringify(hillClimb.participants.map((item) => item.capacity)));
  assert.notEqual(JSON.stringify(bottleHand.participants.map((item) => item.capacity)), JSON.stringify(timeTrial.participants.map((item) => item.capacity)));
  assert.notEqual(result.effectiveVolume, feedZoneResult.effectiveVolume);
  assert.notEqual(result.effectiveVolume, firstAidResult.effectiveVolume);
  assert.notEqual(result.effectiveVolume, teamSprintResult.effectiveVolume);
  assert.notEqual(result.effectiveVolume, teamPursuitResult.effectiveVolume);
  ...
  const cycloCross = clonePreset('cycloCrossCarnivalSplit');
  assert.equal(cycloCross.participants[2].variableCostPerTransaction, 1.56);
  const lastZeroShare = [...bottleHand.participants].reverse().find((item) => item.revenueShare === 0);
  assert.equal(lastZeroShare, undefined);
  const over = clonePreset('bottleHandCyclingCarnivalSplit');
  over.deal.monthlyVolume = 9300;
  const overResult = calculatePartnership(over);
  const firstOver = over.participants.find((item) => item.capacity < overResult.effectiveVolume);
  const lastOver = [...over.participants].reverse().find((item) => item.capacity < overResult.effectiveVolume);
  assert.equal(overResult.effectiveVolume, 9300);
  assert.equal(firstOver.id, 'bottle-hand-cycling-committee');
  assert.equal(lastOver.id, 'bottle-hand-cycling-first-aid');
  assert.notEqual(firstOver.id, lastOver.id);
  assert.equal(lastOver.capacity - overResult.effectiveVolume, -2000);
  assert.equal(firstOver.capacity - overResult.effectiveVolume, -450);
});
test('Echelon cycling carnival split preset is a distinct committee club-hire first-aid starting point', () => {
  const keys = Object.keys(PRESETS);
  assert.ok(keys.indexOf('bottleHandCyclingCarnivalSplit') < keys.indexOf('echelonCyclingCarnivalSplit'));
  assert.equal(keys.indexOf('echelonCyclingCarnivalSplit'), keys.indexOf('bottleHandCyclingCarnivalSplit') + 1);
  assert.equal(keys.indexOf('bottleHandCyclingCarnivalSplit'), keys.indexOf('domestiqueCyclingCarnivalSplit') + 1);
  assert.equal(keys.indexOf('domestiqueCyclingCarnivalSplit'), keys.indexOf('soigneurCyclingCarnivalSplit') + 1);
  const echelon = clonePreset('echelonCyclingCarnivalSplit');
  const bottleHand = clonePreset('bottleHandCyclingCarnivalSplit');
  const domestique = clonePreset('domestiqueCyclingCarnivalSplit');
  const soigneur = clonePreset('soigneurCyclingCarnivalSplit');
  const leadOut = clonePreset('leadOutCyclingCarnivalSplit');
  const feedZone = clonePreset('feedZoneCyclingCarnivalSplit');
  const firstAid = clonePreset('firstAidCyclingCarnivalSplit');
  assert.equal(PRESETS.echelonCyclingCarnivalSplit.name, 'Echelon cycling carnival split');
  assert.equal(echelon.participants.length, 3);
  assert.deepEqual(echelon.participants.map((item) => item.id), ['echelon-cycling-committee', 'echelon-cycling-club-hire', 'echelon-cycling-first-aid']);
  assert.deepEqual(echelon.participants.map((item) => item.name), ['Carnival committee', 'Echelon cycling club hire', 'First-aid']);
  assert.deepEqual(echelon.participants.map((item) => item.revenueShare), [0.37, 0.38, 0.25]);
  assert.equal(echelon.deal.monthlyVolume, 7400);
  assert.equal(echelon.deal.feePerTransaction, 8);
  assert.equal(echelon.deal.addressableVolume, 9400);
  assert.deepEqual(echelon.participants.map((item) => item.capacity), [8950, 10050, 7400]);
  assert.equal(echelon.participants[2].variableCostPerTransaction, 1.16);
  assert.notEqual(echelon.participants[2].variableCostPerTransaction, 1.18);
  assert.notEqual(echelon.participants[2].variableCostPerTransaction, 1.20);
  assert.notEqual(echelon.participants[2].variableCostPerTransaction, 1.56);
  assert.notEqual(echelon.participants[2].variableCostPerTransaction, 1.46);
  assert.notEqual(echelon.participants[2].variableCostPerTransaction, 1.48);
  assert.notEqual(echelon.participants[2].variableCostPerTransaction, 1.50);
  assert.notEqual(echelon.participants[2].variableCostPerTransaction, 1.44);
  assert.notEqual(echelon.participants[2].variableCostPerTransaction, 1.42);
  assert.notEqual(echelon.participants[2].variableCostPerTransaction, 1.40);
  assert.notEqual(echelon.participants[2].variableCostPerTransaction, 1.38);
  assert.notEqual(echelon.participants[2].variableCostPerTransaction, 1.36);
  assert.notEqual(echelon.participants[2].variableCostPerTransaction, 1.34);
  assert.notEqual(echelon.participants[2].variableCostPerTransaction, 1.32);
  assert.notEqual(echelon.participants[2].variableCostPerTransaction, 1.30);
  assert.notEqual(echelon.participants[2].variableCostPerTransaction, 1.28);
  assert.notEqual(echelon.participants[2].variableCostPerTransaction, 1.26);
  assert.notEqual(echelon.participants[2].variableCostPerTransaction, 1.24);
  assert.notEqual(echelon.participants[2].variableCostPerTransaction, 1.22);
  assert.ok(echelon.participants.every((item) => item.capacity != null && Number.isFinite(item.capacity)));
  assert.notEqual(echelon.participants[0].variableCostPerTransaction, echelon.participants[1].variableCostPerTransaction);
  assert.notEqual(echelon.participants[1].variableCostPerTransaction, echelon.participants[2].variableCostPerTransaction);
  assert.notEqual(echelon.participants[0].fixedMonthlyCost, echelon.participants[1].fixedMonthlyCost);
  assert.notEqual(echelon.participants[1].fixedMonthlyCost, echelon.participants[2].fixedMonthlyCost);
  const others = ['balanced', 'thinMargin', 'growthAtCost', 'creatorTakeRate', 'threePartyJv', 'twoPartyStudio', 'fourPartyMarketplace', 'licensorDistributor', 'talentAgentPlatform', 'threePartyJointVenture', 'podcastHostNetwork', 'communityHallSplit', 'festivalStallSplit', 'popupCinemaSplit', 'communityRadioSplit', 'schoolConcertSplit', 'sportsCarnivalSplit', 'netballCarnivalSplit', 'swimmingCarnivalSplit', 'athleticsCarnivalSplit', 'cricketCarnivalSplit', 'tennisCarnivalSplit', 'basketballCarnivalSplit', 'volleyballCarnivalSplit', 'rugbyCarnivalSplit', 'hockeyCarnivalSplit', 'baseballCarnivalSplit', 'softballCarnivalSplit', 'lacrosseCarnivalSplit', 'waterPoloCarnivalSplit', 'rowingCarnivalSplit', 'sailingCarnivalSplit', 'canoeingCarnivalSplit', 'kayakingCarnivalSplit', 'dragonBoatCarnivalSplit', 'surfCarnivalSplit', 'triathlonCarnivalSplit', 'cyclingCarnivalSplit', 'mountainBikeCarnivalSplit', 'bmxCarnivalSplit', 'cycloCrossCarnivalSplit', 'trackCyclingCarnivalSplit', 'gravelCyclingCarnivalSplit', 'roadCyclingCarnivalSplit', 'criteriumCyclingCarnivalSplit', 'timeTrialCyclingCarnivalSplit', 'hillClimbCyclingCarnivalSplit', 'keirinCyclingCarnivalSplit', 'madisonCyclingCarnivalSplit', 'omniumCyclingCarnivalSplit', 'pointsRaceCyclingCarnivalSplit', 'scratchCyclingCarnivalSplit', 'individualPursuitCyclingCarnivalSplit', 'teamPursuitCyclingCarnivalSplit', 'teamSprintCyclingCarnivalSplit', 'firstAidCyclingCarnivalSplit', 'feedZoneCyclingCarnivalSplit', 'leadOutCyclingCarnivalSplit', 'soigneurCyclingCarnivalSplit', 'domestiqueCyclingCarnivalSplit', 'bottleHandCyclingCarnivalSplit'];
  for (const key of others) {
    const other = clonePreset(key);
    assert.notEqual(echelon.participants.map((item) => item.id).join(','), other.participants.map((item) => item.id).join(','), key);
    assert.notEqual(JSON.stringify(echelon.deal), JSON.stringify(other.deal), key);
    assert.notEqual(JSON.stringify(echelon.participants.map((item) => item.variableCostPerTransaction)), JSON.stringify(other.participants.map((item) => item.variableCostPerTransaction)), key);
    assert.notEqual(JSON.stringify(echelon.participants.map((item) => item.fixedMonthlyCost)), JSON.stringify(other.participants.map((item) => item.fixedMonthlyCost)), key);
  }
  const teamSprint = clonePreset('teamSprintCyclingCarnivalSplit');
  const teamPursuit = clonePreset('teamPursuitCyclingCarnivalSplit');
  const pursuit = clonePreset('individualPursuitCyclingCarnivalSplit');
  ...
  const track = clonePreset('trackCyclingCarnivalSplit');
  const cycloCross = clonePreset('cycloCrossCarnivalSplit');
  assert.notEqual(JSON.stringify(echelon), JSON.stringify(bottleHand));
  assert.notEqual(JSON.stringify(echelon), JSON.stringify(domestique));
  assert.notEqual(JSON.stringify(echelon), JSON.stringify(soigneur));
  assert.notEqual(JSON.stringify(echelon), JSON.stringify(leadOut));
  assert.notEqual(JSON.stringify(echelon), JSON.stringify(feedZone));
  assert.notEqual(JSON.stringify(echelon), JSON.stringify(firstAid));
  assert.equal(soigneur.participants[2].variableCostPerTransaction, 1.22);
  assert.equal(leadOut.participants[2].variableCostPerTransaction, 1.24);
  assert.notEqual(JSON.stringify(echelon), JSON.stringify(teamSprint));
  assert.notEqual(JSON.stringify(echelon), JSON.stringify(teamPursuit));
  assert.notEqual(JSON.stringify(echelon), JSON.stringify(pursuit));
  assert.notEqual(JSON.stringify(echelon), JSON.stringify(scratch));
  assert.notEqual(JSON.stringify(echelon), JSON.stringify(pointsRace));
  assert.notEqual(JSON.stringify(echelon), JSON.stringify(omnium));
  assert.notEqual(JSON.stringify(echelon), JSON.stringify(madison));
  assert.notEqual(JSON.stringify(echelon), JSON.stringify(keirin));
  assert.notEqual(JSON.stringify(echelon), JSON.stringify(hillClimb));
  assert.notEqual(echelon.participants[1].id, 'domestique-cycling-club-hire');
  assert.notEqual(echelon.participants[1].id, 'soigneur-cycling-club-hire');
  assert.notEqual(echelon.participants[1].id, 'lead-out-cycling-club-hire');
  assert.notEqual(echelon.participants[1].id, 'feed-zone-cycling-club-hire');
  assert.notEqual(echelon.participants[1].id, 'first-aid-cycling-club-hire');
  assert.notEqual(echelon.participants[1].id, 'team-sprint-cycling-club-hire');
  assert.notEqual(echelon.participants[1].id, 'team-pursuit-cycling-club-hire');
  assert.notEqual(echelon.participants[1].id, 'individual-pursuit-cycling-club-hire');
  assert.notEqual(echelon.participants[1].id, 'scratch-cycling-club-hire');
  assert.notEqual(echelon.participants[1].id, 'points-race-cycling-club-hire');
  assert.notEqual(echelon.participants[1].id, 'omnium-cycling-club-hire');
  assert.equal(echelon.participants[1].name, 'Echelon cycling club hire');
  assert.match(echelon.participants[1].name, /Echelon cycling club/);
  assert.doesNotMatch(echelon.participants[1].name, /Domestique cycling/);
  assert.doesNotMatch(echelon.participants[1].name, /Soigneur cycling/);
  assert.doesNotMatch(echelon.participants[1].name, /Lead-out cycling/);
  assert.doesNotMatch(echelon.participants[1].name, /Feed-zone cycling/);
  assert.doesNotMatch(echelon.participants[1].name, /First-aid cycling/);
  assert.doesNotMatch(echelon.participants[1].name, /Team-sprint cycling/);
  assert.doesNotMatch(echelon.participants[1].name, /Team-pursuit cycling/);
  assert.doesNotMatch(echelon.participants[1].name, /Individual-pursuit cycling/);
  assert.doesNotMatch(echelon.participants[1].name, /Scratch cycling/);
  assert.doesNotMatch(echelon.participants[1].name, /Points-race cycling/);
  assert.doesNotMatch(echelon.participants[1].name, /Omnium cycling/);
  assert.doesNotMatch(echelon.participants[1].name, /Madison cycling/);
  assert.doesNotMatch(echelon.participants[1].name, /Keirin cycling/);
  assert.doesNotMatch(echelon.participants[1].name, /Hill-climb cycling/);
  assert.doesNotMatch(echelon.participants[1].name, /Time-trial cycling/);
  assert.doesNotMatch(echelon.participants[1].name, /Criterium cycling/);
  assert.doesNotMatch(echelon.participants[1].name, /soigneur-station/i);
  assert.doesNotMatch(echelon.participants[1].name, /cape-bag/i);
  assert.doesNotMatch(echelon.participants[1].name, /rice-cake/i);
  assert.doesNotMatch(echelon.participants[1].name, /start-ramp/i);
  assert.doesNotMatch(echelon.participants[1].name, /time-check/i);
  assert.doesNotMatch(echelon.participants[1].name, /timing-hut/i);
  assert.doesNotMatch(echelon.participants[1].name, /pit-lane/i);
  assert.doesNotMatch(echelon.participants[1].name, /sealed-road/i);
  assert.doesNotMatch(echelon.participants[1].name, /circuit/i);
  assert.doesNotMatch(echelon.participants[1].name, /train-line/i);
  assert.doesNotMatch(echelon.participants[1].name, /last-rider/i);
  assert.doesNotMatch(echelon.participants[1].name, /sprinter-launch/i);
  assert.doesNotMatch(echelon.participants[1].name, /musette-line/i);
  assert.doesNotMatch(echelon.participants[1].name, /sticky-bottle/i);
  assert.doesNotMatch(echelon.participants[1].name, /echelon-up/i);
  assert.doesNotMatch(echelon.participants[1].name, /treatment-tent/i);
  assert.doesNotMatch(echelon.participants[1].name, /ice-pack-rota/i);
  assert.doesNotMatch(echelon.participants[1].name, /triage-board/i);
  assert.doesNotMatch(echelon.participants[1].name, /banked-boards/i);
  assert.doesNotMatch(echelon.participants[1].name, /summit-marshal/i);
  assert.doesNotMatch(echelon.participants[1].name, /hairpin/i);
  assert.doesNotMatch(echelon.participants[1].name, /climb-chip/i);
  assert.doesNotMatch(echelon.participants[1].name, /derny-pacer/i);
  assert.doesNotMatch(echelon.participants[1].name, /keirin-chip/i);
  assert.doesNotMatch(echelon.participants[1].name, /keirin-gate/i);
  assert.doesNotMatch(echelon.participants[1].name, /madison-pair/i);
  assert.doesNotMatch(echelon.participants[1].name, /handsling/i);
  assert.doesNotMatch(echelon.participants[1].name, /omnium-board/i);
  assert.doesNotMatch(echelon.participants[1].name, /flying-200/i);
  assert.doesNotMatch(echelon.participants[1].name, /elimination-heat/i);
  assert.doesNotMatch(echelon.participants[1].name, /tempo-lap/i);
  assert.doesNotMatch(echelon.participants[1].name, /points-race/i);
  assert.doesNotMatch(echelon.participants[1].name, /points-sprint/i);
  assert.doesNotMatch(echelon.participants[1].name, /bell-lap/i);
  assert.doesNotMatch(echelon.participants[1].name, /prime-board/i);
  assert.doesNotMatch(echelon.participants[1].name, /scratch-bunch/i);
  assert.doesNotMatch(echelon.participants[1].name, /bunch-finish/i);
  assert.doesNotMatch(echelon.participants[1].name, /scratch-chip/i);
  assert.doesNotMatch(echelon.participants[1].name, /pursuit-line/i);
  assert.doesNotMatch(echelon.participants[1].name, /catch-bell/i);
  assert.doesNotMatch(echelon.participants[1].name, /pursuit-chip/i);
  assert.doesNotMatch(echelon.participants[1].name, /team-pursuit-rota/i);
  assert.doesNotMatch(echelon.participants[1].name, /changeover-bell/i);
  assert.doesNotMatch(echelon.participants[1].name, /quartet-board/i);
  assert.doesNotMatch(echelon.participants[1].name, /start-gate/i);
  assert.doesNotMatch(echelon.participants[1].name, /flying-relay/i);
  assert.doesNotMatch(echelon.participants[1].name, /trio-board/i);
  const payload = JSON.stringify(echelon);
  assert.doesNotMatch(payload, /live roster/i);
  assert.doesNotMatch(payload, /hosted/i);
  assert.doesNotMatch(payload, /\bapi\b/i);
  assert.doesNotMatch(payload, /forecast/i);
  ...
  assert.doesNotMatch(payload, /flying-relay/i);
  assert.doesNotMatch(payload, /trio-board/i);
  assert.match(payload, /Echelon cycling club hire/);
  assert.doesNotMatch(payload, /Domestique cycling club hire/);
  assert.doesNotMatch(payload, /Soigneur cycling club hire/);
  assert.doesNotMatch(payload, /Lead-out cycling club hire/);
  assert.doesNotMatch(payload, /train-line/i);
  ...
  assert.doesNotMatch(payload, /musette-line/i);
  assert.doesNotMatch(payload, /sticky-bottle/i);
  assert.doesNotMatch(payload, /echelon-up/i);
  assert.doesNotMatch(payload, /treatment-tent/i);
  assert.doesNotMatch(payload, /ice-pack-rota/i);
  assert.doesNotMatch(payload, /triage-board/i);
  assert.equal(cycloCross.participants[2].variableCostPerTransaction, 1.56);
  ...
  assert.equal(teamSprint.participants[2].variableCostPerTransaction, 1.30);
  assert.equal(timeTrial.participants[2].variableCostPerTransaction, 1.48);
  const result = calculatePartnership(echelon);
  assert.equal(validateConfiguration(echelon).valid, true);
  assert.equal(domestique.participants[2].variableCostPerTransaction, 1.20);
  assert.equal(soigneur.participants[2].variableCostPerTransaction, 1.22);
  assert.equal(feedZone.participants[2].variableCostPerTransaction, 1.26);
  assert.equal(firstAid.participants[2].variableCostPerTransaction, 1.28);
  assert.equal(new Set(result.participants.map((item) => item.id)).size, 3);
  assert.equal(result.effectiveVolume, 7400);
  assert.equal(result.viable, true);
  assert.ok(result.participants.every((item) => item.profitPass));
  assert.equal(track.deal.monthlyVolume, 5400);
  assert.equal(gravel.deal.monthlyVolume, 5500);
...
test('Domestique cycling carnival remaining listed capacity is distinct from soigneur lead-out feed-zone first-aid team-sprint team-pursuit individual-pursuit scratch points-race omnium madison keirin hill-climb time-trial criterium road gravel track and cyclo-cross', () => {
  ...
  assert.equal(soigneur.participants[2].variableCostPerTransaction, 1.22);
  const echelon = clonePreset('echelonCyclingCarnivalSplit');
  const echelonResult = calculatePartnership(echelon);
  assert.notEqual(JSON.stringify(domestique.participants.map((item) => item.capacity)), JSON.stringify(echelon.participants.map((item) => item.capacity)));
  assert.notEqual(result.effectiveVolume, echelonResult.effectiveVolume);
  assert.deepEqual(echelon.participants.map((item) => item.capacity), [8950, 10050, 7400]);
  assert.equal(echelon.deal.monthlyVolume, 7400);
  assert.equal(echelon.participants[2].variableCostPerTransaction, 1.16);
  const leadOut = clonePreset('leadOutCyclingCarnivalSplit');
...
test('Echelon cycling carnival remaining listed capacity is distinct from domestique soigneur lead-out feed-zone first-aid team-sprint team-pursuit individual-pursuit scratch points-race omnium madison keirin hill-climb time-trial criterium road gravel track and cyclo-cross', () => {
  const echelon = clonePreset('echelonCyclingCarnivalSplit');
  const result = calculatePartnership(echelon);
  assert.equal(result.effectiveVolume, 7400);
  const remaining = echelon.participants.map((item) => item.capacity - result.effectiveVolume);
  assert.deepEqual(remaining, [1550, 2650, 0]);
  const lastWithin = [...echelon.participants].reverse().find((item) => item.capacity - result.effectiveVolume >= -1e-9);
  assert.equal(lastWithin.id, 'echelon-cycling-first-aid');
  const lastSpare = [...echelon.participants].reverse().find((item) => item.capacity - result.effectiveVolume > 1e-9);
  assert.equal(lastSpare.id, 'echelon-cycling-club-hire');
  const firstSpare = echelon.participants.find((item) => item.capacity - result.effectiveVolume > 1e-9);
  assert.equal(firstSpare.id, 'echelon-cycling-committee');
  assert.notEqual(lastSpare.id, lastWithin.id);
  const bottleHandPrev = clonePreset('bottleHandCyclingCarnivalSplit');
  const bottleHandPrevResult = calculatePartnership(bottleHandPrev);
  assert.notEqual(JSON.stringify(echelon.participants.map((item) => item.capacity)), JSON.stringify(bottleHandPrev.participants.map((item) => item.capacity)));
  assert.notEqual(result.effectiveVolume, bottleHandPrevResult.effectiveVolume);
  const domestique = clonePreset('domestiqueCyclingCarnivalSplit');
  const domestiqueResult = calculatePartnership(domestique);
  assert.notEqual(JSON.stringify(echelon.participants.map((item) => item.capacity)), JSON.stringify(domestique.participants.map((item) => item.capacity)));
  assert.notEqual(result.effectiveVolume, domestiqueResult.effectiveVolume);
  assert.deepEqual(domestique.participants.map((item) => item.capacity), [8750, 9850, 7200]);
  assert.equal(domestique.deal.monthlyVolume, 7200);
  assert.equal(domestique.participants[2].variableCostPerTransaction, 1.20);
  const soigneur = clonePreset('soigneurCyclingCarnivalSplit');
  const soigneurResult = calculatePartnership(soigneur);
  assert.notEqual(JSON.stringify(echelon.participants.map((item) => item.capacity)), JSON.stringify(soigneur.participants.map((item) => item.capacity)));
  assert.notEqual(result.effectiveVolume, soigneurResult.effectiveVolume);
  assert.deepEqual(soigneur.participants.map((item) => item.capacity), [8550, 9650, 7100]);
  assert.equal(soigneur.deal.monthlyVolume, 7100);
  assert.equal(soigneur.participants[2].variableCostPerTransaction, 1.22);
  const leadOut = clonePreset('leadOutCyclingCarnivalSplit');
  const leadOutResult = calculatePartnership(leadOut);
  assert.notEqual(JSON.stringify(echelon.participants.map((item) => item.capacity)), JSON.stringify(leadOut.participants.map((item) => item.capacity)));
  assert.notEqual(result.effectiveVolume, leadOutResult.effectiveVolume);
  assert.deepEqual(leadOut.participants.map((item) => item.capacity), [8400, 9500, 7000]);
  assert.equal(leadOut.deal.monthlyVolume, 7000);
  assert.equal(leadOut.participants[2].variableCostPerTransaction, 1.24);
  ...
  const hillClimbResult = calculatePartnership(hillClimb);
  const timeTrialResult = calculatePartnership(timeTrial);
  assert.notEqual(JSON.stringify(echelon.participants.map((item) => item.capacity)), JSON.stringify(feedZone.participants.map((item) => item.capacity)));
  assert.notEqual(JSON.stringify(echelon.participants.map((item) => item.capacity)), JSON.stringify(firstAid.participants.map((item) => item.capacity)));
  assert.notEqual(JSON.stringify(echelon.participants.map((item) => item.capacity)), JSON.stringify(teamSprint.participants.map((item) => item.capacity)));
  assert.notEqual(JSON.stringify(echelon.participants.map((item) => item.capacity)), JSON.stringify(teamPursuit.participants.map((item) => item.capacity)));
  assert.notEqual(JSON.stringify(echelon.participants.map((item) => item.capacity)), JSON.stringify(pursuit.participants.map((item) => item.capacity)));
  assert.notEqual(JSON.stringify(echelon.participants.map((item) => item.capacity)), JSON.stringify(scratch.participants.map((item) => item.capacity)));
  assert.notEqual(JSON.stringify(echelon.participants.map((item) => item.capacity)), JSON.stringify(pointsRace.participants.map((item) => item.capacity)));
  assert.notEqual(JSON.stringify(echelon.participants.map((item) => item.capacity)), JSON.stringify(omnium.participants.map((item) => item.capacity)));
  assert.notEqual(JSON.stringify(echelon.participants.map((item) => item.capacity)), JSON.stringify(madison.participants.map((item) => item.capacity)));
  assert.notEqual(JSON.stringify(echelon.participants.map((item) => item.capacity)), JSON.stringify(keirin.participants.map((item) => item.capacity)));
  assert.notEqual(JSON.stringify(echelon.participants.map((item) => item.capacity)), JSON.stringify(hillClimb.participants.map((item) => item.capacity)));
  assert.notEqual(JSON.stringify(echelon.participants.map((item) => item.capacity)), JSON.stringify(timeTrial.participants.map((item) => item.capacity)));
  assert.notEqual(result.effectiveVolume, feedZoneResult.effectiveVolume);
  assert.notEqual(result.effectiveVolume, firstAidResult.effectiveVolume);
  assert.notEqual(result.effectiveVolume, teamSprintResult.effectiveVolume);
  assert.notEqual(result.effectiveVolume, teamPursuitResult.effectiveVolume);
  ...
  const cycloCross = clonePreset('cycloCrossCarnivalSplit');
  assert.equal(cycloCross.participants[2].variableCostPerTransaction, 1.56);
  const lastZeroShare = [...echelon.participants].reverse().find((item) => item.revenueShare === 0);
  assert.equal(lastZeroShare, undefined);
  const over = clonePreset('echelonCyclingCarnivalSplit');
  over.deal.monthlyVolume = 9400;
  const overResult = calculatePartnership(over);
  const firstOver = over.participants.find((item) => item.capacity < overResult.effectiveVolume);
  const lastOver = [...over.participants].reverse().find((item) => item.capacity < overResult.effectiveVolume);
  assert.equal(overResult.effectiveVolume, 9400);
  assert.equal(firstOver.id, 'echelon-cycling-committee');
  assert.equal(lastOver.id, 'echelon-cycling-first-aid');
  assert.notEqual(firstOver.id, lastOver.id);
  assert.equal(lastOver.capacity - overResult.effectiveVolume, -2000);
  assert.equal(firstOver.capacity - overResult.effectiveVolume, -450);
});
