...
test('hill-climb cycling carnival split preset loads from the starting-point buttons', async () => {
  const app = await workbench();
  ...
  assert.ok(markup.indexOf('data-preset="criteriumCyclingCarnivalSplit"') < markup.indexOf('data-preset="timeTrialCyclingCarnivalSplit"'));
  assert.ok(markup.indexOf('data-preset="timeTrialCyclingCarnivalSplit"') < markup.indexOf('data-preset="hillClimbCyclingCarnivalSplit"'));
  app.click('preset', { preset: 'hillClimbCyclingCarnivalSplit' });
  assert.equal(app.saved().participants.length, 3);
  assert.deepEqual(app.saved().participants.map((item) => item.id), ['hill-climb-cycling-committee', 'hill-climb-cycling-club-hire', 'hill-climb-cycling-first-aid']);
  assert.deepEqual(app.saved().participants.map((item) => item.name), ['Carnival committee', 'Hill-climb cycling club hire', 'First-aid']);
  assert.equal(app.saved().deal.feePerTransaction, 8);
  assert.equal(app.saved().deal.monthlyVolume, 5900);
  assert.equal(app.saved().deal.addressableVolume, 7100);
  assert.deepEqual(app.saved().participants.map((item) => item.capacity), [6900, 8000, 5900]);
  assert.notEqual(app.saved().participants[0].variableCostPerTransaction, app.saved().participants[1].variableCostPerTransaction);
  assert.notEqual(app.saved().participants[1].variableCostPerTransaction, app.saved().participants[2].variableCostPerTransaction);
  assert.notEqual(app.saved().participants[0].fixedMonthlyCost, app.saved().participants[1].fixedMonthlyCost);
  assert.notEqual(app.saved().participants[1].fixedMonthlyCost, app.saved().participants[2].fixedMonthlyCost);
  assert.equal(app.saved().participants[2].variableCostPerTransaction, 1.46);
  assert.match(app.notice(), /Hill-climb cycling carnival split loaded/);
  assert.match(app.markup(), /Operating region holds/);
  assert.notEqual(app.saved().participants.map((item) => item.id).join(','), 'time-trial-cycling-committee,time-trial-cycling-club-hire,time-trial-cycling-first-aid');
  assert.notEqual(app.saved().participants.map((item) => item.id).join(','), 'criterium-cycling-committee,criterium-cycling-club-hire,criterium-cycling-first-aid');
  assert.equal(app.saved().participants.map((item) => item.name)[1], 'Hill-climb cycling club hire');
  const exported = JSON.stringify(app.saved());
  assert.doesNotMatch(exported, /live roster/i);
  assert.doesNotMatch(exported, /hosted/i);
  assert.doesNotMatch(exported, /\bapi\b/i);
  ...
  assert.doesNotMatch(exported, /time-check/i);
  assert.doesNotMatch(exported, /timing-hut/i);
  assert.doesNotMatch(exported, /pit-lane/i);
  assert.doesNotMatch(exported, /sealed-road/i);
  assert.doesNotMatch(exported, /circuit/i);
  assert.doesNotMatch(exported, /feed-zone/i);
  assert.doesNotMatch(exported, /banked-boards/i);
  assert.notEqual(app.saved().participants.map((item) => item.id).join(','), 'creator,platform');
});
...
test('cyclo-cross carnival first-aid stays 1.56 after loading hill-climb cycling carnival', async () => {
  const app = await workbench();
  app.click('preset', { preset: 'hillClimbCyclingCarnivalSplit' });
  assert.equal(app.saved().participants[2].variableCostPerTransaction, 1.46);
  app.click('preset', { preset: 'cycloCrossCarnivalSplit' });
...
test('keyboard last-over-capacity remaining copy shortcuts stay on hill-climb cycling carnival', async () => {
  const hillClimb = await workbench('file:', { clipboard: 'ok' });
  hillClimb.click('preset', { preset: 'hillClimbCyclingCarnivalSplit' });
  assert.match(hillClimb.markup(), /id="copy-last-over-capacity-remaining"[^>]*aria-keyshortcuts="\* Shift\+F7 Shift\+F10"/);
  assert.match(hillClimb.markup(), /id="hide-last-over-capacity-participant"[^>]*aria-keyshortcuts="# Shift\+F9"/);
  assert.match(hillClimb.markup(), /id="hide-first-over-capacity-participant"[^>]*aria-keyshortcuts="@"/);
  assert.doesNotMatch(hillClimb.markup(), /id="hide-first-over-capacity-participant"[^>]*aria-keyshortcuts="Shift\+F9"/);
  assert.match(hillClimb.markup(), /id="copy-first-over-capacity-remaining"[^>]*aria-keyshortcuts="~"/);
  assert.doesNotMatch(hillClimb.markup(), /id="copy-first-over-capacity-remaining"[^>]*aria-keyshortcuts="Shift\+F7"/);
  assert.doesNotMatch(hillClimb.markup(), /id="copy-first-over-capacity-volume"[^>]*aria-keyshortcuts="Shift\+F7"/);
  assert.match(hillClimb.markup(), /id="copy-first-zero-share-participant"[^>]*aria-keyshortcuts="F7"/);
  hillClimb.edit('deal.monthlyVolume', '7000');
  hillClimb.keydown('F7', { shiftKey: true });
  assert.equal(hillClimb.copied().at(-1), 'Last over-capacity remaining listed capacity: 1,100 txn over for First-aid. How far over listed capacity. Not a forecast.');
  hillClimb.keydown('F10', { shiftKey: true });
  assert.equal(hillClimb.copied().at(-1), 'Last over-capacity remaining listed capacity: 1,100 txn over for First-aid. How far over listed capacity. Not a forecast.');
  hillClimb.keydown('~');
  assert.equal(hillClimb.copied().at(-1), 'First over-capacity remaining listed capacity: 100 txn over for Carnival committee. How far over listed capacity. Not a forecast.');
  hillClimb.keydown('F7');
  assert.equal(hillClimb.copied().at(-1), 'First zero-share participant: none entered.');
  hillClimb.keydown('F8', { shiftKey: true });
  assert.ok(hillClimb.focused().includes('#copy-last-over-capacity-remaining'));
  hillClimb.keydown('F11', { shiftKey: true });
  assert.ok(hillClimb.focused().includes('#copy-last-over-capacity-remaining'));
  hillClimb.keydown('F8');
  assert.ok(hillClimb.focused().includes('#copy-first-zero-share-participant'));
  hillClimb.keydown('F9', { shiftKey: true });
  assert.ok(hillClimb.focused().includes('#hide-last-over-capacity-participant'));
  hillClimb.keydown('F9');
  assert.ok(hillClimb.focused().includes('#hide-first-zero-share-participant'));
  hillClimb.keydown('@');
  assert.ok(hillClimb.focused().includes('#hide-first-over-capacity-participant'));
  hillClimb.keydown('F12', { shiftKey: true });
  assert.ok(hillClimb.focused().includes('#hide-first-over-capacity-participant'));
  assert.equal(hillClimb.saved().participants[2].variableCostPerTransaction, 1.46);
  assert.doesNotMatch(JSON.stringify(hillClimb.saved()), /live roster/i);
  assert.doesNotMatch(JSON.stringify(hillClimb.saved()), /hosted/i);
  assert.doesNotMatch(JSON.stringify(hillClimb.saved()), /\bapi\b/i);
  assert.doesNotMatch(JSON.stringify(hillClimb.saved()), /forecast/i);
});
...
test('madison cycling carnival split preset loads from the starting-point buttons', async () => {
  const app = await workbench();
  assert.match(app.markup(), /data-preset="madisonCyclingCarnivalSplit"/);
  assert.match(app.markup(), /Madison cycling carnival split/);
  assert.match(app.markup(), /data-preset="keirinCyclingCarnivalSplit"/);
  assert.match(app.markup(), /Keirin cycling carnival split/);
  const markup = app.markup();
  assert.ok(markup.indexOf('data-preset="hillClimbCyclingCarnivalSplit"') < markup.indexOf('data-preset="keirinCyclingCarnivalSplit"'));
  assert.ok(markup.indexOf('data-preset="keirinCyclingCarnivalSplit"') < markup.indexOf('data-preset="madisonCyclingCarnivalSplit"'));
  app.click('preset', { preset: 'madisonCyclingCarnivalSplit' });
  assert.equal(app.saved().participants.length, 3);
  assert.deepEqual(app.saved().participants.map((item) => item.id), ['madison-cycling-committee', 'madison-cycling-club-hire', 'madison-cycling-first-aid']);
  assert.deepEqual(app.saved().participants.map((item) => item.name), ['Carnival committee', 'Madison cycling club hire', 'First-aid']);
  assert.equal(app.saved().deal.feePerTransaction, 8);
  assert.equal(app.saved().deal.monthlyVolume, 6100);
  assert.equal(app.saved().deal.addressableVolume, 7300);
  assert.deepEqual(app.saved().participants.map((item) => item.capacity), [7100, 8200, 6100]);
  assert.notEqual(app.saved().participants[0].variableCostPerTransaction, app.saved().participants[1].variableCostPerTransaction);
  assert.notEqual(app.saved().participants[1].variableCostPerTransaction, app.saved().participants[2].variableCostPerTransaction);
  assert.equal(app.saved().participants[2].variableCostPerTransaction, 1.42);
  assert.match(app.notice(), /Madison cycling carnival split loaded/);
  assert.match(app.markup(), /Operating region holds/);
  const exported = JSON.stringify(app.saved());
  assert.doesNotMatch(exported, /live roster/i);
  assert.doesNotMatch(exported, /hosted/i);
  ...
  assert.doesNotMatch(exported, /hairpin/i);
  assert.doesNotMatch(exported, /climb-chip/i);
  assert.doesNotMatch(exported, /derny-pacer/i);
  assert.doesNotMatch(exported, /keirin-chip/i);
  assert.doesNotMatch(exported, /keirin-gate/i);
});

test('cyclo-cross carnival first-aid stays 1.56 after loading madison cycling carnival', async () => {
  const app = await workbench();
  app.click('preset', { preset: 'madisonCyclingCarnivalSplit' });
  assert.equal(app.saved().participants[2].variableCostPerTransaction, 1.42);
  app.click('preset', { preset: 'cycloCrossCarnivalSplit' });
...
test('keyboard last-over-capacity remaining copy shortcuts stay on madison cycling carnival', async () => {
  const madison = await workbench('file:', { clipboard: 'ok' });
  madison.click('preset', { preset: 'madisonCyclingCarnivalSplit' });
  assert.match(madison.markup(), /id="copy-last-over-capacity-remaining"[^>]*aria-keyshortcuts="\* Shift\+F7 Shift\+F10"/);
  assert.match(madison.markup(), /id="hide-last-over-capacity-participant"[^>]*aria-keyshortcuts="# Shift\+F9"/);
  assert.match(madison.markup(), /id="hide-first-over-capacity-participant"[^>]*aria-keyshortcuts="@"/);
  assert.doesNotMatch(madison.markup(), /id="hide-first-over-capacity-participant"[^>]*aria-keyshortcuts="Shift\+F9"/);
  assert.match(madison.markup(), /id="copy-first-over-capacity-remaining"[^>]*aria-keyshortcuts="~"/);
  madison.edit('deal.monthlyVolume', '7200');
  madison.keydown('F7', { shiftKey: true });
  assert.equal(madison.copied().at(-1), 'Last over-capacity remaining listed capacity: 1,100 txn over for First-aid. How far over listed capacity. Not a forecast.');
  madison.keydown('F10', { shiftKey: true });
  assert.equal(madison.copied().at(-1), 'Last over-capacity remaining listed capacity: 1,100 txn over for First-aid. How far over listed capacity. Not a forecast.');
  madison.keydown('~');
  assert.equal(madison.copied().at(-1), 'First over-capacity remaining listed capacity: 100 txn over for Carnival committee. How far over listed capacity. Not a forecast.');
  madison.keydown('F7');
  assert.equal(madison.copied().at(-1), 'First zero-share participant: none entered.');
  madison.keydown('F8', { shiftKey: true });
  assert.ok(madison.focused().includes('#copy-last-over-capacity-remaining'));
  madison.keydown('F9', { shiftKey: true });
  assert.ok(madison.focused().includes('#hide-last-over-capacity-participant'));
  madison.keydown('F9');
  assert.ok(madison.focused().includes('#hide-first-zero-share-participant'));
  madison.keydown('@');
  assert.ok(madison.focused().includes('#hide-first-over-capacity-participant'));
  madison.keydown('F12', { shiftKey: true });
  assert.ok(madison.focused().includes('#hide-first-over-capacity-participant'));
  assert.equal(madison.saved().participants[2].variableCostPerTransaction, 1.42);
  assert.doesNotMatch(JSON.stringify(madison.saved()), /live roster/i);
  assert.doesNotMatch(JSON.stringify(madison.saved()), /hosted/i);
  assert.doesNotMatch(JSON.stringify(madison.saved()), /\bapi\b/i);
});

test('omnium cycling carnival split preset loads from the starting-point buttons', async () => {
  const app = await workbench();
  assert.match(app.markup(), /data-preset="omniumCyclingCarnivalSplit"/);
  assert.match(app.markup(), /Omnium cycling carnival split/);
  assert.match(app.markup(), /data-preset="madisonCyclingCarnivalSplit"/);
  assert.match(app.markup(), /Madison cycling carnival split/);
  const markup = app.markup();
  assert.ok(markup.indexOf('data-preset="keirinCyclingCarnivalSplit"') < markup.indexOf('data-preset="madisonCyclingCarnivalSplit"'));
  assert.ok(markup.indexOf('data-preset="madisonCyclingCarnivalSplit"') < markup.indexOf('data-preset="omniumCyclingCarnivalSplit"'));
  app.click('preset', { preset: 'omniumCyclingCarnivalSplit' });
  assert.equal(app.saved().participants.length, 3);
  assert.deepEqual(app.saved().participants.map((item) => item.id), ['omnium-cycling-committee', 'omnium-cycling-club-hire', 'omnium-cycling-first-aid']);
  assert.deepEqual(app.saved().participants.map((item) => item.name), ['Carnival committee', 'Omnium cycling club hire', 'First-aid']);
  assert.equal(app.saved().deal.feePerTransaction, 8);
  assert.equal(app.saved().deal.monthlyVolume, 6200);
  assert.equal(app.saved().deal.addressableVolume, 7400);
  assert.deepEqual(app.saved().participants.map((item) => item.capacity), [7200, 8300, 6200]);
  assert.notEqual(app.saved().participants[0].variableCostPerTransaction, app.saved().participants[1].variableCostPerTransaction);
  assert.notEqual(app.saved().participants[1].variableCostPerTransaction, app.saved().participants[2].variableCostPerTransaction);
  assert.equal(app.saved().participants[2].variableCostPerTransaction, 1.40);
  assert.match(app.notice(), /Omnium cycling carnival split loaded/);
  assert.match(app.markup(), /Operating region holds/);
  const exported = JSON.stringify(app.saved());
  assert.doesNotMatch(exported, /live roster/i);
  assert.doesNotMatch(exported, /hosted/i);
  ...
  assert.doesNotMatch(exported, /keirin-chip/i);
  assert.doesNotMatch(exported, /keirin-gate/i);
  assert.doesNotMatch(exported, /madison-pair/i);
  assert.doesNotMatch(exported, /handsling/i);
});

test('cyclo-cross carnival first-aid stays 1.56 after loading omnium cycling carnival', async () => {
  const app = await workbench();
  app.click('preset', { preset: 'omniumCyclingCarnivalSplit' });
  assert.equal(app.saved().participants[2].variableCostPerTransaction, 1.40);
  app.click('preset', { preset: 'cycloCrossCarnivalSplit' });
...
test('keyboard last-over-capacity remaining copy shortcuts stay on omnium cycling carnival', async () => {
  const omnium = await workbench('file:', { clipboard: 'ok' });
  omnium.click('preset', { preset: 'omniumCyclingCarnivalSplit' });
  assert.match(omnium.markup(), /id="copy-last-over-capacity-remaining"[^>]*aria-keyshortcuts="\* Shift\+F7 Shift\+F10"/);
  assert.match(omnium.markup(), /id="hide-last-over-capacity-participant"[^>]*aria-keyshortcuts="# Shift\+F9"/);
  assert.match(omnium.markup(), /id="hide-first-over-capacity-participant"[^>]*aria-keyshortcuts="@"/);
  assert.doesNotMatch(omnium.markup(), /id="hide-first-over-capacity-participant"[^>]*aria-keyshortcuts="Shift\+F9"/);
  assert.match(omnium.markup(), /id="copy-first-over-capacity-remaining"[^>]*aria-keyshortcuts="~"/);
  omnium.edit('deal.monthlyVolume', '7300');
  omnium.keydown('F7', { shiftKey: true });
  assert.equal(omnium.copied().at(-1), 'Last over-capacity remaining listed capacity: 1,100 txn over for First-aid. How far over listed capacity. Not a forecast.');
  omnium.keydown('F10', { shiftKey: true });
  assert.equal(omnium.copied().at(-1), 'Last over-capacity remaining listed capacity: 1,100 txn over for First-aid. How far over listed capacity. Not a forecast.');
  omnium.keydown('~');
  assert.equal(omnium.copied().at(-1), 'First over-capacity remaining listed capacity: 100 txn over for Carnival committee. How far over listed capacity. Not a forecast.');
  omnium.keydown('F7');
  assert.equal(omnium.copied().at(-1), 'First zero-share participant: none entered.');
  omnium.keydown('F8', { shiftKey: true });
  assert.ok(omnium.focused().includes('#copy-last-over-capacity-remaining'));
  omnium.keydown('F9', { shiftKey: true });
  assert.ok(omnium.focused().includes('#hide-last-over-capacity-participant'));
  omnium.keydown('F9');
  assert.ok(omnium.focused().includes('#hide-first-zero-share-participant'));
  omnium.keydown('@');
  assert.ok(omnium.focused().includes('#hide-first-over-capacity-participant'));
  omnium.keydown('F12', { shiftKey: true });
  assert.ok(omnium.focused().includes('#hide-first-over-capacity-participant'));
  assert.equal(omnium.saved().participants[2].variableCostPerTransaction, 1.40);
  assert.doesNotMatch(JSON.stringify(omnium.saved()), /live roster/i);
  assert.doesNotMatch(JSON.stringify(omnium.saved()), /hosted/i);
  assert.doesNotMatch(JSON.stringify(omnium.saved()), /\bapi\b/i);
});

test('points-race cycling carnival split preset loads from the starting-point buttons', async () => {
  const app = await workbench();
  assert.match(app.markup(), /data-preset="pointsRaceCyclingCarnivalSplit"/);
  assert.match(app.markup(), /Points-race cycling carnival split/);
  assert.match(app.markup(), /data-preset="omniumCyclingCarnivalSplit"/);
  assert.match(app.markup(), /Omnium cycling carnival split/);
  const markup = app.markup();
  assert.ok(markup.indexOf('data-preset="madisonCyclingCarnivalSplit"') < markup.indexOf('data-preset="omniumCyclingCarnivalSplit"'));
  assert.ok(markup.indexOf('data-preset="omniumCyclingCarnivalSplit"') < markup.indexOf('data-preset="pointsRaceCyclingCarnivalSplit"'));
  assert.ok(markup.indexOf('data-preset="pointsRaceCyclingCarnivalSplit"') < markup.indexOf('data-preset="scratchCyclingCarnivalSplit"'));
  app.click('preset', { preset: 'pointsRaceCyclingCarnivalSplit' });
  assert.equal(app.saved().participants.length, 3);
  assert.deepEqual(app.saved().participants.map((item) => item.id), ['points-race-cycling-committee', 'points-race-cycling-club-hire', 'points-race-cycling-first-aid']);
  assert.deepEqual(app.saved().participants.map((item) => item.name), ['Carnival committee', 'Points-race cycling club hire', 'First-aid']);
  assert.equal(app.saved().deal.feePerTransaction, 8);
  assert.equal(app.saved().deal.monthlyVolume, 6300);
  assert.equal(app.saved().deal.addressableVolume, 7500);
  assert.deepEqual(app.saved().participants.map((item) => item.capacity), [7350, 8450, 6300]);
  assert.notEqual(app.saved().participants[0].variableCostPerTransaction, app.saved().participants[1].variableCostPerTransaction);
  assert.notEqual(app.saved().participants[1].variableCostPerTransaction, app.saved().participants[2].variableCostPerTransaction);
  assert.equal(app.saved().participants[2].variableCostPerTransaction, 1.38);
  assert.match(app.notice(), /Points-race cycling carnival split loaded/);
  assert.match(app.markup(), /Operating region holds/);
  const exported = JSON.stringify(app.saved());
  assert.doesNotMatch(exported, /live roster/i);
  assert.doesNotMatch(exported, /hosted/i);
  ...
  assert.doesNotMatch(exported, /madison-pair/i);
  assert.doesNotMatch(exported, /handsling/i);
  assert.doesNotMatch(exported, /omnium-board/i);
  assert.doesNotMatch(exported, /flying-200/i);
  assert.doesNotMatch(exported, /elimination-heat/i);
  assert.doesNotMatch(exported, /tempo-lap/i);
});

test('cyclo-cross carnival first-aid stays 1.56 after loading points-race cycling carnival', async () => {
  const app = await workbench();
  app.click('preset', { preset: 'pointsRaceCyclingCarnivalSplit' });
  assert.equal(app.saved().participants[2].variableCostPerTransaction, 1.38);
  app.click('preset', { preset: 'cycloCrossCarnivalSplit' });
...
test('keyboard last-over-capacity remaining copy shortcuts stay on points-race cycling carnival', async () => {
  const pointsRace = await workbench('file:', { clipboard: 'ok' });
  pointsRace.click('preset', { preset: 'pointsRaceCyclingCarnivalSplit' });
  assert.match(pointsRace.markup(), /id="copy-last-over-capacity-remaining"[^>]*aria-keyshortcuts="\* Shift\+F7 Shift\+F10"/);
  assert.match(pointsRace.markup(), /id="hide-last-over-capacity-participant"[^>]*aria-keyshortcuts="# Shift\+F9"/);
  assert.match(pointsRace.markup(), /id="hide-first-over-capacity-participant"[^>]*aria-keyshortcuts="@"/);
  assert.doesNotMatch(pointsRace.markup(), /id="hide-first-over-capacity-participant"[^>]*aria-keyshortcuts="Shift\+F9"/);
  assert.match(pointsRace.markup(), /id="copy-first-over-capacity-remaining"[^>]*aria-keyshortcuts="~"/);
  pointsRace.edit('deal.monthlyVolume', '7500');
  pointsRace.keydown('F7', { shiftKey: true });
  assert.equal(pointsRace.copied().at(-1), 'Last over-capacity remaining listed capacity: 1,200 txn over for First-aid. How far over listed capacity. Not a forecast.');
  pointsRace.keydown('F10', { shiftKey: true });
  assert.equal(pointsRace.copied().at(-1), 'Last over-capacity remaining listed capacity: 1,200 txn over for First-aid. How far over listed capacity. Not a forecast.');
  pointsRace.keydown('~');
  assert.equal(pointsRace.copied().at(-1), 'First over-capacity remaining listed capacity: 150 txn over for Carnival committee. How far over listed capacity. Not a forecast.');
  pointsRace.keydown('F7');
  assert.equal(pointsRace.copied().at(-1), 'First zero-share participant: none entered.');
  pointsRace.keydown('F8', { shiftKey: true });
  assert.ok(pointsRace.focused().includes('#copy-last-over-capacity-remaining'));
  pointsRace.keydown('F9', { shiftKey: true });
  assert.ok(pointsRace.focused().includes('#hide-last-over-capacity-participant'));
  pointsRace.keydown('F9');
  assert.ok(pointsRace.focused().includes('#hide-first-zero-share-participant'));
  pointsRace.keydown('@');
  assert.ok(pointsRace.focused().includes('#hide-first-over-capacity-participant'));
  pointsRace.keydown('F12', { shiftKey: true });
  assert.ok(pointsRace.focused().includes('#hide-first-over-capacity-participant'));
  assert.equal(pointsRace.saved().participants[2].variableCostPerTransaction, 1.38);
  assert.doesNotMatch(JSON.stringify(pointsRace.saved()), /live roster/i);
  assert.doesNotMatch(JSON.stringify(pointsRace.saved()), /hosted/i);
  assert.doesNotMatch(JSON.stringify(pointsRace.saved()), /\bapi\b/i);
});

test('scratch cycling carnival split preset loads from the starting-point buttons', async () => {
  const app = await workbench();
  assert.match(app.markup(), /data-preset="scratchCyclingCarnivalSplit"/);
  assert.match(app.markup(), /Scratch cycling carnival split/);
  assert.match(app.markup(), /data-preset="pointsRaceCyclingCarnivalSplit"/);
  assert.match(app.markup(), /Points-race cycling carnival split/);
  const markup = app.markup();
  assert.ok(markup.indexOf('data-preset="omniumCyclingCarnivalSplit"') < markup.indexOf('data-preset="pointsRaceCyclingCarnivalSplit"'));
  assert.ok(markup.indexOf('data-preset="pointsRaceCyclingCarnivalSplit"') < markup.indexOf('data-preset="scratchCyclingCarnivalSplit"'));
  app.click('preset', { preset: 'scratchCyclingCarnivalSplit' });
  assert.equal(app.saved().participants.length, 3);
  assert.deepEqual(app.saved().participants.map((item) => item.id), ['scratch-cycling-committee', 'scratch-cycling-club-hire', 'scratch-cycling-first-aid']);
  assert.deepEqual(app.saved().participants.map((item) => item.name), ['Carnival committee', 'Scratch cycling club hire', 'First-aid']);
  assert.equal(app.saved().deal.feePerTransaction, 8);
  assert.equal(app.saved().deal.monthlyVolume, 6400);
  assert.equal(app.saved().deal.addressableVolume, 7700);
  assert.deepEqual(app.saved().participants.map((item) => item.capacity), [7500, 8600, 6400]);
  assert.notEqual(app.saved().participants[0].variableCostPerTransaction, app.saved().participants[1].variableCostPerTransaction);
  assert.notEqual(app.saved().participants[1].variableCostPerTransaction, app.saved().participants[2].variableCostPerTransaction);
  assert.equal(app.saved().participants[2].variableCostPerTransaction, 1.36);
  assert.match(app.notice(), /Scratch cycling carnival split loaded/);
  assert.match(app.markup(), /Operating region holds/);
  const exported = JSON.stringify(app.saved());
  assert.doesNotMatch(exported, /live roster/i);
  assert.doesNotMatch(exported, /hosted/i);
  ...
  assert.doesNotMatch(exported, /elimination-heat/i);
  assert.doesNotMatch(exported, /tempo-lap/i);
  assert.doesNotMatch(exported, /points-race/i);
  assert.doesNotMatch(exported, /points-sprint/i);
  assert.doesNotMatch(exported, /bell-lap/i);
  assert.doesNotMatch(exported, /prime-board/i);
  assert.doesNotMatch(exported, /pursuit-line/i);
  assert.doesNotMatch(exported, /catch-bell/i);
  assert.doesNotMatch(exported, /pursuit-chip/i);
});

test('cyclo-cross carnival first-aid stays 1.56 after loading scratch cycling carnival', async () => {
  const app = await workbench();
  app.click('preset', { preset: 'scratchCyclingCarnivalSplit' });
  assert.equal(app.saved().participants[2].variableCostPerTransaction, 1.36);
  app.click('preset', { preset: 'cycloCrossCarnivalSplit' });
...
test('keyboard last-over-capacity remaining copy shortcuts stay on scratch cycling carnival', async () => {
  const scratch = await workbench('file:', { clipboard: 'ok' });
  scratch.click('preset', { preset: 'scratchCyclingCarnivalSplit' });
  assert.match(scratch.markup(), /id="copy-last-over-capacity-remaining"[^>]*aria-keyshortcuts="\* Shift\+F7 Shift\+F10"/);
  assert.match(scratch.markup(), /id="hide-last-over-capacity-participant"[^>]*aria-keyshortcuts="# Shift\+F9"/);
  assert.match(scratch.markup(), /id="hide-first-over-capacity-participant"[^>]*aria-keyshortcuts="@"/);
  assert.doesNotMatch(scratch.markup(), /id="hide-first-over-capacity-participant"[^>]*aria-keyshortcuts="Shift\+F9"/);
  assert.match(scratch.markup(), /id="copy-first-over-capacity-remaining"[^>]*aria-keyshortcuts="~"/);
  scratch.edit('deal.monthlyVolume', '7700');
  scratch.keydown('F7', { shiftKey: true });
  assert.equal(scratch.copied().at(-1), 'Last over-capacity remaining listed capacity: 1,300 txn over for First-aid. How far over listed capacity. Not a forecast.');
  scratch.keydown('F10', { shiftKey: true });
  assert.equal(scratch.copied().at(-1), 'Last over-capacity remaining listed capacity: 1,300 txn over for First-aid. How far over listed capacity. Not a forecast.');
  scratch.keydown('~');
  assert.equal(scratch.copied().at(-1), 'First over-capacity remaining listed capacity: 200 txn over for Carnival committee. How far over listed capacity. Not a forecast.');
  scratch.keydown('F7');
  assert.equal(scratch.copied().at(-1), 'First zero-share participant: none entered.');
  scratch.keydown('F8', { shiftKey: true });
  assert.ok(scratch.focused().includes('#copy-last-over-capacity-remaining'));
  scratch.keydown('F9', { shiftKey: true });
  assert.ok(scratch.focused().includes('#hide-last-over-capacity-participant'));
  scratch.keydown('F9');
  assert.ok(scratch.focused().includes('#hide-first-zero-share-participant'));
  scratch.keydown('@');
  assert.ok(scratch.focused().includes('#hide-first-over-capacity-participant'));
  scratch.keydown('F12', { shiftKey: true });
  assert.ok(scratch.focused().includes('#hide-first-over-capacity-participant'));
  assert.equal(scratch.saved().participants[2].variableCostPerTransaction, 1.36);
  assert.doesNotMatch(JSON.stringify(scratch.saved()), /live roster/i);
  assert.doesNotMatch(JSON.stringify(scratch.saved()), /hosted/i);
  assert.doesNotMatch(JSON.stringify(scratch.saved()), /\bapi\b/i);
});

test('individual-pursuit cycling carnival split preset loads from the starting-point buttons', async () => {
  const app = await workbench();
  assert.match(app.markup(), /data-preset="individualPursuitCyclingCarnivalSplit"/);
  assert.match(app.markup(), /Individual-pursuit cycling carnival split/);
  assert.match(app.markup(), /data-preset="scratchCyclingCarnivalSplit"/);
  assert.match(app.markup(), /Scratch cycling carnival split/);
  const markup = app.markup();
  assert.ok(markup.indexOf('data-preset="pointsRaceCyclingCarnivalSplit"') < markup.indexOf('data-preset="scratchCyclingCarnivalSplit"'));
  assert.ok(markup.indexOf('data-preset="scratchCyclingCarnivalSplit"') < markup.indexOf('data-preset="individualPursuitCyclingCarnivalSplit"'));
  app.click('preset', { preset: 'individualPursuitCyclingCarnivalSplit' });
  assert.equal(app.saved().participants.length, 3);
  assert.deepEqual(app.saved().participants.map((item) => item.id), ['individual-pursuit-cycling-committee', 'individual-pursuit-cycling-club-hire', 'individual-pursuit-cycling-first-aid']);
  assert.deepEqual(app.saved().participants.map((item) => item.name), ['Carnival committee', 'Individual-pursuit cycling club hire', 'First-aid']);
  assert.equal(app.saved().deal.feePerTransaction, 8);
  assert.equal(app.saved().deal.monthlyVolume, 6500);
  assert.equal(app.saved().deal.addressableVolume, 7900);
  assert.deepEqual(app.saved().participants.map((item) => item.capacity), [7650, 8750, 6500]);
  assert.notEqual(app.saved().participants[0].variableCostPerTransaction, app.saved().participants[1].variableCostPerTransaction);
  assert.notEqual(app.saved().participants[1].variableCostPerTransaction, app.saved().participants[2].variableCostPerTransaction);
  assert.equal(app.saved().participants[2].variableCostPerTransaction, 1.34);
  assert.match(app.notice(), /Individual-pursuit cycling carnival split loaded/);
  assert.match(app.markup(), /Operating region holds/);
  const exported = JSON.stringify(app.saved());
  assert.doesNotMatch(exported, /live roster/i);
  assert.doesNotMatch(exported, /hosted/i);
  ...
  assert.doesNotMatch(exported, /bell-lap/i);
  assert.doesNotMatch(exported, /prime-board/i);
  assert.doesNotMatch(exported, /scratch-bunch/i);
  assert.doesNotMatch(exported, /bunch-finish/i);
  assert.doesNotMatch(exported, /scratch-chip/i);
});

test('cyclo-cross carnival first-aid stays 1.56 after loading individual-pursuit cycling carnival', async () => {
  const app = await workbench();
  app.click('preset', { preset: 'individualPursuitCyclingCarnivalSplit' });
  assert.equal(app.saved().participants[2].variableCostPerTransaction, 1.34);
  app.click('preset', { preset: 'cycloCrossCarnivalSplit' });
...
test('keyboard last-over-capacity remaining copy shortcuts stay on individual-pursuit cycling carnival', async () => {
  const pursuit = await workbench('file:', { clipboard: 'ok' });
  pursuit.click('preset', { preset: 'individualPursuitCyclingCarnivalSplit' });
  assert.match(pursuit.markup(), /id="copy-last-over-capacity-remaining"[^>]*aria-keyshortcuts="\* Shift\+F7 Shift\+F10"/);
  assert.match(pursuit.markup(), /id="hide-last-over-capacity-participant"[^>]*aria-keyshortcuts="# Shift\+F9"/);
  assert.match(pursuit.markup(), /id="hide-first-over-capacity-participant"[^>]*aria-keyshortcuts="@"/);
  assert.doesNotMatch(pursuit.markup(), /id="hide-first-over-capacity-participant"[^>]*aria-keyshortcuts="Shift\+F9"/);
  assert.match(pursuit.markup(), /id="copy-first-over-capacity-remaining"[^>]*aria-keyshortcuts="~"/);
  pursuit.edit('deal.monthlyVolume', '7900');
  pursuit.keydown('F7', { shiftKey: true });
  assert.equal(pursuit.copied().at(-1), 'Last over-capacity remaining listed capacity: 1,400 txn over for First-aid. How far over listed capacity. Not a forecast.');
  pursuit.keydown('F10', { shiftKey: true });
  assert.equal(pursuit.copied().at(-1), 'Last over-capacity remaining listed capacity: 1,400 txn over for First-aid. How far over listed capacity. Not a forecast.');
  pursuit.keydown('~');
  assert.equal(pursuit.copied().at(-1), 'First over-capacity remaining listed capacity: 250 txn over for Carnival committee. How far over listed capacity. Not a forecast.');
  pursuit.keydown('F7');
  assert.equal(pursuit.copied().at(-1), 'First zero-share participant: none entered.');
  pursuit.keydown('F8', { shiftKey: true });
  assert.ok(pursuit.focused().includes('#copy-last-over-capacity-remaining'));
  pursuit.keydown('F9', { shiftKey: true });
  assert.ok(pursuit.focused().includes('#hide-last-over-capacity-participant'));
  pursuit.keydown('F9');
  assert.ok(pursuit.focused().includes('#hide-first-zero-share-participant'));
  pursuit.keydown('@');
  assert.ok(pursuit.focused().includes('#hide-first-over-capacity-participant'));
  pursuit.keydown('F12', { shiftKey: true });
  assert.ok(pursuit.focused().includes('#hide-first-over-capacity-participant'));
  assert.equal(pursuit.saved().participants[2].variableCostPerTransaction, 1.34);
  assert.doesNotMatch(JSON.stringify(pursuit.saved()), /live roster/i);
  assert.doesNotMatch(JSON.stringify(pursuit.saved()), /hosted/i);
  assert.doesNotMatch(JSON.stringify(pursuit.saved()), /\bapi\b/i);
});
test('team-pursuit cycling carnival split preset loads from the starting-point buttons', async () => {
  const app = await workbench();
  assert.match(app.markup(), /data-preset="teamPursuitCyclingCarnivalSplit"/);
  assert.match(app.markup(), /Team-pursuit cycling carnival split/);
  assert.match(app.markup(), /data-preset="individualPursuitCyclingCarnivalSplit"/);
  assert.match(app.markup(), /Individual-pursuit cycling carnival split/);
  const markup = app.markup();
  assert.ok(markup.indexOf('data-preset="scratchCyclingCarnivalSplit"') < markup.indexOf('data-preset="individualPursuitCyclingCarnivalSplit"'));
  assert.ok(markup.indexOf('data-preset="individualPursuitCyclingCarnivalSplit"') < markup.indexOf('data-preset="teamPursuitCyclingCarnivalSplit"'));
  app.click('preset', { preset: 'teamPursuitCyclingCarnivalSplit' });
  assert.equal(app.saved().participants.length, 3);
  assert.deepEqual(app.saved().participants.map((item) => item.id), ['team-pursuit-cycling-committee', 'team-pursuit-cycling-club-hire', 'team-pursuit-cycling-first-aid']);
  assert.deepEqual(app.saved().participants.map((item) => item.name), ['Carnival committee', 'Team-pursuit cycling club hire', 'First-aid']);
  assert.equal(app.saved().deal.feePerTransaction, 8);
  assert.equal(app.saved().deal.monthlyVolume, 6600);
  assert.equal(app.saved().deal.addressableVolume, 8100);
  assert.deepEqual(app.saved().participants.map((item) => item.capacity), [7800, 8900, 6600]);
  assert.notEqual(app.saved().participants[0].variableCostPerTransaction, app.saved().participants[1].variableCostPerTransaction);
  assert.notEqual(app.saved().participants[1].variableCostPerTransaction, app.saved().participants[2].variableCostPerTransaction);
  assert.equal(app.saved().participants[2].variableCostPerTransaction, 1.32);
  assert.match(app.notice(), /Team-pursuit cycling carnival split loaded/);
  assert.match(app.markup(), /Operating region holds/);
  const exported = JSON.stringify(app.saved());
  assert.doesNotMatch(exported, /live roster/i);
  assert.doesNotMatch(exported, /hosted/i);
...
test('cyclo-cross carnival first-aid stays 1.56 after loading team-pursuit cycling carnival', async () => {
  const app = await workbench();
  app.click('preset', { preset: 'teamPursuitCyclingCarnivalSplit' });
  assert.equal(app.saved().participants[2].variableCostPerTransaction, 1.32);
  app.click('preset', { preset: 'cycloCrossCarnivalSplit' });
...
test('keyboard last-over-capacity remaining copy shortcuts stay on team-pursuit cycling carnival', async () => {
  const teamPursuit = await workbench('file:', { clipboard: 'ok' });
  teamPursuit.click('preset', { preset: 'teamPursuitCyclingCarnivalSplit' });
  assert.match(teamPursuit.markup(), /id="copy-last-over-capacity-remaining"[^>]*aria-keyshortcuts="\* Shift\+F7 Shift\+F10"/);
  assert.match(teamPursuit.markup(), /id="hide-last-over-capacity-participant"[^>]*aria-keyshortcuts="# Shift\+F9"/);
  assert.match(teamPursuit.markup(), /id="hide-first-over-capacity-participant"[^>]*aria-keyshortcuts="@"/);
  assert.doesNotMatch(teamPursuit.markup(), /id="hide-first-over-capacity-participant"[^>]*aria-keyshortcuts="Shift\+F9"/);
  assert.match(teamPursuit.markup(), /id="copy-first-over-capacity-remaining"[^>]*aria-keyshortcuts="~"/);
  teamPursuit.edit('deal.monthlyVolume', '8100');
  teamPursuit.keydown('F7', { shiftKey: true });
  assert.equal(teamPursuit.copied().at(-1), 'Last over-capacity remaining listed capacity: 1,500 txn over for First-aid. How far over listed capacity. Not a forecast.');
  teamPursuit.keydown('F10', { shiftKey: true });
  assert.equal(teamPursuit.copied().at(-1), 'Last over-capacity remaining listed capacity: 1,500 txn over for First-aid. How far over listed capacity. Not a forecast.');
  teamPursuit.keydown('~');
  assert.equal(teamPursuit.copied().at(-1), 'First over-capacity remaining listed capacity: 300 txn over for Carnival committee. How far over listed capacity. Not a forecast.');
  teamPursuit.keydown('F7');
  assert.equal(teamPursuit.copied().at(-1), 'First zero-share participant: none entered.');
  teamPursuit.keydown('F8', { shiftKey: true });
  assert.ok(teamPursuit.focused().includes('#copy-last-over-capacity-remaining'));
  teamPursuit.keydown('F9', { shiftKey: true });
  assert.ok(teamPursuit.focused().includes('#hide-last-over-capacity-participant'));
  teamPursuit.keydown('F9');
  assert.ok(teamPursuit.focused().includes('#hide-first-zero-share-participant'));
  teamPursuit.keydown('@');
  assert.ok(teamPursuit.focused().includes('#hide-first-over-capacity-participant'));
  teamPursuit.keydown('F12', { shiftKey: true });
  assert.ok(teamPursuit.focused().includes('#hide-first-over-capacity-participant'));
  assert.equal(teamPursuit.saved().participants[2].variableCostPerTransaction, 1.32);
  assert.doesNotMatch(JSON.stringify(teamPursuit.saved()), /live roster/i);
  assert.doesNotMatch(JSON.stringify(teamPursuit.saved()), /hosted/i);
  assert.doesNotMatch(JSON.stringify(teamPursuit.saved()), /\bapi\b/i);
});

test('team-sprint cycling carnival split preset loads from the starting-point buttons', async () => {
  const app = await workbench();
  assert.match(app.markup(), /data-preset="teamSprintCyclingCarnivalSplit"/);
  assert.match(app.markup(), /Team-sprint cycling carnival split/);
  assert.match(app.markup(), /data-preset="teamPursuitCyclingCarnivalSplit"/);
  assert.match(app.markup(), /Team-pursuit cycling carnival split/);
  const markup = app.markup();
  assert.ok(markup.indexOf('data-preset="individualPursuitCyclingCarnivalSplit"') < markup.indexOf('data-preset="teamPursuitCyclingCarnivalSplit"'));
  assert.ok(markup.indexOf('data-preset="teamPursuitCyclingCarnivalSplit"') < markup.indexOf('data-preset="teamSprintCyclingCarnivalSplit"'));
  app.click('preset', { preset: 'teamSprintCyclingCarnivalSplit' });
  assert.equal(app.saved().participants.length, 3);
  assert.deepEqual(app.saved().participants.map((item) => item.id), ['team-sprint-cycling-committee', 'team-sprint-cycling-club-hire', 'team-sprint-cycling-first-aid']);
  assert.deepEqual(app.saved().participants.map((item) => item.name), ['Carnival committee', 'Team-sprint cycling club hire', 'First-aid']);
  assert.equal(app.saved().deal.feePerTransaction, 8);
  assert.equal(app.saved().deal.monthlyVolume, 6700);
  assert.equal(app.saved().deal.addressableVolume, 8300);
  assert.deepEqual(app.saved().participants.map((item) => item.capacity), [7950, 9050, 6700]);
  assert.notEqual(app.saved().participants[0].variableCostPerTransaction, app.saved().participants[1].variableCostPerTransaction);
  assert.notEqual(app.saved().participants[1].variableCostPerTransaction, app.saved().participants[2].variableCostPerTransaction);
  assert.equal(app.saved().participants[2].variableCostPerTransaction, 1.30);
  assert.match(app.notice(), /Team-sprint cycling carnival split loaded/);
  assert.match(app.markup(), /Operating region holds/);
  const exported = JSON.stringify(app.saved());
  assert.doesNotMatch(exported, /live roster/i);
  assert.doesNotMatch(exported, /hosted/i);
  ...
  assert.doesNotMatch(exported, /catch-bell/i);
  assert.doesNotMatch(exported, /pursuit-chip/i);
  assert.doesNotMatch(exported, /team-pursuit-rota/i);
  assert.doesNotMatch(exported, /changeover-bell/i);
  assert.doesNotMatch(exported, /quartet-board/i);
});

test('cyclo-cross carnival first-aid stays 1.56 after loading team-sprint cycling carnival', async () => {
  const app = await workbench();
  app.click('preset', { preset: 'teamSprintCyclingCarnivalSplit' });
  assert.equal(app.saved().participants[2].variableCostPerTransaction, 1.30);
  app.click('preset', { preset: 'cycloCrossCarnivalSplit' });
...
test('keyboard last-over-capacity remaining copy shortcuts stay on team-sprint cycling carnival', async () => {
  const teamSprint = await workbench('file:', { clipboard: 'ok' });
  teamSprint.click('preset', { preset: 'teamSprintCyclingCarnivalSplit' });
  assert.match(teamSprint.markup(), /id="copy-last-over-capacity-remaining"[^>]*aria-keyshortcuts="\* Shift\+F7 Shift\+F10"/);
  assert.match(teamSprint.markup(), /id="hide-last-over-capacity-participant"[^>]*aria-keyshortcuts="# Shift\+F9"/);
  assert.match(teamSprint.markup(), /id="hide-first-over-capacity-participant"[^>]*aria-keyshortcuts="@"/);
  assert.doesNotMatch(teamSprint.markup(), /id="hide-first-over-capacity-participant"[^>]*aria-keyshortcuts="Shift\+F9"/);
  assert.match(teamSprint.markup(), /id="copy-first-over-capacity-remaining"[^>]*aria-keyshortcuts="~"/);
  teamSprint.edit('deal.monthlyVolume', '8300');
  teamSprint.keydown('F7', { shiftKey: true });
  assert.equal(teamSprint.copied().at(-1), 'Last over-capacity remaining listed capacity: 1,600 txn over for First-aid. How far over listed capacity. Not a forecast.');
  teamSprint.keydown('F10', { shiftKey: true });
  assert.equal(teamSprint.copied().at(-1), 'Last over-capacity remaining listed capacity: 1,600 txn over for First-aid. How far over listed capacity. Not a forecast.');
  teamSprint.keydown('~');
  assert.equal(teamSprint.copied().at(-1), 'First over-capacity remaining listed capacity: 350 txn over for Carnival committee. How far over listed capacity. Not a forecast.');
  teamSprint.keydown('F7');
  assert.equal(teamSprint.copied().at(-1), 'First zero-share participant: none entered.');
  teamSprint.keydown('F8', { shiftKey: true });
  assert.ok(teamSprint.focused().includes('#copy-last-over-capacity-remaining'));
  teamSprint.keydown('F9', { shiftKey: true });
  assert.ok(teamSprint.focused().includes('#hide-last-over-capacity-participant'));
  teamSprint.keydown('F9');
  assert.ok(teamSprint.focused().includes('#hide-first-zero-share-participant'));
  teamSprint.keydown('@');
  assert.ok(teamSprint.focused().includes('#hide-first-over-capacity-participant'));
  teamSprint.keydown('F12', { shiftKey: true });
  assert.ok(teamSprint.focused().includes('#hide-first-over-capacity-participant'));
  assert.equal(teamSprint.saved().participants[2].variableCostPerTransaction, 1.30);
  assert.doesNotMatch(JSON.stringify(teamSprint.saved()), /live roster/i);
  assert.doesNotMatch(JSON.stringify(teamSprint.saved()), /hosted/i);
  assert.doesNotMatch(JSON.stringify(teamSprint.saved()), /\bapi\b/i);
});

test('first-aid cycling carnival split preset loads from the starting-point buttons', async () => {
  const app = await workbench();
  ...
  app.click('preset', { preset: 'firstAidCyclingCarnivalSplit' });
  assert.equal(app.saved().participants.length, 3);
  assert.deepEqual(app.saved().participants.map((item) => item.id), ['first-aid-cycling-committee', 'first-aid-cycling-club-hire', 'first-aid-cycling-first-aid']);
  assert.deepEqual(app.saved().participants.map((item) => item.name), ['Carnival committee', 'First-aid cycling club hire', 'First-aid']);
  assert.equal(app.saved().deal.feePerTransaction, 8);
  assert.equal(app.saved().deal.monthlyVolume, 6800);
  assert.equal(app.saved().deal.addressableVolume, 8500);
  assert.deepEqual(app.saved().participants.map((item) => item.capacity), [8100, 9200, 6800]);
  assert.notEqual(app.saved().participants[0].variableCostPerTransaction, app.saved().participants[1].variableCostPerTransaction);
  assert.notEqual(app.saved().participants[1].variableCostPerTransaction, app.saved().participants[2].variableCostPerTransaction);
  assert.equal(app.saved().participants[2].variableCostPerTransaction, 1.28);
  assert.match(app.notice(), /First-aid cycling carnival split loaded/);
  assert.match(app.markup(), /Operating region holds/);
  const exported = JSON.stringify(app.saved());
  assert.doesNotMatch(exported, /live roster/i);
  assert.doesNotMatch(exported, /hosted/i);
...
test('cyclo-cross carnival first-aid stays 1.56 after loading first-aid cycling carnival', async () => {
  const app = await workbench();
  app.click('preset', { preset: 'firstAidCyclingCarnivalSplit' });
  assert.equal(app.saved().participants[2].variableCostPerTransaction, 1.28);
  app.click('preset', { preset: 'cycloCrossCarnivalSplit' });
...
test('keyboard last-over-capacity remaining copy shortcuts stay on first-aid cycling carnival', async () => {
  const firstAid = await workbench('file:', { clipboard: 'ok' });
  firstAid.click('preset', { preset: 'firstAidCyclingCarnivalSplit' });
  assert.match(firstAid.markup(), /id="copy-last-over-capacity-remaining"[^>]*aria-keyshortcuts="\* Shift\+F7 Shift\+F10"/);
  assert.match(firstAid.markup(), /id="hide-last-over-capacity-participant"[^>]*aria-keyshortcuts="# Shift\+F9"/);
  assert.match(firstAid.markup(), /id="hide-first-over-capacity-participant"[^>]*aria-keyshortcuts="@"/);
  assert.doesNotMatch(firstAid.markup(), /id="hide-first-over-capacity-participant"[^>]*aria-keyshortcuts="Shift\+F9"/);
  assert.match(firstAid.markup(), /id="copy-first-over-capacity-remaining"[^>]*aria-keyshortcuts="~"/);
  firstAid.edit('deal.monthlyVolume', '8500');
  firstAid.keydown('F7', { shiftKey: true });
  assert.equal(firstAid.copied().at(-1), 'Last over-capacity remaining listed capacity: 1,700 txn over for First-aid. How far over listed capacity. Not a forecast.');
  firstAid.keydown('F10', { shiftKey: true });
  assert.equal(firstAid.copied().at(-1), 'Last over-capacity remaining listed capacity: 1,700 txn over for First-aid. How far over listed capacity. Not a forecast.');
  firstAid.keydown('~');
  assert.equal(firstAid.copied().at(-1), 'First over-capacity remaining listed capacity: 400 txn over for Carnival committee. How far over listed capacity. Not a forecast.');
  firstAid.keydown('F7');
  assert.equal(firstAid.copied().at(-1), 'First zero-share participant: none entered.');
  firstAid.keydown('F8', { shiftKey: true });
  assert.ok(firstAid.focused().includes('#copy-last-over-capacity-remaining'));
  firstAid.keydown('F9', { shiftKey: true });
  assert.ok(firstAid.focused().includes('#hide-last-over-capacity-participant'));
  firstAid.keydown('F9');
  assert.ok(firstAid.focused().includes('#hide-first-zero-share-participant'));
  firstAid.keydown('@');
  assert.ok(firstAid.focused().includes('#hide-first-over-capacity-participant'));
  firstAid.keydown('F12', { shiftKey: true });
  assert.ok(firstAid.focused().includes('#hide-first-over-capacity-participant'));
  assert.equal(firstAid.saved().participants[2].variableCostPerTransaction, 1.28);
  assert.doesNotMatch(JSON.stringify(firstAid.saved()), /live roster/i);
  assert.doesNotMatch(JSON.stringify(firstAid.saved()), /hosted/i);
...
test('soigneur cycling carnival split preset loads from the starting-point buttons', async () => {
  const app = await workbench();
  assert.match(app.markup(), /data-preset="soigneurCyclingCarnivalSplit"/);
  assert.match(app.markup(), /Soigneur cycling carnival split/);
  assert.match(app.markup(), /data-preset="leadOutCyclingCarnivalSplit"/);
  assert.match(app.markup(), /Lead-out cycling carnival split/);
  const markup = app.markup();
  assert.ok(markup.indexOf('data-preset="feedZoneCyclingCarnivalSplit"') < markup.indexOf('data-preset="leadOutCyclingCarnivalSplit"'));
  assert.ok(markup.indexOf('data-preset="leadOutCyclingCarnivalSplit"') < markup.indexOf('data-preset="soigneurCyclingCarnivalSplit"'));
  app.click('preset', { preset: 'soigneurCyclingCarnivalSplit' });
  assert.equal(app.saved().participants.length, 3);
  assert.deepEqual(app.saved().participants.map((item) => item.id), ['soigneur-cycling-committee', 'soigneur-cycling-club-hire', 'soigneur-cycling-first-aid']);
  assert.deepEqual(app.saved().participants.map((item) => item.name), ['Carnival committee', 'Soigneur cycling club hire', 'First-aid']);
  assert.equal(app.saved().deal.feePerTransaction, 8);
  assert.equal(app.saved().deal.monthlyVolume, 7100);
  assert.equal(app.saved().deal.addressableVolume, 9100);
  assert.deepEqual(app.saved().participants.map((item) => item.capacity), [8550, 9650, 7100]);
  assert.notEqual(app.saved().participants[0].variableCostPerTransaction, app.saved().participants[1].variableCostPerTransaction);
  assert.notEqual(app.saved().participants[1].variableCostPerTransaction, app.saved().participants[2].variableCostPerTransaction);
  assert.equal(app.saved().participants[2].variableCostPerTransaction, 1.22);
  assert.match(app.notice(), /Soigneur cycling carnival split loaded/);
  assert.match(app.markup(), /Operating region holds/);
  const exported = JSON.stringify(app.saved());
  assert.doesNotMatch(exported, /live roster/i);
  assert.doesNotMatch(exported, /hosted/i);
  ...
  assert.doesNotMatch(exported, /ice-pack-rota/i);
  assert.doesNotMatch(exported, /triage-board/i);
  assert.match(exported, /Soigneur cycling club hire/);
});

test('cyclo-cross carnival first-aid stays 1.56 after loading soigneur cycling carnival', async () => {
  const app = await workbench();
  app.click('preset', { preset: 'soigneurCyclingCarnivalSplit' });
  assert.equal(app.saved().participants[2].variableCostPerTransaction, 1.22);
  app.click('preset', { preset: 'cycloCrossCarnivalSplit' });
...
test('keyboard last-over-capacity remaining copy shortcuts stay on soigneur cycling carnival', async () => {
  const soigneur = await workbench('file:', { clipboard: 'ok' });
  soigneur.click('preset', { preset: 'soigneurCyclingCarnivalSplit' });
  assert.match(soigneur.markup(), /id="copy-last-over-capacity-remaining"[^>]*aria-keyshortcuts="\* Shift\+F7 Shift\+F10"/);
  assert.match(soigneur.markup(), /id="hide-last-over-capacity-participant"[^>]*aria-keyshortcuts="# Shift\+F9"/);
  assert.match(soigneur.markup(), /id="hide-first-over-capacity-participant"[^>]*aria-keyshortcuts="@"/);
  assert.doesNotMatch(soigneur.markup(), /id="hide-first-over-capacity-participant"[^>]*aria-keyshortcuts="Shift\+F9"/);
  assert.match(soigneur.markup(), /id="copy-first-over-capacity-remaining"[^>]*aria-keyshortcuts="~"/);
  soigneur.edit('deal.monthlyVolume', '9100');
  soigneur.keydown('F7', { shiftKey: true });
  assert.equal(soigneur.copied().at(-1), 'Last over-capacity remaining listed capacity: 2,000 txn over for First-aid. How far over listed capacity. Not a forecast.');
  soigneur.keydown('F10', { shiftKey: true });
  assert.equal(soigneur.copied().at(-1), 'Last over-capacity remaining listed capacity: 2,000 txn over for First-aid. How far over listed capacity. Not a forecast.');
  soigneur.keydown('~');
  assert.equal(soigneur.copied().at(-1), 'First over-capacity remaining listed capacity: 550 txn over for Carnival committee. How far over listed capacity. Not a forecast.');
  soigneur.keydown('F7');
  assert.equal(soigneur.copied().at(-1), 'First zero-share participant: none entered.');
  soigneur.keydown('F8', { shiftKey: true });
  assert.ok(soigneur.focused().includes('#copy-last-over-capacity-remaining'));
  soigneur.keydown('F9', { shiftKey: true });
  assert.ok(soigneur.focused().includes('#hide-last-over-capacity-participant'));
  soigneur.keydown('F9');
  assert.ok(soigneur.focused().includes('#hide-first-zero-share-participant'));
  soigneur.keydown('@');
  assert.ok(soigneur.focused().includes('#hide-first-over-capacity-participant'));
  soigneur.keydown('F12', { shiftKey: true });
  assert.ok(soigneur.focused().includes('#hide-first-over-capacity-participant'));
  assert.equal(soigneur.saved().participants[2].variableCostPerTransaction, 1.22);
  assert.doesNotMatch(JSON.stringify(soigneur.saved()), /live roster/i);
  assert.doesNotMatch(JSON.stringify(soigneur.saved()), /hosted/i);
  assert.doesNotMatch(JSON.stringify(soigneur.saved()), /\bapi\b/i);
});

test('domestique cycling carnival split preset loads from the starting-point buttons', async () => {
  const app = await workbench();
  assert.match(app.markup(), /data-preset="domestiqueCyclingCarnivalSplit"/);
  assert.match(app.markup(), /Domestique cycling carnival split/);
  assert.match(app.markup(), /data-preset="soigneurCyclingCarnivalSplit"/);
  assert.match(app.markup(), /Soigneur cycling carnival split/);
  const markup = app.markup();
  assert.ok(markup.indexOf('data-preset="leadOutCyclingCarnivalSplit"') < markup.indexOf('data-preset="soigneurCyclingCarnivalSplit"'));
  assert.ok(markup.indexOf('data-preset="soigneurCyclingCarnivalSplit"') < markup.indexOf('data-preset="domestiqueCyclingCarnivalSplit"'));
  app.click('preset', { preset: 'domestiqueCyclingCarnivalSplit' });
  assert.equal(app.saved().participants.length, 3);
  assert.deepEqual(app.saved().participants.map((item) => item.id), ['domestique-cycling-committee', 'domestique-cycling-club-hire', 'domestique-cycling-first-aid']);
  assert.deepEqual(app.saved().participants.map((item) => item.name), ['Carnival committee', 'Domestique cycling club hire', 'First-aid']);
  assert.equal(app.saved().deal.feePerTransaction, 8);
  assert.equal(app.saved().deal.monthlyVolume, 7200);
  assert.equal(app.saved().deal.addressableVolume, 9200);
  assert.deepEqual(app.saved().participants.map((item) => item.capacity), [8750, 9850, 7200]);
  assert.notEqual(app.saved().participants[0].variableCostPerTransaction, app.saved().participants[1].variableCostPerTransaction);
  assert.notEqual(app.saved().participants[1].variableCostPerTransaction, app.saved().participants[2].variableCostPerTransaction);
  assert.equal(app.saved().participants[2].variableCostPerTransaction, 1.20);
  assert.match(app.notice(), /Domestique cycling carnival split loaded/);
  assert.match(app.markup(), /Operating region holds/);
  const exported = JSON.stringify(app.saved());
  assert.doesNotMatch(exported, /live roster/i);
  assert.doesNotMatch(exported, /hosted/i);
  assert.doesNotMatch(exported, /\bapi\b/i);
  assert.doesNotMatch(exported, /forecast/i);
  assert.doesNotMatch(exported, /soigneur-station/i);
  assert.doesNotMatch(exported, /cape-bag/i);
  assert.doesNotMatch(exported, /rice-cake/i);
  assert.doesNotMatch(exported, /train-line/i);
  assert.doesNotMatch(exported, /last-rider/i);
  assert.doesNotMatch(exported, /sprinter-launch/i);
  assert.doesNotMatch(exported, /musette-line/i);
  ...
  assert.doesNotMatch(exported, /ice-pack-rota/i);
  assert.doesNotMatch(exported, /triage-board/i);
  assert.match(exported, /Domestique cycling club hire/);
});

test('cyclo-cross carnival first-aid stays 1.56 after loading domestique cycling carnival', async () => {
  const app = await workbench();
  app.click('preset', { preset: 'domestiqueCyclingCarnivalSplit' });
  assert.equal(app.saved().participants[2].variableCostPerTransaction, 1.20);
  app.click('preset', { preset: 'cycloCrossCarnivalSplit' });
...
test('keyboard last-over-capacity remaining copy shortcuts stay on domestique cycling carnival', async () => {
  const domestique = await workbench('file:', { clipboard: 'ok' });
  domestique.click('preset', { preset: 'domestiqueCyclingCarnivalSplit' });
  assert.match(domestique.markup(), /id="copy-last-over-capacity-remaining"[^>]*aria-keyshortcuts="\* Shift\+F7 Shift\+F10"/);
  assert.match(domestique.markup(), /id="hide-last-over-capacity-participant"[^>]*aria-keyshortcuts="# Shift\+F9"/);
  assert.match(domestique.markup(), /id="hide-first-over-capacity-participant"[^>]*aria-keyshortcuts="@"/);
  assert.doesNotMatch(domestique.markup(), /id="hide-first-over-capacity-participant"[^>]*aria-keyshortcuts="Shift\+F9"/);
  assert.match(domestique.markup(), /id="copy-first-over-capacity-remaining"[^>]*aria-keyshortcuts="~"/);
  domestique.edit('deal.monthlyVolume', '9200');
  domestique.keydown('F7', { shiftKey: true });
  assert.equal(domestique.copied().at(-1), 'Last over-capacity remaining listed capacity: 2,000 txn over for First-aid. How far over listed capacity. Not a forecast.');
  domestique.keydown('F10', { shiftKey: true });
  assert.equal(domestique.copied().at(-1), 'Last over-capacity remaining listed capacity: 2,000 txn over for First-aid. How far over listed capacity. Not a forecast.');
  domestique.keydown('~');
  assert.equal(domestique.copied().at(-1), 'First over-capacity remaining listed capacity: 450 txn over for Carnival committee. How far over listed capacity. Not a forecast.');
  domestique.keydown('F7');
  assert.equal(domestique.copied().at(-1), 'First zero-share participant: none entered.');
  domestique.keydown('F8', { shiftKey: true });
  assert.ok(domestique.focused().includes('#copy-last-over-capacity-remaining'));
  domestique.keydown('F9', { shiftKey: true });
  assert.ok(domestique.focused().includes('#hide-last-over-capacity-participant'));
  domestique.keydown('F9');
  assert.ok(domestique.focused().includes('#hide-first-zero-share-participant'));
  domestique.keydown('@');
  assert.ok(domestique.focused().includes('#hide-first-over-capacity-participant'));
  domestique.keydown('F12', { shiftKey: true });
  assert.ok(domestique.focused().includes('#hide-first-over-capacity-participant'));
  assert.equal(domestique.saved().participants[2].variableCostPerTransaction, 1.20);
  assert.doesNotMatch(JSON.stringify(domestique.saved()), /live roster/i);
  assert.doesNotMatch(JSON.stringify(domestique.saved()), /hosted/i);
  assert.doesNotMatch(JSON.stringify(domestique.saved()), /\bapi\b/i);
});

test('bottle-hand cycling carnival split preset loads from the starting-point buttons', async () => {
  const app = await workbench();
  assert.match(app.markup(), /data-preset="bottleHandCyclingCarnivalSplit"/);
  assert.match(app.markup(), /Bottle-hand cycling carnival split/);
  assert.match(app.markup(), /data-preset="domestiqueCyclingCarnivalSplit"/);
  assert.match(app.markup(), /Domestique cycling carnival split/);
  const markup = app.markup();
  assert.ok(markup.indexOf('data-preset="soigneurCyclingCarnivalSplit"') < markup.indexOf('data-preset="domestiqueCyclingCarnivalSplit"'));
  assert.ok(markup.indexOf('data-preset="domestiqueCyclingCarnivalSplit"') < markup.indexOf('data-preset="bottleHandCyclingCarnivalSplit"'));
  app.click('preset', { preset: 'bottleHandCyclingCarnivalSplit' });
  assert.equal(app.saved().participants.length, 3);
  assert.deepEqual(app.saved().participants.map((item) => item.id), ['bottle-hand-cycling-committee', 'bottle-hand-cycling-club-hire', 'bottle-hand-cycling-first-aid']);
  assert.deepEqual(app.saved().participants.map((item) => item.name), ['Carnival committee', 'Bottle-hand cycling club hire', 'First-aid']);
  assert.equal(app.saved().deal.feePerTransaction, 8);
  assert.equal(app.saved().deal.monthlyVolume, 7300);
  assert.equal(app.saved().deal.addressableVolume, 9300);
  assert.deepEqual(app.saved().participants.map((item) => item.capacity), [8850, 9950, 7300]);
  assert.notEqual(app.saved().participants[0].variableCostPerTransaction, app.saved().participants[1].variableCostPerTransaction);
  assert.notEqual(app.saved().participants[1].variableCostPerTransaction, app.saved().participants[2].variableCostPerTransaction);
  assert.equal(app.saved().participants[2].variableCostPerTransaction, 1.18);
  assert.match(app.notice(), /Bottle-hand cycling carnival split loaded/);
  assert.match(app.markup(), /Operating region holds/);
  const exported = JSON.stringify(app.saved());
  assert.doesNotMatch(exported, /live roster/i);
  assert.doesNotMatch(exported, /hosted/i);
  ...
  assert.doesNotMatch(exported, /sticky-bottle/i);
  assert.doesNotMatch(exported, /bottle-hand-up/i);
  assert.doesNotMatch(exported, /wheel-change/i);
  assert.doesNotMatch(exported, /neutral-service/i);
  assert.doesNotMatch(exported, /team-car/i);
  assert.doesNotMatch(exported, /treatment-tent/i);
  assert.doesNotMatch(exported, /ice-pack-rota/i);
  assert.doesNotMatch(exported, /triage-board/i);
  assert.match(exported, /Bottle-hand cycling club hire/);
});

test('cyclo-cross carnival first-aid stays 1.56 after loading bottle-hand cycling carnival', async () => {
  const app = await workbench();
  app.click('preset', { preset: 'bottleHandCyclingCarnivalSplit' });
  assert.equal(app.saved().participants[2].variableCostPerTransaction, 1.18);
  app.click('preset', { preset: 'cycloCrossCarnivalSplit' });
...
test('keyboard last-over-capacity remaining copy shortcuts stay on bottle-hand cycling carnival', async () => {
  const bottleHand = await workbench('file:', { clipboard: 'ok' });
  bottleHand.click('preset', { preset: 'bottleHandCyclingCarnivalSplit' });
  assert.match(bottleHand.markup(), /id="copy-last-over-capacity-remaining"[^>]*aria-keyshortcuts="\* Shift\+F7 Shift\+F10"/);
  assert.match(bottleHand.markup(), /id="hide-last-over-capacity-participant"[^>]*aria-keyshortcuts="# Shift\+F9"/);
  assert.match(bottleHand.markup(), /id="hide-first-over-capacity-participant"[^>]*aria-keyshortcuts="@"/);
  assert.doesNotMatch(bottleHand.markup(), /id="hide-first-over-capacity-participant"[^>]*aria-keyshortcuts="Shift\+F9"/);
  assert.match(bottleHand.markup(), /id="copy-first-over-capacity-remaining"[^>]*aria-keyshortcuts="~"/);
  bottleHand.edit('deal.monthlyVolume', '9300');
  bottleHand.keydown('F7', { shiftKey: true });
  assert.equal(bottleHand.copied().at(-1), 'Last over-capacity remaining listed capacity: 2,000 txn over for First-aid. How far over listed capacity. Not a forecast.');
  bottleHand.keydown('F10', { shiftKey: true });
  assert.equal(bottleHand.copied().at(-1), 'Last over-capacity remaining listed capacity: 2,000 txn over for First-aid. How far over listed capacity. Not a forecast.');
  bottleHand.keydown('~');
  assert.equal(bottleHand.copied().at(-1), 'First over-capacity remaining listed capacity: 450 txn over for Carnival committee. How far over listed capacity. Not a forecast.');
  bottleHand.keydown('F7');
  assert.equal(bottleHand.copied().at(-1), 'First zero-share participant: none entered.');
  bottleHand.keydown('F8', { shiftKey: true });
  assert.ok(bottleHand.focused().includes('#copy-last-over-capacity-remaining'));
  bottleHand.keydown('F9', { shiftKey: true });
  assert.ok(bottleHand.focused().includes('#hide-last-over-capacity-participant'));
  bottleHand.keydown('F9');
  assert.ok(bottleHand.focused().includes('#hide-first-zero-share-participant'));
  bottleHand.keydown('@');
  assert.ok(bottleHand.focused().includes('#hide-first-over-capacity-participant'));
  bottleHand.keydown('F12', { shiftKey: true });
  assert.ok(bottleHand.focused().includes('#hide-first-over-capacity-participant'));
  assert.equal(bottleHand.saved().participants[2].variableCostPerTransaction, 1.18);
  assert.doesNotMatch(JSON.stringify(bottleHand.saved()), /live roster/i);
  assert.doesNotMatch(JSON.stringify(bottleHand.saved()), /hosted/i);
  assert.doesNotMatch(JSON.stringify(bottleHand.saved()), /\bapi\b/i);
});

test('echelon cycling carnival split preset loads from the starting-point buttons', async () => {
  const app = await workbench();
  assert.match(app.markup(), /data-preset="echelonCyclingCarnivalSplit"/);
  assert.match(app.markup(), /Echelon cycling carnival split/);
  assert.match(app.markup(), /data-preset="domestiqueCyclingCarnivalSplit"/);
  assert.match(app.markup(), /Domestique cycling carnival split/);
  const markup = app.markup();
  assert.ok(markup.indexOf('data-preset="soigneurCyclingCarnivalSplit"') < markup.indexOf('data-preset="domestiqueCyclingCarnivalSplit"'));
  assert.ok(markup.indexOf('data-preset="domestiqueCyclingCarnivalSplit"') < markup.indexOf('data-preset="bottleHandCyclingCarnivalSplit"'));
  assert.ok(markup.indexOf('data-preset="bottleHandCyclingCarnivalSplit"') < markup.indexOf('data-preset="echelonCyclingCarnivalSplit"'));
  app.click('preset', { preset: 'echelonCyclingCarnivalSplit' });
  assert.equal(app.saved().participants.length, 3);
  assert.deepEqual(app.saved().participants.map((item) => item.id), ['echelon-cycling-committee', 'echelon-cycling-club-hire', 'echelon-cycling-first-aid']);
  assert.deepEqual(app.saved().participants.map((item) => item.name), ['Carnival committee', 'Echelon cycling club hire', 'First-aid']);
  assert.equal(app.saved().deal.feePerTransaction, 8);
  assert.equal(app.saved().deal.monthlyVolume, 7400);
  assert.equal(app.saved().deal.addressableVolume, 9400);
  assert.deepEqual(app.saved().participants.map((item) => item.capacity), [8950, 10050, 7400]);
  assert.notEqual(app.saved().participants[0].variableCostPerTransaction, app.saved().participants[1].variableCostPerTransaction);
  assert.notEqual(app.saved().participants[1].variableCostPerTransaction, app.saved().participants[2].variableCostPerTransaction);
  assert.equal(app.saved().participants[2].variableCostPerTransaction, 1.16);
  assert.notEqual(app.saved().participants[2].variableCostPerTransaction, 1.18);
  assert.match(app.notice(), /Echelon cycling carnival split loaded/);
  assert.match(app.markup(), /Operating region holds/);
  const exported = JSON.stringify(app.saved());
  assert.doesNotMatch(exported, /live roster/i);
  assert.doesNotMatch(exported, /hosted/i);
  ...
  assert.doesNotMatch(exported, /musette-line/i);
  assert.doesNotMatch(exported, /sticky-bottle/i);
  assert.doesNotMatch(exported, /echelon-up/i);
  assert.doesNotMatch(exported, /bidon-pass/i);
  assert.doesNotMatch(exported, /car-window/i);
  assert.doesNotMatch(exported, /crate-lock/i);
  assert.doesNotMatch(exported, /wheel-change/i);
  assert.doesNotMatch(exported, /neutral-service/i);
  assert.doesNotMatch(exported, /team-car/i);
  assert.doesNotMatch(exported, /treatment-tent/i);
  assert.doesNotMatch(exported, /ice-pack-rota/i);
  assert.doesNotMatch(exported, /triage-board/i);
  assert.match(exported, /Echelon cycling club hire/);
});

test('cyclo-cross carnival first-aid stays 1.56 after loading echelon cycling carnival', async () => {
  const app = await workbench();
  app.click('preset', { preset: 'echelonCyclingCarnivalSplit' });
  assert.equal(app.saved().participants[2].variableCostPerTransaction, 1.16);
  app.click('preset', { preset: 'cycloCrossCarnivalSplit' });
...
test('keyboard last-over-capacity remaining copy shortcuts stay on echelon cycling carnival', async () => {
  const echelon = await workbench('file:', { clipboard: 'ok' });
  echelon.click('preset', { preset: 'echelonCyclingCarnivalSplit' });
  assert.match(echelon.markup(), /id="copy-last-over-capacity-remaining"[^>]*aria-keyshortcuts="\* Shift\+F7 Shift\+F10"/);
  assert.match(echelon.markup(), /id="hide-last-over-capacity-participant"[^>]*aria-keyshortcuts="# Shift\+F9"/);
  assert.match(echelon.markup(), /id="hide-first-over-capacity-participant"[^>]*aria-keyshortcuts="@"/);
  assert.doesNotMatch(echelon.markup(), /id="hide-first-over-capacity-participant"[^>]*aria-keyshortcuts="Shift\+F9"/);
  assert.match(echelon.markup(), /id="copy-first-over-capacity-remaining"[^>]*aria-keyshortcuts="~"/);
  echelon.edit('deal.monthlyVolume', '9400');
  echelon.keydown('F7', { shiftKey: true });
  assert.equal(echelon.copied().at(-1), 'Last over-capacity remaining listed capacity: 2,000 txn over for First-aid. How far over listed capacity. Not a forecast.');
  echelon.keydown('F10', { shiftKey: true });
  assert.equal(echelon.copied().at(-1), 'Last over-capacity remaining listed capacity: 2,000 txn over for First-aid. How far over listed capacity. Not a forecast.');
  echelon.keydown('~');
  assert.equal(echelon.copied().at(-1), 'First over-capacity remaining listed capacity: 450 txn over for Carnival committee. How far over listed capacity. Not a forecast.');
  echelon.keydown('F7');
  assert.equal(echelon.copied().at(-1), 'First zero-share participant: none entered.');
  echelon.keydown('F8', { shiftKey: true });
  assert.ok(echelon.focused().includes('#copy-last-over-capacity-remaining'));
  echelon.keydown('F9', { shiftKey: true });
  assert.ok(echelon.focused().includes('#hide-last-over-capacity-participant'));
  echelon.keydown('F9');
  assert.ok(echelon.focused().includes('#hide-first-zero-share-participant'));
  echelon.keydown('@');
  assert.ok(echelon.focused().includes('#hide-first-over-capacity-participant'));
  echelon.keydown('F12', { shiftKey: true });
  assert.ok(echelon.focused().includes('#hide-first-over-capacity-participant'));
  assert.equal(echelon.saved().participants[2].variableCostPerTransaction, 1.16);
  assert.doesNotMatch(JSON.stringify(echelon.saved()), /live roster/i);
  assert.doesNotMatch(JSON.stringify(echelon.saved()), /hosted/i);
  assert.doesNotMatch(JSON.stringify(echelon.saved()), /\bapi\b/i);
});
