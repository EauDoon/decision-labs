test('release 1.5.55 ships echelon cycling carnival, last-over-capacity remaining copy and last-over-capacity hide jump', async () => {
  const pkg = JSON.parse(await readFile(new URL('../package.json', import.meta.url), 'utf8'));
  const readme = await readFile(new URL('../README.md', import.meta.url), 'utf8');
  const changelog = await readFile(new URL('../CHANGELOG.md', import.meta.url), 'utf8');
  const html = await buildStandalone();
  assert.equal(pkg.version, '1.5.55');
  assert.match(readme, /New in v1\.5\.55/);
  assert.match(readme, /Decision workflow \(v1\.5\.55\)/);
  assert.match(readme, /Echelon cycling carnival, last-over-capacity remaining listed capacity copy, and last-over-capacity hide jump in Partnership Breakpoint 1\.5\.55/);
  assert.match(readme, /Cyclo-cross first-aid stays variable cost 1\.56 after the Echelon cycling carnival starting point is loaded/);
  const firstHeading = changelog.match(/^## .+$/m)?.[0];
  assert.equal(firstHeading, '## 1.5.55');
  assert.match(changelog, /Echelon cycling carnival, last-over-capacity remaining listed capacity copy, and last-over-capacity hide jump in Partnership Breakpoint 1\.5\.55/);
  assert.match(html, /echelonCyclingCarnivalSplit/);
  assert.match(html, /Echelon cycling carnival split/);
  assert.match(html, /domestiqueCyclingCarnivalSplit/);
  const echelonPresetAt = html.indexOf('echelonCyclingCarnivalSplit:');
