...
Version 1.4.2 adds a volume-to-hold solver, clipboard copy of the negotiation brief, visible or all stress-grid CSV, keyboard `g` to jump to results, a print one-pager, duplicate-current-case snapshots, a first-to-fail roster label, a sticky invalid-field summary, a Two-party 50/50 studio starting point, and collapse of all-hold stress cases. Counts remain counts. The workbench does not assign probabilities.
...
2. Add, duplicate, reorder, or edit participants. Import participant CSV replaces the roster only after the whole file validates; deal terms stay unchanged. Revenue shares must total exactly 1. Between 2 and 24 participants are required. Removing a participant reallocates that share across whoever remains. Duplicate, Move, and Remove stay outside the card disclosure so they remain usable at 390px.
...
8. Use Solve minimum share to hold, Solve volume to hold on one participant, or Solve fee for all to hold, to preview a deterministic floor. Apply is required. Pin two snapshots against the current draft for a three-way profit and hold table. Duplicate current case saves an independent snapshot. Export filenames include a sanitized deal title when one is present. Export redacted JSON when sharing a case without names. Export stress CSV for all cases, or visible cases after a collapse. Copy negotiation brief uses the clipboard when available. Copy share URL is available when the workbench is served over http. Print report is a one-pager of tornado, waterfall, ledger, and notes.
...
5. Export JSON for portable case backups, redacted JSON with names removed, a Markdown decision report with reproducible inputs, a short negotiation brief, a CSV of every participant in every compound case, or a CSV of currently visible cases. Download names include a sanitized deal title when one is present. Formula-like text is prefixed with an apostrophe in CSV. Print report is a one-pager of tornado, waterfall, ledger, and notes. Copy share URL copies the current hash link in locally served http mode.

The first visit shows a three-step coach. Dismiss it with Got it or Escape; dismissal is stored in this browser. Share links skip the coach. Press `?` for keyboard shortcuts (`u` undo, `r` redo, `e` export JSON, `g` jump to results). Shortcuts are ignored while a field is focused. The invalid-field summary stays sticky until every required field is valid.
