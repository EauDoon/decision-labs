...
Version 1.4.1 adds a sticky results jump nav, participant CSV import, capacity utilization in the ledger, a three-snapshot compare, optional deal notes, dialog focus trapping, title-based export filenames, and a mobile roster toolbar. Copy share URL is available in locally served http mode. Hide in table on the stress ledger is display-only. The workbench does not assign probabilities.
...
6. Edit Compound stress settings. The workbench tests up to 27 combinations of volume decline or growth, fee cuts, and variable-cost increases. Open Inspect all N compound cases to review every outcome. Hide in table removes a ledger row from display only.
...
8. Use Solve minimum share to hold on one participant, or Solve fee for all to hold, to preview a deterministic floor. Apply is required. Pin two snapshots against the current draft for a three-way profit and hold table. Export filenames include a sanitized deal title when one is present. Export redacted JSON when sharing a case without names. Copy share URL is available when the workbench is served over http.
...
4. Inspect a compound case, preview the changed assumptions, and apply it only when you want a new baseline. Existing volume shock resets to zero to avoid double counting. The stress grid then applies the existing settings as additional shocks from this baseline. Hide in table is display-only.
5. Export JSON for portable case backups, redacted JSON with names removed, a Markdown decision report with reproducible inputs, a short negotiation brief, or a CSV containing every participant in every compound case. Download names include a sanitized deal title when one is present. Formula-like text is prefixed with an apostrophe in CSV. Print report includes case assumptions, notes, and the displayed analysis. Copy share URL copies the current hash link in locally served http mode.
