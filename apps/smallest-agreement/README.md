...
4. Optionally set a minimum average support for any group, mark a veto group, a maximum total change cost, an option lock on a clause, and option relationships (prerequisites, incompatible pairs, linked sets) in the relationships section. Option ids must be unique across clauses when relationships are declared; contradictory and dangling rules are rejected with reasons. Blank budget and floor inputs mean no limit. Zero is an active limit. Unlock an option before removing it. A veto is a numerical constraint, not a legal right.
5. Save a named snapshot before changing assumptions. The local library holds up to 20 independent snapshots. Save negotiation rounds with a name, human-authored notes, and an optional recorded outcome; rounds keep immutable baselines for comparison and reopening, and a saved round is not a recorded vote unless the outcome says so. Loading, importing, resetting, and editing can be undone through up to 50 in-tab changes; a new edit clears redo.
