...
/**
 * One-line Markdown of the last group marked as a veto group.
 * Honest when none. Distinct from first veto group copy and first non-veto group copy.
 * A veto is a number you entered, not a legal right.
 */
export function formatLastVetoGroupLabelMarkdown(proposal) {
  const validation = validateProposal(proposal);
  if (!validation.valid) return { status: "invalid", errors: validation.errors };
  const p = canonicalProposal(proposal);
  const disclaimer = "A veto is a number you entered, not a legal right.";
  const vetoGroups = p.groups.filter((group) => group.veto === true);
  const last = vetoGroups[vetoGroups.length - 1];
  if (!last) {
    return {
      status: "ok",
      empty: true,
      text: `No veto group is marked, so there is no last veto group label to copy. ${disclaimer}\n`,
    };
  }
  return {
    status: "ok",
    empty: false,
    label: last.name,
    text: `Last veto group: ${briefText(last.name)}. ${disclaimer}\n`,
  };
}
 ...
 * is below the numeric approval threshold.
 * Honest when none or no inspected package is available.
 * Distinct from last below-threshold group copy and first-below-floor group copy.
 * A threshold is a number you entered, not a legal quorum.
 * Do not treat the label as a legal identity.
 */
export function formatFirstBelowThresholdGroupLabelMarkdown(proposal, options) {
  const validation = validateProposal(proposal);
  if (!validation.valid) return { status: "invalid", errors: validation.errors };
  const disclaimer = "A threshold is a number you entered, not a legal quorum. The label is not a legal identity.";
  if (!Array.isArray(options) || options.length !== proposal.clauses.length) {
    return {
      status: "unavailable",
      empty: true,
      text: `No inspected package is available, so there is no first below-threshold group label to copy. ${disclaimer}\n`,
    };
  }
  const listed = groupsBelowApprovalThreshold(proposal, options);
  if (listed.status !== "ok") return listed;
  ...
  if (!first) {
    return {
      status: "ok",
      empty: true,
      text: `No group is below the approval threshold, so there is no first below-threshold group label to copy. ${disclaimer}\n`,
    };
  }
  return {
    status: "ok",
    empty: false,
    label: first.name,
    text: `First below-threshold group: ${briefText(first.name)}. ${disclaimer}\n`,
  };
}
 ...
 * Uses the same meeting-threshold list as hideLastGroupAtOrAboveThreshold.
 * Honest when none or no inspected package is available.
 * Distinct from last below-threshold group copy, first below-threshold group copy,
 * and threshold-group count copy.
 * A threshold is a number you entered, not a legal quorum.
 * Do not treat the label as a legal identity.
 */
export function formatLastGroupAtOrAboveThresholdLabelMarkdown(proposal, options) {
  const validation = validateProposal(proposal);
  if (!validation.valid) return { status: "invalid", errors: validation.errors };
  const disclaimer = "A threshold is a number you entered, not a legal quorum. The label is not a legal identity.";
  if (!Array.isArray(options) || options.length !== proposal.clauses.length) {
    return {
      status: "unavailable",
      empty: true,
      text: `No inspected package is available, so there is no last at-or-above-threshold group label to copy. ${disclaimer}\n`,
    };
  }
  const listed = groupsMeetingApprovalThreshold(proposal, options);
  if (listed.status !== "ok") return listed;
  ...
  if (!last) {
    return {
      status: "ok",
      empty: true,
      text: `No group is at or above the approval threshold, so there is no last at-or-above-threshold group label to copy. ${disclaimer}\n`,
    };
  }
  return {
    status: "ok",
    empty: false,
    label: last.name,
    text: `Last at-or-above-threshold group: ${briefText(last.name)}. ${disclaimer}\n`,
  };
}
...
/**
 * One-line Markdown of the last group whose average on the inspected package
 * currently meets their declared support floor.
 * Uses the same meeting-floor list as hideLastGroupAtFloor.
 * Honest when none or no inspected package is available.
 * Distinct from last at-or-above-threshold group copy, first at-or-above-threshold
 * group copy, first at-floor group copy, and hideLastGroupAtFloor.
 * A floor is a number you entered, not a legal quorum.
 * Do not treat the label as a legal identity.
 */
export function formatLastGroupAtFloorLabelMarkdown(proposal, options) {
  const validation = validateProposal(proposal);
  if (!validation.valid) return { status: "invalid", errors: validation.errors };
  const disclaimer = "A floor is a number you entered, not a legal quorum. The label is not a legal identity.";
  if (!Array.isArray(options) || options.length !== proposal.clauses.length) {
    return {
      status: "unavailable",
      empty: true,
      text: `No inspected package is available, so there is no last at-floor group label to copy. ${disclaimer}\n`,
    };
  }
  const listed = groupsMeetingDeclaredSupportFloor(proposal, options);
  if (listed.status !== "ok") return listed;
  const last = listed.groups[listed.groups.length - 1];
  if (!last) {
    return {
      status: "ok",
      empty: true,
      text: `No group currently meets their support floor, so there is no last at-floor group label to copy. ${disclaimer}\n`,
    };
  }
  return {
    status: "ok",
    empty: false,
    label: last.name,
    text: `Last at-floor group: ${briefText(last.name)}. ${disclaimer}\n`,
  };
}
 ...
 * One-line Markdown of the first group whose average on the inspected package
 * currently meets their declared support floor.
 * Uses the same meeting-floor list as hideFirstGroupAtFloor.
 * Honest when none or no inspected package is available.
 * Distinct from last at-floor group copy, first at-or-above-threshold group copy,
 * and hideFirstGroupAtFloor.
 * A floor is a number you entered, not a legal quorum.
 * Do not treat the label as a legal identity.
 */
export function formatFirstGroupAtFloorLabelMarkdown(proposal, options) {
  const validation = validateProposal(proposal);
  if (!validation.valid) return { status: "invalid", errors: validation.errors };
  const disclaimer = "A floor is a number you entered, not a legal quorum. The label is not a legal identity.";
  if (!Array.isArray(options) || options.length !== proposal.clauses.length) {
    return {
      status: "unavailable",
      empty: true,
      text: `No inspected package is available, so there is no first at-floor group label to copy. ${disclaimer}\n`,
    };
  }
  const listed = groupsMeetingDeclaredSupportFloor(proposal, options);
  if (listed.status !== "ok") return listed;
  ...
  if (!first) {
    return {
      status: "ok",
      empty: true,
      text: `No group currently meets their support floor, so there is no first at-floor group label to copy. ${disclaimer}\n`,
    };
  }
  return {
    status: "ok",
    empty: false,
    label: first.name,
    text: `First at-floor group: ${briefText(first.name)}. ${disclaimer}\n`,
  };
}
 ...
 * Uses the same below-floor list as hideFirstGroupBelowFloor and hideLastGroupBelowFloor.
 * Honest when none or no inspected package is available.
 * Distinct from first-below-floor group copy, last below-threshold group copy,
 * last at-floor group copy, hideFirstGroupBelowFloor, and hideLastGroupBelowFloor.
 * A floor is a number you entered, not a legal quorum.
 * Do not treat the label as a legal identity.
 */
 ...
 * remaining copy, first-without-floor cost copy, last-without-floor cost copy,
...
/**
 * One-line Markdown of remaining mixing weight of the last group without a
 * declared support floor (minSupport missing).
 ...
 * last-without-floor cost copy, and remaining change-budget copy.
 * A floor is a number you entered, not a legal quorum.
 * Mixing weights are not a legal right.
 * Do not treat labels as legal identities.
 */
export function formatLastGroupWithoutFloorRemainingMarkdown(proposal, options) {
  const validation = validateProposal(proposal);
  if (!validation.valid) return { status: "invalid", errors: validation.errors };
  const disclaimer = "A floor is a number you entered, not a legal quorum.";
  if (!Array.isArray(options) || options.length !== proposal.clauses.length) {
    return {
      ...
      empty: true,
      remaining: 0,
      text: `No inspected package is available, so there is no last-without-floor remaining to copy. ${disclaimer}\n`,
    };
  }
  const selected = proposal.clauses.map((clause, index) => clause.options.find((option) => option.id === options[index]?.id) ?? null);
  if (selected.some((option) => !option)) {
  ...
  if (!last) {
    return {
      ...
      empty: true,
      remaining: 0,
      text: `Last-without-floor remaining: 0. ${disclaimer}\n`,
    };
  }
  const weightsById = new Map(proposal.groups.map((group) => [group.id, group.weight]));
  const remaining = weightsById.get(last.id);
  return {
    status: "ok",
    empty: remaining === 0,
    remaining,
 ...
 * hideLastGroupWithoutFloor.
 * Honest when remaining is zero, none, or no inspected package is available.
 * Distinct prefix from last-without-floor remaining copy, last-without-floor
 * cost copy, aggregate groups-without-floor remaining copy,
 * groups-without-floor count copy, first group-without-floor label copy, last
 * group-without-floor label copy, first-without-floor cost copy, and remaining
 * change-budget copy.
 * A floor is a number you entered, not a legal quorum.
 * Mixing weights are not a legal right.
 * Do not treat labels as legal identities.
 ...
 * Distinct prefix from last-without-floor cost copy, first-without-floor
 * remaining copy, last-without-floor remaining copy, aggregate
...
/**
 * One-line Markdown of mixing-weight cost of the last group without a
 * declared support floor (minSupport missing).
 * Walks groups from the end. Does not call formatFirstGroupWithoutFloorCostMarkdown.
 * Looks up that listed group's weight from proposal.groups.
 * Uses the same without-floor list as hideFirstGroupWithoutFloor,
 * hideLastGroupWithoutFloor, and formatLastGroupWithoutFloorRemainingMarkdown.
 * Honest when cost is zero, none, or no inspected package is available.
 * Distinct prefix from First-without-floor cost, First-without-floor remaining,
 * Last-without-floor remaining, Groups-without-floor remaining, Groups without a
 * support floor count, first/last group-without-floor label, and remaining
 * change-budget.
 * A floor is a number you entered, not a legal quorum.
 * Mixing weights are not a legal right.
 * Do not treat labels as legal identities.
 * Display-only. The solver ignores the copy.
 */
export function formatLastGroupWithoutFloorCostMarkdown(proposal, options) {
  const validation = validateProposal(proposal);
  if (!validation.valid) return { status: "invalid", errors: validation.errors };
  const disclaimer = "A floor is a number you entered, not a legal quorum.";
  if (!Array.isArray(options) || options.length !== proposal.clauses.length) {
    return {
      ...
      empty: true,
      cost: 0,
      text: `No inspected package is available, so there is no last-without-floor cost to copy. ${disclaimer}\n`,
    };
  }
  const selected = proposal.clauses.map((clause, index) => clause.options.find((option) => option.id === options[index]?.id) ?? null);
  if (selected.some((option) => !option)) {
  ...
  const listed = groupsWithoutDeclaredSupportFloor(proposal);
  if (listed.status !== "ok") return listed;
  const listedById = new Map(listed.groups.map((group) => [group.id, group]));
  let last = null;
  for (let index = proposal.groups.length - 1; index >= 0; index -= 1) {
    const candidate = listedById.get(proposal.groups[index].id);
    if (candidate) {
      last = candidate;
      break;
    }
  ...
  if (!last) {
    return {
      ...
      empty: true,
      cost: 0,
      text: `Last-without-floor cost: 0. ${disclaimer}\n`,
    };
  }
  const weightsById = new Map(proposal.groups.map((group) => [group.id, group.weight]));
  const cost = weightsById.get(last.id);
  return {
    status: "ok",
    empty: cost === 0,
    cost,
    text: `Last-without-floor cost: ${cost}. ${disclaimer}\n`,
  };
}
...
export function formatWorkspaceJson(proposal, prefs = {}) {
  ...
  const hideLastGroupAtOrAboveThreshold = readWorkspaceBoolean(prefs, "hideLastGroupAtOrAboveThreshold");
...
export function parseWorkspaceJson(text) {
  ...
  if (hideFirstGroupBelowThreshold.error) return { status: "invalid", errors: [hideFirstGroupBelowThreshold.error] };
  const hideLastGroupAtOrAboveThreshold = readWorkspaceBoolean(raw, "hideLastGroupAtOrAboveThreshold");
  if (hideLastGroupAtOrAboveThreshold.error) return { status: "invalid", errors: [hideLastGroupAtOrAboveThreshold.error] };
