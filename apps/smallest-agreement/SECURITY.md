...
Imported JSON is parsed and structurally validated before it replaces the active draft. Unknown fields on the proposal, groups, clauses, and options are discarded before the draft is stored, exported, or shared. Extra support keys, duplicate identifiers, reserved prototype keys, non-finite weights and costs, and unknown search options are rejected. Version-1 workspace JSON also rejects unknown wrapper keys and non-boolean display filter flags. Import files larger than 250 KB and URL fragments longer than 60,000 characters are rejected. Collection sizes, labels, and identifiers also have explicit limits before search or rendering; see [MODEL.md](MODEL.md). Invalid local storage and malformed URL fragments are ignored. Validation cannot determine whether scores, weights, labels, or costs are truthful or appropriate.
