...
The model compares explicitly supplied structured options. It finds the smallest calculated change that crosses a configurable approval threshold. It makes no claim that the result is a collective decision. Version 1.5.45 keeps the 1.5.0 review packet contract. Workshop display filters, print redaction, pasted tables, lock copy, unique group copies, and `moveClause` do not change search math. A veto is a number, not a legal right. Locks are draft choices, not a legal hold. Scratch cycling club hours is a scratch-bunch / bunch-finish / scratch-chip workshop, distinct from points-race cycling club hours (points-sprint / bell-lap / prime-board), omnium cycling club hours (flying-200 / tempo-board), madison cycling club hours (madison-pair / handsling / pairing-board), keirin cycling club hours (derny-pacer / keirin-chip), hill-climb cycling club hours (summit-marshal / climb-chip), time-trial cycling club hours (start-ramp / time-check / timing-chip), criterium cycling club hours (closed-circuit / pit-lane / number-board) and road cycling club hours (sealed-road / feed-station / wheel-bag).
