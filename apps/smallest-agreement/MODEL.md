...
The model compares explicitly supplied structured options. It finds the smallest calculated change that crosses a configurable approval threshold. It makes no claim that the result is a collective decision. Version 1.5.17 keeps the 1.5.0 review packet contract. Workshop display filters, print redaction, pasted tables, lock copy, unique group copies, and `moveClause` do not change search math. A veto is a number, not a legal right. Locks are draft choices, not a legal hold.
...
- `formatLastBelowThresholdGroupLabelMarkdown(proposal, options)` copies the last group whose average on the inspected package is below the numeric approval threshold as one-line Markdown. Honest when none or no inspected package is available. Distinct from last non-veto group copy and first-below-floor group copy. A threshold is a number you entered, not a legal quorum. Do not treat the label as a legal identity.
