...
The model compares explicitly supplied structured options. It finds the smallest calculated change that crosses a configurable approval threshold. It makes no claim that the result is a collective decision. Version 1.5.22 keeps the 1.5.0 review packet contract. Workshop display filters, print redaction, pasted tables, lock copy, unique group copies, and `moveClause` do not change search math. A veto is a number, not a legal right. Locks are draft choices, not a legal hold.
...
- `formatLastGroupAtFloorLabelMarkdown(proposal, options)` copies the last group whose average on the inspected package currently meets their declared support floor as one-line Markdown. Uses the same meeting-floor list as hideLastGroupAtFloor. Honest when none or no inspected package is available. Distinct from last at-or-above-threshold group copy, first at-or-above-threshold group copy, and hideLastGroupAtFloor. A floor is a number you entered, not a legal quorum. Do not treat the label as a legal identity.
- `formatLastBelowSupportFloorGroupLabelMarkdown(proposal, options)` copies the last group whose average on the inspected package is currently below their declared support floor as one-line Markdown. Uses the same below-floor list as hideFirstGroupBelowFloor. Honest when none or no inspected package is available. Distinct from first-below-floor group copy, last below-threshold group copy, last at-floor group copy, and hideFirstGroupBelowFloor. A floor is a number you entered, not a legal quorum. Do not treat the label as a legal identity.
