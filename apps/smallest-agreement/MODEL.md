...
- `formatWorkspaceJson(proposal, prefs)` writes a version-1 workspace document with `clauseDensity` of `compact` or `comfortable` (default comfortable), `vetoGroupsOnly` and `lockedClausesOnly` booleans (default false, meaning show all), and the canonical proposal. `parseWorkspaceJson` reads that document or a bare proposal. Omitted density restores to comfortable. Omitted filter flags restore to show-all. Invalid density or non-boolean filters are rejected. Density and these filters are display preferences; the solver ignores them.
