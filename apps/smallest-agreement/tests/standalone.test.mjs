...
test("standalone builder appImport only gained formatFirstGroupWithoutFloorLabelMarkdown", () => {
  ...
  assert.match(app, /} else if \(key === "F7"\) \{\n    event\.preventDefault\(\);\n    copyLastGroupWithoutFloor\(\);/u);
  assert.match(app, /} else if \(key === "F8"\) \{\n    event\.preventDefault\(\);\n    jumpToLastGroupWithoutFloorCopy\(\);/u);
  assert.match(app, /} else if \(key === "F9"\) \{\n    event\.preventDefault\(\);\n    jumpToHideLastGroupWithoutFloor\(\);/u);
  assert.match(app, /} else if \(key === "F10"\) \{\n    event\.preventDefault\(\);\n    copyFirstGroupWithoutFloor\(\);/u);
  assert.match(app, /} else if \(key === "F11"\) \{\n    event\.preventDefault\(\);\n    jumpToFirstGroupWithoutFloorCopy\(\);/u);
  assert.match(app, /} else if \(key === "F12"\) \{\n    event\.preventDefault\(\);\n    jumpToHideFirstGroupWithoutFloor\(\);/u);
});
