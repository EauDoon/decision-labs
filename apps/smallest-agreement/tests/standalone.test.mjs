...
test("cyclo-cross club hours preset loads a distinct synthetic cyclo-cross workshop", async () => {
  ...
  assert.equal(pkg.version, "1.5.45");
  const app = await savedWorkbench(new Map());
...
test("scratch cycling club hours preset loads a distinct synthetic scratch-cycling workshop", async () => {
  ...
  assert.equal(firstHeading, "## 1.5.45 - 2026-09-13");
  const app = await savedWorkbench(new Map());
  ...
  assert.doesNotMatch(app.clauses(), /10:30/u);
  assert.doesNotMatch(app.clauses(), /19:15/u);
  assert.doesNotMatch(app.clauses(), /14:25/u);
