...
test("standard navigation and function keys keep their browser behavior outside fields", async () => {
  const ui = await savedWorkbench();
  const keys = ["Home", "End", "PageUp", "PageDown", "ArrowUp", "ArrowDown", "ArrowLeft", "ArrowRight", "Delete", "Insert", "Backspace",
    "F1", "F2", "F3", "F4", "F5", "F6", "F7", "F8", "F9", "F10", "F11", "F12"];
  for (const key of keys) {
    assert.equal(ui.keydown(key), false, `${key} should not be prevented on the document`);
    assert.equal(ui.keydown(key, { tagName: "BODY", isContentEditable: false }, { shiftKey: true }), false, `Shift+${key} should not be prevented on the document`);
  }
});
...
test("madison cycling club hours preset loads a distinct synthetic madison-cycling workshop", async () => {
  ...
  app.click("#load-preset");
  assert.match(app.title(), /Madison cycling club hours: madison course booking, handsling hours, and pairing-board lock-up/u);
  assert.equal(app.disabled("#export-button"), false);
  assert.doesNotMatch(app.alert(), /Fix the proposal/u);
  assert.match(app.clauses(), /Madison course booking/u);
  assert.match(app.clauses(), /Handsling hours/u);
  assert.match(app.clauses(), /Pairing-board lock-up/u);
  assert.match(app.clauses(), /madison course/u);
  assert.match(app.clauses(), /handsling/u);
  assert.match(app.clauses(), /pairing-board/u);
  assert.match(app.clauses(), /madison cycling/u);
  assert.match(app.clauses(), /10:05/u);
  assert.match(app.clauses(), /18:50/u);
  assert.match(app.clauses(), /14:00/u);
  assert.match(app.clauses(), /pairing crew/u);
  assert.match(app.groups(), /Students/u);
  assert.match(app.groups(), /Neighbours/u);
  assert.match(app.groups(), /P&amp;C/u);
  assert.match(app.firstGroupWithoutFloor(), /First group without a support floor: Students/u);
  assert.match(app.lastGroupWithoutFloor(), /Last group without a support floor: P&amp;C/u);
  assert.match(app.groupsWithoutFloorCount(), /Groups without a support floor: 3/u);
  assert.match(app.groupsWithoutFloorRemaining(), /Groups-without-floor remaining: 56/u);
  assert.match(app.lastGroupWithoutFloorRemaining(), /Last-without-floor remaining: 21/u);
  assert.match(app.firstGroupWithoutFloorRemaining(), /First-without-floor remaining: 19/u);
  assert.match(app.firstGroupWithoutFloorCost(), /First-without-floor cost: 19/u);
  assert.match(app.lastGroupWithoutFloorCost(), /Last-without-floor cost: 21/u);
  assert.doesNotMatch(app.title(), /Keirin cycling club hours/u);
  assert.doesNotMatch(app.title(), /Hill-climb cycling club hours/u);
  assert.doesNotMatch(app.title(), /Time-trial cycling club hours/u);
  assert.doesNotMatch(app.title(), /Criterium cycling club hours/u);
  ...
  assert.doesNotMatch(app.clauses(), /keirin-gate/u);
  assert.doesNotMatch(app.clauses(), /09:50/u);
  assert.doesNotMatch(app.clauses(), /18:35/u);
  assert.doesNotMatch(app.clauses(), /13:45/u);
  assert.doesNotMatch(app.groups(), /Residents/u);
...
test("individual-pursuit cycling club hours preset loads a distinct synthetic individual-pursuit-cycling workshop", async () => {
  ...
  const app = await savedWorkbench(new Map());
  app.field("#preset-select", "individual-pursuit-cycling-club-hours");
  app.click("#load-preset");
  assert.match(app.title(), /Individual-pursuit cycling club hours: pursuit-line rota, catch-bell hours, and pursuit-chip lock-up/u);
  assert.equal(app.disabled("#export-button"), false);
  assert.doesNotMatch(app.alert(), /Fix the proposal/u);
  assert.match(app.clauses(), /Pursuit-line rota/u);
  assert.match(app.clauses(), /Catch-bell hours/u);
  assert.match(app.clauses(), /Pursuit-chip lock-up/u);
  assert.match(app.clauses(), /pursuit-line/u);
  assert.match(app.clauses(), /catch-bell/u);
  assert.match(app.clauses(), /pursuit-chip/u);
  assert.match(app.clauses(), /individual-pursuit cycling/u);
  assert.match(app.clauses(), /10:50/u);
  assert.match(app.clauses(), /19:35/u);
  assert.match(app.clauses(), /14:45/u);
  assert.match(app.clauses(), /pursuit crew/u);
  assert.match(app.groups(), /Students/u);
  assert.match(app.groups(), /Neighbours/u);
  assert.match(app.groups(), /P&amp;C/u);
  assert.match(app.firstGroupWithoutFloor(), /First group without a support floor: Students/u);
  assert.match(app.lastGroupWithoutFloor(), /Last group without a support floor: P&amp;C/u);
  assert.match(app.groupsWithoutFloorCount(), /Groups without a support floor: 3/u);
  assert.match(app.groupsWithoutFloorRemaining(), /Groups-without-floor remaining: 67/u);
  assert.match(app.lastGroupWithoutFloorRemaining(), /Last-without-floor remaining: 25/u);
  assert.match(app.firstGroupWithoutFloorRemaining(), /First-without-floor remaining: 23/u);
  assert.match(app.firstGroupWithoutFloorCost(), /First-without-floor cost: 23/u);
  assert.match(app.lastGroupWithoutFloorCost(), /Last-without-floor cost: 25/u);
  assert.doesNotMatch(app.title(), /Scratch cycling club hours/u);
  assert.doesNotMatch(app.title(), /Points-race cycling club hours/u);
  assert.doesNotMatch(app.title(), /Omnium cycling club hours/u);
  assert.doesNotMatch(app.title(), /Madison cycling club hours/u);
  assert.doesNotMatch(app.title(), /Keirin cycling club hours/u);
  assert.doesNotMatch(app.clauses(), /start-ramp/u);
  assert.doesNotMatch(app.clauses(), /timing-hut/u);
  assert.doesNotMatch(app.clauses(), /time-check/u);
  ...
  assert.doesNotMatch(app.clauses(), /keirin-chip/u);
  assert.doesNotMatch(app.clauses(), /keirin-gate/u);
  assert.doesNotMatch(app.clauses(), /madison-pair/u);
  assert.doesNotMatch(app.clauses(), /handsling/u);
  assert.doesNotMatch(app.clauses(), /pairing crew/u);
  assert.doesNotMatch(app.clauses(), /pacer crew/u);
  assert.doesNotMatch(app.clauses(), /flying-200/u);
  assert.doesNotMatch(app.clauses(), /tempo-board/u);
  assert.doesNotMatch(app.clauses(), /omnium-board/u);
  assert.doesNotMatch(app.clauses(), /tempo crew/u);
  assert.doesNotMatch(app.clauses(), /points-race/u);
  assert.doesNotMatch(app.clauses(), /points-sprint/u);
  assert.doesNotMatch(app.clauses(), /bell-lap/u);
  assert.doesNotMatch(app.clauses(), /prime-board/u);
  assert.doesNotMatch(app.clauses(), /bell crew/u);
  assert.doesNotMatch(app.clauses(), /scratch-bunch/u);
  assert.doesNotMatch(app.clauses(), /bunch-finish/u);
  assert.doesNotMatch(app.clauses(), /scratch-chip/u);
  assert.doesNotMatch(app.clauses(), /bunch crew/u);
  assert.doesNotMatch(app.clauses(), /10:40/u);
  assert.doesNotMatch(app.clauses(), /19:25/u);
  assert.doesNotMatch(app.clauses(), /14:35/u);
  assert.doesNotMatch(app.clauses(), /team-pursuit-rota/u);
  assert.doesNotMatch(app.clauses(), /changeover-bell/u);
  assert.doesNotMatch(app.clauses(), /quartet-board/u);
  assert.doesNotMatch(app.clauses(), /changeover crew/u);
  assert.doesNotMatch(app.clauses(), /11:00/u);
  assert.doesNotMatch(app.clauses(), /19:45/u);
  assert.doesNotMatch(app.clauses(), /14:55/u);
  assert.doesNotMatch(app.groups(), /Residents/u);
});
...
test("bottle-hand cycling club hours preset loads a distinct synthetic bottle-hand-cycling workshop", async () => {
  ...
  app.click("#load-preset");
  assert.match(app.title(), /Bottle-hand cycling club hours: bidon-pass, car-window hours, and crate-lock lock-up/u);
  assert.equal(app.disabled("#export-button"), false);
  assert.doesNotMatch(app.alert(), /Fix the proposal/u);
  assert.match(app.clauses(), /Bidon-pass/u);
  assert.match(app.clauses(), /Car-window hours/u);
  assert.match(app.clauses(), /Crate-lock lock-up/u);
  assert.match(app.clauses(), /bidon-pass/u);
  assert.match(app.clauses(), /car-window/u);
  assert.match(app.clauses(), /crate-lock/u);
  assert.match(app.clauses(), /bottle-hand cycling/u);
  assert.match(app.clauses(), /12:10/u);
  assert.match(app.clauses(), /20:55/u);
  assert.match(app.clauses(), /16:05/u);
  assert.match(app.clauses(), /crate crew/u);
  assert.match(app.groups(), /Students/u);
  assert.match(app.groups(), /Neighbours/u);
  assert.match(app.groups(), /P&amp;C/u);
  assert.match(app.firstGroupWithoutFloor(), /First group without a support floor: Students/u);
  assert.match(app.lastGroupWithoutFloor(), /Last group without a support floor: P&amp;C/u);
  assert.match(app.groupsWithoutFloorCount(), /Groups without a support floor: 3/u);
  assert.match(app.groupsWithoutFloorRemaining(), /Groups-without-floor remaining: 83/u);
  assert.match(app.lastGroupWithoutFloorRemaining(), /Last-without-floor remaining: 33/u);
  assert.match(app.firstGroupWithoutFloorRemaining(), /First-without-floor remaining: 31/u);
  assert.match(app.firstGroupWithoutFloorCost(), /First-without-floor cost: 31/u);
  assert.match(app.lastGroupWithoutFloorCost(), /Last-without-floor cost: 33/u);
  assert.doesNotMatch(app.title(), /Domestique cycling club hours/u);
  assert.doesNotMatch(app.title(), /Soigneur cycling club hours/u);
  assert.doesNotMatch(app.title(), /Lead-out cycling club hours/u);
  assert.doesNotMatch(app.title(), /Feed-zone cycling club hours/u);
  assert.doesNotMatch(app.clauses(), /wheel-change/u);
  assert.doesNotMatch(app.clauses(), /Neutral-service hours/u);
  assert.doesNotMatch(app.clauses(), /Team-car lock-up/u);
  assert.doesNotMatch(app.clauses(), /team-car/u);
  assert.doesNotMatch(app.clauses(), /car crew/u);
  assert.doesNotMatch(app.clauses(), /soigneur-station/u);
  assert.doesNotMatch(app.clauses(), /cape-bag/u);
  assert.doesNotMatch(app.clauses(), /rice-cake/u);
  assert.doesNotMatch(app.clauses(), /cake crew/u);
  assert.doesNotMatch(app.clauses(), /train-line/u);
  assert.doesNotMatch(app.clauses(), /last-rider/u);
  assert.doesNotMatch(app.clauses(), /sprinter-launch/u);
  assert.doesNotMatch(app.clauses(), /launch crew/u);
  assert.doesNotMatch(app.clauses(), /musette-line/u);
  assert.doesNotMatch(app.clauses(), /sticky-bottle/u);
  assert.doesNotMatch(app.clauses(), /bottle-hand-up/u);
  assert.doesNotMatch(app.clauses(), /bottle crew/u);
  assert.doesNotMatch(app.clauses(), /12:00/u);
  assert.doesNotMatch(app.clauses(), /20:45/u);
  assert.doesNotMatch(app.clauses(), /15:55/u);
  assert.doesNotMatch(app.groups(), /Residents/u);
});
...
test("domestique cycling club hours preset loads a distinct synthetic domestique-cycling workshop", async () => {
  ...
  const app = await savedWorkbench(new Map());
  app.field("#preset-select", "domestique-cycling-club-hours");
  app.click("#load-preset");
  assert.match(app.title(), /Domestique cycling club hours: wheel-change, neutral-service hours, and team-car lock-up/u);
  assert.equal(app.disabled("#export-button"), false);
  assert.doesNotMatch(app.alert(), /Fix the proposal/u);
  assert.match(app.clauses(), /Wheel-change/u);
  assert.match(app.clauses(), /Neutral-service hours/u);
  assert.match(app.clauses(), /Team-car lock-up/u);
  assert.match(app.clauses(), /wheel-change/u);
  assert.match(app.clauses(), /neutral-service/u);
  assert.match(app.clauses(), /team-car/u);
  assert.match(app.clauses(), /domestique cycling/u);
  assert.match(app.clauses(), /12:00/u);
  assert.match(app.clauses(), /20:45/u);
  assert.match(app.clauses(), /15:55/u);
  assert.match(app.clauses(), /car crew/u);
  assert.match(app.groups(), /Students/u);
  assert.match(app.groups(), /Neighbours/u);
  assert.match(app.groups(), /P&amp;C/u);
  assert.match(app.firstGroupWithoutFloor(), /First group without a support floor: Students/u);
  assert.match(app.lastGroupWithoutFloor(), /Last group without a support floor: P&amp;C/u);
  assert.match(app.groupsWithoutFloorCount(), /Groups without a support floor: 3/u);
  assert.match(app.groupsWithoutFloorRemaining(), /Groups-without-floor remaining: 81/u);
  assert.match(app.lastGroupWithoutFloorRemaining(), /Last-without-floor remaining: 32/u);
  assert.match(app.firstGroupWithoutFloorRemaining(), /First-without-floor remaining: 30/u);
  assert.match(app.firstGroupWithoutFloorCost(), /First-without-floor cost: 30/u);
  assert.match(app.lastGroupWithoutFloorCost(), /Last-without-floor cost: 32/u);
  assert.doesNotMatch(app.title(), /Soigneur cycling club hours/u);
  assert.doesNotMatch(app.title(), /Lead-out cycling club hours/u);
  assert.doesNotMatch(app.title(), /Feed-zone cycling club hours/u);
  assert.doesNotMatch(app.title(), /First-aid cycling club hours/u);
  assert.doesNotMatch(app.title(), /Team-sprint cycling club hours/u);
  assert.doesNotMatch(app.title(), /Team-pursuit cycling club hours/u);
  assert.doesNotMatch(app.title(), /Individual-pursuit cycling club hours/u);
  assert.doesNotMatch(app.title(), /Scratch cycling club hours/u);
  assert.doesNotMatch(app.title(), /Points-race cycling club hours/u);
  assert.doesNotMatch(app.title(), /Omnium cycling club hours/u);
  assert.doesNotMatch(app.title(), /Madison cycling club hours/u);
  ...
  assert.doesNotMatch(app.clauses(), /bottle-hand-up/u);
  assert.doesNotMatch(app.clauses(), /bottle crew/u);
  assert.doesNotMatch(app.clauses(), /treatment-tent/u);
  assert.doesNotMatch(app.clauses(), /ice-pack-rota/u);
  assert.doesNotMatch(app.clauses(), /triage-board/u);
  assert.doesNotMatch(app.clauses(), /ice-pack crew/u);
  assert.doesNotMatch(app.clauses(), /start-gate/u);
  assert.doesNotMatch(app.clauses(), /flying-relay/u);
  assert.doesNotMatch(app.clauses(), /trio-board/u);
  assert.doesNotMatch(app.clauses(), /relay crew/u);
  assert.doesNotMatch(app.clauses(), /team-pursuit-rota/u);
  assert.doesNotMatch(app.clauses(), /changeover-bell/u);
  assert.doesNotMatch(app.clauses(), /quartet-board/u);
  assert.doesNotMatch(app.clauses(), /changeover crew/u);
  ...
  assert.doesNotMatch(app.clauses(), /scratch-chip/u);
  assert.doesNotMatch(app.clauses(), /bunch crew/u);
  assert.doesNotMatch(app.clauses(), /pursuit-line/u);
  assert.doesNotMatch(app.clauses(), /catch-bell/u);
  assert.doesNotMatch(app.clauses(), /pursuit-chip/u);
  assert.doesNotMatch(app.clauses(), /pursuit crew/u);
  assert.doesNotMatch(app.clauses(), /11:50/u);
  assert.doesNotMatch(app.clauses(), /20:35/u);
  assert.doesNotMatch(app.clauses(), /15:45/u);
  assert.doesNotMatch(app.clauses(), /11:40/u);
  assert.doesNotMatch(app.clauses(), /20:25/u);
  assert.doesNotMatch(app.clauses(), /15:35/u);
  assert.doesNotMatch(app.groups(), /Residents/u);
});
...
test("soigneur cycling club hours preset loads a distinct synthetic soigneur-cycling workshop", async () => {
  ...
  const app = await savedWorkbench(new Map());
  app.field("#preset-select", "soigneur-cycling-club-hours");
  app.click("#load-preset");
  assert.match(app.title(), /Soigneur cycling club hours: soigneur-station, cape-bag hours, and rice-cake lock-up/u);
  assert.equal(app.disabled("#export-button"), false);
  assert.doesNotMatch(app.alert(), /Fix the proposal/u);
  assert.match(app.clauses(), /Soigneur-station/u);
  assert.match(app.clauses(), /Cape-bag hours/u);
  assert.match(app.clauses(), /Rice-cake lock-up/u);
  assert.match(app.clauses(), /soigneur-station/u);
  assert.match(app.clauses(), /cape-bag/u);
  assert.match(app.clauses(), /rice-cake/u);
  assert.match(app.clauses(), /soigneur cycling/u);
  assert.match(app.clauses(), /11:50/u);
  assert.match(app.clauses(), /20:35/u);
  assert.match(app.clauses(), /15:45/u);
  assert.match(app.clauses(), /cake crew/u);
  assert.match(app.groups(), /Students/u);
  assert.match(app.groups(), /Neighbours/u);
  assert.match(app.groups(), /P&amp;C/u);
  assert.match(app.firstGroupWithoutFloor(), /First group without a support floor: Students/u);
  assert.match(app.lastGroupWithoutFloor(), /Last group without a support floor: P&amp;C/u);
  assert.match(app.groupsWithoutFloorCount(), /Groups without a support floor: 3/u);
  assert.match(app.groupsWithoutFloorRemaining(), /Groups-without-floor remaining: 79/u);
  assert.match(app.lastGroupWithoutFloorRemaining(), /Last-without-floor remaining: 31/u);
  assert.match(app.firstGroupWithoutFloorRemaining(), /First-without-floor remaining: 29/u);
  assert.match(app.firstGroupWithoutFloorCost(), /First-without-floor cost: 29/u);
  assert.match(app.lastGroupWithoutFloorCost(), /Last-without-floor cost: 31/u);
  assert.doesNotMatch(app.title(), /Domestique cycling club hours/u);
  assert.doesNotMatch(app.title(), /Lead-out cycling club hours/u);
  assert.doesNotMatch(app.title(), /Feed-zone cycling club hours/u);
  assert.doesNotMatch(app.title(), /First-aid cycling club hours/u);
  ...
  assert.doesNotMatch(app.clauses(), /pursuit crew/u);
  assert.doesNotMatch(app.clauses(), /wheel-change/u);
  assert.doesNotMatch(app.clauses(), /neutral-service/u);
  assert.doesNotMatch(app.clauses(), /team-car/u);
  assert.doesNotMatch(app.clauses(), /car crew/u);
  assert.doesNotMatch(app.clauses(), /12:00/u);
  assert.doesNotMatch(app.clauses(), /20:45/u);
  ...
  assert.doesNotMatch(app.clauses(), /20:25/u);
  assert.doesNotMatch(app.clauses(), /15:35/u);
  assert.doesNotMatch(app.clauses(), /11:30/u);
  assert.doesNotMatch(app.clauses(), /20:15/u);
  assert.doesNotMatch(app.clauses(), /15:25/u);
  assert.doesNotMatch(app.groups(), /Residents/u);
});
...
test("lead-out cycling club hours preset loads a distinct synthetic lead-out-cycling workshop", async () => {
  ...
  const app = await savedWorkbench(new Map());
  app.field("#preset-select", "lead-out-cycling-club-hours");
  app.click("#load-preset");
  assert.match(app.title(), /Lead-out cycling club hours: train-line, last-rider hours, and sprinter-launch lock-up/u);
  assert.equal(app.disabled("#export-button"), false);
  assert.doesNotMatch(app.alert(), /Fix the proposal/u);
  assert.match(app.clauses(), /Train-line/u);
  assert.match(app.clauses(), /Last-rider hours/u);
  assert.match(app.clauses(), /Sprinter-launch lock-up/u);
  assert.match(app.clauses(), /train-line/u);
  assert.match(app.clauses(), /last-rider/u);
  assert.match(app.clauses(), /sprinter-launch/u);
  assert.match(app.clauses(), /lead-out cycling/u);
  assert.match(app.clauses(), /11:40/u);
  assert.match(app.clauses(), /20:25/u);
  assert.match(app.clauses(), /15:35/u);
  assert.match(app.clauses(), /launch crew/u);
  assert.match(app.groups(), /Students/u);
  assert.match(app.groups(), /Neighbours/u);
  assert.match(app.groups(), /P&amp;C/u);
  assert.match(app.firstGroupWithoutFloor(), /First group without a support floor: Students/u);
  assert.match(app.lastGroupWithoutFloor(), /Last group without a support floor: P&amp;C/u);
  assert.match(app.groupsWithoutFloorCount(), /Groups without a support floor: 3/u);
  assert.match(app.groupsWithoutFloorRemaining(), /Groups-without-floor remaining: 77/u);
  assert.match(app.lastGroupWithoutFloorRemaining(), /Last-without-floor remaining: 30/u);
  assert.match(app.firstGroupWithoutFloorRemaining(), /First-without-floor remaining: 28/u);
  assert.match(app.firstGroupWithoutFloorCost(), /First-without-floor cost: 28/u);
  assert.match(app.lastGroupWithoutFloorCost(), /Last-without-floor cost: 30/u);
  assert.doesNotMatch(app.title(), /Feed-zone cycling club hours/u);
  assert.doesNotMatch(app.title(), /First-aid cycling club hours/u);
  assert.doesNotMatch(app.title(), /Team-sprint cycling club hours/u);
  assert.doesNotMatch(app.title(), /Team-pursuit cycling club hours/u);
  ...
  assert.doesNotMatch(app.clauses(), /20:15/u);
  assert.doesNotMatch(app.clauses(), /15:25/u);
  assert.doesNotMatch(app.clauses(), /11:20/u);
  assert.doesNotMatch(app.clauses(), /20:05/u);
  assert.doesNotMatch(app.clauses(), /15:15/u);
  assert.doesNotMatch(app.groups(), /Residents/u);
});
...
test("feed-zone cycling club hours preset loads a distinct synthetic feed-zone-cycling workshop", async () => {
  ...
  const app = await savedWorkbench(new Map());
  app.field("#preset-select", "feed-zone-cycling-club-hours");
  app.click("#load-preset");
  assert.match(app.title(), /Feed-zone cycling club hours: musette-line, sticky-bottle hours, and bottle-hand-up lock-up/u);
  assert.equal(app.disabled("#export-button"), false);
  assert.doesNotMatch(app.alert(), /Fix the proposal/u);
  assert.match(app.clauses(), /Musette-line/u);
  assert.match(app.clauses(), /Sticky-bottle hours/u);
  assert.match(app.clauses(), /Bottle-hand-up lock-up/u);
  assert.match(app.clauses(), /musette-line/u);
  assert.match(app.clauses(), /sticky-bottle/u);
  assert.match(app.clauses(), /bottle-hand-up/u);
  assert.match(app.clauses(), /feed-zone cycling/u);
  assert.match(app.clauses(), /11:30/u);
  assert.match(app.clauses(), /20:15/u);
  assert.match(app.clauses(), /15:25/u);
  assert.match(app.clauses(), /bottle crew/u);
  assert.match(app.groups(), /Students/u);
  assert.match(app.groups(), /Neighbours/u);
  assert.match(app.groups(), /P&amp;C/u);
  assert.match(app.firstGroupWithoutFloor(), /First group without a support floor: Students/u);
  assert.match(app.lastGroupWithoutFloor(), /Last group without a support floor: P&amp;C/u);
  assert.match(app.groupsWithoutFloorCount(), /Groups without a support floor: 3/u);
  assert.match(app.groupsWithoutFloorRemaining(), /Groups-without-floor remaining: 75/u);
  assert.match(app.lastGroupWithoutFloorRemaining(), /Last-without-floor remaining: 29/u);
  assert.match(app.firstGroupWithoutFloorRemaining(), /First-without-floor remaining: 27/u);
  assert.match(app.firstGroupWithoutFloorCost(), /First-without-floor cost: 27/u);
  assert.match(app.lastGroupWithoutFloorCost(), /Last-without-floor cost: 29/u);
  assert.doesNotMatch(app.title(), /First-aid cycling club hours/u);
  assert.doesNotMatch(app.title(), /Team-sprint cycling club hours/u);
  assert.doesNotMatch(app.title(), /Team-pursuit cycling club hours/u);
  assert.doesNotMatch(app.title(), /Individual-pursuit cycling club hours/u);
...
test("first-aid cycling club hours preset loads a distinct synthetic first-aid-cycling workshop", async () => {
  ...
  const app = await savedWorkbench(new Map());
  app.field("#preset-select", "first-aid-cycling-club-hours");
  app.click("#load-preset");
  assert.match(app.title(), /First-aid cycling club hours: treatment-tent, ice-pack-rota hours, and triage-board lock-up/u);
  assert.equal(app.disabled("#export-button"), false);
  assert.doesNotMatch(app.alert(), /Fix the proposal/u);
  assert.match(app.clauses(), /Treatment-tent/u);
  assert.match(app.clauses(), /Ice-pack-rota hours/u);
  assert.match(app.clauses(), /Triage-board lock-up/u);
  assert.match(app.clauses(), /treatment-tent/u);
  assert.match(app.clauses(), /ice-pack-rota/u);
  assert.match(app.clauses(), /triage-board/u);
  assert.match(app.clauses(), /first-aid cycling/u);
  assert.match(app.clauses(), /11:20/u);
  assert.match(app.clauses(), /20:05/u);
  assert.match(app.clauses(), /15:15/u);
  assert.match(app.clauses(), /ice-pack crew/u);
  assert.match(app.groups(), /Students/u);
  assert.match(app.groups(), /Neighbours/u);
  assert.match(app.groups(), /P&amp;C/u);
  assert.match(app.firstGroupWithoutFloor(), /First group without a support floor: Students/u);
  assert.match(app.lastGroupWithoutFloor(), /Last group without a support floor: P&amp;C/u);
  assert.match(app.groupsWithoutFloorCount(), /Groups without a support floor: 3/u);
  assert.match(app.groupsWithoutFloorRemaining(), /Groups-without-floor remaining: 73/u);
  assert.match(app.lastGroupWithoutFloorRemaining(), /Last-without-floor remaining: 28/u);
  assert.match(app.firstGroupWithoutFloorRemaining(), /First-without-floor remaining: 26/u);
  assert.match(app.firstGroupWithoutFloorCost(), /First-without-floor cost: 26/u);
  assert.match(app.lastGroupWithoutFloorCost(), /Last-without-floor cost: 28/u);
  assert.doesNotMatch(app.title(), /Team-sprint cycling club hours/u);
  assert.doesNotMatch(app.title(), /Team-pursuit cycling club hours/u);
  assert.doesNotMatch(app.title(), /Individual-pursuit cycling club hours/u);
  assert.doesNotMatch(app.title(), /Scratch cycling club hours/u);
  ...
  assert.doesNotMatch(app.clauses(), /pursuit-chip/u);
  assert.doesNotMatch(app.clauses(), /pursuit crew/u);
  assert.doesNotMatch(app.clauses(), /11:10/u);
  assert.doesNotMatch(app.clauses(), /19:55/u);
  assert.doesNotMatch(app.clauses(), /15:05/u);
  assert.doesNotMatch(app.groups(), /Residents/u);
});
...
test("team-sprint cycling club hours preset loads a distinct synthetic team-sprint-cycling workshop", async () => {
  ...
  const app = await savedWorkbench(new Map());
  app.field("#preset-select", "team-sprint-cycling-club-hours");
  app.click("#load-preset");
  assert.match(app.title(), /Team-sprint cycling club hours: start-gate, flying-relay hours, and trio-board lock-up/u);
  assert.equal(app.disabled("#export-button"), false);
  assert.doesNotMatch(app.alert(), /Fix the proposal/u);
  assert.match(app.clauses(), /Start-gate/u);
  assert.match(app.clauses(), /Flying-relay hours/u);
  assert.match(app.clauses(), /Trio-board lock-up/u);
  assert.match(app.clauses(), /start-gate/u);
  assert.match(app.clauses(), /flying-relay/u);
  assert.match(app.clauses(), /trio-board/u);
  assert.match(app.clauses(), /team-sprint cycling/u);
  assert.match(app.clauses(), /11:10/u);
  assert.match(app.clauses(), /19:55/u);
  assert.match(app.clauses(), /15:05/u);
  assert.match(app.clauses(), /relay crew/u);
  assert.match(app.groups(), /Students/u);
  assert.match(app.groups(), /Neighbours/u);
  assert.match(app.groups(), /P&amp;C/u);
  assert.match(app.firstGroupWithoutFloor(), /First group without a support floor: Students/u);
  assert.match(app.lastGroupWithoutFloor(), /Last group without a support floor: P&amp;C/u);
  assert.match(app.groupsWithoutFloorCount(), /Groups without a support floor: 3/u);
  assert.match(app.groupsWithoutFloorRemaining(), /Groups-without-floor remaining: 71/u);
  assert.match(app.lastGroupWithoutFloorRemaining(), /Last-without-floor remaining: 27/u);
  assert.match(app.firstGroupWithoutFloorRemaining(), /First-without-floor remaining: 25/u);
  assert.match(app.firstGroupWithoutFloorCost(), /First-without-floor cost: 25/u);
  assert.match(app.lastGroupWithoutFloorCost(), /Last-without-floor cost: 27/u);
  assert.doesNotMatch(app.title(), /Team-pursuit cycling club hours/u);
  assert.doesNotMatch(app.title(), /Individual-pursuit cycling club hours/u);
  assert.doesNotMatch(app.title(), /Scratch cycling club hours/u);
  assert.doesNotMatch(app.title(), /Points-race cycling club hours/u);
...
test("team-pursuit cycling club hours preset loads a distinct synthetic team-pursuit-cycling workshop", async () => {
  ...
  const app = await savedWorkbench(new Map());
  app.field("#preset-select", "team-pursuit-cycling-club-hours");
  app.click("#load-preset");
  assert.match(app.title(), /Team-pursuit cycling club hours: team-pursuit-rota, changeover-bell hours, and quartet-board lock-up/u);
  assert.equal(app.disabled("#export-button"), false);
  assert.doesNotMatch(app.alert(), /Fix the proposal/u);
  assert.match(app.clauses(), /Team-pursuit-rota/u);
  assert.match(app.clauses(), /Changeover-bell hours/u);
  assert.match(app.clauses(), /Quartet-board lock-up/u);
  assert.match(app.clauses(), /team-pursuit-rota/u);
  assert.match(app.clauses(), /changeover-bell/u);
  assert.match(app.clauses(), /quartet-board/u);
  assert.match(app.clauses(), /team-pursuit cycling/u);
  assert.match(app.clauses(), /11:00/u);
  assert.match(app.clauses(), /19:45/u);
  assert.match(app.clauses(), /14:55/u);
  assert.match(app.clauses(), /changeover crew/u);
  assert.match(app.groups(), /Students/u);
  assert.match(app.groups(), /Neighbours/u);
  assert.match(app.groups(), /P&amp;C/u);
  assert.match(app.firstGroupWithoutFloor(), /First group without a support floor: Students/u);
  assert.match(app.lastGroupWithoutFloor(), /Last group without a support floor: P&amp;C/u);
  assert.match(app.groupsWithoutFloorCount(), /Groups without a support floor: 3/u);
  assert.match(app.groupsWithoutFloorRemaining(), /Groups-without-floor remaining: 69/u);
  assert.match(app.lastGroupWithoutFloorRemaining(), /Last-without-floor remaining: 26/u);
  assert.match(app.firstGroupWithoutFloorRemaining(), /First-without-floor remaining: 24/u);
  assert.match(app.firstGroupWithoutFloorCost(), /First-without-floor cost: 24/u);
  assert.match(app.lastGroupWithoutFloorCost(), /Last-without-floor cost: 26/u);
  assert.doesNotMatch(app.title(), /Individual-pursuit cycling club hours/u);
  assert.doesNotMatch(app.title(), /Scratch cycling club hours/u);
  assert.doesNotMatch(app.title(), /Points-race cycling club hours/u);
  assert.doesNotMatch(app.title(), /Omnium cycling club hours/u);
  ...
  assert.doesNotMatch(app.clauses(), /pursuit-chip/u);
  assert.doesNotMatch(app.clauses(), /pursuit crew/u);
  assert.doesNotMatch(app.clauses(), /10:50/u);
  assert.doesNotMatch(app.clauses(), /19:35/u);
  assert.doesNotMatch(app.clauses(), /14:45/u);
  assert.doesNotMatch(app.groups(), /Residents/u);
});
...
test("scratch cycling club hours preset loads a distinct synthetic scratch-cycling workshop", async () => {
  ...
  const app = await savedWorkbench(new Map());
  app.field("#preset-select", "scratch-cycling-club-hours");
  app.click("#load-preset");
  assert.match(app.title(), /Scratch cycling club hours: scratch-bunch rota, bunch-finish hours, and scratch-chip lock-up/u);
  assert.equal(app.disabled("#export-button"), false);
  assert.doesNotMatch(app.alert(), /Fix the proposal/u);
  assert.match(app.clauses(), /Scratch-bunch rota/u);
  assert.match(app.clauses(), /Bunch-finish hours/u);
  assert.match(app.clauses(), /Scratch-chip lock-up/u);
  assert.match(app.clauses(), /scratch-bunch/u);
  assert.match(app.clauses(), /bunch-finish/u);
  assert.match(app.clauses(), /scratch-chip/u);
  assert.match(app.clauses(), /scratch cycling/u);
  assert.match(app.clauses(), /10:40/u);
  assert.match(app.clauses(), /19:25/u);
  assert.match(app.clauses(), /14:35/u);
  assert.match(app.clauses(), /bunch crew/u);
  assert.match(app.groups(), /Students/u);
  assert.match(app.groups(), /Neighbours/u);
  assert.match(app.groups(), /P&amp;C/u);
  assert.match(app.firstGroupWithoutFloor(), /First group without a support floor: Students/u);
  assert.match(app.lastGroupWithoutFloor(), /Last group without a support floor: P&amp;C/u);
  assert.match(app.groupsWithoutFloorCount(), /Groups without a support floor: 3/u);
  assert.match(app.groupsWithoutFloorRemaining(), /Groups-without-floor remaining: 65/u);
  assert.match(app.lastGroupWithoutFloorRemaining(), /Last-without-floor remaining: 24/u);
  assert.match(app.firstGroupWithoutFloorRemaining(), /First-without-floor remaining: 22/u);
  assert.match(app.firstGroupWithoutFloorCost(), /First-without-floor cost: 22/u);
  assert.match(app.lastGroupWithoutFloorCost(), /Last-without-floor cost: 24/u);
  assert.doesNotMatch(app.title(), /Points-race cycling club hours/u);
  assert.doesNotMatch(app.title(), /Omnium cycling club hours/u);
  assert.doesNotMatch(app.title(), /Madison cycling club hours/u);
  assert.doesNotMatch(app.title(), /Keirin cycling club hours/u);
...
test("points-race cycling club hours preset loads a distinct synthetic points-race-cycling workshop", async () => {
  ...
  const app = await savedWorkbench(new Map());
  app.field("#preset-select", "points-race-cycling-club-hours");
  app.click("#load-preset");
  assert.match(app.title(), /Points-race cycling club hours: points-race course booking, bell-lap hours, and prime-board lock-up/u);
  assert.equal(app.disabled("#export-button"), false);
  assert.doesNotMatch(app.alert(), /Fix the proposal/u);
  assert.match(app.clauses(), /Points-race course booking/u);
  assert.match(app.clauses(), /Bell-lap hours/u);
  assert.match(app.clauses(), /Prime-board lock-up/u);
  assert.match(app.clauses(), /points-race course/u);
  assert.match(app.clauses(), /bell-lap/u);
  assert.match(app.clauses(), /prime-board/u);
  assert.match(app.clauses(), /points-race cycling/u);
  assert.match(app.clauses(), /10:30/u);
  assert.match(app.clauses(), /19:15/u);
  assert.match(app.clauses(), /14:25/u);
  assert.match(app.clauses(), /bell crew/u);
  assert.match(app.groups(), /Students/u);
  assert.match(app.groups(), /Neighbours/u);
  assert.match(app.groups(), /P&amp;C/u);
  assert.match(app.firstGroupWithoutFloor(), /First group without a support floor: Students/u);
  assert.match(app.lastGroupWithoutFloor(), /Last group without a support floor: P&amp;C/u);
  assert.match(app.groupsWithoutFloorCount(), /Groups without a support floor: 3/u);
  assert.match(app.groupsWithoutFloorRemaining(), /Groups-without-floor remaining: 62/u);
  assert.match(app.lastGroupWithoutFloorRemaining(), /Last-without-floor remaining: 23/u);
  assert.match(app.firstGroupWithoutFloorRemaining(), /First-without-floor remaining: 21/u);
  assert.match(app.firstGroupWithoutFloorCost(), /First-without-floor cost: 21/u);
  assert.match(app.lastGroupWithoutFloorCost(), /Last-without-floor cost: 23/u);
  assert.doesNotMatch(app.title(), /Omnium cycling club hours/u);
  assert.doesNotMatch(app.title(), /Madison cycling club hours/u);
  assert.doesNotMatch(app.title(), /Keirin cycling club hours/u);
  assert.doesNotMatch(app.title(), /Hill-climb cycling club hours/u);
  ...
  assert.doesNotMatch(app.clauses(), /omnium-board/u);
  assert.doesNotMatch(app.clauses(), /tempo crew/u);
  assert.doesNotMatch(app.clauses(), /10:20/u);
  assert.doesNotMatch(app.clauses(), /19:05/u);
  assert.doesNotMatch(app.clauses(), /14:15/u);
  assert.doesNotMatch(app.groups(), /Residents/u);
});
...
test("omnium cycling club hours preset loads a distinct synthetic omnium-cycling workshop", async () => {
  ...
  const app = await savedWorkbench(new Map());
  app.field("#preset-select", "omnium-cycling-club-hours");
  app.click("#load-preset");
  assert.match(app.title(), /Omnium cycling club hours: omnium course booking, flying-200 hours, and tempo-board lock-up/u);
  assert.equal(app.disabled("#export-button"), false);
  assert.doesNotMatch(app.alert(), /Fix the proposal/u);
  assert.match(app.clauses(), /Omnium course booking/u);
  assert.match(app.clauses(), /Flying-200 hours/u);
  assert.match(app.clauses(), /Tempo-board lock-up/u);
  assert.match(app.clauses(), /omnium course/u);
  assert.match(app.clauses(), /flying-200/u);
  assert.match(app.clauses(), /tempo-board/u);
  assert.match(app.clauses(), /omnium cycling/u);
  assert.match(app.clauses(), /10:20/u);
  assert.match(app.clauses(), /19:05/u);
  assert.match(app.clauses(), /14:15/u);
  assert.match(app.clauses(), /tempo crew/u);
  assert.match(app.groups(), /Students/u);
  assert.match(app.groups(), /Neighbours/u);
  assert.match(app.groups(), /P&amp;C/u);
  assert.match(app.firstGroupWithoutFloor(), /First group without a support floor: Students/u);
  assert.match(app.lastGroupWithoutFloor(), /Last group without a support floor: P&amp;C/u);
  assert.match(app.groupsWithoutFloorCount(), /Groups without a support floor: 3/u);
  assert.match(app.groupsWithoutFloorRemaining(), /Groups-without-floor remaining: 59/u);
  assert.match(app.lastGroupWithoutFloorRemaining(), /Last-without-floor remaining: 22/u);
  assert.match(app.firstGroupWithoutFloorRemaining(), /First-without-floor remaining: 20/u);
  assert.match(app.firstGroupWithoutFloorCost(), /First-without-floor cost: 20/u);
  assert.match(app.lastGroupWithoutFloorCost(), /Last-without-floor cost: 22/u);
  assert.doesNotMatch(app.title(), /Madison cycling club hours/u);
  assert.doesNotMatch(app.title(), /Keirin cycling club hours/u);
  assert.doesNotMatch(app.title(), /Hill-climb cycling club hours/u);
  assert.doesNotMatch(app.title(), /Time-trial cycling club hours/u);
  ...
  assert.doesNotMatch(app.clauses(), /pairing crew/u);
  assert.doesNotMatch(app.clauses(), /pacer crew/u);
  assert.doesNotMatch(app.clauses(), /10:05/u);
  assert.doesNotMatch(app.clauses(), /18:50/u);
  assert.doesNotMatch(app.clauses(), /14:00/u);
  assert.doesNotMatch(app.groups(), /Residents/u);
});
