...
test("uncovered leftover Markdown is organizer-private counts and units only", () => {
  ...
  const markdown = uncoveredLeftoverEvidenceToMarkdown(scenario);
  const coverage = computeResidualCoverage(scenario);
  assert.match(markdown, /organizer private/);
  assert.match(markdown, /Not a merchant export/);
...
test("the buyer room copies uncovered leftover evidence with a textarea fallback", async () => {
  const html = await readFile(new URL("../index.html", import.meta.url), "utf8");
  const app = await readFile(new URL("../src/app.js", import.meta.url), "utf8");
  const buyerPanel = html.slice(html.indexOf('id="buyer-panel"'), html.indexOf('id="merchant-panel"'));
  const merchantPanel = html.slice(html.indexOf('id="merchant-panel"'), html.indexOf('id="method-panel"'));
  assert.match(buyerPanel, /id="copy-uncovered-leftover-evidence"/u);
  assert.match(buyerPanel, /Copy uncovered leftover evidence \(organizer private\)/u);
  assert.equal(merchantPanel.includes("copy-uncovered-leftover-evidence"), false);
  assert.match(app, /uncoveredLeftoverEvidenceToMarkdown\(/u);
  assert.match(app, /function copyUncoveredLeftoverEvidence\(/u);
