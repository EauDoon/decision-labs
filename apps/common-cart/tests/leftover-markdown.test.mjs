...
test("soigneur cycling carnival leftover uncovered leftover-only remaining stays distinct from leftover-only allocated, leftover-only headroom, leftover-only capacity, leftover-only minimum, leftover unit price, leftover fill remaining, and lead-out leftover-only remaining", () => {
  const soigneurCycling = clonePreset("soigneurCyclingCarnivalLunch");
  const leftoverOnlyUnitPrice = createLeftoverUncoveredLeftoverOnlyUnitPriceMarkdown(soigneurCycling);
  const leftoverOnlyCapacity = createLeftoverUncoveredLeftoverOnlyCapacityMarkdown(soigneurCycling);
  const leftoverOnlyAllocated = createLeftoverUncoveredLeftoverOnlyAllocatedMarkdown(soigneurCycling);
  const leftoverOnlyHeadroom = createLeftoverUncoveredLeftoverOnlyHeadroomMarkdown(soigneurCycling);
  const leftoverOnlyMinimum = createLeftoverUncoveredLeftoverOnlyMinimumMarkdown(soigneurCycling);
  const leftoverOnlyMaximum = createLeftoverUncoveredLeftoverOnlyMaximumMarkdown(soigneurCycling);
  const leftoverOnlyRemaining = createLeftoverUncoveredLeftoverOnlyRemainingMarkdown(soigneurCycling);
  const leftoverOnlyCount = createLeftoverUncoveredLeftoverOnlyCountMarkdown(soigneurCycling);
  const leftoverOffer = soigneurCycling.offers.find((offer) => offer.id === computeResidualCoverage(soigneurCycling).secondary.offerId);
  assert.equal(leftoverOffer.minimumUnits, 51);
  assert.equal(leftoverOffer.unitPrice, 48);
  assert.equal(leftoverOffer.capacity, 64);
  assert.match(leftoverOnlyRemaining, /leftover uncovered leftover-only remaining \(organizer private\): 52\./);
  assert.match(leftoverOnlyAllocated, /leftover uncovered leftover-only allocated \(organizer private\): 52\./);
  assert.match(leftoverOnlyHeadroom, /leftover uncovered leftover-only headroom \(organizer private\): 12\./);
  assert.match(leftoverOnlyMinimum, /leftover uncovered leftover-only minimum \(organizer private\): 15\./);
  assert.match(leftoverOnlyUnitPrice, /leftover uncovered leftover-only unit price \(organizer private\): 48\./);
  assert.match(leftoverOnlyCapacity, /leftover uncovered leftover-only capacity \(organizer private\): 64\./);
  assert.notEqual(52, leftoverOffer.minimumUnits);
  assert.notEqual(52, leftoverOffer.unitPrice);
  assert.notEqual(52, leftoverOffer.capacity);
  assert.notEqual(52, 48);
  assert.notEqual(52, 12);
  assert.notEqual(52, 15);
  assert.notEqual(52, 51);
  assert.notEqual(52, 49);
  assert.notEqual(52, 47);
  assert.notEqual(leftoverOnlyRemaining, leftoverOnlyAllocated);
  assert.notEqual(leftoverOnlyRemaining, leftoverOnlyUnitPrice);
  assert.notEqual(leftoverOnlyRemaining, leftoverOnlyCapacity);
  assert.notEqual(leftoverOnlyRemaining, leftoverOnlyHeadroom);
  ...
  const keirinCycling = clonePreset("keirinCyclingCarnivalLunch");
  const hillClimbCycling = clonePreset("hillClimbCyclingCarnivalLunch");
  assert.notEqual(createLeftoverUncoveredLeftoverOnlyRemainingMarkdown(soigneurCycling), createLeftoverUncoveredLeftoverOnlyRemainingMarkdown(leadOutCycling));
  assert.notEqual(createLeftoverUncoveredLeftoverOnlyRemainingMarkdown(soigneurCycling), createLeftoverUncoveredLeftoverOnlyRemainingMarkdown(feedZoneCycling));
  assert.notEqual(createLeftoverUncoveredLeftoverOnlyRemainingMarkdown(soigneurCycling), createLeftoverUncoveredLeftoverOnlyRemainingMarkdown(firstAidCycling));
  assert.notEqual(createLeftoverUncoveredLeftoverOnlyRemainingMarkdown(soigneurCycling), createLeftoverUncoveredLeftoverOnlyRemainingMarkdown(teamSprintCycling));
  assert.notEqual(createLeftoverUncoveredLeftoverOnlyRemainingMarkdown(soigneurCycling), createLeftoverUncoveredLeftoverOnlyRemainingMarkdown(teamPursuitCycling));
  assert.notEqual(createLeftoverUncoveredLeftoverOnlyRemainingMarkdown(soigneurCycling), createLeftoverUncoveredLeftoverOnlyRemainingMarkdown(pursuitCycling));
  assert.notEqual(createLeftoverUncoveredLeftoverOnlyRemainingMarkdown(soigneurCycling), createLeftoverUncoveredLeftoverOnlyRemainingMarkdown(scratchCycling));
  assert.notEqual(createLeftoverUncoveredLeftoverOnlyRemainingMarkdown(soigneurCycling), createLeftoverUncoveredLeftoverOnlyRemainingMarkdown(pointsRaceCycling));
  assert.notEqual(createLeftoverUncoveredLeftoverOnlyRemainingMarkdown(soigneurCycling), createLeftoverUncoveredLeftoverOnlyRemainingMarkdown(omniumCycling));
  assert.notEqual(createLeftoverUncoveredLeftoverOnlyRemainingMarkdown(soigneurCycling), createLeftoverUncoveredLeftoverOnlyRemainingMarkdown(madisonCycling));
  assert.notEqual(createLeftoverUncoveredLeftoverOnlyRemainingMarkdown(soigneurCycling), createLeftoverUncoveredLeftoverOnlyRemainingMarkdown(keirinCycling));
  assert.notEqual(createLeftoverUncoveredLeftoverOnlyRemainingMarkdown(soigneurCycling), createLeftoverUncoveredLeftoverOnlyRemainingMarkdown(hillClimbCycling));
  assert.notEqual(createLeftoverUncoveredLeftoverOnlyUnitPriceMarkdown(soigneurCycling), createLeftoverUncoveredLeftoverOnlyUnitPriceMarkdown(leadOutCycling));
  assert.notEqual(createLeftoverUncoveredLeftoverOnlyHeadroomMarkdown(soigneurCycling), createLeftoverUncoveredLeftoverOnlyHeadroomMarkdown(leadOutCycling));
  assert.notEqual(createLeftoverUncoveredLeftoverOnlyUnitPriceMarkdown(soigneurCycling), createLeftoverUncoveredLeftoverOnlyUnitPriceMarkdown(feedZoneCycling));
  assert.notEqual(createLeftoverUncoveredLeftoverOnlyHeadroomMarkdown(soigneurCycling), createLeftoverUncoveredLeftoverOnlyHeadroomMarkdown(feedZoneCycling));
  assert.match(createLeftoverUncoveredLeftoverOnlyRemainingMarkdown(leadOutCycling), /leftover uncovered leftover-only remaining \(organizer private\): 51\./);
  assert.match(createLeftoverUncoveredLeftoverOnlyRemainingMarkdown(feedZoneCycling), /leftover uncovered leftover-only remaining \(organizer private\): 49\./);
  assert.match(createLeftoverUncoveredLeftoverOnlyRemainingMarkdown(firstAidCycling), /leftover uncovered leftover-only remaining \(organizer private\): 47\./);
  assert.match(createLeftoverUncoveredLeftoverOnlyRemainingMarkdown(teamSprintCycling), /leftover uncovered leftover-only remaining \(organizer private\): 45\./);
  ...
  assert.match(createLeftoverUncoveredLeftoverOnlyRemainingMarkdown(keirinCycling), /leftover uncovered leftover-only remaining \(organizer private\): 35\./);
  assert.match(createLeftoverUncoveredLeftoverOnlyRemainingMarkdown(hillClimbCycling), /leftover uncovered leftover-only remaining \(organizer private\): 34\./);
  assert.equal(leadOutCycling.offers.find((offer) => offer.id === computeResidualCoverage(leadOutCycling).secondary.offerId).capacity, 64);
  assert.equal(feedZoneCycling.offers.find((offer) => offer.id === computeResidualCoverage(feedZoneCycling).secondary.offerId).capacity, 64);
  assert.equal(firstAidCycling.offers.find((offer) => offer.id === computeResidualCoverage(firstAidCycling).secondary.offerId).capacity, 64);
  assert.equal(teamSprintCycling.offers.find((offer) => offer.id === computeResidualCoverage(teamSprintCycling).secondary.offerId).capacity, 64);
  assert.equal(teamPursuitCycling.offers.find((offer) => offer.id === computeResidualCoverage(teamPursuitCycling).secondary.offerId).capacity, 64);
...
test("domestique cycling carnival leftover uncovered leftover-only remaining stays distinct from leftover-only allocated, leftover-only headroom, leftover-only capacity, leftover-only minimum, leftover unit price, leftover fill remaining, and lead-out leftover-only remaining", () => {
  const domestiqueCycling = clonePreset("domestiqueCyclingCarnivalLunch");
  const leftoverOnlyUnitPrice = createLeftoverUncoveredLeftoverOnlyUnitPriceMarkdown(domestiqueCycling);
  const leftoverOnlyCapacity = createLeftoverUncoveredLeftoverOnlyCapacityMarkdown(domestiqueCycling);
  const leftoverOnlyAllocated = createLeftoverUncoveredLeftoverOnlyAllocatedMarkdown(domestiqueCycling);
  const leftoverOnlyHeadroom = createLeftoverUncoveredLeftoverOnlyHeadroomMarkdown(domestiqueCycling);
  const leftoverOnlyMinimum = createLeftoverUncoveredLeftoverOnlyMinimumMarkdown(domestiqueCycling);
  const leftoverOnlyMaximum = createLeftoverUncoveredLeftoverOnlyMaximumMarkdown(domestiqueCycling);
  const leftoverOnlyRemaining = createLeftoverUncoveredLeftoverOnlyRemainingMarkdown(domestiqueCycling);
  const leftoverOnlyCount = createLeftoverUncoveredLeftoverOnlyCountMarkdown(domestiqueCycling);
  const leftoverOffer = domestiqueCycling.offers.find((offer) => offer.id === computeResidualCoverage(domestiqueCycling).secondary.offerId);
  assert.equal(leftoverOffer.minimumUnits, 52);
  assert.equal(leftoverOffer.unitPrice, 48);
  assert.equal(leftoverOffer.capacity, 64);
  assert.match(leftoverOnlyRemaining, /leftover uncovered leftover-only remaining \(organizer private\): 53\./);
  assert.match(leftoverOnlyAllocated, /leftover uncovered leftover-only allocated \(organizer private\): 53\./);
  assert.match(leftoverOnlyHeadroom, /leftover uncovered leftover-only headroom \(organizer private\): 11\./);
  assert.match(leftoverOnlyMinimum, /leftover uncovered leftover-only minimum \(organizer private\): 15\./);
  assert.match(leftoverOnlyUnitPrice, /leftover uncovered leftover-only unit price \(organizer private\): 48\./);
  assert.match(leftoverOnlyCapacity, /leftover uncovered leftover-only capacity \(organizer private\): 64\./);
  assert.notEqual(53, leftoverOffer.minimumUnits);
  assert.notEqual(53, leftoverOffer.unitPrice);
  assert.notEqual(53, leftoverOffer.capacity);
  assert.notEqual(53, 48);
  assert.notEqual(53, 11);
  assert.notEqual(53, 12);
  assert.notEqual(53, 15);
  assert.notEqual(53, 52);
  assert.notEqual(53, 51);
  assert.notEqual(53, 49);
  assert.notEqual(53, 47);
  assert.notEqual(leftoverOnlyRemaining, leftoverOnlyAllocated);
  assert.notEqual(leftoverOnlyRemaining, leftoverOnlyUnitPrice);
  assert.notEqual(leftoverOnlyRemaining, leftoverOnlyCapacity);
  assert.notEqual(leftoverOnlyRemaining, leftoverOnlyHeadroom);
  ...
  const bottleHandCycling = clonePreset("bottleHandCyclingCarnivalLunch");
  assert.notEqual(createLeftoverUncoveredLeftoverOnlyRemainingMarkdown(domestiqueCycling), createLeftoverUncoveredLeftoverOnlyRemainingMarkdown(bottleHandCycling));
  assert.match(createLeftoverUncoveredLeftoverOnlyRemainingMarkdown(bottleHandCycling), /leftover uncovered leftover-only remaining \(organizer private\): 54\./);
  const soigneurCycling = clonePreset("soigneurCyclingCarnivalLunch");
  assert.notEqual(createLeftoverUncoveredLeftoverOnlyRemainingMarkdown(domestiqueCycling), createLeftoverUncoveredLeftoverOnlyRemainingMarkdown(soigneurCycling));
  assert.notEqual(createLeftoverUncoveredLeftoverOnlyRemainingMarkdown(domestiqueCycling), createLeftoverUncoveredLeftoverOnlyRemainingMarkdown(leadOutCycling));
  assert.notEqual(createLeftoverUncoveredLeftoverOnlyRemainingMarkdown(domestiqueCycling), createLeftoverUncoveredLeftoverOnlyRemainingMarkdown(feedZoneCycling));
  assert.notEqual(createLeftoverUncoveredLeftoverOnlyRemainingMarkdown(domestiqueCycling), createLeftoverUncoveredLeftoverOnlyRemainingMarkdown(firstAidCycling));
  assert.notEqual(createLeftoverUncoveredLeftoverOnlyRemainingMarkdown(domestiqueCycling), createLeftoverUncoveredLeftoverOnlyRemainingMarkdown(teamSprintCycling));
  assert.notEqual(createLeftoverUncoveredLeftoverOnlyRemainingMarkdown(domestiqueCycling), createLeftoverUncoveredLeftoverOnlyRemainingMarkdown(teamPursuitCycling));
  assert.notEqual(createLeftoverUncoveredLeftoverOnlyRemainingMarkdown(domestiqueCycling), createLeftoverUncoveredLeftoverOnlyRemainingMarkdown(pursuitCycling));
  assert.notEqual(createLeftoverUncoveredLeftoverOnlyRemainingMarkdown(domestiqueCycling), createLeftoverUncoveredLeftoverOnlyRemainingMarkdown(scratchCycling));
  assert.notEqual(createLeftoverUncoveredLeftoverOnlyRemainingMarkdown(domestiqueCycling), createLeftoverUncoveredLeftoverOnlyRemainingMarkdown(pointsRaceCycling));
  assert.notEqual(createLeftoverUncoveredLeftoverOnlyRemainingMarkdown(domestiqueCycling), createLeftoverUncoveredLeftoverOnlyRemainingMarkdown(omniumCycling));
  assert.notEqual(createLeftoverUncoveredLeftoverOnlyRemainingMarkdown(domestiqueCycling), createLeftoverUncoveredLeftoverOnlyRemainingMarkdown(madisonCycling));
  assert.notEqual(createLeftoverUncoveredLeftoverOnlyRemainingMarkdown(domestiqueCycling), createLeftoverUncoveredLeftoverOnlyRemainingMarkdown(keirinCycling));
  assert.notEqual(createLeftoverUncoveredLeftoverOnlyRemainingMarkdown(domestiqueCycling), createLeftoverUncoveredLeftoverOnlyRemainingMarkdown(hillClimbCycling));
  assert.notEqual(createLeftoverUncoveredLeftoverOnlyUnitPriceMarkdown(domestiqueCycling), createLeftoverUncoveredLeftoverOnlyUnitPriceMarkdown(leadOutCycling));
  assert.notEqual(createLeftoverUncoveredLeftoverOnlyHeadroomMarkdown(domestiqueCycling), createLeftoverUncoveredLeftoverOnlyHeadroomMarkdown(leadOutCycling));
  assert.notEqual(createLeftoverUncoveredLeftoverOnlyUnitPriceMarkdown(domestiqueCycling), createLeftoverUncoveredLeftoverOnlyUnitPriceMarkdown(feedZoneCycling));
  assert.notEqual(createLeftoverUncoveredLeftoverOnlyHeadroomMarkdown(domestiqueCycling), createLeftoverUncoveredLeftoverOnlyHeadroomMarkdown(feedZoneCycling));
  assert.match(createLeftoverUncoveredLeftoverOnlyRemainingMarkdown(soigneurCycling), /leftover uncovered leftover-only remaining \(organizer private\): 52\./);
  assert.match(createLeftoverUncoveredLeftoverOnlyRemainingMarkdown(leadOutCycling), /leftover uncovered leftover-only remaining \(organizer private\): 51\./);
  assert.match(createLeftoverUncoveredLeftoverOnlyRemainingMarkdown(feedZoneCycling), /leftover uncovered leftover-only remaining \(organizer private\): 49\./);
  assert.match(createLeftoverUncoveredLeftoverOnlyRemainingMarkdown(firstAidCycling), /leftover uncovered leftover-only remaining \(organizer private\): 47\./);
  assert.match(createLeftoverUncoveredLeftoverOnlyRemainingMarkdown(teamSprintCycling), /leftover uncovered leftover-only remaining \(organizer private\): 45\./);
...
test("bottle-hand cycling carnival leftover uncovered leftover-only remaining stays distinct from leftover-only allocated, leftover-only headroom, leftover-only capacity, leftover-only minimum, leftover unit price, leftover fill remaining, and lead-out leftover-only remaining", () => {
  const bottleHandCycling = clonePreset("bottleHandCyclingCarnivalLunch");
  const leftoverOnlyUnitPrice = createLeftoverUncoveredLeftoverOnlyUnitPriceMarkdown(bottleHandCycling);
  const leftoverOnlyCapacity = createLeftoverUncoveredLeftoverOnlyCapacityMarkdown(bottleHandCycling);
  const leftoverOnlyAllocated = createLeftoverUncoveredLeftoverOnlyAllocatedMarkdown(bottleHandCycling);
  const leftoverOnlyHeadroom = createLeftoverUncoveredLeftoverOnlyHeadroomMarkdown(bottleHandCycling);
  const leftoverOnlyMinimum = createLeftoverUncoveredLeftoverOnlyMinimumMarkdown(bottleHandCycling);
  const leftoverOnlyMaximum = createLeftoverUncoveredLeftoverOnlyMaximumMarkdown(bottleHandCycling);
  const leftoverOnlyRemaining = createLeftoverUncoveredLeftoverOnlyRemainingMarkdown(bottleHandCycling);
  const leftoverOnlyCount = createLeftoverUncoveredLeftoverOnlyCountMarkdown(bottleHandCycling);
  const leftoverOffer = bottleHandCycling.offers.find((offer) => offer.id === computeResidualCoverage(bottleHandCycling).secondary.offerId);
  assert.equal(leftoverOffer.minimumUnits, 53);
  assert.equal(leftoverOffer.unitPrice, 48);
  assert.equal(leftoverOffer.capacity, 64);
  assert.match(leftoverOnlyRemaining, /leftover uncovered leftover-only remaining \(organizer private\): 54\./);
  assert.match(leftoverOnlyAllocated, /leftover uncovered leftover-only allocated \(organizer private\): 54\./);
  assert.match(leftoverOnlyHeadroom, /leftover uncovered leftover-only headroom \(organizer private\): 10\./);
  assert.match(leftoverOnlyMinimum, /leftover uncovered leftover-only minimum \(organizer private\): 15\./);
  assert.match(leftoverOnlyUnitPrice, /leftover uncovered leftover-only unit price \(organizer private\): 48\./);
  assert.match(leftoverOnlyCapacity, /leftover uncovered leftover-only capacity \(organizer private\): 64\./);
  assert.notEqual(54, leftoverOffer.minimumUnits);
  assert.notEqual(54, leftoverOffer.unitPrice);
  assert.notEqual(54, leftoverOffer.capacity);
  assert.notEqual(54, 48);
  assert.notEqual(54, 10);
  assert.notEqual(54, 11);
  assert.notEqual(54, 12);
  assert.notEqual(54, 15);
  assert.notEqual(54, 53);
  assert.notEqual(54, 52);
  assert.notEqual(54, 51);
  assert.notEqual(54, 49);
  assert.notEqual(54, 47);
  assert.notEqual(leftoverOnlyRemaining, leftoverOnlyAllocated);
  assert.notEqual(leftoverOnlyRemaining, leftoverOnlyUnitPrice);
  assert.notEqual(leftoverOnlyRemaining, leftoverOnlyCapacity);
  assert.notEqual(leftoverOnlyRemaining, leftoverOnlyHeadroom);
  ...
  const soigneurCycling = clonePreset("soigneurCyclingCarnivalLunch");
  const domestiqueCycling = clonePreset("domestiqueCyclingCarnivalLunch");
  assert.notEqual(createLeftoverUncoveredLeftoverOnlyRemainingMarkdown(bottleHandCycling), createLeftoverUncoveredLeftoverOnlyRemainingMarkdown(domestiqueCycling));
  assert.notEqual(createLeftoverUncoveredLeftoverOnlyRemainingMarkdown(bottleHandCycling), createLeftoverUncoveredLeftoverOnlyRemainingMarkdown(soigneurCycling));
  assert.notEqual(createLeftoverUncoveredLeftoverOnlyRemainingMarkdown(bottleHandCycling), createLeftoverUncoveredLeftoverOnlyRemainingMarkdown(leadOutCycling));
  assert.notEqual(createLeftoverUncoveredLeftoverOnlyRemainingMarkdown(bottleHandCycling), createLeftoverUncoveredLeftoverOnlyRemainingMarkdown(feedZoneCycling));
  assert.notEqual(createLeftoverUncoveredLeftoverOnlyRemainingMarkdown(bottleHandCycling), createLeftoverUncoveredLeftoverOnlyRemainingMarkdown(firstAidCycling));
  assert.notEqual(createLeftoverUncoveredLeftoverOnlyRemainingMarkdown(bottleHandCycling), createLeftoverUncoveredLeftoverOnlyRemainingMarkdown(teamSprintCycling));
  assert.notEqual(createLeftoverUncoveredLeftoverOnlyRemainingMarkdown(bottleHandCycling), createLeftoverUncoveredLeftoverOnlyRemainingMarkdown(teamPursuitCycling));
  assert.notEqual(createLeftoverUncoveredLeftoverOnlyRemainingMarkdown(bottleHandCycling), createLeftoverUncoveredLeftoverOnlyRemainingMarkdown(pursuitCycling));
  assert.notEqual(createLeftoverUncoveredLeftoverOnlyRemainingMarkdown(bottleHandCycling), createLeftoverUncoveredLeftoverOnlyRemainingMarkdown(scratchCycling));
  assert.notEqual(createLeftoverUncoveredLeftoverOnlyRemainingMarkdown(bottleHandCycling), createLeftoverUncoveredLeftoverOnlyRemainingMarkdown(pointsRaceCycling));
  assert.notEqual(createLeftoverUncoveredLeftoverOnlyRemainingMarkdown(bottleHandCycling), createLeftoverUncoveredLeftoverOnlyRemainingMarkdown(omniumCycling));
  assert.notEqual(createLeftoverUncoveredLeftoverOnlyRemainingMarkdown(bottleHandCycling), createLeftoverUncoveredLeftoverOnlyRemainingMarkdown(madisonCycling));
  assert.notEqual(createLeftoverUncoveredLeftoverOnlyRemainingMarkdown(bottleHandCycling), createLeftoverUncoveredLeftoverOnlyRemainingMarkdown(keirinCycling));
  assert.notEqual(createLeftoverUncoveredLeftoverOnlyRemainingMarkdown(bottleHandCycling), createLeftoverUncoveredLeftoverOnlyRemainingMarkdown(hillClimbCycling));
  assert.notEqual(createLeftoverUncoveredLeftoverOnlyUnitPriceMarkdown(bottleHandCycling), createLeftoverUncoveredLeftoverOnlyUnitPriceMarkdown(leadOutCycling));
  assert.notEqual(createLeftoverUncoveredLeftoverOnlyHeadroomMarkdown(bottleHandCycling), createLeftoverUncoveredLeftoverOnlyHeadroomMarkdown(leadOutCycling));
  assert.notEqual(createLeftoverUncoveredLeftoverOnlyUnitPriceMarkdown(bottleHandCycling), createLeftoverUncoveredLeftoverOnlyUnitPriceMarkdown(feedZoneCycling));
  assert.notEqual(createLeftoverUncoveredLeftoverOnlyHeadroomMarkdown(bottleHandCycling), createLeftoverUncoveredLeftoverOnlyHeadroomMarkdown(feedZoneCycling));
  assert.match(createLeftoverUncoveredLeftoverOnlyRemainingMarkdown(domestiqueCycling), /leftover uncovered leftover-only remaining \(organizer private\): 53\./);
  assert.match(createLeftoverUncoveredLeftoverOnlyRemainingMarkdown(soigneurCycling), /leftover uncovered leftover-only remaining \(organizer private\): 52\./);
  assert.match(createLeftoverUncoveredLeftoverOnlyRemainingMarkdown(leadOutCycling), /leftover uncovered leftover-only remaining \(organizer private\): 51\./);
  assert.match(createLeftoverUncoveredLeftoverOnlyRemainingMarkdown(feedZoneCycling), /leftover uncovered leftover-only remaining \(organizer private\): 49\./);
  assert.match(createLeftoverUncoveredLeftoverOnlyRemainingMarkdown(firstAidCycling), /leftover uncovered leftover-only remaining \(organizer private\): 47\./);
