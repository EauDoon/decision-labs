...
test("gravel cycling carnival leftover uncovered leftover-only remaining stays distinct from leftover-only allocated, leftover-only headroom, leftover-only capacity, leftover-only minimum, leftover unit price, and leftover fill remaining", () => {
  const gravelCycling = clonePreset("gravelCyclingCarnivalLunch");
  const leftoverOnlyUnitPrice = createLeftoverUncoveredLeftoverOnlyUnitPriceMarkdown(gravelCycling);
  const leftoverOnlyCapacity = createLeftoverUncoveredLeftoverOnlyCapacityMarkdown(gravelCycling);
  const leftoverOnlyAllocated = createLeftoverUncoveredLeftoverOnlyAllocatedMarkdown(gravelCycling);
  const leftoverOnlyHeadroom = createLeftoverUncoveredLeftoverOnlyHeadroomMarkdown(gravelCycling);
  const leftoverOnlyMinimum = createLeftoverUncoveredLeftoverOnlyMinimumMarkdown(gravelCycling);
  const leftoverOnlyMaximum = createLeftoverUncoveredLeftoverOnlyMaximumMarkdown(gravelCycling);
  const leftoverOnlyRemaining = createLeftoverUncoveredLeftoverOnlyRemainingMarkdown(gravelCycling);
  const leftoverOnlyCount = createLeftoverUncoveredLeftoverOnlyCountMarkdown(gravelCycling);
  const leftoverOffer = gravelCycling.offers.find((offer) => offer.id === computeResidualCoverage(gravelCycling).secondary.offerId);
  assert.equal(leftoverOffer.minimumUnits, 29);
  assert.equal(leftoverOffer.unitPrice, 24);
  assert.equal(leftoverOffer.capacity, 58);
  assert.match(leftoverOnlyRemaining, /leftover uncovered leftover-only remaining \(organizer private\): 30\./);
  assert.match(leftoverOnlyAllocated, /leftover uncovered leftover-only allocated \(organizer private\): 30\./);
  assert.match(leftoverOnlyHeadroom, /leftover uncovered leftover-only headroom \(organizer private\): 28\./);
  assert.match(leftoverOnlyMinimum, /leftover uncovered leftover-only minimum \(organizer private\): 15\./);
  assert.match(leftoverOnlyUnitPrice, /leftover uncovered leftover-only unit price \(organizer private\): 24\./);
  assert.match(leftoverOnlyCapacity, /leftover uncovered leftover-only capacity \(organizer private\): 58\./);
  assert.notEqual(30, leftoverOffer.minimumUnits);
  assert.notEqual(30, leftoverOffer.unitPrice);
  assert.notEqual(30, leftoverOffer.capacity);
  assert.notEqual(30, 28);
  assert.notEqual(30, 15);
  assert.notEqual(24, leftoverOffer.minimumUnits);
  assert.notEqual(24, leftoverOffer.capacity);
  assert.notEqual(24, 28);
  assert.notEqual(24, 15);
  assert.notEqual(58, leftoverOffer.unitPrice);
  assert.notEqual(58, leftoverOffer.minimumUnits);
  assert.notEqual(58, 30);
  assert.notEqual(58, 28);
  ...
  assert.notEqual(leftoverOnlyCapacity, leftoverOnlyAllocated);
  assert.notEqual(leftoverOnlyCapacity, leftoverOnlyHeadroom);
  assert.notEqual(leftoverOnlyRemaining.startsWith("Common Cart leftover uncovered leftover-only remaining"), false);
  assert.notEqual(leftoverOnlyAllocated.startsWith("Common Cart leftover uncovered leftover-only allocated"), false);
  const trackCycling = clonePreset("trackCyclingCarnivalLunch");
  const trackOffer = trackCycling.offers.find((offer) => offer.id === computeResidualCoverage(trackCycling).secondary.offerId);
  assert.equal(trackOffer.minimumUnits, 28);
  assert.equal(trackOffer.unitPrice, 23);
  assert.equal(trackOffer.capacity, 56);
  assert.match(createLeftoverUncoveredLeftoverOnlyRemainingMarkdown(trackCycling), /leftover uncovered leftover-only remaining \(organizer private\): 29\./);
  assert.match(createLeftoverUncoveredLeftoverOnlyHeadroomMarkdown(trackCycling), /leftover uncovered leftover-only headroom \(organizer private\): 27\./);
  assert.match(createLeftoverUncoveredLeftoverOnlyMinimumMarkdown(trackCycling), /leftover uncovered leftover-only minimum \(organizer private\): 14\./);
  assert.match(createLeftoverUncoveredLeftoverOnlyCapacityMarkdown(trackCycling), /leftover uncovered leftover-only capacity \(organizer private\): 56\./);
  const cycloCross = clonePreset("cycloCrossCarnivalLunch");
  const cycloCrossOffer = cycloCross.offers.find((offer) => offer.id === computeResidualCoverage(cycloCross).secondary.offerId);
  assert.equal(cycloCrossOffer.minimumUnits, 27);
  assert.equal(cycloCrossOffer.unitPrice, 22);
  assert.equal(cycloCrossOffer.capacity, 54);
  assert.match(createLeftoverUncoveredLeftoverOnlyRemainingMarkdown(cycloCross), /leftover uncovered leftover-only remaining \(organizer private\): 28\./);
  assert.match(createLeftoverUncoveredLeftoverOnlyHeadroomMarkdown(cycloCross), /leftover uncovered leftover-only headroom \(organizer private\): 26\./);
  assert.notEqual(createLeftoverUncoveredLeftoverOnlyRemainingMarkdown(gravelCycling), createLeftoverUncoveredLeftoverOnlyRemainingMarkdown(trackCycling));
  assert.notEqual(createLeftoverUncoveredLeftoverOnlyRemainingMarkdown(gravelCycling), createLeftoverUncoveredLeftoverOnlyRemainingMarkdown(cycloCross));
  assert.notEqual(createLeftoverUncoveredLeftoverOnlyHeadroomMarkdown(gravelCycling), createLeftoverUncoveredLeftoverOnlyHeadroomMarkdown(trackCycling));
  assert.notEqual(createLeftoverUncoveredLeftoverOnlyHeadroomMarkdown(gravelCycling), createLeftoverUncoveredLeftoverOnlyHeadroomMarkdown(cycloCross));
  assert.notEqual(createLeftoverUncoveredLeftoverOnlyMinimumMarkdown(gravelCycling), createLeftoverUncoveredLeftoverOnlyMinimumMarkdown(trackCycling));
  assert.notEqual(createLeftoverUncoveredLeftoverOnlyUnitPriceMarkdown(gravelCycling), createLeftoverUncoveredLeftoverOnlyUnitPriceMarkdown(trackCycling));
  assert.notEqual(createLeftoverUncoveredLeftoverOnlyCapacityMarkdown(gravelCycling), createLeftoverUncoveredLeftoverOnlyCapacityMarkdown(trackCycling));
});

test("road cycling carnival leftover uncovered leftover-only remaining stays distinct from leftover-only allocated, leftover-only headroom, leftover-only capacity, leftover-only minimum, leftover unit price, leftover fill remaining, and gravel leftover-only remaining", () => {
  const roadCycling = clonePreset("roadCyclingCarnivalLunch");
  const leftoverOnlyUnitPrice = createLeftoverUncoveredLeftoverOnlyUnitPriceMarkdown(roadCycling);
  const leftoverOnlyCapacity = createLeftoverUncoveredLeftoverOnlyCapacityMarkdown(roadCycling);
  const leftoverOnlyAllocated = createLeftoverUncoveredLeftoverOnlyAllocatedMarkdown(roadCycling);
  const leftoverOnlyHeadroom = createLeftoverUncoveredLeftoverOnlyHeadroomMarkdown(roadCycling);
  const leftoverOnlyMinimum = createLeftoverUncoveredLeftoverOnlyMinimumMarkdown(roadCycling);
  const leftoverOnlyMaximum = createLeftoverUncoveredLeftoverOnlyMaximumMarkdown(roadCycling);
  const leftoverOnlyRemaining = createLeftoverUncoveredLeftoverOnlyRemainingMarkdown(roadCycling);
  const leftoverOnlyCount = createLeftoverUncoveredLeftoverOnlyCountMarkdown(roadCycling);
  const leftoverOffer = roadCycling.offers.find((offer) => offer.id === computeResidualCoverage(roadCycling).secondary.offerId);
  assert.equal(leftoverOffer.minimumUnits, 30);
  assert.equal(leftoverOffer.unitPrice, 25);
  assert.equal(leftoverOffer.capacity, 60);
  assert.match(leftoverOnlyRemaining, /leftover uncovered leftover-only remaining \(organizer private\): 31\./);
  assert.match(leftoverOnlyAllocated, /leftover uncovered leftover-only allocated \(organizer private\): 31\./);
  assert.match(leftoverOnlyHeadroom, /leftover uncovered leftover-only headroom \(organizer private\): 29\./);
  assert.match(leftoverOnlyMinimum, /leftover uncovered leftover-only minimum \(organizer private\): 15\./);
  assert.match(leftoverOnlyUnitPrice, /leftover uncovered leftover-only unit price \(organizer private\): 25\./);
  assert.match(leftoverOnlyCapacity, /leftover uncovered leftover-only capacity \(organizer private\): 60\./);
  assert.notEqual(31, leftoverOffer.minimumUnits);
  assert.notEqual(31, leftoverOffer.unitPrice);
  assert.notEqual(31, leftoverOffer.capacity);
  assert.notEqual(31, 29);
  assert.notEqual(31, 15);
  assert.notEqual(25, leftoverOffer.minimumUnits);
  assert.notEqual(25, leftoverOffer.capacity);
  assert.notEqual(25, 29);
  assert.notEqual(25, 15);
  assert.notEqual(60, leftoverOffer.unitPrice);
  assert.notEqual(60, leftoverOffer.minimumUnits);
  assert.notEqual(60, 31);
  assert.notEqual(60, 29);
  assert.notEqual(leftoverOnlyRemaining, leftoverOnlyAllocated);
  assert.notEqual(leftoverOnlyRemaining, leftoverOnlyUnitPrice);
  assert.notEqual(leftoverOnlyRemaining, leftoverOnlyCapacity);
  assert.notEqual(leftoverOnlyRemaining, leftoverOnlyHeadroom);
  ...
  assert.notEqual(leftoverOnlyAllocated.startsWith("Common Cart leftover uncovered leftover-only allocated"), false);
  const gravelCycling = clonePreset("gravelCyclingCarnivalLunch");
  const gravelOffer = gravelCycling.offers.find((offer) => offer.id === computeResidualCoverage(gravelCycling).secondary.offerId);
  assert.equal(gravelOffer.minimumUnits, 29);
  assert.equal(gravelOffer.unitPrice, 24);
  assert.equal(gravelOffer.capacity, 58);
  assert.match(createLeftoverUncoveredLeftoverOnlyRemainingMarkdown(gravelCycling), /leftover uncovered leftover-only remaining \(organizer private\): 30\./);
  assert.match(createLeftoverUncoveredLeftoverOnlyHeadroomMarkdown(gravelCycling), /leftover uncovered leftover-only headroom \(organizer private\): 28\./);
  assert.match(createLeftoverUncoveredLeftoverOnlyCapacityMarkdown(gravelCycling), /leftover uncovered leftover-only capacity \(organizer private\): 58\./);
  assert.notEqual(createLeftoverUncoveredLeftoverOnlyRemainingMarkdown(roadCycling), createLeftoverUncoveredLeftoverOnlyRemainingMarkdown(gravelCycling));
  assert.notEqual(createLeftoverUncoveredLeftoverOnlyHeadroomMarkdown(roadCycling), createLeftoverUncoveredLeftoverOnlyHeadroomMarkdown(gravelCycling));
  assert.notEqual(createLeftoverUncoveredLeftoverOnlyUnitPriceMarkdown(roadCycling), createLeftoverUncoveredLeftoverOnlyUnitPriceMarkdown(gravelCycling));
  assert.notEqual(createLeftoverUncoveredLeftoverOnlyCapacityMarkdown(roadCycling), createLeftoverUncoveredLeftoverOnlyCapacityMarkdown(gravelCycling));
});

test("criterium cycling carnival leftover uncovered leftover-only remaining stays distinct from leftover-only allocated, leftover-only headroom, leftover-only capacity, leftover-only minimum, leftover unit price, leftover fill remaining, and road leftover-only remaining", () => {
  const criteriumCycling = clonePreset("criteriumCyclingCarnivalLunch");
  const leftoverOnlyUnitPrice = createLeftoverUncoveredLeftoverOnlyUnitPriceMarkdown(criteriumCycling);
  const leftoverOnlyCapacity = createLeftoverUncoveredLeftoverOnlyCapacityMarkdown(criteriumCycling);
  const leftoverOnlyAllocated = createLeftoverUncoveredLeftoverOnlyAllocatedMarkdown(criteriumCycling);
  const leftoverOnlyHeadroom = createLeftoverUncoveredLeftoverOnlyHeadroomMarkdown(criteriumCycling);
  const leftoverOnlyMinimum = createLeftoverUncoveredLeftoverOnlyMinimumMarkdown(criteriumCycling);
  const leftoverOnlyMaximum = createLeftoverUncoveredLeftoverOnlyMaximumMarkdown(criteriumCycling);
  const leftoverOnlyRemaining = createLeftoverUncoveredLeftoverOnlyRemainingMarkdown(criteriumCycling);
  const leftoverOnlyCount = createLeftoverUncoveredLeftoverOnlyCountMarkdown(criteriumCycling);
  const leftoverOffer = criteriumCycling.offers.find((offer) => offer.id === computeResidualCoverage(criteriumCycling).secondary.offerId);
  assert.equal(leftoverOffer.minimumUnits, 31);
  assert.equal(leftoverOffer.unitPrice, 26);
  assert.equal(leftoverOffer.capacity, 62);
  assert.match(leftoverOnlyRemaining, /leftover uncovered leftover-only remaining \(organizer private\): 32\./);
  assert.match(leftoverOnlyAllocated, /leftover uncovered leftover-only allocated \(organizer private\): 32\./);
  assert.match(leftoverOnlyHeadroom, /leftover uncovered leftover-only headroom \(organizer private\): 30\./);
  assert.match(leftoverOnlyMinimum, /leftover uncovered leftover-only minimum \(organizer private\): 15\./);
  assert.match(leftoverOnlyUnitPrice, /leftover uncovered leftover-only unit price \(organizer private\): 26\./);
  assert.match(leftoverOnlyCapacity, /leftover uncovered leftover-only capacity \(organizer private\): 62\./);
  assert.notEqual(32, leftoverOffer.minimumUnits);
  assert.notEqual(32, leftoverOffer.unitPrice);
  assert.notEqual(32, leftoverOffer.capacity);
  assert.notEqual(32, 30);
  assert.notEqual(32, 15);
  assert.notEqual(26, leftoverOffer.minimumUnits);
  assert.notEqual(26, leftoverOffer.capacity);
  assert.notEqual(26, 30);
  assert.notEqual(26, 15);
  assert.notEqual(62, leftoverOffer.unitPrice);
  assert.notEqual(62, leftoverOffer.minimumUnits);
  assert.notEqual(62, 32);
  assert.notEqual(62, 30);
  assert.notEqual(leftoverOnlyRemaining, leftoverOnlyAllocated);
  assert.notEqual(leftoverOnlyRemaining, leftoverOnlyUnitPrice);
  assert.notEqual(leftoverOnlyRemaining, leftoverOnlyCapacity);
  assert.notEqual(leftoverOnlyRemaining, leftoverOnlyHeadroom);
  ...
  assert.notEqual(leftoverOnlyAllocated.startsWith("Common Cart leftover uncovered leftover-only allocated"), false);
  const roadCycling = clonePreset("roadCyclingCarnivalLunch");
  const roadOffer = roadCycling.offers.find((offer) => offer.id === computeResidualCoverage(roadCycling).secondary.offerId);
  assert.equal(roadOffer.minimumUnits, 30);
  assert.equal(roadOffer.unitPrice, 25);
  assert.equal(roadOffer.capacity, 60);
  assert.match(createLeftoverUncoveredLeftoverOnlyRemainingMarkdown(roadCycling), /leftover uncovered leftover-only remaining \(organizer private\): 31\./);
  assert.match(createLeftoverUncoveredLeftoverOnlyHeadroomMarkdown(roadCycling), /leftover uncovered leftover-only headroom \(organizer private\): 29\./);
  assert.match(createLeftoverUncoveredLeftoverOnlyCapacityMarkdown(roadCycling), /leftover uncovered leftover-only capacity \(organizer private\): 60\./);
  assert.notEqual(createLeftoverUncoveredLeftoverOnlyRemainingMarkdown(criteriumCycling), createLeftoverUncoveredLeftoverOnlyRemainingMarkdown(roadCycling));
  assert.notEqual(createLeftoverUncoveredLeftoverOnlyHeadroomMarkdown(criteriumCycling), createLeftoverUncoveredLeftoverOnlyHeadroomMarkdown(roadCycling));
  assert.notEqual(createLeftoverUncoveredLeftoverOnlyUnitPriceMarkdown(criteriumCycling), createLeftoverUncoveredLeftoverOnlyUnitPriceMarkdown(roadCycling));
  assert.notEqual(createLeftoverUncoveredLeftoverOnlyCapacityMarkdown(criteriumCycling), createLeftoverUncoveredLeftoverOnlyCapacityMarkdown(roadCycling));
});

test("time-trial cycling carnival leftover uncovered leftover-only remaining stays distinct from leftover-only allocated, leftover-only headroom, leftover-only capacity, leftover-only minimum, leftover unit price, leftover fill remaining, and criterium leftover-only remaining", () => {
  const timeTrialCycling = clonePreset("timeTrialCyclingCarnivalLunch");
  const leftoverOnlyUnitPrice = createLeftoverUncoveredLeftoverOnlyUnitPriceMarkdown(timeTrialCycling);
  const leftoverOnlyCapacity = createLeftoverUncoveredLeftoverOnlyCapacityMarkdown(timeTrialCycling);
  const leftoverOnlyAllocated = createLeftoverUncoveredLeftoverOnlyAllocatedMarkdown(timeTrialCycling);
  const leftoverOnlyHeadroom = createLeftoverUncoveredLeftoverOnlyHeadroomMarkdown(timeTrialCycling);
  const leftoverOnlyMinimum = createLeftoverUncoveredLeftoverOnlyMinimumMarkdown(timeTrialCycling);
  const leftoverOnlyMaximum = createLeftoverUncoveredLeftoverOnlyMaximumMarkdown(timeTrialCycling);
  const leftoverOnlyRemaining = createLeftoverUncoveredLeftoverOnlyRemainingMarkdown(timeTrialCycling);
  const leftoverOnlyCount = createLeftoverUncoveredLeftoverOnlyCountMarkdown(timeTrialCycling);
  const leftoverOffer = timeTrialCycling.offers.find((offer) => offer.id === computeResidualCoverage(timeTrialCycling).secondary.offerId);
  assert.equal(leftoverOffer.minimumUnits, 32);
  assert.equal(leftoverOffer.unitPrice, 27);
  assert.equal(leftoverOffer.capacity, 64);
  assert.match(leftoverOnlyRemaining, /leftover uncovered leftover-only remaining \(organizer private\): 33\./);
  assert.match(leftoverOnlyAllocated, /leftover uncovered leftover-only allocated \(organizer private\): 33\./);
  assert.match(leftoverOnlyHeadroom, /leftover uncovered leftover-only headroom \(organizer private\): 31\./);
  assert.match(leftoverOnlyMinimum, /leftover uncovered leftover-only minimum \(organizer private\): 15\./);
  assert.match(leftoverOnlyUnitPrice, /leftover uncovered leftover-only unit price \(organizer private\): 27\./);
  assert.match(leftoverOnlyCapacity, /leftover uncovered leftover-only capacity \(organizer private\): 64\./);
  assert.notEqual(33, leftoverOffer.minimumUnits);
  assert.notEqual(33, leftoverOffer.unitPrice);
  assert.notEqual(33, leftoverOffer.capacity);
  assert.notEqual(33, 31);
  assert.notEqual(33, 15);
  assert.notEqual(33, 32);
  assert.notEqual(33, 30);
  assert.notEqual(33, 28);
  assert.notEqual(27, leftoverOffer.minimumUnits);
  assert.notEqual(27, leftoverOffer.capacity);
  assert.notEqual(27, 31);
  assert.notEqual(27, 15);
  assert.notEqual(64, leftoverOffer.unitPrice);
  assert.notEqual(64, leftoverOffer.minimumUnits);
  assert.notEqual(64, 33);
  assert.notEqual(64, 31);
  assert.notEqual(leftoverOnlyRemaining, leftoverOnlyAllocated);
  assert.notEqual(leftoverOnlyRemaining, leftoverOnlyUnitPrice);
  assert.notEqual(leftoverOnlyRemaining, leftoverOnlyCapacity);
  assert.notEqual(leftoverOnlyRemaining, leftoverOnlyHeadroom);
  ...
  assert.notEqual(leftoverOnlyAllocated.startsWith("Common Cart leftover uncovered leftover-only allocated"), false);
  const criteriumCycling = clonePreset("criteriumCyclingCarnivalLunch");
  const criteriumOffer = criteriumCycling.offers.find((offer) => offer.id === computeResidualCoverage(criteriumCycling).secondary.offerId);
  assert.equal(criteriumOffer.minimumUnits, 31);
  assert.equal(criteriumOffer.unitPrice, 26);
  assert.equal(criteriumOffer.capacity, 62);
  ...
  assert.match(createLeftoverUncoveredLeftoverOnlyHeadroomMarkdown(criteriumCycling), /leftover uncovered leftover-only headroom \(organizer private\): 30\./);
  assert.match(createLeftoverUncoveredLeftoverOnlyCapacityMarkdown(criteriumCycling), /leftover uncovered leftover-only capacity \(organizer private\): 62\./);
  assert.notEqual(createLeftoverUncoveredLeftoverOnlyRemainingMarkdown(timeTrialCycling), createLeftoverUncoveredLeftoverOnlyRemainingMarkdown(criteriumCycling));
  assert.notEqual(createLeftoverUncoveredLeftoverOnlyHeadroomMarkdown(timeTrialCycling), createLeftoverUncoveredLeftoverOnlyHeadroomMarkdown(criteriumCycling));
  assert.notEqual(createLeftoverUncoveredLeftoverOnlyUnitPriceMarkdown(timeTrialCycling), createLeftoverUncoveredLeftoverOnlyUnitPriceMarkdown(criteriumCycling));
  assert.notEqual(createLeftoverUncoveredLeftoverOnlyCapacityMarkdown(timeTrialCycling), createLeftoverUncoveredLeftoverOnlyCapacityMarkdown(criteriumCycling));
});


test("hill-climb cycling carnival leftover uncovered leftover-only remaining stays distinct from leftover-only allocated, leftover-only headroom, leftover-only capacity, leftover-only minimum, leftover unit price, leftover fill remaining, and time-trial leftover-only remaining", () => {
  const hillClimbCycling = clonePreset("hillClimbCyclingCarnivalLunch");
  const leftoverOnlyUnitPrice = createLeftoverUncoveredLeftoverOnlyUnitPriceMarkdown(hillClimbCycling);
  const leftoverOnlyCapacity = createLeftoverUncoveredLeftoverOnlyCapacityMarkdown(hillClimbCycling);
  const leftoverOnlyAllocated = createLeftoverUncoveredLeftoverOnlyAllocatedMarkdown(hillClimbCycling);
  const leftoverOnlyHeadroom = createLeftoverUncoveredLeftoverOnlyHeadroomMarkdown(hillClimbCycling);
  const leftoverOnlyMinimum = createLeftoverUncoveredLeftoverOnlyMinimumMarkdown(hillClimbCycling);
  const leftoverOnlyMaximum = createLeftoverUncoveredLeftoverOnlyMaximumMarkdown(hillClimbCycling);
  const leftoverOnlyRemaining = createLeftoverUncoveredLeftoverOnlyRemainingMarkdown(hillClimbCycling);
  const leftoverOnlyCount = createLeftoverUncoveredLeftoverOnlyCountMarkdown(hillClimbCycling);
  const leftoverOffer = hillClimbCycling.offers.find((offer) => offer.id === computeResidualCoverage(hillClimbCycling).secondary.offerId);
  assert.equal(leftoverOffer.minimumUnits, 33);
  assert.equal(leftoverOffer.unitPrice, 28);
  assert.equal(leftoverOffer.capacity, 64);
  assert.match(leftoverOnlyRemaining, /leftover uncovered leftover-only remaining \(organizer private\): 34\./);
  assert.match(leftoverOnlyAllocated, /leftover uncovered leftover-only allocated \(organizer private\): 34\./);
  assert.match(leftoverOnlyHeadroom, /leftover uncovered leftover-only headroom \(organizer private\): 30\./);
  assert.match(leftoverOnlyMinimum, /leftover uncovered leftover-only minimum \(organizer private\): 15\./);
  assert.match(leftoverOnlyUnitPrice, /leftover uncovered leftover-only unit price \(organizer private\): 28\./);
  assert.match(leftoverOnlyCapacity, /leftover uncovered leftover-only capacity \(organizer private\): 64\./);
  assert.notEqual(34, leftoverOffer.minimumUnits);
  assert.notEqual(34, leftoverOffer.unitPrice);
  assert.notEqual(34, leftoverOffer.capacity);
  assert.notEqual(34, 30);
  assert.notEqual(34, 15);
  assert.notEqual(34, 33);
  assert.notEqual(34, 32);
  assert.notEqual(34, 31);
  assert.notEqual(34, 28);
  assert.notEqual(28, leftoverOffer.minimumUnits);
  assert.notEqual(28, leftoverOffer.capacity);
  assert.notEqual(28, 30);
  assert.notEqual(28, 15);
  assert.notEqual(64, leftoverOffer.unitPrice);
  assert.notEqual(64, leftoverOffer.minimumUnits);
  assert.notEqual(64, 34);
  assert.notEqual(64, 30);
  assert.notEqual(leftoverOnlyRemaining, leftoverOnlyAllocated);
  assert.notEqual(leftoverOnlyRemaining, leftoverOnlyUnitPrice);
  assert.notEqual(leftoverOnlyRemaining, leftoverOnlyCapacity);
  assert.notEqual(leftoverOnlyRemaining, leftoverOnlyHeadroom);
  ...
  assert.notEqual(leftoverOnlyAllocated.startsWith("Common Cart leftover uncovered leftover-only allocated"), false);
  const timeTrialCycling = clonePreset("timeTrialCyclingCarnivalLunch");
  const timeTrialOffer = timeTrialCycling.offers.find((offer) => offer.id === computeResidualCoverage(timeTrialCycling).secondary.offerId);
  assert.equal(timeTrialOffer.minimumUnits, 32);
  assert.equal(timeTrialOffer.unitPrice, 27);
  assert.equal(timeTrialOffer.capacity, 64);
  ...
  assert.match(createLeftoverUncoveredLeftoverOnlyHeadroomMarkdown(timeTrialCycling), /leftover uncovered leftover-only headroom \(organizer private\): 31\./);
  assert.match(createLeftoverUncoveredLeftoverOnlyCapacityMarkdown(timeTrialCycling), /leftover uncovered leftover-only capacity \(organizer private\): 64\./);
  assert.notEqual(createLeftoverUncoveredLeftoverOnlyRemainingMarkdown(hillClimbCycling), createLeftoverUncoveredLeftoverOnlyRemainingMarkdown(timeTrialCycling));
  assert.notEqual(createLeftoverUncoveredLeftoverOnlyHeadroomMarkdown(hillClimbCycling), createLeftoverUncoveredLeftoverOnlyHeadroomMarkdown(timeTrialCycling));
  assert.notEqual(createLeftoverUncoveredLeftoverOnlyUnitPriceMarkdown(hillClimbCycling), createLeftoverUncoveredLeftoverOnlyUnitPriceMarkdown(timeTrialCycling));
  const criteriumCycling = clonePreset("criteriumCyclingCarnivalLunch");
  assert.notEqual(createLeftoverUncoveredLeftoverOnlyRemainingMarkdown(hillClimbCycling), createLeftoverUncoveredLeftoverOnlyRemainingMarkdown(criteriumCycling));
  assert.notEqual(createLeftoverUncoveredLeftoverOnlyUnitPriceMarkdown(hillClimbCycling), createLeftoverUncoveredLeftoverOnlyUnitPriceMarkdown(criteriumCycling));
});
...
test("keirin cycling carnival leftover uncovered leftover-only remaining stays distinct from leftover-only allocated, leftover-only headroom, leftover-only capacity, leftover-only minimum, leftover unit price, leftover fill remaining, and hill-climb leftover-only remaining", () => {
  const keirinCycling = clonePreset("keirinCyclingCarnivalLunch");
  const leftoverOnlyUnitPrice = createLeftoverUncoveredLeftoverOnlyUnitPriceMarkdown(keirinCycling);
  const leftoverOnlyCapacity = createLeftoverUncoveredLeftoverOnlyCapacityMarkdown(keirinCycling);
  const leftoverOnlyAllocated = createLeftoverUncoveredLeftoverOnlyAllocatedMarkdown(keirinCycling);
  const leftoverOnlyHeadroom = createLeftoverUncoveredLeftoverOnlyHeadroomMarkdown(keirinCycling);
  const leftoverOnlyMinimum = createLeftoverUncoveredLeftoverOnlyMinimumMarkdown(keirinCycling);
  const leftoverOnlyMaximum = createLeftoverUncoveredLeftoverOnlyMaximumMarkdown(keirinCycling);
  const leftoverOnlyRemaining = createLeftoverUncoveredLeftoverOnlyRemainingMarkdown(keirinCycling);
  const leftoverOnlyCount = createLeftoverUncoveredLeftoverOnlyCountMarkdown(keirinCycling);
  const leftoverOffer = keirinCycling.offers.find((offer) => offer.id === computeResidualCoverage(keirinCycling).secondary.offerId);
  assert.equal(leftoverOffer.minimumUnits, 34);
  assert.equal(leftoverOffer.unitPrice, 30);
  assert.equal(leftoverOffer.capacity, 64);
  assert.match(leftoverOnlyRemaining, /leftover uncovered leftover-only remaining \(organizer private\): 35\./);
  assert.match(leftoverOnlyAllocated, /leftover uncovered leftover-only allocated \(organizer private\): 35\./);
  assert.match(leftoverOnlyHeadroom, /leftover uncovered leftover-only headroom \(organizer private\): 29\./);
  assert.match(leftoverOnlyMinimum, /leftover uncovered leftover-only minimum \(organizer private\): 15\./);
  assert.match(leftoverOnlyUnitPrice, /leftover uncovered leftover-only unit price \(organizer private\): 30\./);
  assert.match(leftoverOnlyCapacity, /leftover uncovered leftover-only capacity \(organizer private\): 64\./);
  assert.notEqual(35, leftoverOffer.minimumUnits);
  assert.notEqual(35, leftoverOffer.unitPrice);
  assert.notEqual(35, leftoverOffer.capacity);
  assert.notEqual(35, 30);
  assert.notEqual(35, 29);
  assert.notEqual(35, 15);
  assert.notEqual(35, 34);
  assert.notEqual(35, 33);
  assert.notEqual(35, 32);
  assert.notEqual(35, 31);
  assert.notEqual(35, 30);
  assert.notEqual(35, 28);
  assert.notEqual(leftoverOnlyRemaining, leftoverOnlyAllocated);
  assert.notEqual(leftoverOnlyRemaining, leftoverOnlyUnitPrice);
  assert.notEqual(leftoverOnlyRemaining, leftoverOnlyCapacity);
  assert.notEqual(leftoverOnlyRemaining, leftoverOnlyHeadroom);
  ...
  const gravelCycling = clonePreset("gravelCyclingCarnivalLunch");
  const cycloCross = clonePreset("cycloCrossCarnivalLunch");
  assert.notEqual(createLeftoverUncoveredLeftoverOnlyRemainingMarkdown(keirinCycling), createLeftoverUncoveredLeftoverOnlyRemainingMarkdown(hillClimbCycling));
  assert.notEqual(createLeftoverUncoveredLeftoverOnlyRemainingMarkdown(keirinCycling), createLeftoverUncoveredLeftoverOnlyRemainingMarkdown(timeTrialCycling));
  assert.notEqual(createLeftoverUncoveredLeftoverOnlyRemainingMarkdown(keirinCycling), createLeftoverUncoveredLeftoverOnlyRemainingMarkdown(criteriumCycling));
  assert.notEqual(createLeftoverUncoveredLeftoverOnlyRemainingMarkdown(keirinCycling), createLeftoverUncoveredLeftoverOnlyRemainingMarkdown(roadCycling));
  assert.notEqual(createLeftoverUncoveredLeftoverOnlyRemainingMarkdown(keirinCycling), createLeftoverUncoveredLeftoverOnlyRemainingMarkdown(gravelCycling));
  assert.notEqual(createLeftoverUncoveredLeftoverOnlyRemainingMarkdown(keirinCycling), createLeftoverUncoveredLeftoverOnlyRemainingMarkdown(cycloCross));
  assert.notEqual(createLeftoverUncoveredLeftoverOnlyUnitPriceMarkdown(keirinCycling), createLeftoverUncoveredLeftoverOnlyUnitPriceMarkdown(hillClimbCycling));
  assert.notEqual(createLeftoverUncoveredLeftoverOnlyHeadroomMarkdown(keirinCycling), createLeftoverUncoveredLeftoverOnlyHeadroomMarkdown(hillClimbCycling));
  ...
  assert.notEqual(createLeftoverUncoveredLeftoverOnlyRemainingMarkdown(keirinCycling), createLeftoverUncoveredLeftoverOnlyRemainingMarkdown(madisonCycling));
  ...
  const scratchCycling = clonePreset("scratchCyclingCarnivalLunch");
  assert.notEqual(createLeftoverUncoveredLeftoverOnlyRemainingMarkdown(keirinCycling), createLeftoverUncoveredLeftoverOnlyRemainingMarkdown(scratchCycling));
  assert.match(createLeftoverUncoveredLeftoverOnlyRemainingMarkdown(scratchCycling), /leftover uncovered leftover-only remaining \(organizer private\): 39\./);
});
...
test("madison cycling carnival leftover uncovered leftover-only remaining stays distinct from leftover-only allocated, leftover-only headroom, leftover-only capacity, leftover-only minimum, leftover unit price, leftover fill remaining, and keirin leftover-only remaining", () => {
  const madisonCycling = clonePreset("madisonCyclingCarnivalLunch");
  const leftoverOnlyUnitPrice = createLeftoverUncoveredLeftoverOnlyUnitPriceMarkdown(madisonCycling);
  const leftoverOnlyCapacity = createLeftoverUncoveredLeftoverOnlyCapacityMarkdown(madisonCycling);
  const leftoverOnlyAllocated = createLeftoverUncoveredLeftoverOnlyAllocatedMarkdown(madisonCycling);
  const leftoverOnlyHeadroom = createLeftoverUncoveredLeftoverOnlyHeadroomMarkdown(madisonCycling);
  const leftoverOnlyMinimum = createLeftoverUncoveredLeftoverOnlyMinimumMarkdown(madisonCycling);
  const leftoverOnlyMaximum = createLeftoverUncoveredLeftoverOnlyMaximumMarkdown(madisonCycling);
  const leftoverOnlyRemaining = createLeftoverUncoveredLeftoverOnlyRemainingMarkdown(madisonCycling);
  const leftoverOnlyCount = createLeftoverUncoveredLeftoverOnlyCountMarkdown(madisonCycling);
  const leftoverOffer = madisonCycling.offers.find((offer) => offer.id === computeResidualCoverage(madisonCycling).secondary.offerId);
  assert.equal(leftoverOffer.minimumUnits, 35);
  assert.equal(leftoverOffer.unitPrice, 31);
  assert.equal(leftoverOffer.capacity, 64);
  assert.match(leftoverOnlyRemaining, /leftover uncovered leftover-only remaining \(organizer private\): 36\./);
  assert.match(leftoverOnlyAllocated, /leftover uncovered leftover-only allocated \(organizer private\): 36\./);
  assert.match(leftoverOnlyHeadroom, /leftover uncovered leftover-only headroom \(organizer private\): 28\./);
  assert.match(leftoverOnlyMinimum, /leftover uncovered leftover-only minimum \(organizer private\): 15\./);
  assert.match(leftoverOnlyUnitPrice, /leftover uncovered leftover-only unit price \(organizer private\): 31\./);
  assert.match(leftoverOnlyCapacity, /leftover uncovered leftover-only capacity \(organizer private\): 64\./);
  assert.notEqual(36, leftoverOffer.minimumUnits);
  assert.notEqual(36, leftoverOffer.unitPrice);
  assert.notEqual(36, leftoverOffer.capacity);
  assert.notEqual(36, 31);
  assert.notEqual(36, 28);
  assert.notEqual(36, 15);
  assert.notEqual(36, 35);
  assert.notEqual(36, 35);
  assert.notEqual(36, 34);
  assert.notEqual(36, 33);
  assert.notEqual(leftoverOnlyRemaining, leftoverOnlyAllocated);
  assert.notEqual(leftoverOnlyRemaining, leftoverOnlyUnitPrice);
  assert.notEqual(leftoverOnlyRemaining, leftoverOnlyCapacity);
  assert.notEqual(leftoverOnlyRemaining, leftoverOnlyHeadroom);
  ...
  const hillClimbCycling = clonePreset("hillClimbCyclingCarnivalLunch");
  const timeTrialCycling = clonePreset("timeTrialCyclingCarnivalLunch");
  assert.notEqual(createLeftoverUncoveredLeftoverOnlyRemainingMarkdown(madisonCycling), createLeftoverUncoveredLeftoverOnlyRemainingMarkdown(keirinCycling));
  assert.notEqual(createLeftoverUncoveredLeftoverOnlyRemainingMarkdown(madisonCycling), createLeftoverUncoveredLeftoverOnlyRemainingMarkdown(hillClimbCycling));
  assert.notEqual(createLeftoverUncoveredLeftoverOnlyRemainingMarkdown(madisonCycling), createLeftoverUncoveredLeftoverOnlyRemainingMarkdown(timeTrialCycling));
  assert.notEqual(createLeftoverUncoveredLeftoverOnlyUnitPriceMarkdown(madisonCycling), createLeftoverUncoveredLeftoverOnlyUnitPriceMarkdown(keirinCycling));
  assert.notEqual(createLeftoverUncoveredLeftoverOnlyHeadroomMarkdown(madisonCycling), createLeftoverUncoveredLeftoverOnlyHeadroomMarkdown(keirinCycling));
  ...
  assert.match(createLeftoverUncoveredLeftoverOnlyRemainingMarkdown(pointsRaceCycling), /leftover uncovered leftover-only remaining \(organizer private\): 38\./);
  const scratchCycling = clonePreset("scratchCyclingCarnivalLunch");
  assert.notEqual(createLeftoverUncoveredLeftoverOnlyRemainingMarkdown(madisonCycling), createLeftoverUncoveredLeftoverOnlyRemainingMarkdown(scratchCycling));
  assert.match(createLeftoverUncoveredLeftoverOnlyRemainingMarkdown(scratchCycling), /leftover uncovered leftover-only remaining \(organizer private\): 39\./);
});
...
test("omnium cycling carnival leftover uncovered leftover-only remaining stays distinct from leftover-only allocated, leftover-only headroom, leftover-only capacity, leftover-only minimum, leftover unit price, leftover fill remaining, and madison leftover-only remaining", () => {
  const omniumCycling = clonePreset("omniumCyclingCarnivalLunch");
  const leftoverOnlyUnitPrice = createLeftoverUncoveredLeftoverOnlyUnitPriceMarkdown(omniumCycling);
  const leftoverOnlyCapacity = createLeftoverUncoveredLeftoverOnlyCapacityMarkdown(omniumCycling);
  const leftoverOnlyAllocated = createLeftoverUncoveredLeftoverOnlyAllocatedMarkdown(omniumCycling);
  const leftoverOnlyHeadroom = createLeftoverUncoveredLeftoverOnlyHeadroomMarkdown(omniumCycling);
  const leftoverOnlyMinimum = createLeftoverUncoveredLeftoverOnlyMinimumMarkdown(omniumCycling);
  const leftoverOnlyMaximum = createLeftoverUncoveredLeftoverOnlyMaximumMarkdown(omniumCycling);
  const leftoverOnlyRemaining = createLeftoverUncoveredLeftoverOnlyRemainingMarkdown(omniumCycling);
  const leftoverOnlyCount = createLeftoverUncoveredLeftoverOnlyCountMarkdown(omniumCycling);
  const leftoverOffer = omniumCycling.offers.find((offer) => offer.id === computeResidualCoverage(omniumCycling).secondary.offerId);
  assert.equal(leftoverOffer.minimumUnits, 36);
  assert.equal(leftoverOffer.unitPrice, 32);
  assert.equal(leftoverOffer.capacity, 64);
  assert.match(leftoverOnlyRemaining, /leftover uncovered leftover-only remaining \(organizer private\): 37\./);
  assert.match(leftoverOnlyAllocated, /leftover uncovered leftover-only allocated \(organizer private\): 37\./);
  assert.match(leftoverOnlyHeadroom, /leftover uncovered leftover-only headroom \(organizer private\): 27\./);
  assert.match(leftoverOnlyMinimum, /leftover uncovered leftover-only minimum \(organizer private\): 15\./);
  assert.match(leftoverOnlyUnitPrice, /leftover uncovered leftover-only unit price \(organizer private\): 32\./);
  assert.match(leftoverOnlyCapacity, /leftover uncovered leftover-only capacity \(organizer private\): 64\./);
  assert.notEqual(37, leftoverOffer.minimumUnits);
  assert.notEqual(37, leftoverOffer.unitPrice);
  assert.notEqual(37, leftoverOffer.capacity);
  assert.notEqual(37, 32);
  assert.notEqual(37, 27);
  assert.notEqual(37, 15);
  assert.notEqual(37, 36);
  assert.notEqual(37, 35);
  assert.notEqual(37, 34);
  assert.notEqual(37, 33);
  assert.notEqual(37, 32);
  assert.notEqual(leftoverOnlyRemaining, leftoverOnlyAllocated);
  assert.notEqual(leftoverOnlyRemaining, leftoverOnlyUnitPrice);
  assert.notEqual(leftoverOnlyRemaining, leftoverOnlyCapacity);
  ...
  const timeTrialCycling = clonePreset("timeTrialCyclingCarnivalLunch");
  const criteriumCycling = clonePreset("criteriumCyclingCarnivalLunch");
  assert.notEqual(createLeftoverUncoveredLeftoverOnlyRemainingMarkdown(omniumCycling), createLeftoverUncoveredLeftoverOnlyRemainingMarkdown(madisonCycling));
  assert.notEqual(createLeftoverUncoveredLeftoverOnlyRemainingMarkdown(omniumCycling), createLeftoverUncoveredLeftoverOnlyRemainingMarkdown(keirinCycling));
  assert.notEqual(createLeftoverUncoveredLeftoverOnlyRemainingMarkdown(omniumCycling), createLeftoverUncoveredLeftoverOnlyRemainingMarkdown(hillClimbCycling));
  assert.notEqual(createLeftoverUncoveredLeftoverOnlyRemainingMarkdown(omniumCycling), createLeftoverUncoveredLeftoverOnlyRemainingMarkdown(timeTrialCycling));
  assert.notEqual(createLeftoverUncoveredLeftoverOnlyRemainingMarkdown(omniumCycling), createLeftoverUncoveredLeftoverOnlyRemainingMarkdown(criteriumCycling));
  assert.notEqual(createLeftoverUncoveredLeftoverOnlyUnitPriceMarkdown(omniumCycling), createLeftoverUncoveredLeftoverOnlyUnitPriceMarkdown(madisonCycling));
  assert.notEqual(createLeftoverUncoveredLeftoverOnlyHeadroomMarkdown(omniumCycling), createLeftoverUncoveredLeftoverOnlyHeadroomMarkdown(madisonCycling));
  assert.match(createLeftoverUncoveredLeftoverOnlyRemainingMarkdown(madisonCycling), /leftover uncovered leftover-only remaining \(organizer private\): 36\./);
  assert.match(createLeftoverUncoveredLeftoverOnlyRemainingMarkdown(keirinCycling), /leftover uncovered leftover-only remaining \(organizer private\): 35\./);
  assert.match(createLeftoverUncoveredLeftoverOnlyRemainingMarkdown(hillClimbCycling), /leftover uncovered leftover-only remaining \(organizer private\): 34\./);
  assert.match(createLeftoverUncoveredLeftoverOnlyRemainingMarkdown(timeTrialCycling), /leftover uncovered leftover-only remaining \(organizer private\): 33\./);
  ...
  assert.match(createLeftoverUncoveredLeftoverOnlyRemainingMarkdown(pointsRaceCycling), /leftover uncovered leftover-only remaining \(organizer private\): 38\./);
  const scratchCycling = clonePreset("scratchCyclingCarnivalLunch");
  assert.notEqual(createLeftoverUncoveredLeftoverOnlyRemainingMarkdown(omniumCycling), createLeftoverUncoveredLeftoverOnlyRemainingMarkdown(scratchCycling));
  assert.match(createLeftoverUncoveredLeftoverOnlyRemainingMarkdown(scratchCycling), /leftover uncovered leftover-only remaining \(organizer private\): 39\./);
});
...
test("points-race cycling carnival leftover uncovered leftover-only remaining stays distinct from leftover-only allocated, leftover-only headroom, leftover-only capacity, leftover-only minimum, leftover unit price, leftover fill remaining, and omnium leftover-only remaining", () => {
  ...
  assert.equal(hillClimbCycling.offers.find((offer) => offer.id === computeResidualCoverage(hillClimbCycling).secondary.offerId).capacity, 64);
  const scratchCycling = clonePreset("scratchCyclingCarnivalLunch");
  assert.notEqual(createLeftoverUncoveredLeftoverOnlyRemainingMarkdown(pointsRaceCycling), createLeftoverUncoveredLeftoverOnlyRemainingMarkdown(scratchCycling));
  assert.match(createLeftoverUncoveredLeftoverOnlyRemainingMarkdown(scratchCycling), /leftover uncovered leftover-only remaining \(organizer private\): 39\./);
});

test("scratch cycling carnival leftover uncovered leftover-only remaining stays distinct from leftover-only allocated, leftover-only headroom, leftover-only capacity, leftover-only minimum, leftover unit price, leftover fill remaining, and points-race leftover-only remaining", () => {
  const scratchCycling = clonePreset("scratchCyclingCarnivalLunch");
  const leftoverOnlyUnitPrice = createLeftoverUncoveredLeftoverOnlyUnitPriceMarkdown(scratchCycling);
  const leftoverOnlyCapacity = createLeftoverUncoveredLeftoverOnlyCapacityMarkdown(scratchCycling);
  const leftoverOnlyAllocated = createLeftoverUncoveredLeftoverOnlyAllocatedMarkdown(scratchCycling);
  const leftoverOnlyHeadroom = createLeftoverUncoveredLeftoverOnlyHeadroomMarkdown(scratchCycling);
  const leftoverOnlyMinimum = createLeftoverUncoveredLeftoverOnlyMinimumMarkdown(scratchCycling);
  const leftoverOnlyMaximum = createLeftoverUncoveredLeftoverOnlyMaximumMarkdown(scratchCycling);
  const leftoverOnlyRemaining = createLeftoverUncoveredLeftoverOnlyRemainingMarkdown(scratchCycling);
  const leftoverOnlyCount = createLeftoverUncoveredLeftoverOnlyCountMarkdown(scratchCycling);
  const leftoverOffer = scratchCycling.offers.find((offer) => offer.id === computeResidualCoverage(scratchCycling).secondary.offerId);
  assert.equal(leftoverOffer.minimumUnits, 38);
  assert.equal(leftoverOffer.unitPrice, 34);
  assert.equal(leftoverOffer.capacity, 64);
  assert.match(leftoverOnlyRemaining, /leftover uncovered leftover-only remaining \(organizer private\): 39\./);
  assert.match(leftoverOnlyAllocated, /leftover uncovered leftover-only allocated \(organizer private\): 39\./);
  assert.match(leftoverOnlyHeadroom, /leftover uncovered leftover-only headroom \(organizer private\): 25\./);
  assert.match(leftoverOnlyMinimum, /leftover uncovered leftover-only minimum \(organizer private\): 15\./);
  assert.match(leftoverOnlyUnitPrice, /leftover uncovered leftover-only unit price \(organizer private\): 34\./);
  assert.match(leftoverOnlyCapacity, /leftover uncovered leftover-only capacity \(organizer private\): 64\./);
  assert.notEqual(39, leftoverOffer.minimumUnits);
  assert.notEqual(39, leftoverOffer.unitPrice);
  assert.notEqual(39, leftoverOffer.capacity);
  assert.notEqual(39, 34);
  assert.notEqual(39, 25);
  assert.notEqual(39, 15);
  assert.notEqual(39, 38);
  assert.notEqual(39, 37);
  assert.notEqual(39, 36);
  assert.notEqual(leftoverOnlyRemaining, leftoverOnlyAllocated);
  assert.notEqual(leftoverOnlyRemaining, leftoverOnlyUnitPrice);
  assert.notEqual(leftoverOnlyRemaining, leftoverOnlyCapacity);
  assert.notEqual(leftoverOnlyRemaining, leftoverOnlyHeadroom);
  ...
  const hillClimbCycling = clonePreset("hillClimbCyclingCarnivalLunch");
  const timeTrialCycling = clonePreset("timeTrialCyclingCarnivalLunch");
  assert.notEqual(createLeftoverUncoveredLeftoverOnlyRemainingMarkdown(scratchCycling), createLeftoverUncoveredLeftoverOnlyRemainingMarkdown(pointsRaceCycling));
  assert.notEqual(createLeftoverUncoveredLeftoverOnlyRemainingMarkdown(scratchCycling), createLeftoverUncoveredLeftoverOnlyRemainingMarkdown(omniumCycling));
  assert.notEqual(createLeftoverUncoveredLeftoverOnlyRemainingMarkdown(scratchCycling), createLeftoverUncoveredLeftoverOnlyRemainingMarkdown(madisonCycling));
  assert.notEqual(createLeftoverUncoveredLeftoverOnlyRemainingMarkdown(scratchCycling), createLeftoverUncoveredLeftoverOnlyRemainingMarkdown(keirinCycling));
  assert.notEqual(createLeftoverUncoveredLeftoverOnlyRemainingMarkdown(scratchCycling), createLeftoverUncoveredLeftoverOnlyRemainingMarkdown(hillClimbCycling));
  assert.notEqual(createLeftoverUncoveredLeftoverOnlyRemainingMarkdown(scratchCycling), createLeftoverUncoveredLeftoverOnlyRemainingMarkdown(timeTrialCycling));
  assert.notEqual(createLeftoverUncoveredLeftoverOnlyUnitPriceMarkdown(scratchCycling), createLeftoverUncoveredLeftoverOnlyUnitPriceMarkdown(pointsRaceCycling));
  assert.notEqual(createLeftoverUncoveredLeftoverOnlyHeadroomMarkdown(scratchCycling), createLeftoverUncoveredLeftoverOnlyHeadroomMarkdown(pointsRaceCycling));
  assert.match(createLeftoverUncoveredLeftoverOnlyRemainingMarkdown(pointsRaceCycling), /leftover uncovered leftover-only remaining \(organizer private\): 38\./);
  assert.match(createLeftoverUncoveredLeftoverOnlyRemainingMarkdown(omniumCycling), /leftover uncovered leftover-only remaining \(organizer private\): 37\./);
  assert.match(createLeftoverUncoveredLeftoverOnlyRemainingMarkdown(madisonCycling), /leftover uncovered leftover-only remaining \(organizer private\): 36\./);
  assert.match(createLeftoverUncoveredLeftoverOnlyRemainingMarkdown(keirinCycling), /leftover uncovered leftover-only remaining \(organizer private\): 35\./);
  assert.match(createLeftoverUncoveredLeftoverOnlyRemainingMarkdown(hillClimbCycling), /leftover uncovered leftover-only remaining \(organizer private\): 34\./);
  assert.equal(pointsRaceCycling.offers.find((offer) => offer.id === computeResidualCoverage(pointsRaceCycling).secondary.offerId).capacity, 64);
  assert.equal(omniumCycling.offers.find((offer) => offer.id === computeResidualCoverage(omniumCycling).secondary.offerId).capacity, 64);
  assert.equal(madisonCycling.offers.find((offer) => offer.id === computeResidualCoverage(madisonCycling).secondary.offerId).capacity, 64);
  assert.equal(keirinCycling.offers.find((offer) => offer.id === computeResidualCoverage(keirinCycling).secondary.offerId).capacity, 64);
  assert.equal(hillClimbCycling.offers.find((offer) => offer.id === computeResidualCoverage(hillClimbCycling).secondary.offerId).capacity, 64);
  const individualPursuitCycling = clonePreset("individualPursuitCyclingCarnivalLunch");
  assert.notEqual(createLeftoverUncoveredLeftoverOnlyRemainingMarkdown(scratchCycling), createLeftoverUncoveredLeftoverOnlyRemainingMarkdown(individualPursuitCycling));
  assert.match(createLeftoverUncoveredLeftoverOnlyRemainingMarkdown(individualPursuitCycling), /leftover uncovered leftover-only remaining \(organizer private\): 41\./);
  const teamPursuitCyclingFromScratch = clonePreset("teamPursuitCyclingCarnivalLunch");
  assert.notEqual(createLeftoverUncoveredLeftoverOnlyRemainingMarkdown(scratchCycling), createLeftoverUncoveredLeftoverOnlyRemainingMarkdown(teamPursuitCyclingFromScratch));
  assert.match(createLeftoverUncoveredLeftoverOnlyRemainingMarkdown(teamPursuitCyclingFromScratch), /leftover uncovered leftover-only remaining \(organizer private\): 43\./);
  const teamSprintCyclingFromScratch = clonePreset("teamSprintCyclingCarnivalLunch");
  assert.notEqual(createLeftoverUncoveredLeftoverOnlyRemainingMarkdown(scratchCycling), createLeftoverUncoveredLeftoverOnlyRemainingMarkdown(teamSprintCyclingFromScratch));
  assert.match(createLeftoverUncoveredLeftoverOnlyRemainingMarkdown(teamSprintCyclingFromScratch), /leftover uncovered leftover-only remaining \(organizer private\): 45\./);
  const firstAidCyclingFromScratch = clonePreset("firstAidCyclingCarnivalLunch");
  assert.notEqual(createLeftoverUncoveredLeftoverOnlyRemainingMarkdown(scratchCycling), createLeftoverUncoveredLeftoverOnlyRemainingMarkdown(firstAidCyclingFromScratch));
  assert.match(createLeftoverUncoveredLeftoverOnlyRemainingMarkdown(firstAidCyclingFromScratch), /leftover uncovered leftover-only remaining \(organizer private\): 47\./);
  const feedZoneCyclingFromScratch = clonePreset("feedZoneCyclingCarnivalLunch");
  assert.notEqual(createLeftoverUncoveredLeftoverOnlyRemainingMarkdown(scratchCycling), createLeftoverUncoveredLeftoverOnlyRemainingMarkdown(feedZoneCyclingFromScratch));
  assert.notEqual(createLeftoverUncoveredLeftoverOnlyRemainingMarkdown(firstAidCyclingFromScratch), createLeftoverUncoveredLeftoverOnlyRemainingMarkdown(feedZoneCyclingFromScratch));
  assert.match(createLeftoverUncoveredLeftoverOnlyRemainingMarkdown(feedZoneCyclingFromScratch), /leftover uncovered leftover-only remaining \(organizer private\): 49\./);
  const leadOutCyclingFromScratch = clonePreset("leadOutCyclingCarnivalLunch");
  assert.notEqual(createLeftoverUncoveredLeftoverOnlyRemainingMarkdown(scratchCycling), createLeftoverUncoveredLeftoverOnlyRemainingMarkdown(leadOutCyclingFromScratch));
  assert.notEqual(createLeftoverUncoveredLeftoverOnlyRemainingMarkdown(feedZoneCyclingFromScratch), createLeftoverUncoveredLeftoverOnlyRemainingMarkdown(leadOutCyclingFromScratch));
  assert.notEqual(createLeftoverUncoveredLeftoverOnlyRemainingMarkdown(firstAidCyclingFromScratch), createLeftoverUncoveredLeftoverOnlyRemainingMarkdown(leadOutCyclingFromScratch));
  assert.match(createLeftoverUncoveredLeftoverOnlyRemainingMarkdown(leadOutCyclingFromScratch), /leftover uncovered leftover-only remaining \(organizer private\): 51\./);
});

test("individual-pursuit cycling carnival leftover uncovered leftover-only remaining stays distinct from leftover-only allocated, leftover-only headroom, leftover-only capacity, leftover-only minimum, leftover unit price, leftover fill remaining, and scratch leftover-only remaining", () => {
  const pursuitCycling = clonePreset("individualPursuitCyclingCarnivalLunch");
  const leftoverOnlyUnitPrice = createLeftoverUncoveredLeftoverOnlyUnitPriceMarkdown(pursuitCycling);
  const leftoverOnlyCapacity = createLeftoverUncoveredLeftoverOnlyCapacityMarkdown(pursuitCycling);
  const leftoverOnlyAllocated = createLeftoverUncoveredLeftoverOnlyAllocatedMarkdown(pursuitCycling);
  const leftoverOnlyHeadroom = createLeftoverUncoveredLeftoverOnlyHeadroomMarkdown(pursuitCycling);
  const leftoverOnlyMinimum = createLeftoverUncoveredLeftoverOnlyMinimumMarkdown(pursuitCycling);
  const leftoverOnlyMaximum = createLeftoverUncoveredLeftoverOnlyMaximumMarkdown(pursuitCycling);
  const leftoverOnlyRemaining = createLeftoverUncoveredLeftoverOnlyRemainingMarkdown(pursuitCycling);
  const leftoverOnlyCount = createLeftoverUncoveredLeftoverOnlyCountMarkdown(pursuitCycling);
  const leftoverOffer = pursuitCycling.offers.find((offer) => offer.id === computeResidualCoverage(pursuitCycling).secondary.offerId);
  assert.equal(leftoverOffer.minimumUnits, 40);
  assert.equal(leftoverOffer.unitPrice, 36);
  assert.equal(leftoverOffer.capacity, 64);
  assert.match(leftoverOnlyRemaining, /leftover uncovered leftover-only remaining \(organizer private\): 41\./);
  assert.match(leftoverOnlyAllocated, /leftover uncovered leftover-only allocated \(organizer private\): 41\./);
  assert.match(leftoverOnlyHeadroom, /leftover uncovered leftover-only headroom \(organizer private\): 23\./);
  assert.match(leftoverOnlyMinimum, /leftover uncovered leftover-only minimum \(organizer private\): 15\./);
  assert.match(leftoverOnlyUnitPrice, /leftover uncovered leftover-only unit price \(organizer private\): 36\./);
  assert.match(leftoverOnlyCapacity, /leftover uncovered leftover-only capacity \(organizer private\): 64\./);
  assert.notEqual(41, leftoverOffer.minimumUnits);
  assert.notEqual(41, leftoverOffer.unitPrice);
  assert.notEqual(41, leftoverOffer.capacity);
  assert.notEqual(41, 36);
  assert.notEqual(41, 23);
  assert.notEqual(41, 15);
  assert.notEqual(41, 40);
  assert.notEqual(41, 39);
  assert.notEqual(41, 38);
  assert.notEqual(leftoverOnlyRemaining, leftoverOnlyAllocated);
  assert.notEqual(leftoverOnlyRemaining, leftoverOnlyUnitPrice);
  assert.notEqual(leftoverOnlyRemaining, leftoverOnlyCapacity);
  assert.notEqual(leftoverOnlyRemaining, leftoverOnlyHeadroom);
  ...
  const keirinCycling = clonePreset("keirinCyclingCarnivalLunch");
  const hillClimbCycling = clonePreset("hillClimbCyclingCarnivalLunch");
  assert.notEqual(createLeftoverUncoveredLeftoverOnlyRemainingMarkdown(pursuitCycling), createLeftoverUncoveredLeftoverOnlyRemainingMarkdown(scratchCycling));
  assert.notEqual(createLeftoverUncoveredLeftoverOnlyRemainingMarkdown(pursuitCycling), createLeftoverUncoveredLeftoverOnlyRemainingMarkdown(pointsRaceCycling));
  assert.notEqual(createLeftoverUncoveredLeftoverOnlyRemainingMarkdown(pursuitCycling), createLeftoverUncoveredLeftoverOnlyRemainingMarkdown(omniumCycling));
  assert.notEqual(createLeftoverUncoveredLeftoverOnlyRemainingMarkdown(pursuitCycling), createLeftoverUncoveredLeftoverOnlyRemainingMarkdown(madisonCycling));
  assert.notEqual(createLeftoverUncoveredLeftoverOnlyRemainingMarkdown(pursuitCycling), createLeftoverUncoveredLeftoverOnlyRemainingMarkdown(keirinCycling));
  assert.notEqual(createLeftoverUncoveredLeftoverOnlyRemainingMarkdown(pursuitCycling), createLeftoverUncoveredLeftoverOnlyRemainingMarkdown(hillClimbCycling));
  assert.notEqual(createLeftoverUncoveredLeftoverOnlyUnitPriceMarkdown(pursuitCycling), createLeftoverUncoveredLeftoverOnlyUnitPriceMarkdown(scratchCycling));
  assert.notEqual(createLeftoverUncoveredLeftoverOnlyHeadroomMarkdown(pursuitCycling), createLeftoverUncoveredLeftoverOnlyHeadroomMarkdown(scratchCycling));
  assert.match(createLeftoverUncoveredLeftoverOnlyRemainingMarkdown(scratchCycling), /leftover uncovered leftover-only remaining \(organizer private\): 39\./);
  assert.match(createLeftoverUncoveredLeftoverOnlyRemainingMarkdown(pointsRaceCycling), /leftover uncovered leftover-only remaining \(organizer private\): 38\./);
  assert.match(createLeftoverUncoveredLeftoverOnlyRemainingMarkdown(omniumCycling), /leftover uncovered leftover-only remaining \(organizer private\): 37\./);
  assert.match(createLeftoverUncoveredLeftoverOnlyRemainingMarkdown(madisonCycling), /leftover uncovered leftover-only remaining \(organizer private\): 36\./);
  assert.match(createLeftoverUncoveredLeftoverOnlyRemainingMarkdown(keirinCycling), /leftover uncovered leftover-only remaining \(organizer private\): 35\./);
  assert.match(createLeftoverUncoveredLeftoverOnlyRemainingMarkdown(hillClimbCycling), /leftover uncovered leftover-only remaining \(organizer private\): 34\./);
  assert.equal(scratchCycling.offers.find((offer) => offer.id === computeResidualCoverage(scratchCycling).secondary.offerId).capacity, 64);
  assert.equal(pointsRaceCycling.offers.find((offer) => offer.id === computeResidualCoverage(pointsRaceCycling).secondary.offerId).capacity, 64);
  assert.equal(omniumCycling.offers.find((offer) => offer.id === computeResidualCoverage(omniumCycling).secondary.offerId).capacity, 64);
  assert.equal(madisonCycling.offers.find((offer) => offer.id === computeResidualCoverage(madisonCycling).secondary.offerId).capacity, 64);
  assert.equal(keirinCycling.offers.find((offer) => offer.id === computeResidualCoverage(keirinCycling).secondary.offerId).capacity, 64);
...
test("team-pursuit cycling carnival leftover uncovered leftover-only remaining stays distinct from leftover-only allocated, leftover-only headroom, leftover-only capacity, leftover-only minimum, leftover unit price, leftover fill remaining, and individual-pursuit leftover-only remaining", () => {
  const teamPursuitCycling = clonePreset("teamPursuitCyclingCarnivalLunch");
  const leftoverOnlyUnitPrice = createLeftoverUncoveredLeftoverOnlyUnitPriceMarkdown(teamPursuitCycling);
  const leftoverOnlyCapacity = createLeftoverUncoveredLeftoverOnlyCapacityMarkdown(teamPursuitCycling);
  const leftoverOnlyAllocated = createLeftoverUncoveredLeftoverOnlyAllocatedMarkdown(teamPursuitCycling);
  const leftoverOnlyHeadroom = createLeftoverUncoveredLeftoverOnlyHeadroomMarkdown(teamPursuitCycling);
  const leftoverOnlyMinimum = createLeftoverUncoveredLeftoverOnlyMinimumMarkdown(teamPursuitCycling);
  const leftoverOnlyMaximum = createLeftoverUncoveredLeftoverOnlyMaximumMarkdown(teamPursuitCycling);
  const leftoverOnlyRemaining = createLeftoverUncoveredLeftoverOnlyRemainingMarkdown(teamPursuitCycling);
  const leftoverOnlyCount = createLeftoverUncoveredLeftoverOnlyCountMarkdown(teamPursuitCycling);
  const leftoverOffer = teamPursuitCycling.offers.find((offer) => offer.id === computeResidualCoverage(teamPursuitCycling).secondary.offerId);
  assert.equal(leftoverOffer.minimumUnits, 42);
  assert.equal(leftoverOffer.unitPrice, 38);
  assert.equal(leftoverOffer.capacity, 64);
  assert.match(leftoverOnlyRemaining, /leftover uncovered leftover-only remaining \(organizer private\): 43\./);
  assert.match(leftoverOnlyAllocated, /leftover uncovered leftover-only allocated \(organizer private\): 43\./);
  assert.match(leftoverOnlyHeadroom, /leftover uncovered leftover-only headroom \(organizer private\): 21\./);
  assert.match(leftoverOnlyMinimum, /leftover uncovered leftover-only minimum \(organizer private\): 15\./);
  assert.match(leftoverOnlyUnitPrice, /leftover uncovered leftover-only unit price \(organizer private\): 38\./);
  assert.match(leftoverOnlyCapacity, /leftover uncovered leftover-only capacity \(organizer private\): 64\./);
  assert.notEqual(43, leftoverOffer.minimumUnits);
  assert.notEqual(43, leftoverOffer.unitPrice);
  assert.notEqual(43, leftoverOffer.capacity);
  assert.notEqual(43, 38);
  assert.notEqual(43, 21);
  assert.notEqual(43, 15);
  assert.notEqual(43, 42);
  assert.notEqual(43, 41);
  assert.notEqual(43, 39);
  assert.notEqual(leftoverOnlyRemaining, leftoverOnlyAllocated);
  assert.notEqual(leftoverOnlyRemaining, leftoverOnlyUnitPrice);
  assert.notEqual(leftoverOnlyRemaining, leftoverOnlyCapacity);
  assert.notEqual(leftoverOnlyRemaining, leftoverOnlyHeadroom);
  ...
  const keirinCycling = clonePreset("keirinCyclingCarnivalLunch");
  const hillClimbCycling = clonePreset("hillClimbCyclingCarnivalLunch");
  assert.notEqual(createLeftoverUncoveredLeftoverOnlyRemainingMarkdown(teamPursuitCycling), createLeftoverUncoveredLeftoverOnlyRemainingMarkdown(pursuitCycling));
  assert.notEqual(createLeftoverUncoveredLeftoverOnlyRemainingMarkdown(teamPursuitCycling), createLeftoverUncoveredLeftoverOnlyRemainingMarkdown(scratchCycling));
  assert.notEqual(createLeftoverUncoveredLeftoverOnlyRemainingMarkdown(teamPursuitCycling), createLeftoverUncoveredLeftoverOnlyRemainingMarkdown(pointsRaceCycling));
  assert.notEqual(createLeftoverUncoveredLeftoverOnlyRemainingMarkdown(teamPursuitCycling), createLeftoverUncoveredLeftoverOnlyRemainingMarkdown(omniumCycling));
  assert.notEqual(createLeftoverUncoveredLeftoverOnlyRemainingMarkdown(teamPursuitCycling), createLeftoverUncoveredLeftoverOnlyRemainingMarkdown(madisonCycling));
  assert.notEqual(createLeftoverUncoveredLeftoverOnlyRemainingMarkdown(teamPursuitCycling), createLeftoverUncoveredLeftoverOnlyRemainingMarkdown(keirinCycling));
  assert.notEqual(createLeftoverUncoveredLeftoverOnlyRemainingMarkdown(teamPursuitCycling), createLeftoverUncoveredLeftoverOnlyRemainingMarkdown(hillClimbCycling));
  assert.notEqual(createLeftoverUncoveredLeftoverOnlyUnitPriceMarkdown(teamPursuitCycling), createLeftoverUncoveredLeftoverOnlyUnitPriceMarkdown(pursuitCycling));
  assert.notEqual(createLeftoverUncoveredLeftoverOnlyHeadroomMarkdown(teamPursuitCycling), createLeftoverUncoveredLeftoverOnlyHeadroomMarkdown(pursuitCycling));
  assert.match(createLeftoverUncoveredLeftoverOnlyRemainingMarkdown(pursuitCycling), /leftover uncovered leftover-only remaining \(organizer private\): 41\./);
  assert.match(createLeftoverUncoveredLeftoverOnlyRemainingMarkdown(scratchCycling), /leftover uncovered leftover-only remaining \(organizer private\): 39\./);
  assert.match(createLeftoverUncoveredLeftoverOnlyRemainingMarkdown(pointsRaceCycling), /leftover uncovered leftover-only remaining \(organizer private\): 38\./);
  assert.match(createLeftoverUncoveredLeftoverOnlyRemainingMarkdown(omniumCycling), /leftover uncovered leftover-only remaining \(organizer private\): 37\./);
  assert.match(createLeftoverUncoveredLeftoverOnlyRemainingMarkdown(madisonCycling), /leftover uncovered leftover-only remaining \(organizer private\): 36\./);
  assert.match(createLeftoverUncoveredLeftoverOnlyRemainingMarkdown(keirinCycling), /leftover uncovered leftover-only remaining \(organizer private\): 35\./);
  assert.match(createLeftoverUncoveredLeftoverOnlyRemainingMarkdown(hillClimbCycling), /leftover uncovered leftover-only remaining \(organizer private\): 34\./);
  assert.equal(pursuitCycling.offers.find((offer) => offer.id === computeResidualCoverage(pursuitCycling).secondary.offerId).capacity, 64);
  assert.equal(scratchCycling.offers.find((offer) => offer.id === computeResidualCoverage(scratchCycling).secondary.offerId).capacity, 64);
  assert.equal(pointsRaceCycling.offers.find((offer) => offer.id === computeResidualCoverage(pointsRaceCycling).secondary.offerId).capacity, 64);
  assert.equal(omniumCycling.offers.find((offer) => offer.id === computeResidualCoverage(omniumCycling).secondary.offerId).capacity, 64);
  assert.equal(madisonCycling.offers.find((offer) => offer.id === computeResidualCoverage(madisonCycling).secondary.offerId).capacity, 64);
...
test("team-sprint cycling carnival leftover uncovered leftover-only remaining stays distinct from leftover-only allocated, leftover-only headroom, leftover-only capacity, leftover-only minimum, leftover unit price, leftover fill remaining, and team-pursuit leftover-only remaining", () => {
  const teamSprintCycling = clonePreset("teamSprintCyclingCarnivalLunch");
  const leftoverOnlyUnitPrice = createLeftoverUncoveredLeftoverOnlyUnitPriceMarkdown(teamSprintCycling);
  const leftoverOnlyCapacity = createLeftoverUncoveredLeftoverOnlyCapacityMarkdown(teamSprintCycling);
  const leftoverOnlyAllocated = createLeftoverUncoveredLeftoverOnlyAllocatedMarkdown(teamSprintCycling);
  const leftoverOnlyHeadroom = createLeftoverUncoveredLeftoverOnlyHeadroomMarkdown(teamSprintCycling);
  const leftoverOnlyMinimum = createLeftoverUncoveredLeftoverOnlyMinimumMarkdown(teamSprintCycling);
  const leftoverOnlyMaximum = createLeftoverUncoveredLeftoverOnlyMaximumMarkdown(teamSprintCycling);
  const leftoverOnlyRemaining = createLeftoverUncoveredLeftoverOnlyRemainingMarkdown(teamSprintCycling);
  const leftoverOnlyCount = createLeftoverUncoveredLeftoverOnlyCountMarkdown(teamSprintCycling);
  const leftoverOffer = teamSprintCycling.offers.find((offer) => offer.id === computeResidualCoverage(teamSprintCycling).secondary.offerId);
  assert.equal(leftoverOffer.minimumUnits, 43);
  assert.equal(leftoverOffer.unitPrice, 40);
  assert.equal(leftoverOffer.capacity, 64);
  assert.match(leftoverOnlyRemaining, /leftover uncovered leftover-only remaining \(organizer private\): 45\./);
  assert.match(leftoverOnlyAllocated, /leftover uncovered leftover-only allocated \(organizer private\): 45\./);
  assert.match(leftoverOnlyHeadroom, /leftover uncovered leftover-only headroom \(organizer private\): 19\./);
  assert.match(leftoverOnlyMinimum, /leftover uncovered leftover-only minimum \(organizer private\): 15\./);
  assert.match(leftoverOnlyUnitPrice, /leftover uncovered leftover-only unit price \(organizer private\): 40\./);
  assert.match(leftoverOnlyCapacity, /leftover uncovered leftover-only capacity \(organizer private\): 64\./);
  assert.notEqual(45, leftoverOffer.minimumUnits);
  assert.notEqual(45, leftoverOffer.unitPrice);
  assert.notEqual(45, leftoverOffer.capacity);
  assert.notEqual(45, 40);
  assert.notEqual(45, 19);
  assert.notEqual(45, 15);
  assert.notEqual(45, 43);
  assert.notEqual(45, 41);
  assert.notEqual(45, 39);
  assert.notEqual(leftoverOnlyRemaining, leftoverOnlyAllocated);
  assert.notEqual(leftoverOnlyRemaining, leftoverOnlyUnitPrice);
  assert.notEqual(leftoverOnlyRemaining, leftoverOnlyCapacity);
  assert.notEqual(leftoverOnlyRemaining, leftoverOnlyHeadroom);
  ...
  const keirinCycling = clonePreset("keirinCyclingCarnivalLunch");
  const hillClimbCycling = clonePreset("hillClimbCyclingCarnivalLunch");
  assert.notEqual(createLeftoverUncoveredLeftoverOnlyRemainingMarkdown(teamSprintCycling), createLeftoverUncoveredLeftoverOnlyRemainingMarkdown(teamPursuitCycling));
  assert.notEqual(createLeftoverUncoveredLeftoverOnlyRemainingMarkdown(teamSprintCycling), createLeftoverUncoveredLeftoverOnlyRemainingMarkdown(pursuitCycling));
  assert.notEqual(createLeftoverUncoveredLeftoverOnlyRemainingMarkdown(teamSprintCycling), createLeftoverUncoveredLeftoverOnlyRemainingMarkdown(scratchCycling));
  assert.notEqual(createLeftoverUncoveredLeftoverOnlyRemainingMarkdown(teamSprintCycling), createLeftoverUncoveredLeftoverOnlyRemainingMarkdown(pointsRaceCycling));
  assert.notEqual(createLeftoverUncoveredLeftoverOnlyRemainingMarkdown(teamSprintCycling), createLeftoverUncoveredLeftoverOnlyRemainingMarkdown(omniumCycling));
  assert.notEqual(createLeftoverUncoveredLeftoverOnlyRemainingMarkdown(teamSprintCycling), createLeftoverUncoveredLeftoverOnlyRemainingMarkdown(madisonCycling));
  assert.notEqual(createLeftoverUncoveredLeftoverOnlyRemainingMarkdown(teamSprintCycling), createLeftoverUncoveredLeftoverOnlyRemainingMarkdown(keirinCycling));
  assert.notEqual(createLeftoverUncoveredLeftoverOnlyRemainingMarkdown(teamSprintCycling), createLeftoverUncoveredLeftoverOnlyRemainingMarkdown(hillClimbCycling));
  assert.notEqual(createLeftoverUncoveredLeftoverOnlyUnitPriceMarkdown(teamSprintCycling), createLeftoverUncoveredLeftoverOnlyUnitPriceMarkdown(teamPursuitCycling));
  assert.notEqual(createLeftoverUncoveredLeftoverOnlyHeadroomMarkdown(teamSprintCycling), createLeftoverUncoveredLeftoverOnlyHeadroomMarkdown(teamPursuitCycling));
  assert.notEqual(createLeftoverUncoveredLeftoverOnlyUnitPriceMarkdown(teamSprintCycling), createLeftoverUncoveredLeftoverOnlyUnitPriceMarkdown(pursuitCycling));
  assert.notEqual(createLeftoverUncoveredLeftoverOnlyHeadroomMarkdown(teamSprintCycling), createLeftoverUncoveredLeftoverOnlyHeadroomMarkdown(pursuitCycling));
  assert.match(createLeftoverUncoveredLeftoverOnlyRemainingMarkdown(teamPursuitCycling), /leftover uncovered leftover-only remaining \(organizer private\): 43\./);
  assert.match(createLeftoverUncoveredLeftoverOnlyRemainingMarkdown(pursuitCycling), /leftover uncovered leftover-only remaining \(organizer private\): 41\./);
  assert.match(createLeftoverUncoveredLeftoverOnlyRemainingMarkdown(scratchCycling), /leftover uncovered leftover-only remaining \(organizer private\): 39\./);
  assert.match(createLeftoverUncoveredLeftoverOnlyRemainingMarkdown(pointsRaceCycling), /leftover uncovered leftover-only remaining \(organizer private\): 38\./);
  assert.match(createLeftoverUncoveredLeftoverOnlyRemainingMarkdown(omniumCycling), /leftover uncovered leftover-only remaining \(organizer private\): 37\./);
  ...
  assert.match(createLeftoverUncoveredLeftoverOnlyRemainingMarkdown(keirinCycling), /leftover uncovered leftover-only remaining \(organizer private\): 35\./);
  assert.match(createLeftoverUncoveredLeftoverOnlyRemainingMarkdown(hillClimbCycling), /leftover uncovered leftover-only remaining \(organizer private\): 34\./);
  assert.equal(teamPursuitCycling.offers.find((offer) => offer.id === computeResidualCoverage(teamPursuitCycling).secondary.offerId).capacity, 64);
  assert.equal(pursuitCycling.offers.find((offer) => offer.id === computeResidualCoverage(pursuitCycling).secondary.offerId).capacity, 64);
  assert.equal(scratchCycling.offers.find((offer) => offer.id === computeResidualCoverage(scratchCycling).secondary.offerId).capacity, 64);
  assert.equal(pointsRaceCycling.offers.find((offer) => offer.id === computeResidualCoverage(pointsRaceCycling).secondary.offerId).capacity, 64);
  assert.equal(omniumCycling.offers.find((offer) => offer.id === computeResidualCoverage(omniumCycling).secondary.offerId).capacity, 64);
...
test("first-aid cycling carnival leftover uncovered leftover-only remaining stays distinct from leftover-only allocated, leftover-only headroom, leftover-only capacity, leftover-only minimum, leftover unit price, leftover fill remaining, and team-sprint leftover-only remaining", () => {
  const firstAidCycling = clonePreset("firstAidCyclingCarnivalLunch");
  const leftoverOnlyUnitPrice = createLeftoverUncoveredLeftoverOnlyUnitPriceMarkdown(firstAidCycling);
  const leftoverOnlyCapacity = createLeftoverUncoveredLeftoverOnlyCapacityMarkdown(firstAidCycling);
  const leftoverOnlyAllocated = createLeftoverUncoveredLeftoverOnlyAllocatedMarkdown(firstAidCycling);
  const leftoverOnlyHeadroom = createLeftoverUncoveredLeftoverOnlyHeadroomMarkdown(firstAidCycling);
  const leftoverOnlyMinimum = createLeftoverUncoveredLeftoverOnlyMinimumMarkdown(firstAidCycling);
  const leftoverOnlyMaximum = createLeftoverUncoveredLeftoverOnlyMaximumMarkdown(firstAidCycling);
  const leftoverOnlyRemaining = createLeftoverUncoveredLeftoverOnlyRemainingMarkdown(firstAidCycling);
  const leftoverOnlyCount = createLeftoverUncoveredLeftoverOnlyCountMarkdown(firstAidCycling);
  const leftoverOffer = firstAidCycling.offers.find((offer) => offer.id === computeResidualCoverage(firstAidCycling).secondary.offerId);
  assert.equal(leftoverOffer.minimumUnits, 45);
  assert.equal(leftoverOffer.unitPrice, 42);
  assert.equal(leftoverOffer.capacity, 64);
  assert.match(leftoverOnlyRemaining, /leftover uncovered leftover-only remaining \(organizer private\): 47\./);
  assert.match(leftoverOnlyAllocated, /leftover uncovered leftover-only allocated \(organizer private\): 47\./);
  assert.match(leftoverOnlyHeadroom, /leftover uncovered leftover-only headroom \(organizer private\): 17\./);
  assert.match(leftoverOnlyMinimum, /leftover uncovered leftover-only minimum \(organizer private\): 15\./);
  assert.match(leftoverOnlyUnitPrice, /leftover uncovered leftover-only unit price \(organizer private\): 42\./);
  assert.match(leftoverOnlyCapacity, /leftover uncovered leftover-only capacity \(organizer private\): 64\./);
  assert.notEqual(47, leftoverOffer.minimumUnits);
  assert.notEqual(47, leftoverOffer.unitPrice);
  assert.notEqual(47, leftoverOffer.capacity);
  assert.notEqual(47, 42);
  assert.notEqual(47, 17);
  assert.notEqual(47, 15);
  assert.notEqual(47, 45);
  assert.notEqual(47, 43);
  assert.notEqual(47, 41);
  assert.notEqual(leftoverOnlyRemaining, leftoverOnlyAllocated);
  assert.notEqual(leftoverOnlyRemaining, leftoverOnlyUnitPrice);
  assert.notEqual(leftoverOnlyRemaining, leftoverOnlyCapacity);
  assert.notEqual(leftoverOnlyRemaining, leftoverOnlyHeadroom);
  ...
  const keirinCycling = clonePreset("keirinCyclingCarnivalLunch");
  const hillClimbCycling = clonePreset("hillClimbCyclingCarnivalLunch");
  const leadOutCyclingFromFirstAid = clonePreset("leadOutCyclingCarnivalLunch");
  assert.notEqual(createLeftoverUncoveredLeftoverOnlyRemainingMarkdown(firstAidCycling), createLeftoverUncoveredLeftoverOnlyRemainingMarkdown(leadOutCyclingFromFirstAid));
  assert.match(createLeftoverUncoveredLeftoverOnlyRemainingMarkdown(leadOutCyclingFromFirstAid), /leftover uncovered leftover-only remaining \(organizer private\): 51\./);
  assert.notEqual(createLeftoverUncoveredLeftoverOnlyRemainingMarkdown(firstAidCycling), createLeftoverUncoveredLeftoverOnlyRemainingMarkdown(teamSprintCycling));
  assert.notEqual(createLeftoverUncoveredLeftoverOnlyRemainingMarkdown(firstAidCycling), createLeftoverUncoveredLeftoverOnlyRemainingMarkdown(teamPursuitCycling));
  assert.notEqual(createLeftoverUncoveredLeftoverOnlyRemainingMarkdown(firstAidCycling), createLeftoverUncoveredLeftoverOnlyRemainingMarkdown(pursuitCycling));
  assert.notEqual(createLeftoverUncoveredLeftoverOnlyRemainingMarkdown(firstAidCycling), createLeftoverUncoveredLeftoverOnlyRemainingMarkdown(scratchCycling));
  assert.notEqual(createLeftoverUncoveredLeftoverOnlyRemainingMarkdown(firstAidCycling), createLeftoverUncoveredLeftoverOnlyRemainingMarkdown(pointsRaceCycling));
  assert.notEqual(createLeftoverUncoveredLeftoverOnlyRemainingMarkdown(firstAidCycling), createLeftoverUncoveredLeftoverOnlyRemainingMarkdown(omniumCycling));
  assert.notEqual(createLeftoverUncoveredLeftoverOnlyRemainingMarkdown(firstAidCycling), createLeftoverUncoveredLeftoverOnlyRemainingMarkdown(madisonCycling));
  assert.notEqual(createLeftoverUncoveredLeftoverOnlyRemainingMarkdown(firstAidCycling), createLeftoverUncoveredLeftoverOnlyRemainingMarkdown(keirinCycling));
  assert.notEqual(createLeftoverUncoveredLeftoverOnlyRemainingMarkdown(firstAidCycling), createLeftoverUncoveredLeftoverOnlyRemainingMarkdown(hillClimbCycling));
  assert.notEqual(createLeftoverUncoveredLeftoverOnlyUnitPriceMarkdown(firstAidCycling), createLeftoverUncoveredLeftoverOnlyUnitPriceMarkdown(teamSprintCycling));
  assert.notEqual(createLeftoverUncoveredLeftoverOnlyHeadroomMarkdown(firstAidCycling), createLeftoverUncoveredLeftoverOnlyHeadroomMarkdown(teamSprintCycling));
  assert.notEqual(createLeftoverUncoveredLeftoverOnlyUnitPriceMarkdown(firstAidCycling), createLeftoverUncoveredLeftoverOnlyUnitPriceMarkdown(teamPursuitCycling));
  assert.notEqual(createLeftoverUncoveredLeftoverOnlyHeadroomMarkdown(firstAidCycling), createLeftoverUncoveredLeftoverOnlyHeadroomMarkdown(teamPursuitCycling));
  assert.match(createLeftoverUncoveredLeftoverOnlyRemainingMarkdown(teamSprintCycling), /leftover uncovered leftover-only remaining \(organizer private\): 45\./);
  assert.match(createLeftoverUncoveredLeftoverOnlyRemainingMarkdown(teamPursuitCycling), /leftover uncovered leftover-only remaining \(organizer private\): 43\./);
  assert.match(createLeftoverUncoveredLeftoverOnlyRemainingMarkdown(pursuitCycling), /leftover uncovered leftover-only remaining \(organizer private\): 41\./);
  assert.match(createLeftoverUncoveredLeftoverOnlyRemainingMarkdown(scratchCycling), /leftover uncovered leftover-only remaining \(organizer private\): 39\./);
  assert.match(createLeftoverUncoveredLeftoverOnlyRemainingMarkdown(pointsRaceCycling), /leftover uncovered leftover-only remaining \(organizer private\): 38\./);
  ...
  assert.match(createLeftoverUncoveredLeftoverOnlyRemainingMarkdown(keirinCycling), /leftover uncovered leftover-only remaining \(organizer private\): 35\./);
  assert.match(createLeftoverUncoveredLeftoverOnlyRemainingMarkdown(hillClimbCycling), /leftover uncovered leftover-only remaining \(organizer private\): 34\./);
  assert.equal(teamSprintCycling.offers.find((offer) => offer.id === computeResidualCoverage(teamSprintCycling).secondary.offerId).capacity, 64);
  assert.equal(teamPursuitCycling.offers.find((offer) => offer.id === computeResidualCoverage(teamPursuitCycling).secondary.offerId).capacity, 64);
  assert.equal(pursuitCycling.offers.find((offer) => offer.id === computeResidualCoverage(pursuitCycling).secondary.offerId).capacity, 64);
  assert.equal(scratchCycling.offers.find((offer) => offer.id === computeResidualCoverage(scratchCycling).secondary.offerId).capacity, 64);
  assert.equal(pointsRaceCycling.offers.find((offer) => offer.id === computeResidualCoverage(pointsRaceCycling).secondary.offerId).capacity, 64);
...
test("feed-zone cycling carnival leftover uncovered leftover-only remaining stays distinct from leftover-only allocated, leftover-only headroom, leftover-only capacity, leftover-only minimum, leftover unit price, leftover fill remaining, and first-aid leftover-only remaining", () => {
  const feedZoneCycling = clonePreset("feedZoneCyclingCarnivalLunch");
  const leftoverOnlyUnitPrice = createLeftoverUncoveredLeftoverOnlyUnitPriceMarkdown(feedZoneCycling);
  const leftoverOnlyCapacity = createLeftoverUncoveredLeftoverOnlyCapacityMarkdown(feedZoneCycling);
  const leftoverOnlyAllocated = createLeftoverUncoveredLeftoverOnlyAllocatedMarkdown(feedZoneCycling);
  const leftoverOnlyHeadroom = createLeftoverUncoveredLeftoverOnlyHeadroomMarkdown(feedZoneCycling);
  const leftoverOnlyMinimum = createLeftoverUncoveredLeftoverOnlyMinimumMarkdown(feedZoneCycling);
  const leftoverOnlyMaximum = createLeftoverUncoveredLeftoverOnlyMaximumMarkdown(feedZoneCycling);
  const leftoverOnlyRemaining = createLeftoverUncoveredLeftoverOnlyRemainingMarkdown(feedZoneCycling);
  const leftoverOnlyCount = createLeftoverUncoveredLeftoverOnlyCountMarkdown(feedZoneCycling);
  const leftoverOffer = feedZoneCycling.offers.find((offer) => offer.id === computeResidualCoverage(feedZoneCycling).secondary.offerId);
  assert.equal(leftoverOffer.minimumUnits, 47);
  assert.equal(leftoverOffer.unitPrice, 44);
  assert.equal(leftoverOffer.capacity, 64);
  assert.match(leftoverOnlyRemaining, /leftover uncovered leftover-only remaining \(organizer private\): 49\./);
  assert.match(leftoverOnlyAllocated, /leftover uncovered leftover-only allocated \(organizer private\): 49\./);
  assert.match(leftoverOnlyHeadroom, /leftover uncovered leftover-only headroom \(organizer private\): 15\./);
  assert.match(leftoverOnlyMinimum, /leftover uncovered leftover-only minimum \(organizer private\): 15\./);
  assert.match(leftoverOnlyUnitPrice, /leftover uncovered leftover-only unit price \(organizer private\): 44\./);
  assert.match(leftoverOnlyCapacity, /leftover uncovered leftover-only capacity \(organizer private\): 64\./);
  assert.notEqual(49, leftoverOffer.minimumUnits);
  assert.notEqual(49, leftoverOffer.unitPrice);
  assert.notEqual(49, leftoverOffer.capacity);
  assert.notEqual(49, 44);
  assert.notEqual(49, 15);
  assert.notEqual(49, 47);
  assert.notEqual(49, 45);
  assert.notEqual(49, 43);
  assert.notEqual(leftoverOnlyRemaining, leftoverOnlyAllocated);
  assert.notEqual(leftoverOnlyRemaining, leftoverOnlyUnitPrice);
  assert.notEqual(leftoverOnlyRemaining, leftoverOnlyCapacity);
  assert.notEqual(leftoverOnlyRemaining, leftoverOnlyHeadroom);
  ...
  const keirinCycling = clonePreset("keirinCyclingCarnivalLunch");
  const hillClimbCycling = clonePreset("hillClimbCyclingCarnivalLunch");
  const leadOutCycling = clonePreset("leadOutCyclingCarnivalLunch");
  assert.notEqual(createLeftoverUncoveredLeftoverOnlyRemainingMarkdown(feedZoneCycling), createLeftoverUncoveredLeftoverOnlyRemainingMarkdown(leadOutCycling));
  assert.match(createLeftoverUncoveredLeftoverOnlyRemainingMarkdown(leadOutCycling), /leftover uncovered leftover-only remaining \(organizer private\): 51\./);
  assert.notEqual(createLeftoverUncoveredLeftoverOnlyRemainingMarkdown(feedZoneCycling), createLeftoverUncoveredLeftoverOnlyRemainingMarkdown(firstAidCycling));
  assert.notEqual(createLeftoverUncoveredLeftoverOnlyRemainingMarkdown(feedZoneCycling), createLeftoverUncoveredLeftoverOnlyRemainingMarkdown(teamSprintCycling));
  assert.notEqual(createLeftoverUncoveredLeftoverOnlyRemainingMarkdown(feedZoneCycling), createLeftoverUncoveredLeftoverOnlyRemainingMarkdown(teamPursuitCycling));
  assert.notEqual(createLeftoverUncoveredLeftoverOnlyRemainingMarkdown(feedZoneCycling), createLeftoverUncoveredLeftoverOnlyRemainingMarkdown(pursuitCycling));
  assert.notEqual(createLeftoverUncoveredLeftoverOnlyRemainingMarkdown(feedZoneCycling), createLeftoverUncoveredLeftoverOnlyRemainingMarkdown(scratchCycling));
  assert.notEqual(createLeftoverUncoveredLeftoverOnlyRemainingMarkdown(feedZoneCycling), createLeftoverUncoveredLeftoverOnlyRemainingMarkdown(pointsRaceCycling));
  assert.notEqual(createLeftoverUncoveredLeftoverOnlyRemainingMarkdown(feedZoneCycling), createLeftoverUncoveredLeftoverOnlyRemainingMarkdown(omniumCycling));
  assert.notEqual(createLeftoverUncoveredLeftoverOnlyRemainingMarkdown(feedZoneCycling), createLeftoverUncoveredLeftoverOnlyRemainingMarkdown(madisonCycling));
  assert.notEqual(createLeftoverUncoveredLeftoverOnlyRemainingMarkdown(feedZoneCycling), createLeftoverUncoveredLeftoverOnlyRemainingMarkdown(keirinCycling));
  assert.notEqual(createLeftoverUncoveredLeftoverOnlyRemainingMarkdown(feedZoneCycling), createLeftoverUncoveredLeftoverOnlyRemainingMarkdown(hillClimbCycling));
  assert.notEqual(createLeftoverUncoveredLeftoverOnlyUnitPriceMarkdown(feedZoneCycling), createLeftoverUncoveredLeftoverOnlyUnitPriceMarkdown(firstAidCycling));
  assert.notEqual(createLeftoverUncoveredLeftoverOnlyHeadroomMarkdown(feedZoneCycling), createLeftoverUncoveredLeftoverOnlyHeadroomMarkdown(firstAidCycling));
  assert.notEqual(createLeftoverUncoveredLeftoverOnlyUnitPriceMarkdown(feedZoneCycling), createLeftoverUncoveredLeftoverOnlyUnitPriceMarkdown(teamSprintCycling));
  assert.notEqual(createLeftoverUncoveredLeftoverOnlyHeadroomMarkdown(feedZoneCycling), createLeftoverUncoveredLeftoverOnlyHeadroomMarkdown(teamSprintCycling));
  assert.match(createLeftoverUncoveredLeftoverOnlyRemainingMarkdown(firstAidCycling), /leftover uncovered leftover-only remaining \(organizer private\): 47\./);
  assert.match(createLeftoverUncoveredLeftoverOnlyRemainingMarkdown(teamSprintCycling), /leftover uncovered leftover-only remaining \(organizer private\): 45\./);
  assert.match(createLeftoverUncoveredLeftoverOnlyRemainingMarkdown(teamPursuitCycling), /leftover uncovered leftover-only remaining \(organizer private\): 43\./);
  assert.match(createLeftoverUncoveredLeftoverOnlyRemainingMarkdown(pursuitCycling), /leftover uncovered leftover-only remaining \(organizer private\): 41\./);
  assert.match(createLeftoverUncoveredLeftoverOnlyRemainingMarkdown(scratchCycling), /leftover uncovered leftover-only remaining \(organizer private\): 39\./);
  ...
  assert.match(createLeftoverUncoveredLeftoverOnlyRemainingMarkdown(keirinCycling), /leftover uncovered leftover-only remaining \(organizer private\): 35\./);
  assert.match(createLeftoverUncoveredLeftoverOnlyRemainingMarkdown(hillClimbCycling), /leftover uncovered leftover-only remaining \(organizer private\): 34\./);
  assert.equal(firstAidCycling.offers.find((offer) => offer.id === computeResidualCoverage(firstAidCycling).secondary.offerId).capacity, 64);
  assert.equal(teamSprintCycling.offers.find((offer) => offer.id === computeResidualCoverage(teamSprintCycling).secondary.offerId).capacity, 64);
  assert.equal(teamPursuitCycling.offers.find((offer) => offer.id === computeResidualCoverage(teamPursuitCycling).secondary.offerId).capacity, 64);
  assert.equal(pursuitCycling.offers.find((offer) => offer.id === computeResidualCoverage(pursuitCycling).secondary.offerId).capacity, 64);
  assert.equal(scratchCycling.offers.find((offer) => offer.id === computeResidualCoverage(scratchCycling).secondary.offerId).capacity, 64);
...
test("lead-out cycling carnival leftover uncovered leftover-only remaining stays distinct from leftover-only allocated, leftover-only headroom, leftover-only capacity, leftover-only minimum, leftover unit price, leftover fill remaining, and feed-zone leftover-only remaining", () => {
  const leadOutCycling = clonePreset("leadOutCyclingCarnivalLunch");
  const leftoverOnlyUnitPrice = createLeftoverUncoveredLeftoverOnlyUnitPriceMarkdown(leadOutCycling);
  const leftoverOnlyCapacity = createLeftoverUncoveredLeftoverOnlyCapacityMarkdown(leadOutCycling);
  const leftoverOnlyAllocated = createLeftoverUncoveredLeftoverOnlyAllocatedMarkdown(leadOutCycling);
  const leftoverOnlyHeadroom = createLeftoverUncoveredLeftoverOnlyHeadroomMarkdown(leadOutCycling);
  const leftoverOnlyMinimum = createLeftoverUncoveredLeftoverOnlyMinimumMarkdown(leadOutCycling);
  const leftoverOnlyMaximum = createLeftoverUncoveredLeftoverOnlyMaximumMarkdown(leadOutCycling);
  const leftoverOnlyRemaining = createLeftoverUncoveredLeftoverOnlyRemainingMarkdown(leadOutCycling);
  const leftoverOnlyCount = createLeftoverUncoveredLeftoverOnlyCountMarkdown(leadOutCycling);
  const leftoverOffer = leadOutCycling.offers.find((offer) => offer.id === computeResidualCoverage(leadOutCycling).secondary.offerId);
  assert.equal(leftoverOffer.minimumUnits, 49);
  assert.equal(leftoverOffer.unitPrice, 46);
  assert.equal(leftoverOffer.capacity, 64);
  assert.match(leftoverOnlyRemaining, /leftover uncovered leftover-only remaining \(organizer private\): 51\./);
  assert.match(leftoverOnlyAllocated, /leftover uncovered leftover-only allocated \(organizer private\): 51\./);
  assert.match(leftoverOnlyHeadroom, /leftover uncovered leftover-only headroom \(organizer private\): 13\./);
  assert.match(leftoverOnlyMinimum, /leftover uncovered leftover-only minimum \(organizer private\): 15\./);
  assert.match(leftoverOnlyUnitPrice, /leftover uncovered leftover-only unit price \(organizer private\): 46\./);
  assert.match(leftoverOnlyCapacity, /leftover uncovered leftover-only capacity \(organizer private\): 64\./);
  assert.notEqual(51, leftoverOffer.minimumUnits);
  assert.notEqual(51, leftoverOffer.unitPrice);
  assert.notEqual(51, leftoverOffer.capacity);
  assert.notEqual(51, 46);
  assert.notEqual(51, 13);
  assert.notEqual(51, 15);
  assert.notEqual(51, 49);
  assert.notEqual(51, 47);
  assert.notEqual(51, 45);
  assert.notEqual(leftoverOnlyRemaining, leftoverOnlyAllocated);
  assert.notEqual(leftoverOnlyRemaining, leftoverOnlyUnitPrice);
  assert.notEqual(leftoverOnlyRemaining, leftoverOnlyCapacity);
  assert.notEqual(leftoverOnlyRemaining, leftoverOnlyHeadroom);
  ...
  const keirinCycling = clonePreset("keirinCyclingCarnivalLunch");
  const hillClimbCycling = clonePreset("hillClimbCyclingCarnivalLunch");
  assert.notEqual(createLeftoverUncoveredLeftoverOnlyRemainingMarkdown(leadOutCycling), createLeftoverUncoveredLeftoverOnlyRemainingMarkdown(feedZoneCycling));
  assert.notEqual(createLeftoverUncoveredLeftoverOnlyRemainingMarkdown(leadOutCycling), createLeftoverUncoveredLeftoverOnlyRemainingMarkdown(firstAidCycling));
  assert.notEqual(createLeftoverUncoveredLeftoverOnlyRemainingMarkdown(leadOutCycling), createLeftoverUncoveredLeftoverOnlyRemainingMarkdown(teamSprintCycling));
  assert.notEqual(createLeftoverUncoveredLeftoverOnlyRemainingMarkdown(leadOutCycling), createLeftoverUncoveredLeftoverOnlyRemainingMarkdown(teamPursuitCycling));
  assert.notEqual(createLeftoverUncoveredLeftoverOnlyRemainingMarkdown(leadOutCycling), createLeftoverUncoveredLeftoverOnlyRemainingMarkdown(pursuitCycling));
  assert.notEqual(createLeftoverUncoveredLeftoverOnlyRemainingMarkdown(leadOutCycling), createLeftoverUncoveredLeftoverOnlyRemainingMarkdown(scratchCycling));
  assert.notEqual(createLeftoverUncoveredLeftoverOnlyRemainingMarkdown(leadOutCycling), createLeftoverUncoveredLeftoverOnlyRemainingMarkdown(pointsRaceCycling));
  assert.notEqual(createLeftoverUncoveredLeftoverOnlyRemainingMarkdown(leadOutCycling), createLeftoverUncoveredLeftoverOnlyRemainingMarkdown(omniumCycling));
  assert.notEqual(createLeftoverUncoveredLeftoverOnlyRemainingMarkdown(leadOutCycling), createLeftoverUncoveredLeftoverOnlyRemainingMarkdown(madisonCycling));
  assert.notEqual(createLeftoverUncoveredLeftoverOnlyRemainingMarkdown(leadOutCycling), createLeftoverUncoveredLeftoverOnlyRemainingMarkdown(keirinCycling));
  assert.notEqual(createLeftoverUncoveredLeftoverOnlyRemainingMarkdown(leadOutCycling), createLeftoverUncoveredLeftoverOnlyRemainingMarkdown(hillClimbCycling));
  assert.notEqual(createLeftoverUncoveredLeftoverOnlyUnitPriceMarkdown(leadOutCycling), createLeftoverUncoveredLeftoverOnlyUnitPriceMarkdown(feedZoneCycling));
  assert.notEqual(createLeftoverUncoveredLeftoverOnlyHeadroomMarkdown(leadOutCycling), createLeftoverUncoveredLeftoverOnlyHeadroomMarkdown(feedZoneCycling));
  assert.notEqual(createLeftoverUncoveredLeftoverOnlyUnitPriceMarkdown(leadOutCycling), createLeftoverUncoveredLeftoverOnlyUnitPriceMarkdown(firstAidCycling));
  assert.notEqual(createLeftoverUncoveredLeftoverOnlyHeadroomMarkdown(leadOutCycling), createLeftoverUncoveredLeftoverOnlyHeadroomMarkdown(firstAidCycling));
  assert.match(createLeftoverUncoveredLeftoverOnlyRemainingMarkdown(feedZoneCycling), /leftover uncovered leftover-only remaining \(organizer private\): 49\./);
  assert.match(createLeftoverUncoveredLeftoverOnlyRemainingMarkdown(firstAidCycling), /leftover uncovered leftover-only remaining \(organizer private\): 47\./);
  assert.match(createLeftoverUncoveredLeftoverOnlyRemainingMarkdown(teamSprintCycling), /leftover uncovered leftover-only remaining \(organizer private\): 45\./);
  assert.match(createLeftoverUncoveredLeftoverOnlyRemainingMarkdown(teamPursuitCycling), /leftover uncovered leftover-only remaining \(organizer private\): 43\./);
  assert.match(createLeftoverUncoveredLeftoverOnlyRemainingMarkdown(pursuitCycling), /leftover uncovered leftover-only remaining \(organizer private\): 41\./);
  ...
  assert.match(createLeftoverUncoveredLeftoverOnlyRemainingMarkdown(keirinCycling), /leftover uncovered leftover-only remaining \(organizer private\): 35\./);
  assert.match(createLeftoverUncoveredLeftoverOnlyRemainingMarkdown(hillClimbCycling), /leftover uncovered leftover-only remaining \(organizer private\): 34\./);
  assert.equal(feedZoneCycling.offers.find((offer) => offer.id === computeResidualCoverage(feedZoneCycling).secondary.offerId).capacity, 64);
  assert.equal(firstAidCycling.offers.find((offer) => offer.id === computeResidualCoverage(firstAidCycling).secondary.offerId).capacity, 64);
  assert.equal(teamSprintCycling.offers.find((offer) => offer.id === computeResidualCoverage(teamSprintCycling).secondary.offerId).capacity, 64);
  assert.equal(teamPursuitCycling.offers.find((offer) => offer.id === computeResidualCoverage(teamPursuitCycling).secondary.offerId).capacity, 64);
  assert.equal(pursuitCycling.offers.find((offer) => offer.id === computeResidualCoverage(pursuitCycling).secondary.offerId).capacity, 64);
