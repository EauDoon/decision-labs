...
test("hide tertiary leftover row is display-only and leaves matching unchanged", () => {
  ...
  const all = filterLeftoverCoverageRowsHidingTertiary(scenario, false);
  const shown = filterLeftoverCoverageRowsHidingTertiary(scenario, true);
  assert.deepEqual(all.map((row) => row.id), leftoverCoverageRows(scenario).map((row) => row.id));
  assert.equal(all.length, 4);
  assert.ok(coverage.tertiary);
  assert.equal(shown.some((row) => row.id === "tertiary-fill"), false);
  assert.equal(shown.some((row) => row.id === "leftover-after-winner"), true);
  assert.equal(shown.some((row) => row.id === "leftover-fill"), true);
...
test("hide tertiary leftover row does not break hide covered leftover rows", () => {
  const scenario = leftoverFixture();
  const covered = filterLeftoverCoverageRowsHidingCovered(scenario, true);
  const tertiary = filterLeftoverCoverageRowsHidingTertiary(scenario, true);
  assert.equal(covered.some((row) => row.id === "leftover-fill"), false);
  assert.equal(tertiary.some((row) => row.id === "tertiary-fill"), false);
  assert.equal(tertiary.some((row) => row.id === "leftover-fill"), true);
  assert.equal(filterLeftoverCoverageRowsHidingCovered(scenario, false).length, 4);
  assert.equal(filterLeftoverCoverageRowsHidingTertiary(scenario, false).length, 4);
});

test("hide tertiary leftover row rejects prototype-like flags", () => {
  const scenario = clonePreset("studio");
  assert.throws(() => filterLeftoverCoverageRowsHidingTertiary(scenario, "true"), /true or false/);
  assert.throws(() => filterLeftoverCoverageRowsHidingTertiary(scenario, 1), ScenarioError);
});

test("the organizer hide-tertiary leftover filter is not on the merchant table", async () => {
  const html = await readFile(new URL("../index.html", import.meta.url), "utf8");
  const app = await readFile(new URL("../src/app.js", import.meta.url), "utf8");
  const buyerPanel = html.slice(html.indexOf('id="buyer-panel"'), html.indexOf('id="merchant-panel"'));
  const merchantPanel = html.slice(html.indexOf('id="merchant-panel"'), html.indexOf('id="method-panel"'));
  assert.match(buyerPanel, /id="hide-tertiary-leftover-row"/u);
  assert.match(buyerPanel, /Hide the tertiary leftover coverage row/u);
  assert.match(buyerPanel, /id="hide-covered-leftover-rows"/u);
  assert.match(buyerPanel, /Merchant views still show counts only/u);
  assert.equal(merchantPanel.includes("hide-tertiary-leftover-row"), false);
