...
test("Shift+F8 leftover uncovered leftover-only capacity copy jump stays on the organizer leftover uncovered leftover-only capacity control", async () => {
  ...
  assert.match(html, /<kbd>Shift\+F8<\/kbd> Focus the leftover uncovered leftover-only capacity copy control, or leftover heading if missing/u);
  ...
  assert.match(app, /if \(event\.shiftKey && key === "F8"\) \{\s*event\.preventDefault\(\);\s*focusLeftoverUncoveredLeftoverOnlyCapacityCopy\(\);/u);
