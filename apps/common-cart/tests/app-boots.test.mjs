...
// Regression test for the M9 final review finding: the 1.6.0 contingency work
// left `let contingencyRuns` after the load-time refresh() call, so every page
// load died with a temporal-dead-zone ReferenceError. No test executed app.js,
// so the suite stayed green while the workbench was dead. This test boots the
// real module top-to-bottom in a stub DOM and fails on any load-time throw.
// The stub is intentionally tolerant (unknown selectors yield generic nodes):
// it guards evaluation order, not markup completeness or behavior.
...
test("app module evaluates top-to-bottom without a load-time throw", async () => {
  ...
  const status = nodes.get("contingency-status");
  assert.ok(status, "contingency status node exists in markup");
  assert.match(status.textContent, /No contingency experiment has run|Contingency experiments will appear/);
});
