...
test("leftover print one-pager uses evidence blocks and omits private buyer rows", async () => {
  ...
  assert.match(leftover, /leftover-print-fill-evidence/u);
  assert.match(leftover, /Leftover fill evidence: none/u);
  assert.match(leftover, /leftover-print-tertiary-evidence/u);
  assert.match(leftover, /Tertiary fill evidence: none/u);
  assert.match(leftover, /leftover-print-uncovered-evidence/u);
  assert.match(leftover, /Uncovered leftover evidence: none/u);
  ...
  assert.equal(merchantPanel.includes("leftover-print-fill-evidence"), false);
  assert.equal(merchantPanel.includes("copy-leftover-fill-evidence"), false);
});
...
test("merchant-facing 1.4.1 surfaces omit buyer labels, ids, budgets, and allocations", () => {
  ...
  const merchantSurfaces = [
    ...
    winningOfferEvidenceToMarkdown(left)
  ];
