...
test("leftover uncovered leftover-only allocated copy stays on its control without Shift+F7", async () => {
  ...
  assert.match(app, /createLeftoverUncoveredLeftoverOnlyAllocatedMarkdown\(scenario\)/u);
  ...
  assert.doesNotMatch(app, /if \(key === "F7"\) \{\s*event\.preventDefault\(\);\s*copyLeftoverUncoveredLeftoverOnlyAllocated/u);
  assert.match(app, /if \(event\.shiftKey && key === "F7"\) \{\s*event\.preventDefault\(\);\s*copyLeftoverUncoveredLeftoverOnlyUnitPrice\(\);/u);
