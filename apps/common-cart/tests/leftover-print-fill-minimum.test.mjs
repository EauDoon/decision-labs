...
test("print leftover one-pager includes leftover-fill minimum when leftover fill exists", async () => {
  ...
  assert.match(css, /body\.print-leftover #leftover-print-fill-minimum/u);
  const leftover = html.slice(html.indexOf('id="leftover-print-fill-minimum"'), html.indexOf('id="copy-winning-merchant"'));
  ...
  assert.match(app, /leftover-print-fill-minimum/u);
  assert.match(app, /Leftover fill minimum:/u);
  assert.match(app, /Leftover fill minimum: \$\{leftoverOffer\.minimumUnits\} units/u);
