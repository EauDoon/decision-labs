...
test("Shift+F8 leftover uncovered leftover-only minimum copy jump stays on the organizer leftover uncovered leftover-only minimum control", async () => {
  ...
  assert.match(html, /<kbd>Shift\+F8<\/kbd> Focus the leftover uncovered leftover-only minimum copy control, or leftover heading if missing/u);
  ...
  assert.match(app, /if \(event\.shiftKey && key === "F8"\) \{\s*event\.preventDefault\(\);\s*focusLeftoverUncoveredLeftoverOnlyMinimumCopy\(\);/u);
  assert.doesNotMatch(app, /if \(event\.shiftKey && key === "F8"\) \{\s*event\.preventDefault\(\);\s*copyLeftoverUncoveredLeftoverOnlyMinimum/u);
...
test("Shift+F9 hide first uncovered leftover buyer jump stays organizer-only", async () => {
  const html = await readFile(new URL("../index.html", import.meta.url), "utf8");
  const app = await readFile(new URL("../src/app.js", import.meta.url), "utf8");
  const buyerPanel = html.slice(html.indexOf('id="buyer-panel"'), html.indexOf('id="merchant-panel"'));
  const merchantPanel = html.slice(html.indexOf('id="merchant-panel"'), html.indexOf('id="method-panel"'));
  ...
  assert.match(html, /<kbd>Shift\+F9<\/kbd> Focus hide first uncovered leftover buyer, or the buyer list if missing/u);
