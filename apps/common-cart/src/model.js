...
export const presets = Object.freeze({
  ...
  swimmingCarnivalLunch: {
    title: "Swimming carnival lunch",
    currency: "AUD",
    buyers: [
      buyer("B01", "Lane crate", "Carnival lunch pack", 8, 16, 4, ["Carnival pie", "Lane salad"]),
      buyer("B02", "Dive bench", "Carnival lunch pack", 12, 12, 3, ["Carnival pie"]),
      buyer("B03", "Marshal hamper", "Carnival lunch pack", 6, 14, 5, ["Lane salad", "Pool water"]),
      buyer("B04", "Pool cooler", "Carnival lunch pack", 5, 10, 2, ["Pool water"]),
      buyer("B05", "Grandstand trolley", "Carnival lunch pack", 9, 15, 4, ["Carnival pie", "Pool water"]),
      buyer("B06", "Timekeeper table", "Carnival lunch pack", 7, 13, 3, ["Lane salad", "Carnival pie"])
    ],
    offers: [
      offer("O01", "Pool-deck Carnival Delivery", "Carnival lunch pack", "Carnival pie", 10, 10, 3, 40, 3),
      offer("O02", "Lane Salad Run", "Carnival lunch pack", "Lane salad", 8, 8, 4, 24, 2),
      { ...offer("O03", "Hall Carnival Pickup", "Carnival lunch pack", "Pool water", 6, 8, 1, 30, 5), fulfillment: "pickup" }
    ]
  },
  athleticsCarnivalLunch: {
    title: "Athletics carnival lunch",
    currency: "AUD",
    buyers: [
      buyer("B01", "Track crate", "Athletics lunch pack", 8, 16, 4, ["Athletics pie", "Track salad"]),
      buyer("B02", "Jump bench", "Athletics lunch pack", 12, 12, 3, ["Athletics pie"]),
      buyer("B03", "Marshal hamper", "Athletics lunch pack", 6, 14, 5, ["Track salad", "Track water"]),
      buyer("B04", "Track cooler", "Athletics lunch pack", 5, 10, 2, ["Track water"]),
      buyer("B05", "Grandstand trolley", "Athletics lunch pack", 9, 15, 4, ["Athletics pie", "Track water"]),
      buyer("B06", "Timekeeper table", "Athletics lunch pack", 7, 13, 3, ["Track salad", "Athletics pie"])
    ],
    offers: [
      offer("O01", "Track-side Athletics Delivery", "Athletics lunch pack", "Athletics pie", 10, 10, 3, 40, 3),
      offer("O02", "Track Salad Run", "Athletics lunch pack", "Track salad", 8, 8, 4, 24, 2),
      { ...offer("O03", "Hall Athletics Pickup", "Athletics lunch pack", "Track water", 6, 8, 1, 30, 5), fulfillment: "pickup" }
    ]
  },
  cricketCarnivalLunch: {
    title: "Cricket carnival lunch",
    currency: "AUD",
    buyers: [
      buyer("B01", "Pitch crate", "Cricket lunch pack", 9, 17, 4, ["Cricket pie", "Pitch salad"]),
      buyer("B02", "Crease bench", "Cricket lunch pack", 13, 13, 3, ["Cricket pie"]),
      buyer("B03", "Umpire hamper", "Cricket lunch pack", 7, 15, 5, ["Pitch salad", "Pitch water"]),
      buyer("B04", "Boundary cooler", "Cricket lunch pack", 6, 11, 2, ["Pitch water"]),
      buyer("B05", "Pavilion trolley", "Cricket lunch pack", 10, 16, 4, ["Cricket pie", "Pitch water"]),
      buyer("B06", "Scorer table", "Cricket lunch pack", 8, 14, 3, ["Pitch salad", "Cricket pie"])
    ],
    offers: [
      offer("O01", "Pitch-side Cricket Delivery", "Cricket lunch pack", "Cricket pie", 11, 11, 3, 42, 3),
  ...
  volleyballCarnivalLunch: {
    title: "Volleyball carnival lunch",
    currency: "AUD",
    buyers: [
      buyer("B01", "Court crate", "Volleyball lunch pack", 12, 20, 4, ["Volleyball pie", "Sideline salad"]),
      buyer("B02", "Net bench", "Volleyball lunch pack", 16, 16, 3, ["Volleyball pie"]),
      buyer("B03", "Referee hamper", "Volleyball lunch pack", 10, 18, 5, ["Sideline salad", "Sideline water"]),
      buyer("B04", "Sideline cooler", "Volleyball lunch pack", 7, 14, 2, ["Sideline water"]),
      buyer("B05", "Scoreboard trolley", "Volleyball lunch pack", 13, 19, 4, ["Volleyball pie", "Sideline water"]),
      buyer("B06", "Bench table", "Volleyball lunch pack", 11, 17, 3, ["Sideline salad", "Volleyball pie"])
    ],
    offers: [
      offer("O01", "Court-side Volleyball Delivery", "Volleyball lunch pack", "Volleyball pie", 14, 14, 3, 48, 3),
      offer("O02", "Sideline Salad Run", "Volleyball lunch pack", "Sideline salad", 12, 12, 4, 34, 2),
      { ...offer("O03", "Hall Volleyball Pickup", "Volleyball lunch pack", "Sideline water", 10, 10, 1, 38, 5), fulfillment: "pickup" }
    ]
  },
  soccerCarnivalLunch: {
    title: "Soccer carnival lunch",
    currency: "AUD",
    buyers: [
      buyer("B01", "Pitch crate", "Soccer lunch pack", 13, 21, 4, ["Soccer pie", "Pitch salad"]),
      buyer("B02", "Goal bench", "Soccer lunch pack", 17, 17, 3, ["Soccer pie"]),
      buyer("B03", "Referee hamper", "Soccer lunch pack", 11, 19, 5, ["Pitch salad", "Pitch water"]),
      buyer("B04", "Sideline cooler", "Soccer lunch pack", 8, 15, 2, ["Pitch water"]),
      buyer("B05", "Scoreboard trolley", "Soccer lunch pack", 14, 20, 4, ["Soccer pie", "Pitch water"]),
      buyer("B06", "Bench table", "Soccer lunch pack", 12, 18, 3, ["Pitch salad", "Soccer pie"])
    ],
    offers: [
      offer("O01", "Court-side Soccer Delivery", "Soccer lunch pack", "Soccer pie", 15, 15, 3, 50, 3),
      offer("O02", "Pitch Salad Run", "Soccer lunch pack", "Pitch salad", 13, 13, 4, 38, 2),
      { ...offer("O03", "Hall Soccer Pickup", "Soccer lunch pack", "Pitch water", 11, 11, 1, 40, 5), fulfillment: "pickup" }
    ]
  },
  rugbyCarnivalLunch: {
    title: "Rugby carnival lunch",
    currency: "AUD",
    buyers: [
      buyer("B01", "Scrum crate", "Rugby lunch pack", 14, 22, 4, ["Rugby pie", "Tryline salad"]),
      buyer("B02", "Touch bench", "Rugby lunch pack", 18, 18, 3, ["Rugby pie"]),
      buyer("B03", "Referee hamper", "Rugby lunch pack", 12, 20, 5, ["Tryline salad", "Tryline water"]),
      buyer("B04", "Sideline cooler", "Rugby lunch pack", 9, 16, 2, ["Tryline water"]),
      buyer("B05", "Scoreboard trolley", "Rugby lunch pack", 15, 21, 4, ["Rugby pie", "Tryline water"]),
      buyer("B06", "Bench table", "Rugby lunch pack", 13, 19, 3, ["Tryline salad", "Rugby pie"])
    ],
    offers: [
      offer("O01", "Court-side Rugby Delivery", "Rugby lunch pack", "Rugby pie", 16, 16, 3, 52, 3),
      offer("O02", "Tryline Salad Run", "Rugby lunch pack", "Tryline salad", 14, 14, 4, 40, 2),
      { ...offer("O03", "Hall Rugby Pickup", "Rugby lunch pack", "Tryline water", 12, 12, 1, 42, 5), fulfillment: "pickup" }
    ]
  },
  hockeyCarnivalLunch: {
    title: "Hockey carnival lunch",
    currency: "AUD",
    buyers: [
      buyer("B01", "Boards crate", "Hockey lunch pack", 15, 23, 4, ["Hockey pie", "Boards salad"]),
      buyer("B02", "Goal bench", "Hockey lunch pack", 19, 19, 3, ["Hockey pie"]),
      buyer("B03", "Umpire hamper", "Hockey lunch pack", 13, 21, 5, ["Boards salad", "Boards water"]),
      buyer("B04", "Sideline cooler", "Hockey lunch pack", 10, 17, 2, ["Boards water"]),
      buyer("B05", "Scoreboard trolley", "Hockey lunch pack", 16, 22, 4, ["Hockey pie", "Boards water"]),
      buyer("B06", "Bench table", "Hockey lunch pack", 14, 20, 3, ["Boards salad", "Hockey pie"])
    ],
    offers: [
      offer("O01", "Court-side Hockey Delivery", "Hockey lunch pack", "Hockey pie", 17, 17, 3, 54, 3),
      offer("O02", "Boards Salad Run", "Hockey lunch pack", "Boards salad", 15, 15, 4, 42, 2),
      { ...offer("O03", "Hall Hockey Pickup", "Hockey lunch pack", "Boards water", 13, 13, 1, 44, 5), fulfillment: "pickup" }
    ]
  },
  baseballCarnivalLunch: {
    title: "Baseball carnival lunch",
    currency: "AUD",
    buyers: [
      buyer("B01", "Dugout crate", "Baseball lunch pack", 15, 23, 4, ["Baseball pie", "Boards salad"]),
      buyer("B02", "Mound bench", "Baseball lunch pack", 19, 19, 3, ["Baseball pie"]),
      buyer("B03", "Umpire hamper", "Baseball lunch pack", 13, 21, 5, ["Boards salad", "Boards water"]),
      buyer("B04", "Sideline cooler", "Baseball lunch pack", 10, 17, 2, ["Boards water"]),
      buyer("B05", "Scoreboard trolley", "Baseball lunch pack", 16, 22, 4, ["Baseball pie", "Boards water"]),
      buyer("B06", "Bench table", "Baseball lunch pack", 14, 20, 3, ["Boards salad", "Baseball pie"])
    ],
    offers: [
      offer("O01", "Court-side Baseball Delivery", "Baseball lunch pack", "Baseball pie", 17, 17, 3, 54, 3),
  ...
  waterPoloCarnivalLunch: {
    title: "Water polo carnival lunch",
    currency: "AUD",
    buyers: [
      buyer("B01", "Pool crate", "Water polo lunch pack", 15, 23, 4, ["Water polo pie", "Pool salad"]),
      buyer("B02", "Goal bench", "Water polo lunch pack", 19, 19, 3, ["Water polo pie"]),
      buyer("B03", "Referee hamper", "Water polo lunch pack", 13, 21, 5, ["Pool salad", "Pool water"]),
      buyer("B04", "Sideline cooler", "Water polo lunch pack", 10, 17, 2, ["Pool water"]),
      buyer("B05", "Scoreboard trolley", "Water polo lunch pack", 16, 22, 4, ["Water polo pie", "Pool water"]),
      buyer("B06", "Bench table", "Water polo lunch pack", 14, 20, 3, ["Pool salad", "Water polo pie"])
    ],
    offers: [
      offer("O01", "Court-side Water Polo Delivery", "Water polo lunch pack", "Water polo pie", 17, 17, 3, 54, 3),
      offer("O02", "Pool Salad Run", "Water polo lunch pack", "Pool salad", 15, 15, 4, 42, 2),
      { ...offer("O03", "Hall Water Polo Pickup", "Water polo lunch pack", "Pool water", 16, 16, 1, 44, 5), fulfillment: "pickup" }
    ]
  },
  rowingCarnivalLunch: {
    title: "Rowing carnival lunch",
    currency: "AUD",
    buyers: [
      buyer("B01", "Dock crate", "Rowing lunch pack", 15, 23, 4, ["Rowing pie", "Pontoon salad"]),
      buyer("B02", "Stroke bench", "Rowing lunch pack", 19, 19, 3, ["Rowing pie"]),
      buyer("B03", "Coxswain hamper", "Rowing lunch pack", 13, 21, 5, ["Pontoon salad", "Pontoon water"]),
      buyer("B04", "Sideline cooler", "Rowing lunch pack", 10, 17, 2, ["Pontoon water"]),
      buyer("B05", "Scoreboard trolley", "Rowing lunch pack", 16, 22, 4, ["Rowing pie", "Pontoon water"]),
      buyer("B06", "Bench table", "Rowing lunch pack", 14, 20, 3, ["Pontoon salad", "Rowing pie"])
    ],
    offers: [
      offer("O01", "Court-side Rowing Delivery", "Rowing lunch pack", "Rowing pie", 17, 17, 3, 54, 3),
      offer("O02", "Pontoon Salad Run", "Rowing lunch pack", "Pontoon salad", 15, 15, 4, 42, 2),
      { ...offer("O03", "Hall Rowing Pickup", "Rowing lunch pack", "Pontoon water", 17, 17, 1, 44, 5), fulfillment: "pickup" }
    ]
  },
  sailingCarnivalLunch: {
    title: "Sailing carnival lunch",
    currency: "AUD",
    buyers: [
      buyer("B01", "Jetty crate", "Sailing lunch pack", 15, 23, 4, ["Sailing pie", "Marina salad"]),
      buyer("B02", "Helm bench", "Sailing lunch pack", 19, 19, 3, ["Sailing pie"]),
      buyer("B03", "Skipper hamper", "Sailing lunch pack", 13, 21, 5, ["Marina salad", "Marina water"]),
      buyer("B04", "Sideline cooler", "Sailing lunch pack", 10, 17, 2, ["Marina water"]),
      buyer("B05", "Scoreboard trolley", "Sailing lunch pack", 16, 22, 4, ["Sailing pie", "Marina water"]),
      buyer("B06", "Bench table", "Sailing lunch pack", 14, 20, 3, ["Marina salad", "Sailing pie"])
    ],
    offers: [
      offer("O01", "Court-side Sailing Delivery", "Sailing lunch pack", "Sailing pie", 17, 17, 3, 54, 3),
  ...
  kayakingCarnivalLunch: {
    title: "Kayaking carnival lunch",
    currency: "AUD",
    buyers: [
      buyer("B01", "Kayak-rack crate", "Kayaking lunch pack", 15, 23, 4, ["Kayaking pie", "Boathouse salad"]),
      buyer("B02", "Spraydeck bench", "Kayaking lunch pack", 19, 19, 3, ["Kayaking pie"]),
      buyer("B03", "Bow hamper", "Kayaking lunch pack", 13, 21, 5, ["Boathouse salad", "Boathouse water"]),
      buyer("B04", "Sideline cooler", "Kayaking lunch pack", 10, 17, 2, ["Boathouse water"]),
      buyer("B05", "Scoreboard trolley", "Kayaking lunch pack", 16, 22, 4, ["Kayaking pie", "Boathouse water"]),
      buyer("B06", "Bench table", "Kayaking lunch pack", 14, 20, 3, ["Boathouse salad", "Kayaking pie"])
    ],
    offers: [
      offer("O01", "Court-side Kayaking Delivery", "Kayaking lunch pack", "Kayaking pie", 17, 17, 3, 54, 3),
  ...
  cyclingCarnivalLunch: {
    title: "Cycling carnival lunch",
    currency: "AUD",
    buyers: [
      buyer("B01", "Cycle-rack crate", "Cycling lunch pack", 15, 23, 4, ["Cycle pie", "Velodrome salad"]),
      buyer("B02", "Board-bag bench", "Cycling lunch pack", 19, 19, 3, ["Cycle pie"]),
      buyer("B03", "Break hamper", "Cycling lunch pack", 15, 21, 5, ["Velodrome salad", "Velodrome water"]),
      buyer("B04", "Sideline cooler", "Cycling lunch pack", 10, 19, 2, ["Velodrome water"]),
      buyer("B05", "Scoreboard trolley", "Cycling lunch pack", 16, 22, 4, ["Cycle pie", "Velodrome water"]),
      buyer("B06", "Bench table", "Cycling lunch pack", 14, 20, 3, ["Velodrome salad", "Cycle pie"])
    ],
    offers: [
      offer("O01", "Court-side Cycling Delivery", "Cycling lunch pack", "Cycle pie", 17, 17, 3, 54, 3),
      offer("O02", "Velodrome Salad Run", "Cycling lunch pack", "Velodrome salad", 15, 15, 4, 42, 2),
      { ...offer("O03", "Hall Cycling Pickup", "Cycling lunch pack", "Velodrome water", 19, 24, 1, 48, 5), fulfillment: "pickup" }
    ]
  },
  mountainBikeCarnivalLunch: {
    title: "Mountain bike carnival lunch",
    currency: "AUD",
    buyers: [
      buyer("B01", "Trail-rack crate", "Mountain bike lunch pack", 15, 23, 4, ["Trail pie", "Trail salad"]),
      buyer("B02", "Board-bag bench", "Mountain bike lunch pack", 19, 19, 3, ["Trail pie"]),
      buyer("B03", "Break hamper", "Mountain bike lunch pack", 15, 21, 5, ["Trail salad", "Trail water"]),
      buyer("B04", "Sideline cooler", "Mountain bike lunch pack", 11, 20, 2, ["Trail water"]),
      buyer("B05", "Scoreboard trolley", "Mountain bike lunch pack", 16, 22, 4, ["Trail pie", "Trail water"]),
      buyer("B06", "Bench table", "Mountain bike lunch pack", 14, 20, 3, ["Trail salad", "Trail pie"])
    ],
    offers: [
      offer("O01", "Court-side Mountain Bike Delivery", "Mountain bike lunch pack", "Trail pie", 17, 17, 3, 54, 3),
...
export function validateWorkspace(candidate) {
  ...
  const hideBuyersFilledByLeftoverFill = own(candidate, "hideBuyersFilledByLeftoverFill");
  let hideLeftoverFillBuyers = false;
  if (hideBuyersFilledByLeftoverFill !== undefined) {
    if (hideBuyersFilledByLeftoverFill !== true && hideBuyersFilledByLeftoverFill !== false) {
      throw new ScenarioError("Hide leftover-fill buyers must be true or false.");
    }
  ...
  const hideLastBuyerFilledByTertiaryFill = own(candidate, "hideLastBuyerFilledByTertiaryFill");
  let hideLastTertiaryFillBuyer = false;
  if (hideLastBuyerFilledByTertiaryFill !== undefined) {
    if (hideLastBuyerFilledByTertiaryFill !== true && hideLastBuyerFilledByTertiaryFill !== false) {
      throw new ScenarioError("Hide last tertiary-fill buyer must be true or false.");
    }
    hideLastTertiaryFillBuyer = hideLastBuyerFilledByTertiaryFill;
  }
  const hideLastUnservedBuyer = own(candidate, "hideLastUnservedBuyer");
  let hideLastUnserved = false;
  if (hideLastUnservedBuyer !== undefined) {
    if (hideLastUnservedBuyer !== true && hideLastUnservedBuyer !== false) {
      throw new ScenarioError("Hide last unserved buyer must be true or false.");
    }
    hideLastUnserved = hideLastUnservedBuyer;
  }
  const hideFirstUnservedBuyer = own(candidate, "hideFirstUnservedBuyer");
  let hideFirstUnserved = false;
  if (hideFirstUnservedBuyer !== undefined) {
    if (hideFirstUnservedBuyer !== true && hideFirstUnservedBuyer !== false) {
      throw new ScenarioError("Hide first unserved buyer must be true or false.");
    }
    hideFirstUnserved = hideFirstUnservedBuyer;
  }
  const hideLastLeftoverOnlyBuyer = own(candidate, "hideLastLeftoverOnlyBuyer");
  let hideLastLeftoverOnly = false;
  if (hideLastLeftoverOnlyBuyer !== undefined) {
    if (hideLastLeftoverOnlyBuyer !== true && hideLastLeftoverOnlyBuyer !== false) {
      throw new ScenarioError("Hide last leftover-only buyer must be true or false.");
    }
    hideLastLeftoverOnly = hideLastLeftoverOnlyBuyer;
  }
  const hideFirstLeftoverOnlyBuyer = own(candidate, "hideFirstLeftoverOnlyBuyer");
  let hideFirstLeftoverOnly = false;
  if (hideFirstLeftoverOnlyBuyer !== undefined) {
    if (hideFirstLeftoverOnlyBuyer !== true && hideFirstLeftoverOnlyBuyer !== false) {
      throw new ScenarioError("Hide first leftover-only buyer must be true or false.");
    }
    hideFirstLeftoverOnly = hideFirstLeftoverOnlyBuyer;
  }
  const hideLastWinnerAllocatedBuyer = own(candidate, "hideLastWinnerAllocatedBuyer");
  let hideLastWinnerAllocated = false;
  if (hideLastWinnerAllocatedBuyer !== undefined) {
    if (hideLastWinnerAllocatedBuyer !== true && hideLastWinnerAllocatedBuyer !== false) {
      throw new ScenarioError("Hide last winner-allocated buyer must be true or false.");
    }
    hideLastWinnerAllocated = hideLastWinnerAllocatedBuyer;
  }
  const hideFirstWinnerAllocatedBuyer = own(candidate, "hideFirstWinnerAllocatedBuyer");
  let hideFirstWinnerAllocated = false;
  if (hideFirstWinnerAllocatedBuyer !== undefined) {
    if (hideFirstWinnerAllocatedBuyer !== true && hideFirstWinnerAllocatedBuyer !== false) {
      throw new ScenarioError("Hide first winner-allocated buyer must be true or false.");
    }
    hideFirstWinnerAllocated = hideFirstWinnerAllocatedBuyer;
  }
  const hideFirstUncoveredLeftoverBuyer = own(candidate, "hideFirstUncoveredLeftoverBuyer");
  let hideFirstUncoveredLeftover = false;
  if (hideFirstUncoveredLeftoverBuyer !== undefined) {
    if (hideFirstUncoveredLeftoverBuyer !== true && hideFirstUncoveredLeftoverBuyer !== false) {
      throw new ScenarioError("Hide first uncovered leftover buyer must be true or false.");
    }
    hideFirstUncoveredLeftover = hideFirstUncoveredLeftoverBuyer;
  }
  const hideLastUncoveredLeftoverBuyer = own(candidate, "hideLastUncoveredLeftoverBuyer");
  let hideLastUncoveredLeftover = false;
  if (hideLastUncoveredLeftoverBuyer !== undefined) {
    if (hideLastUncoveredLeftoverBuyer !== true && hideLastUncoveredLeftoverBuyer !== false) {
      throw new ScenarioError("Hide last uncovered leftover buyer must be true or false.");
    }
    hideLastUncoveredLeftover = hideLastUncoveredLeftoverBuyer;
  }
  return { version: 1, rooms: candidate.rooms.map(validateScenario), fulfillmentFilter: filter, hideExcludedBuyers: hideExcluded, hideUnwinnableOffers: hideUnwinnable, hideCoveredLeftoverRows: hideCoveredLeftover, hideTertiaryLeftoverRow: hideTertiaryLeftover, hideLeftoverFillRow: hideLeftoverFill, hideZeroRemainingCapacityOffers: hideZeroRemaining, hideOffersWithRemainingCapacity: hideRemainingCapacity, hideFullyFilledBuyers: hideFullyFilled, hideBuyersWithLeftover: hideLeftoverBuyers, hideUnservedBuyers: hideUnserved, hideLeftoverOnlyBuyers: hideLeftoverOnly, hideWinnerAllocatedBuyers: hideWinnerAllocated, hideBuyersFilledByLeftoverFill: hideLeftoverFillBuyers, hideLastBuyerFilledByLeftoverFill: hideLastLeftoverFillBuyer, hideFirstBuyerFilledByLeftoverFill: hideFirstLeftoverFillBuyer, hideFirstBuyerFilledByTertiaryFill: hideFirstTertiaryFillBuyer, hideLastBuyerFilledByTertiaryFill: hideLastTertiaryFillBuyer, hideLastUnservedBuyer: hideLastUnserved, hideFirstUnservedBuyer: hideFirstUnserved, hideLastLeftoverOnlyBuyer: hideLastLeftoverOnly, hideFirstLeftoverOnlyBuyer: hideFirstLeftoverOnly, hideLastWinnerAllocatedBuyer: hideLastWinnerAllocated, hideFirstWinnerAllocatedBuyer: hideFirstWinnerAllocated, hideFirstUncoveredLeftoverBuyer: hideFirstUncoveredLeftover, hideLastUncoveredLeftoverBuyer: hideLastUncoveredLeftover };
}
...
/** Display-only. Matching is unchanged. Hides organizer buyer rows in leftover-fill selectedBuyerIds. Winner-allocated and unserved buyers stay visible. */
export function filterBuyerIdsHidingBuyersFilledByLeftoverFill(rawScenario, hideBuyersFilledByLeftoverFill) {
  if (hideBuyersFilledByLeftoverFill !== true && hideBuyersFilledByLeftoverFill !== false) {
    throw new ScenarioError("Hide leftover-fill buyers must be true or false.");
  }
  const scenario = validateScenario(rawScenario);
  if (!hideBuyersFilledByLeftoverFill) return scenario.buyers.map((buyer) => buyer.id);
  const market = evaluateMarket(scenario);
  const coverage = computeResidualCoverage(scenario);
  const winnerIds = new Set(market.winner?.selectedBuyerIds ?? []);
  const leftoverFillIds = new Set(coverage.secondary?.selectedBuyerIds ?? []);
...
/** Display-only. Matching is unchanged. Hides only the last leftover-only buyer. Winner-allocated, unserved, leftover-fill, tertiary-fill, and other leftover-only buyers stay visible. Unserved buyers are not leftover-only. */
export function filterBuyerIdsHidingLastLeftoverOnlyBuyer(rawScenario, hideLastLeftoverOnlyBuyer) {
  if (hideLastLeftoverOnlyBuyer !== true && hideLastLeftoverOnlyBuyer !== false) {
    throw new ScenarioError("Hide last leftover-only buyer must be true or false.");
  }
  const scenario = validateScenario(rawScenario);
  if (!hideLastLeftoverOnlyBuyer) return scenario.buyers.map((buyer) => buyer.id);
  const market = evaluateMarket(scenario);
  const coverage = computeResidualCoverage(scenario);
  const winnerIds = new Set(market.winner?.selectedBuyerIds ?? []);
  const leftoverFillIds = new Set(coverage.secondary?.selectedBuyerIds ?? []);
  const tertiaryIds = new Set(coverage.tertiary?.selectedBuyerIds ?? []);
  let lastLeftoverOnlyId = null;
  for (const buyer of scenario.buyers) {
    const leftoverOnly = !winnerIds.has(buyer.id) && (leftoverFillIds.has(buyer.id) || tertiaryIds.has(buyer.id));
    if (leftoverOnly) lastLeftoverOnlyId = buyer.id;
  }
  return scenario.buyers.filter((buyer) => buyer.id !== lastLeftoverOnlyId).map((buyer) => buyer.id);
}

/** Display-only. Matching is unchanged. Hides only the first leftover-only buyer. Winner-allocated, unserved, leftover-fill, tertiary-fill, and other leftover-only buyers stay visible. Unserved buyers are not leftover-only. */
export function filterBuyerIdsHidingFirstLeftoverOnlyBuyer(rawScenario, hideFirstLeftoverOnlyBuyer) {
  if (hideFirstLeftoverOnlyBuyer !== true && hideFirstLeftoverOnlyBuyer !== false) {
    throw new ScenarioError("Hide first leftover-only buyer must be true or false.");
  }
  const scenario = validateScenario(rawScenario);
  if (!hideFirstLeftoverOnlyBuyer) return scenario.buyers.map((buyer) => buyer.id);
  const market = evaluateMarket(scenario);
  const coverage = computeResidualCoverage(scenario);
  const winnerIds = new Set(market.winner?.selectedBuyerIds ?? []);
  const leftoverFillIds = new Set(coverage.secondary?.selectedBuyerIds ?? []);
  const tertiaryIds = new Set(coverage.tertiary?.selectedBuyerIds ?? []);
  let firstLeftoverOnlyId = null;
  for (const buyer of scenario.buyers) {
    const leftoverOnly = !winnerIds.has(buyer.id) && (leftoverFillIds.has(buyer.id) || tertiaryIds.has(buyer.id));
    if (leftoverOnly) {
      firstLeftoverOnlyId = buyer.id;
      break;
    }
  }
  return scenario.buyers.filter((buyer) => buyer.id !== firstLeftoverOnlyId).map((buyer) => buyer.id);
}

/** Display-only. Matching is unchanged. Hides only the last winner-allocated buyer. Leftover-only, unserved, leftover-fill, tertiary-fill, and other winner-allocated buyers stay visible. */
export function filterBuyerIdsHidingLastWinnerAllocatedBuyer(rawScenario, hideLastWinnerAllocatedBuyer) {
  if (hideLastWinnerAllocatedBuyer !== true && hideLastWinnerAllocatedBuyer !== false) {
    throw new ScenarioError("Hide last winner-allocated buyer must be true or false.");
  }
  const scenario = validateScenario(rawScenario);
  if (!hideLastWinnerAllocatedBuyer) return scenario.buyers.map((buyer) => buyer.id);
  const market = evaluateMarket(scenario);
  const winnerIds = new Set(market.winner?.selectedBuyerIds ?? []);
  let lastWinnerAllocatedId = null;
  for (const buyer of scenario.buyers) {
    if (winnerIds.has(buyer.id)) lastWinnerAllocatedId = buyer.id;
  }
  return scenario.buyers.filter((buyer) => buyer.id !== lastWinnerAllocatedId).map((buyer) => buyer.id);
}

/** Display-only. Matching is unchanged. Hides only the first winner-allocated buyer. Leftover-only, unserved, leftover-fill, tertiary-fill, and other winner-allocated buyers stay visible. */
export function filterBuyerIdsHidingFirstWinnerAllocatedBuyer(rawScenario, hideFirstWinnerAllocatedBuyer) {
  if (hideFirstWinnerAllocatedBuyer !== true && hideFirstWinnerAllocatedBuyer !== false) {
    throw new ScenarioError("Hide first winner-allocated buyer must be true or false.");
  }
  const scenario = validateScenario(rawScenario);
  if (!hideFirstWinnerAllocatedBuyer) return scenario.buyers.map((buyer) => buyer.id);
  const market = evaluateMarket(scenario);
  const winnerIds = new Set(market.winner?.selectedBuyerIds ?? []);
  let firstWinnerAllocatedId = null;
  for (const buyer of scenario.buyers) {
    if (winnerIds.has(buyer.id)) {
      firstWinnerAllocatedId = buyer.id;
      break;
    }
  }
  return scenario.buyers.filter((buyer) => buyer.id !== firstWinnerAllocatedId).map((buyer) => buyer.id);
}

/** Display-only. Matching is unchanged. Hides only the first uncovered leftover buyer. Uncovered leftover buyers are leftover-after-winner buyers still unfilled after leftover fill and tertiary fill. Leftover-only, winner-allocated, leftover-fill, tertiary-fill, and other leftover buyers stay visible. */
export function filterBuyerIdsHidingFirstUncoveredLeftoverBuyer(rawScenario, hideFirstUncoveredLeftoverBuyer) {
  if (hideFirstUncoveredLeftoverBuyer !== true && hideFirstUncoveredLeftoverBuyer !== false) {
    throw new ScenarioError("Hide first uncovered leftover buyer must be true or false.");
  }
  const scenario = validateScenario(rawScenario);
  if (!hideFirstUncoveredLeftoverBuyer) return scenario.buyers.map((buyer) => buyer.id);
  const coverage = computeResidualCoverage(scenario);
  const leftoverIds = new Set(coverage.leftoverBuyerIds);
  const filled = new Set([
    ...(coverage.secondary?.selectedBuyerIds ?? []),
    ...(coverage.tertiary?.selectedBuyerIds ?? [])
  ]);
  let firstUncoveredLeftoverId = null;
  for (const buyer of scenario.buyers) {
    if (leftoverIds.has(buyer.id) && !filled.has(buyer.id)) {
      firstUncoveredLeftoverId = buyer.id;
      break;
    }
  }
  return scenario.buyers.filter((buyer) => buyer.id !== firstUncoveredLeftoverId).map((buyer) => buyer.id);
}

/** Display-only. Matching is unchanged. Hides only the last uncovered leftover buyer. Uncovered leftover buyers are leftover-after-winner buyers still unfilled after leftover fill and tertiary fill. Leftover-only, winner-allocated, leftover-fill, tertiary-fill, first uncovered leftover, and other leftover buyers stay visible. */
export function filterBuyerIdsHidingLastUncoveredLeftoverBuyer(rawScenario, hideLastUncoveredLeftoverBuyer) {
  if (hideLastUncoveredLeftoverBuyer !== true && hideLastUncoveredLeftoverBuyer !== false) {
    throw new ScenarioError("Hide last uncovered leftover buyer must be true or false.");
  }
  const scenario = validateScenario(rawScenario);
  if (!hideLastUncoveredLeftoverBuyer) return scenario.buyers.map((buyer) => buyer.id);
  const coverage = computeResidualCoverage(scenario);
  const leftoverIds = new Set(coverage.leftoverBuyerIds);
  const filled = new Set([
    ...(coverage.secondary?.selectedBuyerIds ?? []),
  ...
  let lastUncoveredLeftoverId = null;
  for (const buyer of scenario.buyers) {
    if (leftoverIds.has(buyer.id) && !filled.has(buyer.id)) {
      lastUncoveredLeftoverId = buyer.id;
    }
  }
  return scenario.buyers.filter((buyer) => buyer.id !== lastUncoveredLeftoverId).map((buyer) => buyer.id);
}

/** Display-only. Matching is unchanged. Hides only the first buyer with no winner, leftover-fill, or tertiary units. Winner-allocated, leftover-fill, tertiary-fill, and other unserved buyers stay visible. */
export function filterBuyerIdsHidingFirstUnservedBuyer(rawScenario, hideFirstUnservedBuyer) {
  if (hideFirstUnservedBuyer !== true && hideFirstUnservedBuyer !== false) {
    throw new ScenarioError("Hide first unserved buyer must be true or false.");
  }
  const scenario = validateScenario(rawScenario);
  if (!hideFirstUnservedBuyer) return scenario.buyers.map((buyer) => buyer.id);
  const market = evaluateMarket(scenario);
  const coverage = computeResidualCoverage(scenario);
  const served = new Set(market.winner?.selectedBuyerIds ?? []);
  for (const id of coverage.secondary?.selectedBuyerIds ?? []) served.add(id);
  for (const id of coverage.tertiary?.selectedBuyerIds ?? []) served.add(id);
  let firstUnservedId = null;
  for (const buyer of scenario.buyers) {
    if (!served.has(buyer.id)) {
      firstUnservedId = buyer.id;
      break;
    }
  }
  return scenario.buyers.filter((buyer) => buyer.id !== firstUnservedId).map((buyer) => buyer.id);
}

/** Display-only. Matching is unchanged. Hides only the last buyer with no winner, leftover-fill, or tertiary units. Winner-allocated, leftover-fill, tertiary-fill, and other unserved buyers stay visible. */
export function filterBuyerIdsHidingLastUnservedBuyer(rawScenario, hideLastUnservedBuyer) {
  if (hideLastUnservedBuyer !== true && hideLastUnservedBuyer !== false) {
    throw new ScenarioError("Hide last unserved buyer must be true or false.");
  }
  const scenario = validateScenario(rawScenario);
  if (!hideLastUnservedBuyer) return scenario.buyers.map((buyer) => buyer.id);
  const market = evaluateMarket(scenario);
  const coverage = computeResidualCoverage(scenario);
  const served = new Set(market.winner?.selectedBuyerIds ?? []);
  for (const id of coverage.secondary?.selectedBuyerIds ?? []) served.add(id);
  for (const id of coverage.tertiary?.selectedBuyerIds ?? []) served.add(id);
  let lastUnservedId = null;
  for (const buyer of scenario.buyers) {
    if (!served.has(buyer.id)) lastUnservedId = buyer.id;
  }
  return scenario.buyers.filter((buyer) => buyer.id !== lastUnservedId).map((buyer) => buyer.id);
}

/** Display-only. Matching is unchanged. Hides only the last tertiary-fill selectedBuyerIds entry. Winner-allocated, leftover-fill, unserved, and other tertiary-fill buyers stay visible. */
export function filterBuyerIdsHidingLastBuyerFilledByTertiaryFill(rawScenario, hideLastBuyerFilledByTertiaryFill) {
  if (hideLastBuyerFilledByTertiaryFill !== true && hideLastBuyerFilledByTertiaryFill !== false) {
    throw new ScenarioError("Hide last tertiary-fill buyer must be true or false.");
  }
  const scenario = validateScenario(rawScenario);
  if (!hideLastBuyerFilledByTertiaryFill) return scenario.buyers.map((buyer) => buyer.id);
  const market = evaluateMarket(scenario);
  const coverage = computeResidualCoverage(scenario);
  const tertiaryIds = coverage.tertiary?.selectedBuyerIds ?? [];
  const lastId = tertiaryIds.length > 0 ? tertiaryIds[tertiaryIds.length - 1] : null;
  const winnerIds = new Set(market.winner?.selectedBuyerIds ?? []);
  return scenario.buyers.filter((buyer) => {
    if (winnerIds.has(buyer.id)) return true;
    if (lastId && buyer.id === lastId) return false;
...
export function createLeftoverFillRemainingCapacityMarkdown(rawScenario) {
  const scenario = validateScenario(rawScenario);
  const coverage = computeResidualCoverage(scenario);
  if (!coverage.secondary) {
    return "Common Cart leftover fill remaining capacity (organizer private): none. Not a merchant export.\n";
  }
  const leftoverOffer = scenario.offers.find((offer) => offer.id === coverage.secondary.offerId);
  const remaining = leftoverOffer ? Math.max(0, leftoverOffer.capacity - coverage.secondary.fulfilledUnits) : 0;
...
export function createLeftoverFillPickupMarkdown(rawScenario) {
  const scenario = validateScenario(rawScenario);
  const coverage = computeResidualCoverage(scenario);
  if (!coverage.secondary) {
    return "Common Cart leftover fill pickup (organizer private): none. Not a merchant export.\n";
  }
  const leftoverOffer = scenario.offers.find((offer) => offer.id === coverage.secondary.offerId);
  if (!leftoverOffer || leftoverOffer.fulfillment !== "pickup") {
    return "Common Cart leftover fill pickup (organizer private): none. Not a merchant export.\n";
  }
...
export function createLeftoverFillMaximumMarkdown(rawScenario) {
  const scenario = validateScenario(rawScenario);
  const coverage = computeResidualCoverage(scenario);
  if (!coverage.secondary) {
    return "Common Cart leftover fill maximum (organizer private): none. Not a merchant export.\n";
  }
  const leftoverOffer = scenario.offers.find((offer) => offer.id === coverage.secondary.offerId);
  const maximum = leftoverOffer ? leftoverOffer.capacity : 0;
  return `Common Cart leftover fill maximum (organizer private): ${maximum}. Not a merchant export.\n`;
}

/** Organizer-private one-line tertiary fill offer capacity. Count only. Not a merchant export. */
export function createTertiaryFillMaximumMarkdown(rawScenario) {
  const scenario = validateScenario(rawScenario);
  const coverage = computeResidualCoverage(scenario);
  if (!coverage.tertiary) {
    return "Common Cart tertiary fill maximum (organizer private): none. Not a merchant export.\n";
  }
  const tertiaryOffer = scenario.offers.find((offer) => offer.id === coverage.tertiary.offerId);
  const maximum = tertiaryOffer ? tertiaryOffer.capacity : 0;
  return `Common Cart tertiary fill maximum (organizer private): ${maximum}. Not a merchant export.\n`;
}

/** Organizer-private one-line tertiary fill remaining capacity. Count only. Not a merchant export. */
export function createTertiaryFillRemainingCapacityMarkdown(rawScenario) {
  const scenario = validateScenario(rawScenario);
  const coverage = computeResidualCoverage(scenario);
  if (!coverage.tertiary) {
    return "Common Cart tertiary fill remaining capacity (organizer private): none. Not a merchant export.\n";
  }
  const tertiaryOffer = scenario.offers.find((offer) => offer.id === coverage.tertiary.offerId);
  const remaining = tertiaryOffer ? Math.max(0, tertiaryOffer.capacity - coverage.tertiary.fulfilledUnits) : 0;
  return `Common Cart tertiary fill remaining capacity (organizer private): ${remaining}. Not a merchant export.\n`;
}
...
export function createLeftoverUncoveredRemainingMarkdown(rawScenario) {
  const coverage = computeResidualCoverage(rawScenario);
  const amount = coverage.leftoverBuyerCount > 0 ? String(coverage.unfilledUnits) : "none";
  return `Common Cart leftover uncovered remaining (organizer private): ${amount}. Not a merchant export.\n`;
}
...
export function createLeftoverUncoveredMaximumMarkdown(rawScenario) {
  const scenario = validateScenario(rawScenario);
  const coverage = computeResidualCoverage(scenario);
  if (!coverage.secondary) {
    return "Common Cart leftover uncovered maximum (organizer private): none. Not a merchant export.\n";
  }
  const leftoverOffer = scenario.offers.find((offer) => offer.id === coverage.secondary.offerId);
  const maximum = leftoverOffer ? leftoverOffer.capacity : 0;
  return `Common Cart leftover uncovered maximum (organizer private): ${maximum}. Not a merchant export.\n`;
}
...
export function createLeftoverUncoveredMinimumMarkdown(rawScenario) {
  const scenario = validateScenario(rawScenario);
  const coverage = computeResidualCoverage(scenario);
  if (!coverage.secondary) {
    return "Common Cart leftover uncovered minimum (organizer private): none. Not a merchant export.\n";
  }
  const leftoverOffer = scenario.offers.find((offer) => offer.id === coverage.secondary.offerId);
  const minimum = leftoverOffer ? leftoverOffer.minimumUnits : 0;
  return `Common Cart leftover uncovered minimum (organizer private): ${minimum}. Not a merchant export.\n`;
}
...
export function createLeftoverUncoveredCountMarkdown(rawScenario) {
  const coverage = computeResidualCoverage(rawScenario);
  const amount = coverage.leftoverBuyerCount > 0 ? String(coverage.unfilledBuyerCount) : "none";
  return `Common Cart leftover uncovered count (organizer private): ${amount}. Not a merchant export.\n`;
}

function leftoverOnlyAllocatedIds(rawScenario) {
  const coverage = computeResidualCoverage(rawScenario);
  const market = evaluateMarket(rawScenario);
  const winnerIds = new Set(market.winner?.selectedBuyerIds ?? []);
...
function leftoverOnlyAllocatedCount(rawScenario) {
  return leftoverOnlyAllocatedIds(rawScenario).size;
}

function leftoverOnlyAllocatedUnits(rawScenario) {
  const leftoverOnly = leftoverOnlyAllocatedIds(rawScenario);
  const scenario = validateScenario(rawScenario);
  let units = 0;
  for (const buyer of scenario.buyers) {
    if (leftoverOnly.has(buyer.id)) units += buyer.quantity;
  }
  return units;
}
...
function leftoverOnlyAllocatedMinimum(rawScenario) {
  const leftoverOnly = leftoverOnlyAllocatedIds(rawScenario);
  const scenario = validateScenario(rawScenario);
  let minimum = null;
  for (const buyer of scenario.buyers) {
    if (leftoverOnly.has(buyer.id) && (minimum === null || buyer.quantity < minimum)) minimum = buyer.quantity;
  }
  return minimum ?? 0;
}
...
function leftoverOnlyAllocatedUnitPrice(rawScenario) {
  const scenario = validateScenario(rawScenario);
  const coverage = computeResidualCoverage(scenario);
  if (!coverage.secondary) return 0;
  const leftoverOffer = scenario.offers.find((offer) => offer.id === coverage.secondary.offerId);
  if (!leftoverOffer) return 0;
  return leftoverOffer.unitPrice;
}
...
export function createLeftoverUncoveredLeftoverOnlyCountMarkdown(rawScenario) {
  const coverage = computeResidualCoverage(rawScenario);
  const amount = coverage.leftoverBuyerCount > 0 ? String(leftoverOnlyAllocatedCount(rawScenario)) : "none";
  return `Common Cart leftover uncovered leftover-only count (organizer private): ${amount}. Not a merchant export.\n`;
}

/** Organizer-private one-line leftover uncovered leftover-only remaining units. Leftover-only buyer units after the winner. Honest empty none. Not a merchant export. Distinct prefix from leftover uncovered remaining, leftover uncovered leftover-only count, leftover uncovered count, leftover uncovered maximum, leftover uncovered minimum, uncovered leftover unit-count, leftover-fill remaining, and tertiary remaining. */
export function createLeftoverUncoveredLeftoverOnlyRemainingMarkdown(rawScenario) {
  const coverage = computeResidualCoverage(rawScenario);
...
export function createLeftoverUncoveredLeftoverOnlyUnitPriceMarkdown(rawScenario) {
  const coverage = computeResidualCoverage(rawScenario);
  const amount = coverage.leftoverBuyerCount > 0 ? String(leftoverOnlyAllocatedUnitPrice(rawScenario)) : "none";
  return `Common Cart leftover uncovered leftover-only unit price (organizer private): ${amount}. Not a merchant export.\n`;
}
