...
Scenario JSON is untrusted input. The app validates types, lengths, ranges, collection sizes, unique identifiers, and a closed field set before calculation. Extra keys, including prototype names, are rejected, and field values are read from own properties only. User-provided values are inserted into the page as text or input values, not executable HTML.
