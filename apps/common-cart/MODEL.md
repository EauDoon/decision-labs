...
`restoreExampleOffers` replaces offers with a named preset's offers and keeps the current buyers, title, and currency. Named lunch presets include Bottle-hand cycling carnival lunch after Domestique cycling carnival lunch after Soigneur cycling carnival lunch after Lead-out cycling carnival lunch after Feed-zone cycling carnival lunch after First-aid cycling carnival lunch after Team-sprint cycling carnival lunch after Team-pursuit cycling carnival lunch after Individual-pursuit cycling carnival lunch after Scratch cycling carnival lunch after Points-race cycling carnival lunch after Omnium cycling carnival lunch after Madison cycling carnival lunch. Bottle-hand leftover uncovered leftover-only remaining is 54 leftover-only units. Pickup minimum 53, unit price 48, capacity 64. Leftover-only minimum stays 15. Distinct from Domestique leftover remaining 53, Soigneur leftover remaining 52. Domestique leftover uncovered leftover-only remaining is 53 leftover-only units. Pickup minimum 52, unit price 48, capacity 64. Leftover-only minimum stays 15. Distinct from Soigneur leftover remaining 52, Lead-out leftover remaining 51,  Feed-zone leftover remaining 49, First-aid leftover remaining 47, Team-sprint leftover remaining 45, Team-pursuit leftover remaining 43, Individual-pursuit leftover remaining 41, and Scratch leftover remaining 39. Lead-out leftover uncovered leftover-only remaining stays 51 leftover-only units. Pickup minimum 49, unit price 46, capacity 64.
...
`leftoverFillEvidenceToMarkdown` writes one organizer-private leftover-fill document: the fill summary plus units, merchant, offer label, remaining capacity, fulfillment mode, delivery and pickup days, minimum units, and offer capacity, each with honest empty values when leftover fill is missing. It is not a merchant export. Labels, IDs, budgets, and allocations stay out.

`uncoveredLeftoverEvidenceToMarkdown` writes one organizer-private uncovered-leftover document: uncovered buyer and unit counts plus the remaining, count, leftover-fill capacity, and leftover-fill minimum facts. It is not a merchant export. Labels, IDs, budgets, and allocations stay out.

`leftoverOnlyEvidenceToMarkdown` writes one organizer-private leftover-only document: buyer count, units, largest and smallest buyer quantities, headroom, allocated units, offer capacity, and offer unit price. It is not a merchant export. Labels, IDs, budgets, and allocations stay out.

`tertiaryFillEvidenceToMarkdown` writes one organizer-private tertiary-fill document: remaining capacity and offer capacity. It is not a merchant export. Labels, IDs, budgets, and allocations stay out.

`winningOfferEvidenceToMarkdown` writes the winning merchant label, fulfillment mode, and remaining capacity in one merchant-safe document, or `None unlocked` values. It omits buyer identities, IDs, budgets, and allocations.
