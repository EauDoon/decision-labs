...
`createLeftoverUncoveredLeftoverOnlyRemainingMarkdown` writes leftover-only buyer units after the winner as one-line organizer-private Markdown. Sum of leftover-only buyer quantities when leftover after the winner exists, or honest empty when leftover after the winner is missing. It is leftover-only buyer units after the winner, not leftover-fill offer capacity and not leftover-only headroom remaining. Omnium cycling carnival leftover-only remaining is 37 leftover-only units, leftover uncovered leftover-only headroom is 27, leftover uncovered leftover-only capacity reports 64, leftover uncovered leftover-only minimum is 15, and leftover-fill offer unit price is 32. Gravel cycling carnival leftover-only remaining is 30 leftover-only units, leftover uncovered leftover-only headroom is 28, leftover uncovered leftover-only capacity reports 58, leftover uncovered leftover-only minimum is 15, and leftover-fill offer unit price is 24. It is not a merchant export. Distinct prefix from `createLeftoverUncoveredRemainingMarkdown`, `createLeftoverUncoveredLeftoverOnlyCountMarkdown`, `createLeftoverUncoveredLeftoverOnlyMaximumMarkdown`, `createLeftoverUncoveredLeftoverOnlyMinimumMarkdown`, `createLeftoverUncoveredLeftoverOnlyHeadroomMarkdown`, `createLeftoverUncoveredLeftoverOnlyCapacityMarkdown`, `createLeftoverUncoveredLeftoverOnlyUnitPriceMarkdown`, `createLeftoverUncoveredCountMarkdown`, `createLeftoverUncoveredMaximumMarkdown`, `createLeftoverUncoveredMinimumMarkdown`, `createUncoveredLeftoverUnitCountMarkdown`, `createLeftoverFillRemainingCapacityMarkdown`, and `createTertiaryFillRemainingCapacityMarkdown` even when the number matches. Buyer identities, IDs, budgets, and allocations stay out.
