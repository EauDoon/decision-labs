...
`createLeftoverFillLabelMarkdown` writes leftover-fill offer label as one-line organizer-private Markdown. Merchant and variant only when leftover fill exists, or honest empty when leftover fill is missing. It is not a merchant export. Distinct from `createLeftoverFillMerchantLabelMarkdown`, `createLeftoverFillPickupMarkdown`, and `createWinningMerchantLabelMarkdown`. Buyer identities, IDs, budgets, and allocations stay out.
