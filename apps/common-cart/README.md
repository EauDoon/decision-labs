...
- Start from coffee, office chair, community pantry, office pantry bulk, hardware tools, community garden bulk seed, school fete catering, office fruit box, library photocopy paper, sports club match-day kit, or price-ladder examples.
...
- Inspect a merchant view that contains aggregate ranges, a variant overlap matrix, and a delivery heatmap, rather than individual records. Copy overlap counts as formula-safe CSV or as Markdown. Copy winner aggregates as Markdown. Copy leftover coverage, leftover fill, uncovered leftover counts, leftover unspent item headroom, and the winner inspector summary as organizer-private Markdown. Copy the winning merchant label only. Copy the winning fulfillment mode as pickup or shipping. Copy remaining capacity on the unlocked winner.
- Import or export a scenario as JSON, duplicate the room as a compare snapshot, export an organizer briefing, export heatmap CSV, export offer CSV, or encode the current scenario in a share link. Press `p` for an organizer leftover one-pager, `v` for variant overlap, `j` for uncovered leftover, `d` for uncovered leftover coverage, `s` for buyer paste, `f` for the fulfillment filter, `h` for leftover headroom, `i` for leftover headroom copy, `k` for the organizer review panel, `c` for leftover coverage copy, `t` for tertiary leftover fill, `y` for leftover fill copy, `z` for leftover fill, `g` for group headroom, `q` for requested units, and `x` for Export private buyer report.
- Hide locked offers, fully covered leftover-coverage rows, the leftover-fill coverage row, or the tertiary leftover-coverage row on screen without changing matching. Restore shows locked offers again.
