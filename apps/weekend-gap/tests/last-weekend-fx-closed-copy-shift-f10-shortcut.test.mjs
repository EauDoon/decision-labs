...
test("1.5.34 keeps last-weekend-FX-closed copy control without taking Shift+F10", async () => {
  ...
  assert.notEqual(app.match(/function copyLastWeekendFxClosedHourMarkdown/)?.[0], app.match(/function copyLastWeekendFxOpenHourMarkdown/)?.[0]);
  ...
  const f7 = handler.indexOf('event.key === "F7"');
  ...
  const f7Slice = handler.slice(f7, f7 + 180);
  assert.match(f7Slice, /copyLastWeekendFxClosedHourMarkdown\(\)/);
