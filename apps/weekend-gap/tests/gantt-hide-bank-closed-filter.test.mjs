...
test("bank-closed helper hides hours whose bank gate is closed", () => {
  ...
  assert.equal(ganttHourBankClosed(null), false);
  assert.equal(ganttHourBankClosed({}), false);
  const bankClosed = schedule.hours.filter((point) => point.hour < SIMULATION_HOURS && ganttHourBankClosed(point)).length;
  const bankOpen = schedule.hours.filter((point) => point.hour < SIMULATION_HOURS && !ganttHourBankClosed(point)).length;
  assert.ok(bankClosed > 0);
  assert.ok(bankOpen > 0);
  assert.equal(bankClosed + bankOpen, SIMULATION_HOURS);
});

test("hide-bank-closed Gantt SVG is display-only and keeps the selected hour visible", () => {
  ...
  const composedOpen = buildGateGanttSvg(DEFAULT_SCENARIO, 0, { hideBankClosedHours: true, hideOpenHours: true });
  const hiddenClosed = buildGateGanttSvg(DEFAULT_SCENARIO, 0, { hideClosedHours: true });
  const composedClosed = buildGateGanttSvg(DEFAULT_SCENARIO, 0, { hideBankClosedHours: true, hideClosedHours: true });
  const weekdayOnly = buildGateGanttSvg(DEFAULT_SCENARIO, 0, { hideWeekdayHours: true });
  const composedWeekday = buildGateGanttSvg(DEFAULT_SCENARIO, 0, { hideBankClosedHours: true, hideWeekdayHours: true });
  const weekendOnly = buildGateGanttSvg(DEFAULT_SCENARIO, 0, { hideWeekendHours: true });
  const composedWeekend = buildGateGanttSvg(DEFAULT_SCENARIO, 0, { hideBankClosedHours: true, hideWeekendHours: true });
  const hiddenZero = buildGateGanttSvg(DEFAULT_SCENARIO, 0, { hideZeroQueueHours: true });
  const composedZero = buildGateGanttSvg(DEFAULT_SCENARIO, 0, { hideBankClosedHours: true, hideZeroQueueHours: true });
  const holiday = { ...DEFAULT_SCENARIO, mondayHoliday: true };
  const everyClosed = buildGateGanttSvg(holiday, 0, { everyClosedOnly: true });
  const composedEvery = buildGateGanttSvg(holiday, 0, { hideBankClosedHours: true, everyClosedOnly: true });
  const closedOnly = buildGateGanttSvg(DEFAULT_SCENARIO, 0, { closedOnly: true });
  const composedClosedOnly = buildGateGanttSvg(DEFAULT_SCENARIO, 0, { hideBankClosedHours: true, closedOnly: true });
  const fxOnly = buildGateGanttSvg(DEFAULT_SCENARIO, 0, { hideBankClosedHours: true, gateFilter: "fx" });
  const selectedClosed = buildGateGanttSvg(DEFAULT_SCENARIO, 2, { hideBankClosedHours: true });
  assert.notEqual(full, hiddenBank);
  assert.notEqual(hiddenBank, hiddenOpen);
  assert.notEqual(hiddenBank, hiddenClosed);
  assert.notEqual(hiddenBank, hiddenZero);
  assert.match(hiddenBank, /viewBox="0 0 720/);
  const hiddenRects = (hiddenBank.match(/<rect /g) || []).length;
...
test("hide-bank-closed Gantt filter is a display control that can restore all hours", async () => {
  const html = await readFile(new URL("../index.html", import.meta.url), "utf8");
  const app = await readFile(new URL("../src/app.js", import.meta.url), "utf8");
  assert.match(html, /id="gantt-hide-bank-closed"/);
  assert.match(html, /Hide hours where the bank gate is closed/);
  assert.match(app, /ganttHourBankClosed/);
