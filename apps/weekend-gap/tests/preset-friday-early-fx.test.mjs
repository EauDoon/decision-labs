...
test("Friday early FX open keeps the Normal Friday calendar with a Friday morning FX window", () => {
  ...
  assert.equal(firstFridayEarlyFxOpenHour(PRESETS.saturdayMiddayFxOpen), null);
...
test("Friday early FX open ORs into fxWeekday through isFridayEarlyFxOpenHour, not Friday late or Thursday late helpers", async () => {
  ...
  const helperNext = model.indexOf("\n/** Saturday 12:00-14:00", helperStart);
  const commentStart = model.lastIndexOf("/** Friday 08:00-10:00", helperStart);
  ...
  assert.match(helper, /fridayFxLateClose/);
  assert.match(helper, /fridayEarlyIssuerOpen/);
  assert.match(helper, /fridayEarlyBankOpen/);
  assert.match(helper, /fridayEarlyPayoutOpen/);
  assert.match(helper, /thursdayLateFxOpen/);
  ...
  assert.doesNotMatch(helper, /isThursdayLateFxOpenHour/);
  ...
  assert.match(model, /function isFridayLateFxOpenHour/);
  assert.match(model, /function isFridayLateFxHour/);
  assert.match(model, /function isFridayEarlyIssuerHour/);
  assert.match(model, /function isFridayEarlyBankHour/);
  assert.match(model, /function isFridayEarlyPayoutHour/);
  assert.match(model, /function isThursdayLateFxOpenHour/);
  ...
  assert.ok(saturdayMiddayCall > fridayEarlyCall);
