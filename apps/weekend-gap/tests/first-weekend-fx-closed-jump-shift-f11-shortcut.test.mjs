...
test("1.5.37 keeps first-weekend-FX-closed jump without taking Shift+F11", async () => {
  ...
  assert.notEqual(app.match(/function jumpToFirstWeekendFxClosedCopy/)?.[0], app.match(/function jumpToLastWeekendFxClosedCopy/)?.[0]);
  assert.notEqual(app.match(/function jumpToFirstWeekendFxClosedCopy/)?.[0], app.match(/function jumpToLastWeekdayFxClosedCopy/)?.[0]);
  assert.notEqual(app.match(/function jumpToFirstWeekendFxClosedCopy/)?.[0], app.match(/function jumpToGantt/)?.[0]);
  const closedFn = app.slice(app.indexOf("function jumpToFirstWeekendFxClosedCopy"), app.indexOf("function jumpToHideWeekdayFxOpenFilter"));
  assert.match(closedFn, /jumpToGantt\(\)/);
  assert.doesNotMatch(closedFn, /copyFirstWeekendFxClosedHourMarkdown/);
