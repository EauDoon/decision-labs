...
test("keyboard shortcuts skip inputs and cover help, play, first settlement, undo, redo and export", () => {
  ...
  assert.match(html, /Jump the timeline to the first closed bank hour/);
  ...
  assert.match(appSource, /event\.key === "f"/);
  assert.match(appSource, /jumpToFirstClosedBank\(\)/);
  assert.match(appSource, /event\.key === "s"/);
  assert.match(appSource, /jumpToScenarioInputs\(\)/);
