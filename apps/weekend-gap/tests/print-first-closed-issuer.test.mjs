...
import { DEFAULT_SCENARIO, firstClosedIssuerGanttHour, formatTime, scenarioToJSON } from "../src/model.js";

test("print and print redacted include the first closed issuer hour label with an honest empty", async () => {
  const html = await readFile(new URL("../index.html", import.meta.url), "utf8");
  const css = await readFile(new URL("../styles.css", import.meta.url), "utf8");
  const app = await readFile(new URL("../src/app.js", import.meta.url), "utf8");
  assert.match(html, /id="first-closed-issuer-hour-line"/);
  assert.match(html, /id="first-closed-issuer-hour-value"/);
  assert.match(html, /First closed issuer hour/);
