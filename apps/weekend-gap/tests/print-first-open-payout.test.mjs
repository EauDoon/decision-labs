import assert from "node:assert/strict";
import test from "node:test";
import { readFile } from "node:fs/promises";
import { DEFAULT_SCENARIO, firstOpenPayoutGanttHour, formatTime, scenarioToJSON } from "../src/model.js";

test("print and print redacted include the first open payout hour label with an honest empty", async () => {
  const html = await readFile(new URL("../index.html", import.meta.url), "utf8");
  const css = await readFile(new URL("../styles.css", import.meta.url), "utf8");
  const app = await readFile(new URL("../src/app.js", import.meta.url), "utf8");
  assert.match(html, /id="first-open-payout-hour-line"/);
  assert.match(html, /id="first-open-payout-hour-value"/);
  assert.match(html, /First open payout hour/);
  assert.match(html, /id="first-closed-payout-hour-line"/);
  assert.match(html, /class="outcome-card card print-keep"/);
  assert.match(css, /\.outcome-card/);
  assert.match(css, /\.print-keep/);
  assert.match(app, /firstOpenPayoutHour\.textContent/);
  assert.match(app, /firstOpenPayoutGanttHour\(scenario\)/);
      assert.match(app, /The hour evidence lines stay on the printed brief with honest empty values when none exist/);
  assert.match(app, /openPayoutHour === null/);
  const printHandler = app.slice(app.indexOf('document.querySelector("#print")'), app.indexOf('document.querySelector("#copy-dashboard-markdown")'));
  assert.doesNotMatch(printHandler, /setScenario\(/);
  assert.match(printHandler, /The saved scenario was not changed/);
  const hour = firstOpenPayoutGanttHour(DEFAULT_SCENARIO);
  assert.equal(hour, 0);
  assert.equal(formatTime(hour), "Fri 15:00");
  const before = scenarioToJSON(DEFAULT_SCENARIO);
  assert.equal(scenarioToJSON(DEFAULT_SCENARIO), before);
});
