...
test("1.5.35 keeps last-weekend-FX-open jump without taking Shift+F11", async () => {
  ...
  assert.doesNotMatch(html, /Jump to the last-weekend-FX-open-hour copy control/);
  ...
  assert.notEqual(app.match(/function jumpToLastWeekendFxOpenCopy/)?.[0], app.match(/function jumpToFirstWeekendFxOpenCopy/)?.[0]);
  assert.notEqual(app.match(/function jumpToLastWeekendFxOpenCopy/)?.[0], app.match(/function jumpToLastWeekendFxClosedCopy/)?.[0]);
  assert.notEqual(app.match(/function jumpToLastWeekendFxOpenCopy/)?.[0], app.match(/function jumpToGantt/)?.[0]);
  const openFn = app.slice(app.indexOf("function jumpToLastWeekendFxOpenCopy"), app.indexOf("function jumpToLastWeekdayFxOpenCopy"));
  assert.match(openFn, /jumpToGantt\(\)/);
  assert.doesNotMatch(openFn, /copyLastWeekendFxOpenHourMarkdown/);
  assert.doesNotMatch(openFn, /copyFirstWeekendFxOpenHourMarkdown/);
