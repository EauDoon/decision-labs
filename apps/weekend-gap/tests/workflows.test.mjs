...
test("keyboard Shift+F10 copies first weekend-FX-open hour through the new control and ignores the key while typing", async () => {
  const ui = await boot(new Map([["weekend-gap:coach:v1", "dismissed"]]));
  ...
  assert.doesNotMatch(ui.nodes.get("first-weekend-fx-open-copy-fallback").value, /Last weekday-FX-open hour:/);
  assert.doesNotMatch(ui.nodes.get("first-weekend-fx-open-copy-fallback").value, /Last weekend-FX-open hour:/);
  assert.doesNotMatch(ui.nodes.get("first-weekend-fx-open-copy-fallback").value, /Last weekday-FX-closed hour:/);
  assert.doesNotMatch(ui.nodes.get("first-weekend-fx-open-copy-fallback").value, /Last weekend-FX-closed hour:/);
  assert.doesNotMatch(ui.nodes.get("first-weekend-fx-open-copy-fallback").value, /Last closed FX hour:/);
  ui.nodes.get("first-weekend-fx-open-copy-fallback").hidden = true;
  ui.nodes.get("first-weekend-fx-open-copy-fallback").value = "";
  ...
  assert.equal(ui.nodes.get("last-weekday-fx-closed-copy-fallback").hidden, false);
  assert.match(ui.nodes.get("last-weekday-fx-closed-copy-fallback").value, /Last weekday-FX-closed hour:/);
  assert.notEqual(ui.nodes.get("first-weekend-fx-open-copy-fallback").value, ui.nodes.get("last-weekday-fx-closed-copy-fallback").value);
});

test("keyboard Shift+F11 jumps to the first-weekend-FX-open copy control and does not copy", async () => {
  const ui = await boot(new Map([["weekend-gap:coach:v1", "dismissed"]]));
  await ui.keydown("F11", { tagName: "BODY" }, { shiftKey: true });
  assert.equal(ui.nodes.get("copy-first-weekend-fx-open").focused, true);
  ...
  assert.equal(ui.nodes.get("first-weekend-fx-open-copy-fallback").hidden, true);
  ui.nodes.get("copy-first-weekend-fx-open").focused = false;
  ui.nodes.get("gantt-title").focused = false;
  await ui.keydown("F11", { tagName: "INPUT" }, { shiftKey: true });
  assert.equal(ui.nodes.get("copy-first-weekend-fx-open").focused, false);
  assert.equal(ui.nodes.get("gantt-title").focused, false);
