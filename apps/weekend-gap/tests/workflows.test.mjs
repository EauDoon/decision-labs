...
test("source mode runs library, sensitivity, undo, hourly table and workspace reload workflows", async () => {
  ...
  await ui.edit("gantt-hour-filter", "any-closed", "change");
  ...
  await ui.edit("gantt-hour-filter", "all", "change");
  assert.equal(ui.nodes.get("gantt-table").children.length, 73);
  await ui.edit("timeline-range", 65);
  ...
  assert.equal(persisted.ganttHourFilter, "all");
...
test("consolidated Gantt hour filter persists and older hide flags map into it", async () => {
  const ui = await boot();
  await ui.edit("gantt-hour-filter", "any-closed", "change");
  assert.equal(JSON.parse(ui.storage.get("weekend-gap:workspace:v1")).ganttHourFilter, "any-closed");
  assert.match(ui.nodes.get("gantt-filter-note").textContent, /hours closed on at least one gate remain/);
  await ui.edit("gantt-hour-filter", "every-closed", "change");
  assert.match(ui.nodes.get("gantt-filter-note").textContent, /Showing only hours where every gate is closed/);
  await ui.edit("gantt-hour-filter", "weekend", "change");
  assert.match(ui.nodes.get("gantt-filter-note").textContent, /Weekday hours are hidden/);
  await ui.edit("gantt-hour-filter", "weekday", "change");
  assert.match(ui.nodes.get("gantt-filter-note").textContent, /Weekend hours are hidden/);
  await ui.edit("gantt-hour-filter", "queued", "change");
  assert.match(ui.nodes.get("gantt-filter-note").textContent, /synthetic queue is zero/);
  await ui.edit("gantt-hour-filter", "all", "change");
  assert.match(ui.nodes.get("gantt-filter-note").textContent, /All 72 model hours/);
  await ui.edit("gantt-hour-filter", "weekend", "change");
  const restored = await boot(ui.storage);
  assert.equal(restored.nodes.get("gantt-hour-filter").value, "weekend");
  const raw = JSON.parse(ui.storage.get("weekend-gap:workspace:v1"));
  delete raw.ganttHourFilter;
  const legacyClosed = await boot(new Map([["weekend-gap:workspace:v1", JSON.stringify({ ...raw, ganttClosedOnly: true })]]));
  assert.equal(legacyClosed.nodes.get("gantt-hour-filter").value, "any-closed");
  const legacyEvery = await boot(new Map([["weekend-gap:workspace:v1", JSON.stringify({ ...raw, ganttEveryGateClosed: true })]]));
  assert.equal(legacyEvery.nodes.get("gantt-hour-filter").value, "every-closed");
  const legacyBankHide = await boot(new Map([["weekend-gap:workspace:v1", JSON.stringify({ ...raw, hideBankClosedGanttHours: true })]]));
  assert.equal(legacyBankHide.nodes.get("gantt-hour-filter").value, "any-closed");
  const legacyWeekendHide = await boot(new Map([["weekend-gap:workspace:v1", JSON.stringify({ ...raw, hideWeekendGanttHours: true })]]));
  assert.equal(legacyWeekendHide.nodes.get("gantt-hour-filter").value, "weekday");
  const legacyZero = await boot(new Map([["weekend-gap:workspace:v1", JSON.stringify({ ...raw, hideZeroQueueGanttHours: true })]]));
  assert.equal(legacyZero.nodes.get("gantt-hour-filter").value, "queued");
});

test("gate hour evidence buttons copy first and last open and closed hours per gate", async () => {
  const ui = await boot();
  await ui.nodes.get("copy-gate-evidence-bank").click();
  assert.match(ui.nodes.get("gate-bank-evidence-fallback").value, /First open Bank hour|First closed Bank hour/);
  assert.match(ui.nodes.get("gate-bank-evidence-fallback").value, /Last open Bank hour|Last closed Bank hour/);
  assert.match(ui.nodes.get("gate-bank-evidence-fallback").value, /Bank hours: \d+ open, \d+ closed/);
  await ui.nodes.get("copy-gate-evidence-fx").click();
  assert.match(ui.nodes.get("gate-fx-evidence-fallback").value, /FX hours: \d+ open, \d+ closed/);
});
