...
test("keyboard Shift+F10 copies first weekday-FX-open hour through the existing control and ignores the key while typing", async () => {
  const ui = await boot(new Map([["weekend-gap:coach:v1", "dismissed"]]));
  ...
  assert.doesNotMatch(ui.nodes.get("first-weekday-fx-open-copy-fallback").value, /First weekend-FX-closed hour:/);
  assert.doesNotMatch(ui.nodes.get("first-weekday-fx-open-copy-fallback").value, /First weekend-FX-open hour:/);
  ...
  assert.doesNotMatch(ui.nodes.get("first-weekday-fx-open-copy-fallback").value, /First weekday-FX-closed hour:/);
