...
async function boot(storage = new Map(), { blockedStorage = false, hash = "", reduced = false, canvasAvailable = true } = {}) {
  ...
  const presets = ["normal", "weekendRush", "marketStress", "thinFxTightWindows", "longWeekendFridayStart", "compressedFridayClose", "paydayFridayBurst", "publicHolidayMonday", "saturdayMarketBurst", "sundayStallClose", "thinSaturdayFx", "earlyMondayBankOpen", "fridayLateFxClose", "mondayLateIssuerOpen", "saturdayEarlyFxOpen", "sundayLateBankClose", "sundayLatePayoutClose", "saturdayEarlyPayoutOpen", "fridayEarlyPayoutOpen", "saturdayLatePayoutOpen", "sundayEarlyPayoutOpen", "sundayLateIssuerClose", "sundayEarlyIssuerOpen", "saturdayEarlyIssuerOpen", "fridayEarlyIssuerOpen", "saturdayEarlyBankOpen", "fridayEarlyBankOpen", "saturdayLateBankOpen", "fridayLateBankOpen", "fridayLateFxOpen", "saturdayLateFxOpen", "sundayLateFxOpen", "sundayEarlyFxOpen", "mondayEarlyFxOpen", "mondayLateFxOpen", "tuesdayEarlyFxOpen", "tuesdayLateFxOpen", "wednesdayEarlyFxOpen", "wednesdayLateFxOpen", "thursdayEarlyFxOpen"].map(key => { const element = new Element(); element.dataset.preset = key; return element; });
  const document = {
...
test("keyboard Shift+F10 copies last weekend-FX-open hour through the existing control and ignores the key while typing", async () => {
  const ui = await boot(new Map([["weekend-gap:coach:v1", "dismissed"]]));
  ...
  assert.doesNotMatch(ui.nodes.get("last-weekend-fx-open-copy-fallback").value, /First weekend-FX-closed hour:/);
  assert.doesNotMatch(ui.nodes.get("last-weekend-fx-open-copy-fallback").value, /First weekend-FX-open hour:/);
  assert.doesNotMatch(ui.nodes.get("last-weekend-fx-open-copy-fallback").value, /First weekday-FX-closed hour:/);
  assert.doesNotMatch(ui.nodes.get("last-weekend-fx-open-copy-fallback").value, /First weekday-FX-open hour:/);
  assert.doesNotMatch(ui.nodes.get("last-weekend-fx-open-copy-fallback").value, /Last weekday-FX-open hour:/);
