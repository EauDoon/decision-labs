...
test("release 1.5.51 ships Sunday sunrise FX open, first-weekday-FX-open copy and weekday-FX-closed hide jump", async () => {
  ...
  assert.match(html, /isSundaySunriseFxOpenHour/);
  assert.match(html, /isSundayBreakfastFxOpenHour/);
  assert.match(html, /isSundayDaybreakFxOpenHour/);
...
test("release 1.5.38 ships Sunday midday FX open, first-weekday-FX-open copy and weekday-FX-closed hide jump", async () => {
  const pkg = JSON.parse(await readFile(new URL("../package.json", import.meta.url), "utf8"));
  const readme = await readFile(new URL("../README.md", import.meta.url), "utf8");
  const changelog = await readFile(new URL("../CHANGELOG.md", import.meta.url), "utf8");
  const html = await buildStandalone();
  ...
  const saturdayPreset = html.indexOf('data-preset="saturdayMiddayFxOpen"');
  const sundayPreset = html.indexOf('data-preset="sundayMiddayFxOpen"');
  assert.ok(fridayPreset !== -1 && saturdayPreset > fridayPreset);
  assert.ok(sundayPreset > saturdayPreset);
  assert.match(html, /hideWeekdayFxClosedGanttHours/);
  assert.match(html, /firstWeekdayFxOpenHourToMarkdown/);
  assert.match(html, /lastWeekdayFxOpenHourToMarkdown/);
  assert.match(html, /isSundayMiddayFxOpenHour/);
...
test("release 1.5.37 ships Saturday midday FX open, last-weekday-FX-open copy and weekday-FX-closed hide jump", async () => {
  const pkg = JSON.parse(await readFile(new URL("../package.json", import.meta.url), "utf8"));
  const readme = await readFile(new URL("../README.md", import.meta.url), "utf8");
  const changelog = await readFile(new URL("../CHANGELOG.md", import.meta.url), "utf8");
  const html = await buildStandalone();
...
test("release 1.5.36 ships Friday early FX open, first-weekend-FX-closed copy and weekday-FX-closed hide jump", async () => {
  const pkg = JSON.parse(await readFile(new URL("../package.json", import.meta.url), "utf8"));
  const readme = await readFile(new URL("../README.md", import.meta.url), "utf8");
  const changelog = await readFile(new URL("../CHANGELOG.md", import.meta.url), "utf8");
  const html = await buildStandalone();
  ...
  assert.match(html, /hideWeekdayFxClosedGanttHours/);
  assert.match(html, /firstWeekendFxClosedHourToMarkdown/);
  assert.match(html, /isFridayEarlyFxOpenHour/);
  assert.match(html, /isThursdayLateFxOpenHour/);
  assert.match(html, /ganttHourWeekdayFxClosed/);
  assert.match(html, /if \(event\.defaultPrevented\) return/);
  assert.match(html, /event\.key\.length === 1 \? event\.key\.toLowerCase\(\) : event\.key/);
  assert.ok(html.includes('event.key === "F10"'));
...
test("release 1.5.35 ships Thursday late FX open, first-weekend-FX-open copy and weekday-FX-closed hide jump", async () => {
  const pkg = JSON.parse(await readFile(new URL("../package.json", import.meta.url), "utf8"));
  const readme = await readFile(new URL("../README.md", import.meta.url), "utf8");
  const changelog = await readFile(new URL("../CHANGELOG.md", import.meta.url), "utf8");
  const html = await buildStandalone();
  ...
  assert.match(html, /isThursdayLateFxOpenHour/);
  assert.match(html, /isThursdayEarlyFxOpenHour/);
  assert.match(html, /ganttHourWeekdayFxClosed/);
  assert.match(html, /if \(event\.defaultPrevented\) return/);
  assert.match(html, /event\.key\.length === 1 \? event\.key\.toLowerCase\(\) : event\.key/);
  assert.ok(html.includes('event.key === "F10"'));
...
test("release 1.5.34 ships Thursday early FX open, last-weekend-FX-open copy and weekday-FX-open hide jump", async () => {
  const pkg = JSON.parse(await readFile(new URL("../package.json", import.meta.url), "utf8"));
  const readme = await readFile(new URL("../README.md", import.meta.url), "utf8");
  const changelog = await readFile(new URL("../CHANGELOG.md", import.meta.url), "utf8");
  const html = await buildStandalone();
...
test("release 1.5.33 ships Wednesday late FX open, last-weekend-FX-closed copy and weekend-FX-open hide jump", async () => {
  const pkg = JSON.parse(await readFile(new URL("../package.json", import.meta.url), "utf8"));
  const readme = await readFile(new URL("../README.md", import.meta.url), "utf8");
  const changelog = await readFile(new URL("../CHANGELOG.md", import.meta.url), "utf8");
  const html = await buildStandalone();
  ...
  assert.match(html, /isWednesdayLateFxOpenHour/);
...
test("release 1.5.32 ships Wednesday early FX open, first-weekend-FX-closed copy and weekday-FX-open hide jump", async () => {
  const pkg = JSON.parse(await readFile(new URL("../package.json", import.meta.url), "utf8"));
  const readme = await readFile(new URL("../README.md", import.meta.url), "utf8");
  const changelog = await readFile(new URL("../CHANGELOG.md", import.meta.url), "utf8");
  const html = await buildStandalone();
  ...
  assert.match(html, /hideWeekdayFxOpenGanttHours/);
  assert.match(html, /firstWeekendFxClosedHourToMarkdown/);
  assert.match(html, /isWednesdayEarlyFxOpenHour/);
